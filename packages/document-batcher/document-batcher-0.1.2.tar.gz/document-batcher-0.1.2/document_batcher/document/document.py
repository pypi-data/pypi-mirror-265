...
class Document:
    ...
    @staticmethod
    def _sent_tokenize(text) -> list[str]:
        """
        This static method is based on
        https://github.com/nltk/nltk/blob/3.8.1/nltk/tokenize/__init__.py#L95-L107
        """
        if not hasattr(Document, '_english_sentence_segmentation_model'):
            
            english_sentence_segmentation_model_file_path = os.path.join(
                os.path.dirname(__file__), '..',
                'nltk_data/tokenizers/punkt/PY3/english.pickle'
            )
            
            assert os.path.isfile(english_sentence_segmentation_model_file_path), \
                'the English sentence segmentation model file is not available at: ' \
                + english_sentence_segmentation_model_file_path
            
            # https://github.com/nltk/nltk/blob/3.8.1/nltk/data.py#L754-L755
            with open(english_sentence_segmentation_model_file_path, 'rb') as model_file:
                
                Document._english_sentence_segmentation_model = pickle.load(model_file)
            
        assert isinstance(
            Document._english_sentence_segmentation_model,
            nltk.tokenize.PunktSentenceTokenizer
        )
        
        return Document._english_sentence_segmentation_model.tokenize(text)
    
    
    #######################################################
    #### document id
    
    _ID_KEYS = ['documentID', 'document_id']
    ...
    def _set_id(
        ...
        assert isinstance(doc_id, str), \
            'in each input json line, ' \
            + 'the document id value pointed to by one of the keys in ' \
            + str(Document._get_id_keys()) \
            + ' must be of type str; however, the pointed to value ``' \
            + str(doc_id) \
            + '\'\' is of type ' \
            + str(type(doc_id))
        self._id = doc_id
    ...
    def from_input_doc_dict(
        ...
        list_of_sents = Document._sent_tokenize(input_doc.get_full_text())
