...
class Mapper():
        ...
        if self.mapper_type in ['training','forecasting']:
            ...
            query = f"""
                ...
                LEFT JOIN (
                    ...
                    INNER JOIN (CURRENT VALIDTIME SELECT * FROM {self.schema_name}.{self.code_repository}) BB
                    ON AA.ID_CODE = BB.ID
                    INNER JOIN {self.schema_name}.{self.trained_model_repository} CC
        ...
        if self.mapper_type == 'training':
            ...
            query = f"""
                    ...
                    FROM Script(
                        ...
                        ORDER BY {self.id_row}
                        SCRIPT_COMMAND(
                            'tdpython3 ./{self.SEARCHUIFDBPATH}/tds_feature_engineering_reducer.py  | awk -F"\t" "NF == {6 + len(self.id_partition.split(','))}"'
                        )
                        RETURNS(
                                {output_sql}
    ...
    def execute_mapper(self):
        ...
        if self.mapper_type == 'feature engineering reducer':
            ...
            (TD_TIMECODE, ID_PROCESS, {self.id_partition}, FEATURE_NAME, FEATURE_VALUE, FEATURE_TYPE, STATUS, ID_MODEL)
            {self.generate_sto_query()}
            WHERE STATUS LIKE '%successful%'
            """
