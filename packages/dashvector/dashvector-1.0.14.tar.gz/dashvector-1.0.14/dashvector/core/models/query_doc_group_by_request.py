...
class QueryDocGroupByRequest(RPCRequest):
    def __init__(
        ...
        *,
        ...
        if group_topk < 1 or group_topk > 16:
            raise DashVectorException(
                code=DashVectorCode.InvalidTopk,
                reason=f"DashVectorSDK QueryDocGroupByRequest group_topk value({group_topk}) is invalid and must be in [1, 16]",
            )
        ...
        if group_count < 1 or group_count > 64:
            raise DashVectorException(
                code=DashVectorCode.InvalidTopk,
