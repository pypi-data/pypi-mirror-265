...
from startai.data_classes.container import Container
from startai.utils.exceptions import StartaiException
...
def test_container_all_false(on_device):
    ...
    try:
        assert Container(
            {"a": startai.array([1], device=on_device), "b": {"c": [1], "d": True}}
        ).cont_all_false(assert_is_bool=True)
        error_raised = False
    except StartaiException:
        error_raised = True
    assert error_raised
...
def test_container_all_key_chains(include_empty, on_device):
    a_val = Container() if include_empty else startai.array([1], device=on_device)
    bc_val = Container() if include_empty else startai.array([2], device=on_device)
    bd_val = Container() if include_empty else startai.array([3], device=on_device)
...
def test_container_all_true(on_device):
    assert not Container(
        {"a": startai.array([1], device=on_device), "b": {"c": [], "d": True}}
    ).cont_all_true()
    assert Container(
        {"a": startai.array([1], device=on_device), "b": {"c": [1], "d": True}}
    ).cont_all_true()
...
def test_container_as_bools(on_device):
    dict_in = {"a": startai.array([1], device=on_device), "b": {"c": [], "d": True}}
    container = Container(dict_in)
...
def test_container_assert_contains(on_device):
    arr0 = startai.array([0.0], device=on_device)
    arr1 = startai.array([1.0], device=on_device)
    arr2 = startai.array([2.0], device=on_device)
    ...
    sub_struc = Container(
        {
            "c": startai.array([3.0], device=on_device),
            "d": startai.array([4.0], device=on_device),
        }
    )
    try:
    ...
    partial_sub_struc = Container({"b": {"d": startai.array([4.0], device=on_device)}})
...
def test_container_assert_identical(on_device):
    ...
    arr1 = startai.array([1], device=on_device)
    arr2 = startai.array([2], device=on_device)
    arr3 = startai.array([3], device=on_device)
    container0 = Container({"a": arr1, "b": {"c": arr2, "d": arr3}})
    container1 = Container({"a": arr1, "b": {"c": arr2, "d": arr3}})
    container2 = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": {
                "c": startai.array([2], device=on_device),
                "d": startai.array([3], device=on_device),
            },
        }
    )
    ...
    # the same
    startai.Container.cont_assert_identical([container0, container1])
    startai.Container.cont_assert_identical([container1, container0])

    # not the same
    try:
        startai.Container.cont_assert_identical([container0, container2])
        error_caught = False
    except StartaiException:
        error_caught = True
    assert error_caught
    try:
        startai.Container.cont_assert_identical([container1, container2])
        error_caught = False
    except StartaiException:
        error_caught = True
    assert error_caught
    ...
    # partial
    startai.Container.cont_assert_identical([container0, container3], partial=True)
    startai.Container.cont_assert_identical([container3, container0], partial=True)
    try:
        startai.Container.cont_assert_identical([container4, container0], partial=True)
        error_caught = False
    except StartaiException:
        error_caught = True
    assert error_caught
...
def test_container_assert_identical_structure(on_device):
    ...
    container1 = Container(
        {
            "a": startai.array([3], device=on_device),
            "b": {
                "c": startai.array([4], device=on_device),
                "d": startai.array([5], device=on_device),
            },
        }
    )
    container2 = Container(
        {
            ...
            "b": {
                "c": startai.array([4], device=on_device),
                "d": startai.array([5], device=on_device),
                "e": startai.array([6], device=on_device),
            },
        }
    )
    ...
    container4 = Container({"b": {"d": startai.array([4], device=on_device)}})
    container5 = Container({"d": startai.array([4], device=on_device)})

    # with identical
    startai.Container.cont_assert_identical_structure([container0, container1])
    startai.Container.cont_assert_identical_structure([container1, container0])
    startai.Container.cont_assert_identical_structure([container1, container0, container1])

    # without identical
    try:
        startai.Container.cont_assert_identical_structure(
...
def test_container_at_key_chain(on_device):
    ...
    assert np.allclose(startai.to_numpy(sub_container["c"]), np.array([2]))
    sub_container = container.cont_at_key_chain("b/c")
    assert np.allclose(startai.to_numpy(sub_container), np.array([2]))
...
def test_container_at_key_chains(on_device):
    ...
    assert np.allclose(startai.to_numpy(new_container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(new_container["b"]["c"]), np.array([2]))
    assert "d" not in new_container["b"]
    new_container = container.cont_at_key_chains(["b/c", "b/d"])
    assert "a" not in new_container
    assert np.allclose(startai.to_numpy(new_container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(new_container["b"]["d"]), np.array([3]))
...
def test_container_combine(on_device):
    ...
    container_1 = Container(
        {
            "a": startai.array([4], device=on_device),
            "b": {
                "c": startai.array([5], device=on_device),
                "e": startai.array([6], device=on_device),
            },
        }
    )
    container_comb = startai.Container.cont_combine(container_0, container_1)
    assert np.equal(startai.to_numpy(container_comb.a), np.array([4]))
    assert np.equal(startai.to_numpy(container_comb.b.c), np.array([5]))
    assert np.equal(startai.to_numpy(container_comb.b.d), np.array([3]))
    assert np.equal(startai.to_numpy(container_comb.b.e), np.array([6]))
...
def test_container_cont_inplace_update(on_device):
    container0 = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": {
                "c": startai.array([1], device=on_device),
                "d": startai.array([2], device=on_device),
            },
        }
    )
    id0 = id(container0)
    container1 = Container(
        {
            "a": startai.array([0], device=on_device),
            "b": {
                "c": startai.array([0], device=on_device),
                "d": startai.array([0], device=on_device),
            },
        }
    )
    id1 = id(container1)
    assert startai.Container.cont_all_false(container0.all_equal(container1))
    container0.inplace_update(container1)
    assert id0 == id(container0)
    assert id1 == id(container1)
    assert startai.Container.cont_all_true(container0.all_equal(container1))
...
def test_container_copy(on_device):
    dict_in = {
        "a": startai.array([0.0], device=on_device),
        "b": {
            "c": startai.array([1.0], device=on_device),
            "d": startai.array([2.0], device=on_device),
        },
    }
    cont = Container(dict_in)
    cont_deepcopy = cont.cont_copy()
    assert np.allclose(startai.to_numpy(cont.a), startai.to_numpy(cont_deepcopy.a))
    assert np.allclose(startai.to_numpy(cont.b.c), startai.to_numpy(cont_deepcopy.b.c))
    assert np.allclose(startai.to_numpy(cont.b.d), startai.to_numpy(cont_deepcopy.b.d))
...
def test_container_create_if_absent(on_device):
    dict_in = {
        "a": startai.array([[[1.0], [2.0], [3.0]]], device=on_device),
        "b": {
            "c": startai.array([[[2.0], [4.0], [6.0]]], device=on_device),
            "d": startai.array([[[3.0], [6.0], [9.0]]], device=on_device),
        },
    }
    ...
    assert np.allclose(startai.to_numpy(container.a), np.array([[[1.0], [2.0], [3.0]]]))
    container.cont_create_if_absent("e", startai.array([[[4.0], [8.0], [12.0]]]), True)
    assert np.allclose(startai.to_numpy(container.e), np.array([[[4.0], [8.0], [12.0]]]))

    # depth 2
    container.cont_create_if_absent("f/g", np.array([[[5.0], [10.0], [15.0]]]), True)
    assert np.allclose(startai.to_numpy(container.f.g), np.array([[[5.0], [10.0], [15.0]]]))


@pytest.mark.parametrize("inplace", [True, False])
def test_container_cutoff_at_depth(inplace, on_device):
    # values
    a_val = startai.array([1], device=on_device)
    bcde_val = startai.array([2], device=on_device)
    ...
    if inplace:
        cont_cutoff = cont
    assert np.allclose(startai.to_numpy(cont_cutoff.a), startai.to_numpy(a_val))
    ...
    if inplace:
        cont_cutoff = cont
    assert np.allclose(startai.to_numpy(cont_cutoff.a), startai.to_numpy(a_val))
    assert np.allclose(startai.to_numpy(cont_cutoff.b.c.d.e), startai.to_numpy(bcde_val))


@pytest.mark.parametrize("inplace", [True, False])
def test_container_cutoff_at_height(inplace, on_device):
    # values
    d_val = startai.array([2], device=on_device)
    e_val = startai.array([3], device=on_device)
    ...
    if inplace:
        cont_cutoff = cont
    assert np.allclose(startai.to_numpy(cont_cutoff.a.c.d), startai.to_numpy(d_val))
    assert np.allclose(startai.to_numpy(cont_cutoff.b.c.d.e), startai.to_numpy(e_val))
...
def test_container_depth(on_device):
    cont_depth1 = Container(
        {"a": startai.array([1], device=on_device), "b": startai.array([2], device=on_device)}
    )
    ...
    cont_depth3 = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": {
                "c": {"d": startai.array([2], device=on_device)},
                "e": startai.array([3], device=on_device),
            },
        }
    )
    ...
    cont_depth4 = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": {"c": {"d": {"e": startai.array([2], device=on_device)}}},
        }
    )
...
def test_container_diff(on_device):
    ...
    container_1 = Container(
        {
            ...
            "b": {
                "c": startai.array([5], device=on_device),
                "d": startai.array([6], device=on_device),
            },
        }
    )
    container_diff = startai.Container.cont_diff(container_0, container_1)
    assert np.equal(startai.to_numpy(container_diff.a.diff_0), np.array([1]))
    assert np.equal(startai.to_numpy(container_diff.a.diff_1), np.array([4]))
    assert np.equal(startai.to_numpy(container_diff.b.c.diff_0), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.c.diff_1), np.array([5]))
    assert np.equal(startai.to_numpy(container_diff.b.d.diff_0), np.array([3]))
    assert np.equal(startai.to_numpy(container_diff.b.d.diff_1), np.array([6]))
    container_diff_diff_only = startai.Container.cont_diff(
        container_0, container_1, mode="diff_only"
    )
    assert container_diff_diff_only.cont_to_dict() == container_diff.cont_to_dict()
    container_diff_same_only = startai.Container.cont_diff(
    ...
    container_diff = startai.Container.cont_diff(container_0, container_1)
    assert np.equal(startai.to_numpy(container_diff.a), np.array([1]))
    assert np.equal(startai.to_numpy(container_diff.b.c.diff_0), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.c.diff_1), np.array([5]))
    assert np.equal(startai.to_numpy(container_diff.b.d), np.array([3]))
    container_diff_diff_only = startai.Container.cont_diff(
        container_0, container_1, mode="diff_only"
    )
    ...
    container_1 = Container(
        {
            "e": startai.array([1], device=on_device),
            "f": {
                "g": startai.array([2], device=on_device),
                "h": startai.array([3], device=on_device),
            },
        }
    )
    container_diff = startai.Container.cont_diff(container_0, container_1)
    assert np.equal(startai.to_numpy(container_diff.a.diff_0), np.array([1]))
    assert np.equal(startai.to_numpy(container_diff.b.diff_0.c), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.diff_0.d), np.array([3]))
    assert np.equal(startai.to_numpy(container_diff.e.diff_1), np.array([1]))
    assert np.equal(startai.to_numpy(container_diff.f.diff_1.g), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.f.diff_1.h), np.array([3]))
    container_diff_diff_only = startai.Container.cont_diff(
        container_0, container_1, mode="diff_only"
    )
    assert container_diff_diff_only.cont_to_dict() == container_diff.cont_to_dict()
    ...
    container_diff = startai.Container.cont_diff(container_0, container_1)
    assert np.equal(startai.to_numpy(container_diff.a), np.array([1]))
    assert np.equal(startai.to_numpy(container_diff.b.c), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.d.diff_0), np.array([3]))
    assert np.equal(startai.to_numpy(container_diff.b.e.diff_1), np.array([3]))
    container_diff_diff_only = startai.Container.cont_diff(
        container_0, container_1, mode="diff_only"
    )
    assert "a" not in container_diff_diff_only
...
def test_container_duplicate_array_keychains(on_device):
    ...
    res = startai.Container.cont_duplicate_array_keychains(container0)
    assert res == (("a", "b/c"),)
    res = startai.Container.cont_duplicate_array_keychains(container1)
    assert res == ()
...
def test_container_find_sub_structure(on_device):
    ...
    # full
    sub_cont = Container(
        {"c": startai.array([4], device=on_device), "d": startai.array([5], device=on_device)}
    )
    ...
    # partial
    partial_sub_cont = Container({"d": startai.array([5], device=on_device)})
    found_kc = top_cont.cont_find_sub_structure(partial_sub_cont, partial=True)
    assert found_kc == "b"
    partial_sub_cont = Container({"b": {"d": startai.array([5], device=on_device)}})
    found_kc = top_cont.cont_find_sub_structure(partial_sub_cont, partial=True)
    assert found_kc == ""
...
def test_container_flatten_key_chains(on_device):
    container = Container(
        {
            ...
            "b": {
                "c": {"d": startai.array([2], device=on_device)},
                "e": {"f": {"g": startai.array([3], device=on_device)}},
            },
        }
    )
    ...
    assert np.allclose(startai.to_numpy(container_flat["a"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat.a), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat["b__c__d"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat.b__c__d), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat["b__e__f__g"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_flat.b__e__f__g), np.array([[3]]))
    ...
    assert np.allclose(startai.to_numpy(container_flat["a"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat.a), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat["b__c"]["d"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat.b__c.d), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat["b__e__f"]["g"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_flat.b__e__f.g), np.array([[3]]))
    ...
    assert np.allclose(startai.to_numpy(container_flat["a"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat.a), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat["b"]["c__d"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat.b.c__d), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat["b"]["e__f__g"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_flat.b.e__f__g), np.array([[3]]))
    ...
    assert np.allclose(startai.to_numpy(container_flat["a"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat.a), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_flat["b"]["c"]["d"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat.b.c.d), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_flat["b"]["e__f"]["g"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_flat.b.e__f.g), np.array([[3]]))


def test_container_format_key_chains(on_device):
    dict_in = {
        "_a": startai.array([1], device=on_device),
        "b ": {
            "c": startai.array([2], device=on_device),
            "d-": startai.array([3], device=on_device),
        },
    ...
    assert np.allclose(startai.to_numpy(cont_formatted["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(cont_formatted.a), np.array([1]))
    assert np.allclose(startai.to_numpy(cont_formatted["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(cont_formatted.b.c), np.array([2]))
    assert np.allclose(startai.to_numpy(cont_formatted["b"]["d"]), np.array([3]))
    assert np.allclose(startai.to_numpy(cont_formatted.b.d), np.array([3]))
...
def test_container_from_dict(on_device):
    ...
    container = Container(dict_in)
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container.a), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container.b.c), np.array([2]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([3]))
    assert np.allclose(startai.to_numpy(container.b.d), np.array([3]))
...
def test_container_from_flat_list(on_device):
    ...
    assert np.allclose(startai.to_numpy(container["a"]), np.array([4]))
    assert np.allclose(startai.to_numpy(container.a), np.array([4]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([5]))
    assert np.allclose(startai.to_numpy(container.b.c), np.array([5]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([6]))
    assert np.allclose(startai.to_numpy(container.b.d), np.array([6]))
...
def test_container_from_list(on_device):
    list_in = [
        startai.array([1], device=on_device),
        [startai.array([2], device=on_device), startai.array([3], device=on_device)],
    ]
    container = Container(list_in, types_to_iteratively_nest=[list])
    assert np.allclose(startai.to_numpy(container["it_0"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container.it_0), np.array([1]))
    assert np.allclose(startai.to_numpy(container["it_1"]["it_0"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container.it_1.it_0), np.array([2]))
    assert np.allclose(startai.to_numpy(container["it_1"]["it_1"]), np.array([3]))
    assert np.allclose(startai.to_numpy(container.it_1.it_1), np.array([3]))
...
def test_container_from_queues(on_device):
    ...
    if startai.gpu_is_available() and startai.current_backend_str() == "jax":
    ...
    def worker_fn(in_queue, out_queue, load_size, worker_id):
        ...
        while keep_going:
            ...
            out_queue.put(
                {
                    "a": [
                        startai.to_native(startai.array([1.0, 2.0, 3.0], device=on_device))
                        * worker_id
                    ]
                    * load_size
                }
    ...
    assert np.allclose(startai.to_numpy(container[0].a), np.array([1.0, 2.0, 3.0]))
    assert np.allclose(startai.to_numpy(container[0].a), np.array([1.0, 2.0, 3.0]))
    ...
    assert np.allclose(startai.to_numpy(container[1].a), np.array([2.0, 4.0, 6.0]))
    assert np.allclose(startai.to_numpy(container[1].a), np.array([2.0, 4.0, 6.0]))
    assert np.allclose(startai.to_numpy(container[2].a), np.array([2.0, 4.0, 6.0]))
    assert np.allclose(startai.to_numpy(container[2].a), np.array([2.0, 4.0, 6.0]))
    ...
    assert np.allclose(startai.to_numpy(container[3].a), np.array([3.0, 6.0, 9.0]))
    assert np.allclose(startai.to_numpy(container[3].a), np.array([3.0, 6.0, 9.0]))
...
def test_container_identical(on_device):
    ...
    # the same
    assert startai.Container.cont_identical([container0, container1])
    assert startai.Container.cont_identical([container1, container0])

    # not the same
    assert not startai.Container.cont_identical([container0, container2])
    assert not startai.Container.cont_identical([container2, container0])
    assert not startai.Container.cont_identical([container1, container2])
    assert not startai.Container.cont_identical([container2, container1])

    # partial
    assert startai.Container.cont_identical([container0, container3], partial=True)
    assert startai.Container.cont_identical([container3, container0], partial=True)
    assert not startai.Container.cont_identical([container0, container4], partial=True)
    assert not startai.Container.cont_identical([container4, container0], partial=True)
...
def test_container_identical_array_shapes(on_device):
    ...
    container0 = Container(
        {
            "a": startai.array([1, 2], device=on_device),
            "b": {
                "c": startai.array([2, 3, 4], device=on_device),
                "d": startai.array([3, 4, 5, 6], device=on_device),
            },
        }
    )
    container1 = Container(
        {
            "a": startai.array([1, 2, 3, 4], device=on_device),
            "b": {
                "c": startai.array([3, 4], device=on_device),
                "d": startai.array([3, 4, 5], device=on_device),
            },
        }
    )
    container2 = Container(
    ...
    # with identical
    assert startai.Container.cont_identical_array_shapes([container0, container1])
    assert startai.Container.cont_identical_array_shapes([container1, container0])
    assert startai.Container.cont_identical_array_shapes(
        [container1, container0, container1]
    )
    assert not startai.Container.cont_identical([container0, container2])
    assert not startai.Container.cont_identical([container1, container2])
    assert not startai.Container.cont_identical([container0, container1, container2])


def test_container_identical_configs(on_device):
    container0 = Container({"a": startai.array([1], device=on_device)}, print_limit=5)
    container1 = Container({"a": startai.array([1], device=on_device)}, print_limit=5)
    container2 = Container({"a": startai.array([1], device=on_device)}, print_limit=10)

    # with identical
    assert startai.Container.cont_identical_configs([container0, container1])
    assert startai.Container.cont_identical_configs([container1, container0])
    assert startai.Container.cont_identical_configs([container1, container0, container1])

    # without identical
    assert not startai.Container.cont_identical_configs([container1, container2])
    assert not startai.Container.cont_identical_configs(
...
def test_container_identical_structure(on_device):
    ...
    # with identical
    assert startai.Container.cont_identical_structure([container0, container1])
    assert startai.Container.cont_identical_structure([container1, container0])
    assert startai.Container.cont_identical_structure([container1, container0, container1])

    # without identical
    assert not startai.Container.cont_identical_structure([container2, container3])
    assert not startai.Container.cont_identical_structure([container0, container3])
    assert not startai.Container.cont_identical_structure([container1, container2])
    assert not startai.Container.cont_identical_structure(
        [container1, container0, container2]
    )

    # partial
    assert startai.Container.cont_identical_structure(
...
def test_container_if_exists(on_device):
    ...
    container = Container(dict_in)
    assert np.allclose(
        startai.to_numpy(container.cont_if_exists("a")), np.array([[[1.0], [2.0], [3.0]]])
    )
    assert "c" not in container
    assert container.cont_if_exists("c") is None
    container["c"] = startai.array([[[1.0], [2.0], [3.0]]], device=on_device)
    assert np.allclose(
        startai.to_numpy(container.cont_if_exists("c")), np.array([[[1.0], [2.0], [3.0]]])
    )
    assert container.cont_if_exists("d") is None
    container.d = startai.array([[[1.0], [2.0], [3.0]]], device=on_device)
    assert np.allclose(
        startai.to_numpy(container.cont_if_exists("d")), np.array([[[1.0], [2.0], [3.0]]])
    )
...
def test_container_inplace(on_device):
    ...
    const = 3
    arr = startai.array([1], device=on_device)
    container1 = Container(
        {
            "a": startai.array([3], device=on_device),
            "b": {
...
def test_container_list_join(on_device):
    ...
    container_list_joined = startai.Container.cont_list_join([container_0, container_1])
    assert np.allclose(startai.to_numpy(container_list_joined["a"][0]), np.array([1]))
    assert np.allclose(startai.to_numpy(container_list_joined.a[0]), np.array([1]))
    assert np.allclose(startai.to_numpy(container_list_joined["b"]["c"][0]), np.array([2]))
    assert np.allclose(startai.to_numpy(container_list_joined.b.c[0]), np.array([2]))
    assert np.allclose(startai.to_numpy(container_list_joined["b"]["d"][0]), np.array([3]))
    assert np.allclose(startai.to_numpy(container_list_joined.b.d[0]), np.array([3]))
    assert np.allclose(startai.to_numpy(container_list_joined["a"][1]), np.array([4]))
    assert np.allclose(startai.to_numpy(container_list_joined.a[1]), np.array([4]))
    assert np.allclose(startai.to_numpy(container_list_joined["b"]["c"][1]), np.array([5]))
    assert np.allclose(startai.to_numpy(container_list_joined.b.c[1]), np.array([5]))
    assert np.allclose(startai.to_numpy(container_list_joined["b"]["d"][1]), np.array([6]))
    assert np.allclose(startai.to_numpy(container_list_joined.b.d[1]), np.array([6]))
...
def test_container_list_stack(on_device):
    ...
    container_list_stacked = startai.Container.cont_list_stack(
        [container_0, container_1], 0
    )
    assert np.allclose(startai.to_numpy(container_list_stacked["a"][0]), np.array([1]))
    assert np.allclose(startai.to_numpy(container_list_stacked.a[0]), np.array([1]))
    assert np.allclose(startai.to_numpy(container_list_stacked["b"]["c"][0]), np.array([2]))
    assert np.allclose(startai.to_numpy(container_list_stacked.b.c[0]), np.array([2]))
    assert np.allclose(startai.to_numpy(container_list_stacked["b"]["d"][0]), np.array([3]))
    assert np.allclose(startai.to_numpy(container_list_stacked.b.d[0]), np.array([3]))
    assert np.allclose(startai.to_numpy(container_list_stacked["a"][1]), np.array([4]))
    assert np.allclose(startai.to_numpy(container_list_stacked.a[1]), np.array([4]))
    assert np.allclose(startai.to_numpy(container_list_stacked["b"]["c"][1]), np.array([5]))
    assert np.allclose(startai.to_numpy(container_list_stacked.b.c[1]), np.array([5]))
    assert np.allclose(startai.to_numpy(container_list_stacked["b"]["d"][1]), np.array([6]))
    assert np.allclose(startai.to_numpy(container_list_stacked.b.d[1]), np.array([6]))
...
def test_container_map(inplace, on_device):
    ...
    for (key, value), expected_value in zip(
        container_iterator,
        [
            startai.array([2], device=on_device),
            startai.array([3], device=on_device),
            startai.array([4], device=on_device),
        ],
    ):
        assert startai.to_numpy(value) == startai.to_numpy(expected_value)
    ...
    assert np.allclose(startai.to_numpy(container_mapped["a"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_mapped.a), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_mapped["b"]["c"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_mapped.b.c), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_mapped["b"]["d"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_mapped.b.d), np.array([[3]]))
    ...
    if inplace:
        container_mapped = container
    assert np.allclose(startai.to_numpy(container_mapped["a"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_mapped.a), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_mapped["b"]["c"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_mapped.b.c), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_mapped["b"]["d"]), np.array([[3]]))
    ...
    container_orig = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": [startai.array([2], device=on_device), startai.array([3], device=on_device)],
        }
    )
    container = container_orig.cont_deep_copy()
    container_mapped = container.cont_map(
    ...
    if inplace:
        container_mapped = container
    assert np.allclose(startai.to_numpy(container_mapped["a"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container_mapped["b"][0]), np.array([3]))
    assert np.allclose(startai.to_numpy(container_mapped["b"][1]), np.array([4]))
...
def test_container_map_sub_conts(inplace, on_device):
    ...
    def _add_e_attr(cont_in):
        cont_in.e = startai.array([4], device=on_device)
        return cont_in
    ...
    assert np.array_equal(startai.to_numpy(container_mapped.e), np.array([4]))
    assert "e" in container_mapped.b
    assert np.array_equal(startai.to_numpy(container_mapped.b.e), np.array([4]))
...
def test_container_multi_map(on_device):
    ...
    # with key_chains to apply
    container_mapped = startai.Container.cont_multi_map(
        lambda x, _: x[0] + x[1], [container0, container1], assert_identical=True
    )
    assert np.allclose(startai.to_numpy(container_mapped["a"]), np.array([[4]]))
    assert np.allclose(startai.to_numpy(container_mapped.a), np.array([[4]]))
    assert np.allclose(startai.to_numpy(container_mapped["b"]["c"]), np.array([[6]]))
    assert np.allclose(startai.to_numpy(container_mapped.b.c), np.array([[6]]))
    assert np.allclose(startai.to_numpy(container_mapped["b"]["d"]), np.array([[8]]))
    assert np.allclose(startai.to_numpy(container_mapped.b.d), np.array([[8]]))
    ...
    container1 = Container(
        {
            ...
            "b": [
                startai.array([4], device=on_device),
                startai.array([5], device=on_device),
            ],
        }
    )
    ...
    assert np.allclose(startai.to_numpy(container_mapped["a"]), np.array([4]))
    assert np.allclose(startai.to_numpy(container_mapped["b"][0]), np.array([6]))
    assert np.allclose(startai.to_numpy(container_mapped["b"][1]), np.array([8]))

    # Non identical containers
    a = startai.Container(a={"b": 2, "c": 4}, d={"e": 6, "f": 9})
    b = startai.Container(a=2, d=3)
    container_mapped = startai.Container.cont_multi_map(lambda xs, _: xs[0] / xs[1], [a, b])

    assert np.allclose(startai.to_numpy(container_mapped["a"].b), 1)
    assert np.allclose(startai.to_numpy(container_mapped["a"]["c"]), 2)
    assert np.allclose(startai.to_numpy(container_mapped.d.e), 2)
    assert np.allclose(startai.to_numpy(container_mapped["d"].f), 3)


def test_container_num_arrays(on_device):
    dict_in = {
        "a": startai.array([[0.0, 1.0, 2.0, 3.0]], device=on_device),
        "b": {
            "c": startai.array([[5.0, 10.0, 15.0, 20.0]], device=on_device),
            "d": startai.array([[10.0, 9.0, 8.0, 7.0]], device=on_device),
        },
    }
    container = Container(dict_in)
    ...
    dict_in = {
        "a": startai.array([[0.0, 1.0, 2.0, 3.0]], device=on_device),
        "b": {
            "c": _variable(startai.array([[5.0, 10.0, 15.0, 20.0]], device=on_device)),
            "d": startai.array([[10.0, 9.0, 8.0, 7.0]], device=on_device),
        },
    }
    container = Container(dict_in)
    assert (
        ...
        if startai.current_backend_str() in ("numpy", "jax")
        else 2
    )
...
def test_container_overwrite_at_key_chain(on_device):
    ...
    # noinspection PyBroadException
    try:
        container.cont_overwrite_at_key_chain("b/e", startai.array([4], device=on_device))
        exception_raised = False
    except Exception:
        exception_raised = True
    assert exception_raised
    container = container.cont_overwrite_at_key_chain(
        "b/d", startai.array([4], device=on_device)
    )
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([4]))
...
def test_container_overwrite_at_key_chains(on_device):
    ...
    target_container = Container(
        {
            "a": startai.array([4], device=on_device),
            "b": {"d": startai.array([5], device=on_device)},
        }
    )
    new_container = container.cont_overwrite_at_key_chains(
        target_container, inplace=False
    )
    assert np.allclose(startai.to_numpy(new_container["a"]), np.array([4]))
    assert np.allclose(startai.to_numpy(new_container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(new_container["b"]["d"]), np.array([5]))
    target_container = Container({"b": {"c": startai.array([7], device=on_device)}})
    new_container = container.cont_overwrite_at_key_chains(
        target_container, inplace=False
    )
    assert np.allclose(startai.to_numpy(new_container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(new_container["b"]["c"]), np.array([7]))
    assert np.allclose(startai.to_numpy(new_container["b"]["d"]), np.array([3]))
    # noinspection PyBroadException
    try:
        container.cont_overwrite_at_key_chains(
            Container({"b": {"e": startai.array([5], device=on_device)}})
        )
        exception_raised = False
    except Exception:
        exception_raised = True
...
def test_container_pickle(on_device):
    dict_in = {
        "a": startai.array([np.float32(1.0)], device=on_device),
        "b": {
            "c": startai.array([np.float32(2.0)], device=on_device),
            "d": startai.array([np.float32(3.0)], device=on_device),
        },
    }
    ...
    if startai.backend == "paddle":
        cont = cont.to_numpy()

    assert cont._local_startai is None
    pickled = pickle.dumps(cont)
    cont_again = pickle.loads(pickled)
    assert cont_again._local_startai is None
    startai.Container.cont_identical_structure([cont, cont_again])
    startai.Container.cont_identical_configs([cont, cont_again])

    # with module attribute
    cont = Container(dict_in, startaih=startai)
    ...
    assert cont._local_startai is startai
    pickled = pickle.dumps(cont)
    cont_again = pickle.loads(pickled)
    # noinspection PyUnresolvedReferences
    assert cont_again._local_startai.current_backend_str() is startai.current_backend_str()
    startai.Container.cont_identical_structure([cont, cont_again])
    startai.Container.cont_identical_configs([cont, cont_again])
...
def test_container_prune_empty(on_device):
    dict_in = {
        "a": startai.array([1], device=on_device),
        "b": {"c": {}, "d": startai.array([3], device=on_device)},
    }
    container = Container(dict_in)
    container_pruned = container.cont_prune_empty()
    assert np.allclose(startai.to_numpy(container_pruned["a"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_pruned.a), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_pruned["b"]["d"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_pruned.b.d), np.array([[3]]))
...
def test_container_prune_key_chain(on_device):
    dict_in = {
        "a": startai.array([1], device=on_device),
        "b": {"c": startai.array([2], device=on_device), "d": None},
    }
    container = Container(dict_in)
...
def test_container_prune_key_from_key_chains(on_device):
    container = Container(
        {
            "Ayy": startai.array([1], device=on_device),
            "Bee": {
                "Cee": startai.array([2], device=on_device),
                "Dee": startai.array([3], device=on_device),
            },
            "Beh": {
                "Ceh": startai.array([4], device=on_device),
                "Deh": startai.array([5], device=on_device),
            },
        }
    )
    ...
    assert np.allclose(startai.to_numpy(container_pruned["Ayy"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_pruned.Ayy), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_pruned["Cee"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_pruned.Cee), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_pruned["Dee"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_pruned.Dee), np.array([[3]]))
    ...
    assert np.allclose(startai.to_numpy(container_pruned["Dee"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_pruned.Dee), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_pruned["Ceh"]), np.array([[4]]))
    assert np.allclose(startai.to_numpy(container_pruned.Ceh), np.array([[4]]))
    assert np.allclose(startai.to_numpy(container_pruned["Deh"]), np.array([[5]]))
    assert np.allclose(startai.to_numpy(container_pruned.Deh), np.array([[5]]))
    assert "Bee" not in container_pruned
...
def test_container_prune_keys(on_device):
    ...
    assert "a" not in container_pruned
    assert np.allclose(startai.to_numpy(container_pruned["b"]["c"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_pruned.b.c), np.array([[2]]))
...
def test_container_prune_keys_from_key_chains(on_device):
    container = Container(
        {
            ...
            "Eee": {"Fff": startai.array([4], device=on_device)},
        }
    )
    ...
    assert np.allclose(startai.to_numpy(container_pruned["Dee"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_pruned.Dee), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_pruned["Fff"]), np.array([[4]]))
    assert np.allclose(startai.to_numpy(container_pruned.Fff), np.array([[4]]))
    assert "Bee" not in container_pruned
...
def test_container_reduce(on_device):
    ...
    container_b = startai.Container(
        {
            "a": startai.array([2], device=on_device),
            "b": {
                "c": startai.array([4], device=on_device),
                "d": startai.array([6], device=on_device),
            },
    ...
    res = startai.Container.cont_reduce([container_a, container_b], lambda x: x[0] + x[1])
    assert np.allclose(startai.to_numpy(res.a), np.array([3.0]))
    assert np.allclose(startai.to_numpy(res.b.c), np.array([6]))
    assert np.allclose(startai.to_numpy(res.b.d), np.array([9]))
...
def test_container_reshape_like(on_device):
    container = Container(
        {
            "a": startai.array([[1.0]], device=on_device),
            "b": {
                "c": startai.array([[3.0], [4.0]], device=on_device),
                "d": startai.array([[5.0], [6.0], [7.0]], device=on_device),
            },
        }
    )
    ...
    container = Container(
        {
            "a": startai.array([[[1.0]], [[1.0]], [[1.0]]], device=on_device),
            "b": {
                "c": startai.array(
...
def test_container_restructure(on_device):
    container = Container(
        {
            "a": startai.array([[1, 2], [3, 4]], device=on_device),
            "b": {
                "c": startai.array([[2, 4], [6, 8]], device=on_device),
                "d": startai.array([3, 6, 9, 12], device=on_device),
            },
        }
    )
    ...
    assert np.allclose(
        startai.to_numpy(container_restructured["A"]), np.array([[1, 3], [2, 4]])
    )
    assert np.allclose(
        startai.to_numpy(container_restructured.A), np.array([[1, 3], [2, 4]])
    )
    assert np.allclose(
        startai.to_numpy(container_restructured["B/C"]), np.array([2, 4, 6, 8])
    )
    assert np.allclose(startai.to_numpy(container_restructured.B.C), np.array([2, 4, 6, 8]))
    assert np.allclose(
        startai.to_numpy(container_restructured["B/D"]), np.array([[3, 6], [9, 12]])
    )
    assert np.allclose(
        startai.to_numpy(container_restructured.B.D), np.array([[3, 6], [9, 12]])
    )
...
def test_container_restructure_key_chains(on_device):
    ...
    assert np.allclose(startai.to_numpy(container_restructured["A"]), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_restructured.A), np.array([[1]]))
    assert np.allclose(startai.to_numpy(container_restructured["b/c"]), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_restructured.b.c), np.array([[2]]))
    assert np.allclose(startai.to_numpy(container_restructured["b/d"]), np.array([[3]]))
    assert np.allclose(startai.to_numpy(container_restructured.b.d), np.array([[3]]))
...
def test_container_set_at_key_chain(on_device):
    ...
    # explicit function call
    container = container_orig.cont_copy()
    container = container.cont_set_at_key_chain("b/e", startai.array([4], device=on_device))
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([3]))
    assert np.allclose(startai.to_numpy(container["b"]["e"]), np.array([4]))
    container = container.cont_set_at_key_chain("f", startai.array([5], device=on_device))
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([3]))
    assert np.allclose(startai.to_numpy(container["b"]["e"]), np.array([4]))
    assert np.allclose(startai.to_numpy(container["f"]), np.array([5]))

    # overridden built-in function call
    container = container_orig.cont_copy()
    assert "b/e" not in container
    container["b/e"] = startai.array([4], device=on_device)
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([3]))
    assert np.allclose(startai.to_numpy(container["b"]["e"]), np.array([4]))
    assert "f" not in container
    container["f"] = startai.array([5], device=on_device)
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([2]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([3]))
    assert np.allclose(startai.to_numpy(container["b"]["e"]), np.array([4]))
...
def test_container_set_at_keys(on_device):
    ...
    container = orig_container.cont_set_at_keys({"b": startai.array([4], device=on_device)})
    assert np.allclose(startai.to_numpy(container["a"]), np.array([1]))
    assert np.allclose(startai.to_numpy(container["b"]), np.array([4]))
    ...
    container = orig_container.cont_set_at_keys(
        {"a": startai.array([5], device=on_device), "c": startai.array([6], device=on_device)}
    )
    assert np.allclose(startai.to_numpy(container["a"]), np.array([5]))
    assert np.allclose(startai.to_numpy(container["b"]["c"]), np.array([6]))
    assert np.allclose(startai.to_numpy(container["b"]["d"]), np.array([3]))
...
def test_container_shapes(on_device):
    dict_in = {
        "a": startai.array([[[1.0], [2.0], [3.0]]], device=on_device),
        "b": {
            "c": startai.array([[[2.0], [4.0]]], device=on_device),
            "d": startai.array([[9.0]], device=on_device),
        },
    }
...
def test_container_size_ordered_arrays(on_device):
    dict_in = {
        "a": startai.array([[0.0, 1.0, 2.0, 3.0]], device=on_device),
        "b": {
            "c": startai.array([[5.0, 10.0]], device=on_device),
            "d": startai.array([[10.0, 9.0, 8.0]], device=on_device),
        },
    }
    container = Container(dict_in)
    size_ordered = container.cont_size_ordered_arrays()
    assert np.allclose(startai.to_numpy(size_ordered.a), np.array([[0.0, 1.0, 2.0, 3.0]]))
    assert np.allclose(startai.to_numpy(size_ordered.b__c), np.array([[5.0, 10.0]]))
    assert np.allclose(startai.to_numpy(size_ordered.b__d), np.array([[10.0, 9.0, 8.0]]))
    for v, arr in zip(
        size_ordered.values(),
        [
            np.array([[5.0, 10.0]]),
        ...
        assert np.allclose(startai.to_numpy(v), arr)


def test_container_slice(on_device):
    dict_in = {
        "a": startai.array([[0.0], [1.0]], device=on_device),
        "b": {
            "c": startai.array([[1.0], [2.0]], device=on_device),
            "d": startai.array([[2.0], [3.0]], device=on_device),
        },
    }
    container = Container(dict_in)
    container0 = container[0]
    container1 = container[1]
    assert np.array_equal(startai.to_numpy(container0["a"]), np.array([0.0]))
    assert np.array_equal(startai.to_numpy(container0.a), np.array([0.0]))
    assert np.array_equal(startai.to_numpy(container0["b"]["c"]), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(container0.b.c), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(container0["b"]["d"]), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(container0.b.d), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(container1["a"]), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(container1.a), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(container1["b"]["c"]), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(container1.b.c), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(container1["b"]["d"]), np.array([3.0]))
    assert np.array_equal(startai.to_numpy(container1.b.d), np.array([3.0]))
...
def test_container_slice_keys(str_slice, on_device):
    # values
    a_val = startai.array([1], device=on_device)
    b_val = startai.array([2], device=on_device)
    c_val = startai.array([3], device=on_device)
    d_val = startai.array([4], device=on_device)
    e_val = startai.array([5], device=on_device)
    ...
    assert np.allclose(startai.to_numpy(cont_sliced.b), startai.to_numpy(b_val))
    assert np.allclose(startai.to_numpy(cont_sliced.c), startai.to_numpy(c_val))
    assert np.allclose(startai.to_numpy(cont_sliced.d), startai.to_numpy(d_val))
...
def test_container_slice_via_key(on_device):
    dict_in = {
        "a": {
            "x": startai.array([0.0], device=on_device),
            "y": startai.array([1.0], device=on_device),
        },
        "b": {
            "c": {
                "x": startai.array([1.0], device=on_device),
                "y": startai.array([2.0], device=on_device),
            },
            "d": {
                "x": startai.array([2.0], device=on_device),
                "y": startai.array([3.0], device=on_device),
            },
        },
    }
    container = Container(dict_in)
    ...
    assert np.array_equal(startai.to_numpy(containerx["a"]), np.array([0.0]))
    assert np.array_equal(startai.to_numpy(containerx.a), np.array([0.0]))
    assert np.array_equal(startai.to_numpy(containerx["b"]["c"]), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(containerx.b.c), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(containerx["b"]["d"]), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(containerx.b.d), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(containery["a"]), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(containery.a), np.array([1.0]))
    assert np.array_equal(startai.to_numpy(containery["b"]["c"]), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(containery.b.c), np.array([2.0]))
    assert np.array_equal(startai.to_numpy(containery["b"]["d"]), np.array([3.0]))
    assert np.array_equal(startai.to_numpy(containery.b.d), np.array([3.0]))


def test_container_sort_by_key(on_device):
    dict_in = {
        "b": startai.array([1], device=on_device),
        "a": {
            "d": startai.array([2], device=on_device),
            "c": startai.array([3], device=on_device),
        },
    }
    container = Container(dict_in)
...
def test_container_split_conts(on_device):
    dict_in = {
        "a": startai.array([[1], [2], [3]], device=on_device),
        "b": {
            "c": startai.array([[2], [3], [4]], device=on_device),
            "d": startai.array([[3], [4], [5]], device=on_device),
        },
    ...
    for cont, a, bc, bd in zip(container_split, [1, 2, 3], [2, 3, 4], [3, 4, 5]):
        assert np.array_equal(startai.to_numpy(cont["a"])[0], np.array([a]))
        assert np.array_equal(startai.to_numpy(cont.a)[0], np.array([a]))
        assert np.array_equal(startai.to_numpy(cont["b"]["c"])[0], np.array([bc]))
        assert np.array_equal(startai.to_numpy(cont.b.c)[0], np.array([bc]))
        assert np.array_equal(startai.to_numpy(cont["b"]["d"])[0], np.array([bd]))
        assert np.array_equal(startai.to_numpy(cont.b.d)[0], np.array([bd]))
...
def test_container_structural_diff(on_device):
    ...
    container_diff = startai.Container.cont_structural_diff(container_0, container_1)
    assert np.equal(startai.to_numpy(container_diff.a.diff_0), np.array([1]))
    assert np.equal(startai.to_numpy(container_diff.a.diff_1), np.array([[4]]))
    assert np.equal(startai.to_numpy(container_diff.b.c.diff_0), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.c.diff_1), np.array([[[5]]]))
    assert np.equal(startai.to_numpy(container_diff.b.d.diff_0), np.array([3]))
    assert np.equal(startai.to_numpy(container_diff.b.e.diff_1), np.array([3]))
    container_diff_diff_only = startai.Container.cont_structural_diff(
        container_0, container_1, mode="diff_only"
    )
    assert container_diff_diff_only.cont_to_dict() == container_diff.cont_to_dict()
    container_diff_same_only = startai.Container.cont_structural_diff(
        container_0, container_1, mode="same_only"
    )
    assert container_diff_same_only.cont_to_dict() == {}
    ...
    container_1 = Container(
        {
            "e": startai.array([4], device=on_device),
            "f": {
                "g": startai.array([5], device=on_device),
                "h": startai.array([6], device=on_device),
            },
        }
    )
    container_diff = startai.Container.cont_structural_diff(container_0, container_1)
    ...
    assert np.equal(startai.to_numpy(container_diff.b.diff_0.c), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.diff_0.d), np.array([3]))
    assert np.equal(startai.to_numpy(container_diff.e.diff_1), np.array([4]))
    assert np.equal(startai.to_numpy(container_diff.f.diff_1.g), np.array([5]))
    assert np.equal(startai.to_numpy(container_diff.f.diff_1.h), np.array([6]))
    container_diff_diff_only = startai.Container.cont_structural_diff(
        container_0, container_1, mode="diff_only"
    )
    assert container_diff_diff_only.cont_to_dict() == container_diff.cont_to_dict()
    ...
    assert np.equal(startai.to_numpy(container_diff.b.c), np.array([2]))
    assert np.equal(startai.to_numpy(container_diff.b.d.diff_0), np.array([3]))
    assert np.equal(startai.to_numpy(container_diff.b.e.diff_1), np.array([6]))
    container_diff_diff_only = startai.Container.cont_structural_diff(
        container_0, container_1, mode="diff_only"
    )
    assert "a" not in container_diff_diff_only
...
def test_container_to_and_from_disk_as_hdf5(on_device):
    if startai.current_backend_str() == "tensorflow":
    ...
    dict_in_2 = {
        "a": startai.array([np.float32(1.0), np.float32(1.0)], device=on_device),
        "b": {
            "c": startai.array([np.float32(2.0), np.float32(2.0)], device=on_device),
            "d": startai.array([np.float32(3.0), np.float32(3.0)], device=on_device),
        },
    }
    ...
    assert np.array_equal(startai.to_numpy(loaded_container.a), startai.to_numpy(container1.a))
    assert np.array_equal(
        startai.to_numpy(loaded_container.b.c), startai.to_numpy(container1.b.c)
    )
    assert np.array_equal(
        startai.to_numpy(loaded_container.b.d), startai.to_numpy(container1.b.d)
    )
    ...
    assert np.array_equal(startai.to_numpy(loaded_container.a), startai.to_numpy(container2.a))
    assert np.array_equal(
        startai.to_numpy(loaded_container.b.c), startai.to_numpy(container2.b.c)
    )
    assert np.array_equal(
        startai.to_numpy(loaded_container.b.d), startai.to_numpy(container2.b.d)
    )
    ...
    assert np.array_equal(
        startai.to_numpy(loaded_sliced_container.a), startai.to_numpy(container1.a)
    )
    assert np.array_equal(
        startai.to_numpy(loaded_sliced_container.b.c), startai.to_numpy(container1.b.c)
    )
    assert np.array_equal(
        startai.to_numpy(loaded_sliced_container.b.d), startai.to_numpy(container1.b.d)
    )
...
def test_container_to_and_from_disk_as_json(on_device):
    ...
    dict_in = {
        "a": 1.274e-7,
        "b": {"c": True, "d": startai.array([np.float32(3.0)], device=on_device)},
    }
    container = Container(dict_in)
...
def test_container_to_and_from_disk_as_pickled(on_device):
    ...
    assert np.array_equal(startai.to_numpy(loaded_container.a), startai.to_numpy(container.a))
    assert np.array_equal(
        startai.to_numpy(loaded_container.b.c), startai.to_numpy(container.b.c)
    )
    assert np.array_equal(
        startai.to_numpy(loaded_container.b.d), startai.to_numpy(container.b.d)
    )
    ...
    container0 = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": {
                "c": startai.array([True], device=on_device),
                "d": {
                    "g": startai.array([2.0], device=on_device),
                    "h": startai.array([3], device=on_device),
                },
            },
        }
    )
    res = startai.Container.cont_to_dict(container0)
...
def test_container_to_disk_shuffle_and_from_disk_as_hdf5(on_device):
    ...
    dict_in = {
        "a": startai.array([1, 2, 3], device=on_device),
        "b": {
            "c": startai.array([1, 2, 3], device=on_device),
            "d": startai.array([1, 2, 3], device=on_device),
        },
    }
    container = Container(dict_in)
    ...
    assert (startai.to_numpy(container_shuffled["a"]) == data).all()
    assert (startai.to_numpy(container_shuffled.a) == data).all()
    assert (startai.to_numpy(container_shuffled["b"]["c"]) == data).all()
    assert (startai.to_numpy(container_shuffled.b.c) == data).all()
    assert (startai.to_numpy(container_shuffled["b"]["d"]) == data).all()
    assert (startai.to_numpy(container_shuffled.b.d) == data).all()
...
def test_container_to_nested_list(on_device):
    ...
    res = startai.Container.cont_to_nested_list(container0)
...
def test_container_to_raw(on_device):
    ...
    assert np.allclose(startai.to_numpy(raw[0]), np.array([1]))
    assert np.allclose(startai.to_numpy(raw[1][0]), np.array([2]))
    assert np.allclose(startai.to_numpy(raw[1][1]), np.array([3]))


def test_container_trim_key(on_device):
    key = "abcdefg"
    max_length = 3
    trimmed_key = startai.Container.cont_trim_key(key, max_length)
...
def test_container_unify(on_device):
    ...
    conts[dev0] = Container(
        {
            "a": startai.array([1], device=dev0),
            "b": {"c": startai.array([2], device=dev0), "d": startai.array([3], device=dev0)},
        }
    )
    if "gpu" in on_device and startai.num_gpus() > 1:
        idx = startai.num_gpus() - 1
        dev1 = on_device[:-1] + str(idx)
        on_devices.append(dev1)
        conts[dev1] = Container(
            {
                "a": startai.array([4], device=dev1),
                "b": {
                    "c": startai.array([5], device=dev1),
                    "d": startai.array([6], device=dev1),
                },
            }
        )

    # test
    container_unified = startai.Container.cont_unify(conts, dev0, "concat", 0)
    assert np.allclose(startai.to_numpy(container_unified.a[0]), np.array([1]))
    assert np.allclose(startai.to_numpy(container_unified.b.c[0]), np.array([2]))
    assert np.allclose(startai.to_numpy(container_unified.b.d[0]), np.array([3]))
    if len(on_devices) > 1:
        assert np.allclose(startai.to_numpy(container_unified.a[1]), np.array([4]))
        assert np.allclose(startai.to_numpy(container_unified.b.c[1]), np.array([5]))
        assert np.allclose(startai.to_numpy(container_unified.b.d[1]), np.array([6]))
...
def test_container_unstack_conts(on_device):
    ...
    for cont, a, bc, bd in zip(container_unstacked, [1, 2, 3], [2, 3, 4], [3, 4, 5]):
        assert np.array_equal(startai.to_numpy(cont["a"]), np.array([a]))
        assert np.array_equal(startai.to_numpy(cont.a), np.array([a]))
        assert np.array_equal(startai.to_numpy(cont["b"]["c"]), np.array([bc]))
        assert np.array_equal(startai.to_numpy(cont.b.c), np.array([bc]))
        assert np.array_equal(startai.to_numpy(cont["b"]["d"]), np.array([bd]))
        assert np.array_equal(startai.to_numpy(cont.b.d), np.array([bd]))
...
def test_container_with_entries_as_lists(on_device):
    ...
    dict_in = {
        "a": startai.array([1], device=on_device),
        "b": {"c": startai.array([2.0], device=on_device), "d": "some string"},
    }
    container = Container(dict_in)
...
def test_container_with_startai_backend(on_device):
    container0 = Container(
        {
            "a": startai.array([1], device=on_device),
            "b": {
    ...
    container0 = startai.Container.cont_with_startai_backend(container0, "numpy")
    assert container0.cont_config["startaih"] == "numpy"
    assert id_container0 != id(container0)
    id_container0 = id(container0)
    startai.Container.cont_with_startai_backend(container0, "torch", inplace=True)
    assert container0.cont_config["startaih"] == "torch"
...
def test_jax_pytree_compatibility(on_device):
    ...
    for i, true_val in enumerate(true_values):
        assert np.array_equal(startai.to_numpy(cont_values[i]), startai.to_numpy(true_val))
