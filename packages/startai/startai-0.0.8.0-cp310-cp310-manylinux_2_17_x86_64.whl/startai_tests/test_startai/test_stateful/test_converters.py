...
def get_converter(startai_backend, converter):
    return getattr(startai_backend.Module, converter)
...
def test_from_backend_module(bs_ic_oc, from_class_and_args, backend_fw):
    ...
    # using startai_backend.utils.backend.ContextManager instead of update_backend,
    # because with_backend doesn't work here
    with startai.utils.backend.ContextManager(backend_fw) as startai_backend:
        x = startai_backend.astype(
            startai_backend.linspace(
                startai_backend.zeros(batch_shape),
                startai_backend.ones(batch_shape),
                input_channels,
            ),
            "float32",
        )
        native_module_class = NATIVE_MODULES[startai_backend.current_backend_str()]
        module_converter = get_converter(
            startai_backend, FROM_CONVERTERS[startai_backend.current_backend_str()]
        )

        if from_class_and_args:
            startai_module = module_converter(
                native_module_class,
                instance_args=[x],
                constructor_kwargs={
            ...
            if startai_backend.current_backend_str() == "tensorflow":
            ...
            fw_kwargs = {}
            startai_module = module_converter(native_module, **fw_kwargs)

        def loss_fn(v_=None):
            out = startai_module(x, v=v_)
            return startai_backend.mean(out)
        ...
        for i in range(10):
            loss, grads = startai_backend.execute_with_gradients(loss_fn, startai_module.v)
            w = startai_backend.gradient_descent_update(startai_module.v, grads, 1e-3)
            startai_backend.inplace_update(startai_module.v, w)
            assert loss <= loss_tm1
            loss_tm1 = loss

        # type test
        assert startai_backend.is_array(loss)
        assert isinstance(grads, startai_backend.Container)
        # cardinality test
        assert loss.shape == ()
        # value test
...
def test_from_jax_module(bs_ic_oc, from_class_and_args, module_type, backend_fw):
    ...
    module_converter = get_converter(startai, FROM_CONVERTERS["jax"][module_type])
    if from_class_and_args:
        startai_module = module_converter(
            native_module_class,
            instance_args=[x],
    ...
    with startai.utils.backend.ContextManager(backend_fw) as startai_backend:
        for _ in range(10):
            loss, grads = startai_backend.execute_with_gradients(loss_fn, startai_module.v)
            startai_module.v = startai_backend.gradient_descent_update(
                startai_module.v, grads, 1e-3
            )
            assert loss < loss_tm1
            loss_tm1 = loss
