...
from startai.functional.frontends.tensorflow.nn import _convolution_broadcast_helper
from startai_tests.test_startai.test_frontends.test_tensorflow.test_nn import _x_and_filters
import numpy as np
import math
...
def _pow_helper_shared_dtype(draw):
    ...
    x1, x2 = x
    if "int" in dtype2:
        x2 = startai.nested_map(lambda x: abs(x), x2, include_derived={"list": True})

    if startai.is_int_dtype(dtype2):
        max_val = startai.iinfo(dtype2).max
    else:
        max_val = startai.finfo(dtype2).max
...
# startai.exceptions.StartaiBackendException: torch: constant_pad: constant_pad_nd(): argument
...
def test_tensorflow_Svd(
    *,
    ...
    ret = [startai.to_numpy(x) for x in ret]
...
@handle_frontend_test(
    fn_tree="tensorflow.raw_ops.Xdstartai",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("float"),
        num_arrays=2,
        shared_dtype=True,
...
def test_tensorflow_Xdstartai(  # NOQA
    *,
    dtype_and_x,
    frontend,
    test_flags,
