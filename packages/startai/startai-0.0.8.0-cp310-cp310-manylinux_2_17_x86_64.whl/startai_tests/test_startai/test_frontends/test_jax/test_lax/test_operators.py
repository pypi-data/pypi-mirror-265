# global
import numpy as np
import startai.functional.frontends.jax.lax as jlax
import startai.functional.frontends.jax.numpy as jnp
...
from startai_tests.test_startai.helpers import handle_frontend_test, BackendHandler
from startai_tests.test_startai.test_functional.test_experimental.test_nn.test_layers import (
    _reduce_window_helper,
)
from startai_tests.test_startai.test_functional.test_nn.test_layers import (
    _assume_tf_dilation_gt_1,
)
from startai.functional.frontends.jax.numpy import can_cast
from startai.functional.frontends.jax.lax.operators import (
    _dimension_numbers,
    _argsort_tuple,
)
...
def _fill_value(draw):
    ...
    with BackendHandler.update_backend(test_globals.CURRENT_BACKEND) as startai_backend:
        ...
        elif startai_backend.is_int_dtype(dtype):
...
def _general_dot_helper(draw):
    ...
    if is_pref:
        ...
        if "uint" in input_dtype[0]:
            ...
            raise startai.exceptions.StartaiException("unsupported dtype")
    return (
        input_dtype * 2,
        (lhs[0], rhs[0]),
...
def _x_and_filters(draw, dim=2, transpose=False, general=False):
    ...
    vals = startai.permute_dims(vals, axes=_argsort_tuple(dim_nums[0]))
    filters = draw(
        helpers.array_values(
            dtype=dtype[0],
            shape=filter_shape,
    ...
    filters = startai.permute_dims(filters, axes=_argsort_tuple(dim_nums[1]))
...
def test_jax_conv_general_dilated(
    *,
    ...
    _assume_tf_dilation_gt_1(startai.current_backend_str(), on_device, dilations[0])
...
def test_jax_conv_transpose(
    *,
    ...
    dtype, x, filters, dilation, dim_num, stride, pad, fc, pref = x_f_d_other
    _assume_tf_dilation_gt_1(startai.current_backend_str(), on_device, dilation)
    helpers.test_frontend_function(
        input_dtypes=dtype,
        backend_to_test=backend_fw,
        test_flags=test_flags,
