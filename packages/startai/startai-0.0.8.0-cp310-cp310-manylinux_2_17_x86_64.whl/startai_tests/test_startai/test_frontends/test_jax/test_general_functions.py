...
from startai.functional.frontends.jax import vmap
...
def test_jax_device_get(
    *,
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        x_on_dev = startai_backend.functional.frontends.jax.device_get(x).startai_array
...
def test_jax_device_put(
    *,
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        x_on_dev = startai_backend.functional.frontends.jax.device_put(
            x, on_device
        ).startai_array
        dev_from_new_x = startai_backend.dev(x_on_dev)
...
def test_jax_vmap(
    ...
    arrays = [startai.native_array(array) for array in generated_arrays]
    if in_axes_as_cont:
