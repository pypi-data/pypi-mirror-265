...
from startai_tests.test_startai.test_frontends.test_torch.test_comparison_ops import (
    _topk_helper,
)
from startai_tests.test_startai.test_frontends.test_torch.test_creation_ops import (
    _as_strided_helper,
)
from startai_tests.test_startai.test_frontends.test_torch.test_indexing_slicing_joining_mutating_ops import (  # noqa: E501
    _dtype_input_dim_start_length,
)
from startai_tests.test_startai.test_frontends.test_torch.test_reduction_ops import (
    _get_axis_and_p,
)
...
from startai_tests.test_startai.helpers.hypothesis_helpers.general_helpers import sizes_
from startai_tests.test_startai.test_frontends.test_torch.test_blas_and_lapack_ops import (
    _get_dtype_and_3dbatch_matrices,
    _get_dtype_input_and_matrices,
    _get_dtype_input_and_mat_vec,
)
from startai.functional.frontends.torch import Tensor
from startai_tests.test_startai.helpers import handle_frontend_method, BackendHandler
from startai_tests.test_startai.test_functional.test_core.test_searching import (
    _broadcastable_trio,
)
from startai_tests.test_startai.test_functional.test_core.test_manipulation import (  # noqa
    _get_splits,
)
from startai_tests.test_startai.test_frontends.test_torch.test_miscellaneous_ops import (  # noqa
    dtype_value1_value2_axis,
    _get_dtype_value1_value2_cov,
)
from startai_tests.test_startai.test_frontends.test_torch.test_linalg import (  # noqa
    _get_dtype_and_matrix,
)
from startai_tests.test_startai.test_functional.test_core.test_statistical import (
    _get_castable_dtype,
    _statistical_dtype_values,
)
from startai_tests.test_startai.test_functional.test_experimental.test_core.test_manipulation import (  # noqa
    put_along_axis_helper,
)
from startai_tests.test_startai.test_functional.test_experimental.test_core.test_statistical import (  # noqa
    _quantile_helper,
)
...
CLASS_TREE = "startai.functional.frontends.torch.Tensor"
...
def _masked_fill_helper(draw):
    ...
    if startai.is_uint_dtype(dtypes[0]):
        fill_value = draw(helpers.ints(min_value=0, max_value=5))
    elif startai.is_int_dtype(dtypes[0]):
...
def _requires_grad_and_dtypes(draw):
    dtypes = draw(_dtypes())
    dtype = dtypes[0]
    if startai.is_int_dtype(dtype) or startai.is_uint_dtype(dtype):
...
def test_torch___array__(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        function_module = local_importer.import_module("startai.functional.frontends.torch")
...
def test_torch___array_wrap__(
    ...
    helpers.value_test(
        ret_np_flat=np.array(ret.startai_array).ravel(),
...
def test_torch_backward(
    ...
    startai.set_backend(backend_fw)
    if startai.current_backend_str() == "numpy":
        startai.warnings.warn("Gradient calculation unavailable for numpy backend")
        return
    if startai.current_backend_str() == "paddle":
        startai.warnings.warn("torch.Tensor.backward() unavailable for paddle backend")
        return
    _, values = dtype_x
    ...
    helpers.assertions.value_test(
        ret_np_flat=helpers.flatten_and_to_np(
            ret=x._grads.startai_array, backend=backend_fw
        ),
        ret_np_from_gt_flat=helpers.flatten_and_to_np(
            ret=startai.to_startai(x_torch.grad.numpy()), backend=backend_fw
        ),
        rtol=1e-3,
        atol=1e-3,
        backend="torch",
    ...
    helpers.assertions.value_test(
        ret_np_flat=helpers.flatten_and_to_np(
            ret=y._grads.startai_array, backend=backend_fw
        ),
        ret_np_from_gt_flat=helpers.flatten_and_to_np(
            ret=startai.to_startai(y_torch.grad.numpy()), backend=backend_fw
        ),
        rtol=1e-3,
        atol=1e-3,
        backend="torch",
    ...
    helpers.assertions.value_test(
        ret_np_flat=helpers.flatten_and_to_np(
            ret=z._grads.startai_array, backend=backend_fw
        ),
        ret_np_from_gt_flat=helpers.flatten_and_to_np(
            ret=startai.to_startai(z_torch.grad.numpy()), backend=backend_fw
        ),
        rtol=1e-3,
        atol=1e-3,
        backend="torch",
...
def test_torch_cuda(dtype_x, backend_fw):
    ...
    device = "gpu:0"
    startai.utils.assertions.check_equal(x.cuda, device, as_array=False)
    startai.previous_backend()
...
def test_torch_device(
    ...
    _, data = dtype_x
    x = Tensor(data[0])
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(
        x.device, startai.dev(startai.array(data[0])), as_array=False
    )
    startai.previous_backend()
...
def test_torch_dtype(dtype_x, backend_fw):
    ...
    x = Tensor(data[0])
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(x.dtype, dtype[0], as_array=False)
    startai.previous_backend()
...
def test_torch_get_device(
    ...
    _, data = dtype_x
    x = Tensor(data[0])
    startai.utils.assertions.check_equal(x.get_device, -1, as_array=False)
    x = Tensor(data[0], "gpu:0")
    startai.utils.assertions.check_equal(x.get_device, 0, as_array=False)
    x = Tensor(data[0], "tpu:3")
    startai.utils.assertions.check_equal(x.get_device, 3, as_array=False)
    startai.previous_backend()


def test_torch_grad(backend_fw):
    startai.set_backend(backend_fw)
    x = Tensor(startai.array([1.0, 2.0, 3.0]))
    grads = startai.array([1.0, 2.0, 3.0])
    x._grads = grads
    assert startai.array_equal(x.grad, grads)
    startai.previous_backend()


def test_torch_grad_fn(backend_fw):
    startai.set_backend(backend_fw)
    x = Tensor(startai.array([3.0]), requires_grad=True)
    startai.utils.assertions.check_equal(x.grad_fn, None, as_array=False)
    y = x.pow(2)
    startai.utils.assertions.check_equal(y.grad_fn, "PowBackward", as_array=False)
    startai.utils.assertions.check_equal(
        y.grad_fn.next_functions[0], "AccumulateGrad", as_array=False
    )
    z = y.detach()
    startai.utils.assertions.check_equal(z.grad_fn, None, as_array=False)
    startai.previous_backend()
...
def test_torch_imag(dtype_x, backend_fw):
    ...
    x = Tensor(data[0])
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(x.imag, startai.imag(data[0]))
    startai.previous_backend()
...
def test_torch_index_fill(
    ...
    if indices.ndim != 1:
        indices = startai.flatten(indices)
    helpers.test_frontend_method(
        init_input_dtypes=[input_dtypes[0]],
        backend_to_test=backend_fw,
        init_all_as_kwargs_np={"data": x},
...
def test_torch_is_cuda(
    ...
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(
        x.is_cuda, "gpu" in startai.dev(startai.array(data[0])), as_array=False
    )
    startai.previous_backend()
...
def test_torch_is_leaf(requires_grad, backend_fw):
    startai.set_backend(backend_fw)
    x = Tensor(startai.array([3.0]), requires_grad=requires_grad)
    startai.utils.assertions.check_equal(x.is_leaf, True, as_array=False)
    y = x.pow(2)
    startai.utils.assertions.check_equal(y.is_leaf, not requires_grad, as_array=False)
    z = y.detach()
    startai.utils.assertions.check_equal(z.is_leaf, True, as_array=False)
    startai.previous_backend()
...
def test_torch_is_meta(
    ...
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(
        x.is_meta, "meta" in startai.dev(startai.array(data[0])), as_array=False
    )
    startai.previous_backend()
...
def test_torch_is_quantized(
    ...
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(
        x.is_quantized, "q" in startai.dtype(startai.array(data[0])), as_array=False
    )
    startai.previous_backend()
...
def test_torch_startai_array(
    ...
    x = Tensor(data[0])
    x.startai_array = data[0]
    ret = helpers.flatten_and_to_np(ret=x.startai_array.data, backend=backend_fw)
...
def test_torch_ndim(dtype_x, backend_fw):
    ...
    x = Tensor(data[0])
    startai.utils.assertions.check_equal(x.ndim, data[0].ndim, as_array=False)
    startai.previous_backend()
...
def test_torch_new_empty(
    ...
    helpers.test_frontend_method(
        ...
        method_input_dtypes=[startai.int32],
        method_all_as_kwargs_np={
            "size": size,
        },
        frontend_method_data=frontend_method_data,
...
def test_torch_real(dtype_x, backend_fw):
    ...
    x = Tensor(data[0])
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(x.real, startai.real(data[0]))
    startai.previous_backend()
...
def test_torch_requires_grad(dtype_x, requires_grad, backend_fw):
    ...
    startai.utils.assertions.check_equal(x.requires_grad, requires_grad, as_array=False)
    x.requires_grad = not requires_grad
    startai.utils.assertions.check_equal(x.requires_grad, not requires_grad, as_array=False)
    startai.previous_backend()
...
def test_torch_shape(dtype_x, backend_fw):
    ...
    x = Tensor(data[0])
    startai.utils.assertions.check_equal(
        x.startai_array.shape, startai.Shape(shape), as_array=False
    )
    startai.previous_backend()
...
def test_torch_svd(
    ...
    with helpers.update_backend(backend_fw) as startai_backend:
        ret = [startai_backend.to_numpy(x) for x in ret]
