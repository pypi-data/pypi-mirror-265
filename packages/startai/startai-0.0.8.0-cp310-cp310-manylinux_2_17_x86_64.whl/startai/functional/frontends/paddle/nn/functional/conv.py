...
from startai.functional.frontends.torch.nn.functional import convolution_functions
...
def _channel_first_input(x, data_format):
    ...
    dims = ndims - 2
    if 1 >= dims >= 3:
        raise startai.utils.exceptions.StartaiError(f"invalid for input with {dims} dims")
    # channel first input
    if data_format not in ["NCL", "NCHW", "NCDHW"]:
        if data_format in ["NLC", "NHWC", "NDHWC"]:
            x = startai.permute_dims(x, axes=(0, ndims - 1, *range(1, ndims - 1)))
        else:
