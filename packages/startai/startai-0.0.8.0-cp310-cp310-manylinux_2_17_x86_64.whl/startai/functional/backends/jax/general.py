"""Collection of Jax general functions, wrapped to fit Startai syntax and
signature."""
...
from startai.functional.backends.jax import JaxArray, NativeArray
from startai.functional.backends.jax.device import _to_array, _to_device
...
def _mask_to_index(query, x):
    if query.shape != x.shape:
        if len(query.shape) > len(x.shape):
            raise startai.exceptions.StartaiException("too many indices")
...
def set_item(
    ...
    *,
    ...
    if copy:
        return ret
    return startai.inplace_update(x, _to_device(ret))
...
def inplace_decrement(
    x: Union[startai.Array, JaxArray], val: Union[startai.Array, JaxArray]
) -> startai.Array:
    (x_native, val_native), _ = startai.args_to_native(x, val)
    if startai.is_startai_array(x):
        x.data -= val_native
    else:
        x = startai.Array(x_native - val_native)
    return x
...
def inplace_update(
    x: Union[startai.Array, JaxArray],
    val: Union[startai.Array, JaxArray],
    /,
    *,
...
def scatter_flat(
    ...
    *,
    ...
    if target_given:
        return startai.inplace_update(out, target)
    return target
...
def scatter_nd(
    ...
    *,
    ...
    updates = jnp.array(
        updates,
        dtype=(
            startai.dtype(out, as_native=True)
    ...
    target = out
    target_given = startai.exists(target)
    if startai.exists(shape) and startai.exists(target):
...
def shape(
    ...
    *,
    ...
    if as_array:
        return startai.array(jnp.shape(x), dtype=startai.default_int_dtype())
    else:
...
def vmap(
    ...
    func = startai.output_to_native_arrays(func)
    return startai.inputs_to_native_arrays(
