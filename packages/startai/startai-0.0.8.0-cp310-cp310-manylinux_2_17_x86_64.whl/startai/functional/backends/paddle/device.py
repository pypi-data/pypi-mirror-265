...
) -> Union[startai.Device, core.Place]:
    return x.place if as_native else as_startai_dev(x.place)
...
def as_startai_dev(device: core.Place, /):
    # TODO: add handling to string inputs without indices for gpu
    if isinstance(device, str):
        return startai.Device(device)

    # TODO: remove this once startai.Device accepts native device inputs
    if device.is_cpu_place():
        return startai.Device("cpu")
    elif device.is_gpu_place():
        dev_idx = device.gpu_device_id()
        return startai.Device(f"gpu:{str(dev_idx)}")


def as_native_dev(
    device: Optional[Union[startai.Device, core.Place]] = None,
    /,
) -> core.Place:
...
def handle_soft_device_variable(*args, fn, **kwargs):
    ...
    paddle.device.set_device(startai.as_startai_dev(device_shifting_dev))
    ret = fn(*args, **kwargs)
    paddle.device.set_device(startai.as_startai_dev(prev_def_dev))
    return ret
