...
from startai.functional.startai.gradients import _get_required_float_variables
...
def bind_custom_gradient_function(func, custom_grad_fn):
    ...
    def custom_module(x):
        ...
        return startai.to_native((ret, grad), nested=True, include_derived=True)
...
def vjp(func: Callable, *primals):
    ...
    native_flattened_primals = startai.to_native(flattened_primals, nested=True)
    ...
    def vjpfun(x_in):
        grads = tape.gradient(
            ...
            output_gradients=startai.to_native(_flatten_containers(x_in)[0], nested=True),
        )
...
def jvp(func: Callable, primals, tangents):
    ...
    flattened_primals = startai.to_native(flattened_primals, nested=True)
    flattened_tangents = startai.to_native(flattened_tangents, nested=True)
