...
def histogram(
    ...
    /,
    *,
    bins: Optional[Union[int, startai.Array, startai.NativeArray]] = None,
    ...
    >>> y = startai.array([0., 0.5, 1., 1.5, 2.])
    >>> z = startai.histogram(x, bins=y)
    >>> print(z)
    ...
    >>> range = (0., 5.)
    >>> dtype = startai.int32
    >>> y = startai.histogram(x, bins=bins, range=range, dtype=dtype)
    >>> print(y)
    startai.array([2, 1, 1, 1])
    ...
    >>> dtype = startai.float32
    >>> weights = startai.array([[1., 1., 1.], [1., 1., 1.]])
    >>> z = startai.histogram(
    ...                     x,
    ...                     bins=y,
    ...                     axis=axis,
    ...
    >>> y = startai.array([0., 1., 2., 3., 4., 5.])
    >>> dtype = startai.int32
    >>> z = startai.histogram(x, bins=y, dtype=dtype)
    >>> print(z)
    {
    ...
    """
    return startai.current_backend(a).histogram(
        a,
        bins=bins,
        axis=axis,
        extend_lower_interval=extend_lower_interval,
...
def median(
    ...
    *,
    ...
    >>> startai.median(a)
    3.5
    >>> startai.median(a, axis=0)
    startai.array([6.5, 4.5, 2.5])
    """
    return startai.current_backend().median(input, axis=axis, keepdims=keepdims, out=out)
...
def nanmean(
    ...
    *,
    ...
    Examples
    --------
    >>> a = startai.array([[1, startai.nan], [3, 4]])
    >>> startai.nanmean(a)
    2.6666666666666665
    >>> startai.nanmean(a, axis=0)
    startai.array([2.,  4.])
    """
    return startai.current_backend(a).nanmean(
...
def nanmin(
    ...
    *,
    ...
    >>> startai.nanmin(a)
    1.0
    >>> startai.nanmin(a, axis=1)
    [1. 3.]
    >>> startai.nanmin(a, axis=0, keepdims=True)
    [[1. 2.]]
    """
    return startai.current_backend(x).nanmin(
        x,
        axis=axis,
        keepdims=keepdims,
        out=out,
...
def nanprod(
    ...
    *,
    ...
    >>> startai.nanprod(a)
    12.0
    >>> startai.nanprod(a, axis=0)
    [3. 4.]
    >>> startai.nanprod(a, axis=0, keepdims=True)
    [[3. 4.]]
    """
    return startai.current_backend(a).nanprod(
        a,
        axis=axis,
        keepdims=keepdims,
        dtype=dtype,
...
def quantile(
    ...
    *,
    ...
    >>> startai.quantile(a, q, axis=0)
    startai.array([6.5, 4.5, 2.5])

    >>> startai.quantile(a, q, axis=1)
    startai.array([7.,  2.])

    >>> startai.quantile(a, q, axis=1, keepdims=True)
    startai.array([[7.],[2.]])

    >>> a = startai.array([1., 2., 3., 4.])
    >>> q = startai.array([0.3, 0.7])
    >>> startai.quantile(a, q, interpolation='lower')
    startai.array([1., 3.])
    """
    return startai.current_backend(a).quantile(
...
def corrcoef(
    ...
    /,
    *,
    y: Optional[startai.Array] = None,
    rowvar: bool = True,
    out: Optional[startai.Array] = None,
) -> startai.Array:
    return startai.current_backend(x).corrcoef(x, y=y, rowvar=rowvar, out=out)
...
def nanmedian(
    ...
    *,
    ...
    """startai.Array instance method variant of startai.nanmedian. This method simply
    wraps the function, and so the docstring for startai.nanmedian also applies to
    ...
    >>> x = startai.array([[12.0, 10.0, 34.0], [45.0, 23.0, startai.nan]])
    >>> startai.nanmedian(x)
        startai.array(23.)
    ...
    """
    return startai.current_backend().nanmedian(
...
def bincount(
    ...
    /,
    *,
    weights: Optional[startai.Array] = None,
    minlength: int = 0,
    out: Optional[startai.Array] = None,
) -> startai.Array:
    ...
    >>> a.bincount(a, axis=0)
        array([6.5, 2. , 2.5])
    """
    return startai.current_backend(x).bincount(
...
def igamma(
    ...
    /,
    *,
    x: Optional[Union[startai.Array, startai.NativeArray]] = None,
    ...
    """
    return startai.current_backend().igamma(a, x=x, out=out)
...
def cov(
    ...
    x2: Union[startai.Array, startai.NativeArray] = None,
    /,
    *,
    rowVar: bool = True,
    bias: bool = False,
    ddof: Optional[int] = None,
    fweights: Optional[startai.Array] = None,
    aweights: Optional[startai.Array] = None,
    dtype: Optional[Union[startai.Dtype, startai.NativeDtype]] = None,
) -> startai.Array:
    ...
    With :class:`startai.Array` input and rowVar flag set to False (True by default):
    >>> x = startai.array([[1,2,3],
    ...                [4,5,6]])
    ...
    With :class:`startai.Array` input and bias flag set to True (False by default):
    >>> x = startai.array([[1,2,3],
    ...                [4,5,6]])
    >>> y = x[0].cov(x[1], bias=True)
    >>> print(y)
    startai.array([[0.66666667, 0.66666667],
           [0.66666667, 0.66666667]])

    With :class:`startai.Array` input with both fweights and aweights given:
    >>> x = startai.array([[1,2,3],
    ...                [4,5,6]])
    >>> fw = startai.array([1,2,3])
    >>> aw = startai.array([ 1.2, 2.3, 3.4 ])
    >>> y = x[0].cov(x[1], fweights=fw, aweights=aw)
    >>> print(y)
    startai.array([[0.48447205, 0.48447205],
           [0.48447205, 0.48447205]])
    """
    return startai.current_backend(x1).cov(
        x1,
        x2,
        rowVar=rowVar,
        bias=bias,
...
def cummax(
    ...
    *,
    ...
    >>> x = startai.array([-86, -19, 41, 88, -5, 80, 32, 87, -90, -12])
    >>> y = startai.cummax(x, exclusive=False, reverse=False)
    >>> print(y)
    (startai.array([-86, -19,  41,  88,  88,  88,  88,  88,  88,  88]),
    startai.array([0, 1, 2, 3, 3, 3, 3, 3, 3, 3]))

    >>> x = startai.array([ 14,  15,  49, -24, -39])
    >>> y = startai.cummax(x, axis=0, exclusive=False, reverse=False)
    >>> print(y)
    (startai.array([14, 15, 49, 49, 49]), startai.array([0, 1, 2, 2, 2]))

    >>> x = startai.array([[ 63,  43, -16,  -4],[ 21,  82,  59,  33]])
    >>> startai.cummax(x, axis=0, reverse=False, dtype='int64', out=x)
    >>> print(x)
    startai.array([[0, 0, 0, 0],
           [0, 1, 1, 1]])

    >>> x = startai.array([[-36,  83, -81],
    ...                [ 23,  29,  63],
    ...                [-83,  85,   2],
    ...                [ 31,  25, -86],
    ...                [-10, -52,   0],
    ...                [ 22,  38,  55],
    ...                [ 33,  54, -16]])
    >>> y = startai.cummax(x, axis=1, exclusive=True, reverse=False)
    >>> print(y)
    (startai.array([[ 0,  0, 83],
           [ 0, 23, 29],
           [ 0,  0, 85],
           [ 0, 31, 31],
           [ 0,  0,  0],
           [ 0, 22, 38],
           [ 0, 33, 54]]), startai.array([[0, 0, 2],
           [0, 1, 2],
           [0, 0, 2],
           [0, 1, 1],
           [0, 0, 0],
    ...
    >>> x = startai.array([73, 15, 47])
    >>> y = startai.cummax(x, axis=0, reverse=True, exclusive=True)
    >>> print(y)
    (startai.array([47, 47,  0]), startai.array([0, 0, 0]))

    >>> x = startai.array([-47, -14, -67, 15, -23, -45])
    >>> y = startai.cummax(x, axis=0, reverse=True, exclusive=False)
    >>> print(y)
    (startai.array([ 15,  15,  15,  15, -23, -45]), startai.array([2, 2, 2, 2, 1, 0]))
    """
    return startai.current_backend(x).cummax(
...
def cummin(
    ...
    *,
    ...
    >>> x = startai.array([1, 5, 2, 0])
    >>> y = startai.cummin(x)
    >>> print(y)
    startai.array([1, 1, 1, 0])
    >>> x = startai.array([[6, 4, 2],
    ...                [1, 3, 0]])
    >>> y = startai.zeros((2,3))
    >>> startai.cummin(x, axis=0, reverse=True, out=y)
    >>> print(y)
    startai.array([[1., 3., 0.],
           [1., 3., 0.]])

    >>> x = startai.array([[2, 4, 5],
    ...                [3, 6, 5],
    ...                [1, 3, 10]])
    >>> startai.cummin(x,axis=1,reverse=True, dtype='int64', out=x)
    >>> print(x)
    startai.array([[ 2,  4,  5],
           [ 3,  5,  5],
           [ 1,  3, 10]])
    ...
    ...                   b = startai.zeros((2, 3)),
    ...                   c = startai.zeros((3,3)))
    >>> startai.cummin(x,axis=1,reverse=True, out=y)
    >>> print(y)
    {
        a: startai.array([[1., 3., 4.]]),
        b: startai.array([[3., 5., 8.],
                      [5., 5., 5.]]),
    ...
    >>> x = startai.Container(a=startai.array([[0],[5]]),
    ...                   b=startai.array([[6, 8, 7],
    ...                                [4, 2, 3]]),
    ...                   c=startai.array([[1, 2],
    ...                                [3, 4],
    ...                                [6, 4]]))
    >>> startai.cummin(x,axis=0,out=x)
    >>> print(x)
    {
        a: startai.array([[0],
                      [0]]),
    ...
    """
    return startai.current_backend(x).cummin(
        x, axis=axis, exclusive=exclusive, reverse=reverse, dtype=dtype, out=out
    )
