...
from startai.utils.backend import ast_helpers


import_cache = {}
path_hooks = []

# Note that any modules listed as 'to skip' should not depend on the Startai backend state.
# If they do, the behavior of startai.with_backend is undefined and may not function as
# expected. Import these modules along with Startai initialization, as the import logic
# assumes they exist in sys.modules.

MODULES_TO_SKIP = ["startai.compiler", "startai.engines", "startai.wrappers"]

IS_COMPILING_WITH_BACKEND = False


class LocalStartaiImporter:
    def __init__(self):
        self.finder = ast_helpers.StartaiPathFinder()
...
def _import_module(name, package=None):
    ...
    # Return the one from global Startai if the module is marked to skip
    ...
    if startai.is_local():
        spec.loader.exec_module(module, startai._compiled_id)
    else:
