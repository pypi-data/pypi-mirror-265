...
class Parafac2Tensor(FactorizedTensor):
    def __init__(self, parafac2_tensor):
        ...
        shape, rank = startai.Parafac2Tensor.validate_parafac2_tensor(parafac2_tensor)
    ...
    # ---------------#
    def to_tensor(self):
        return startai.Parafac2Tensor.parafac2_to_tensor(self)

    def to_vec(self):
        return startai.Parafac2Tensor.parafac2_to_vec(self)

    def to_unfolded(self, mode):
        return startai.Parafac2Tensor.parafac2_to_unfolded(self, mode)
    ...
    def from_CPTensor(cls, cp_tensor, parafac2_tensor_ok=False):
        ...
        Q, R = startai.qr(B)
        projections = [Q for _ in range(startai.shape(A)[0])]
        B = R
    ...
    def validate_parafac2_tensor(parafac2_tensor):
        ...
        """
        if isinstance(parafac2_tensor, startai.Parafac2Tensor):
        ...
        for i, projection in enumerate(projections):
            current_mode_size, current_rank = startai.shape(projection)
            if current_rank != rank:
                raise ValueError(
                    ...
                    f" projections[{i}].shape[1]={startai.shape(projection)[1]}"
                )

            inner_product = startai.dot(startai.permute_dims(projection, (1, 0)), projection)
            if (
                startai.max(
                    startai.abs(inner_product - startai.eye(rank, dtype=inner_product[0].dtype))
                )
                > 1e-5
            ):
                raise ValueError(
                    ...
                    " T.eye(rank)) = "
                    f"""{startai.sqrt(startai.sum(startai.square(inner_product -
                                  startai.eye(rank,dtype=inner_product[0].dtype)),
                                    axis=0))}"""
                )
        ...
        for i, factor in enumerate(factors[1:]):
            current_mode_size, current_rank = startai.shape(factor)
            if current_rank != rank:
                raise ValueError(
        ...
        if weights is not None and startai.shape(weights)[0] != rank:
            raise ValueError(
                f"Given factors for a rank-{rank} PARAFAC2 tensor but"
                f" len(weights)={startai.shape(weights)[0]}."
            )
    ...
    def parafac2_normalise(parafac2_tensor):
        ...
        _, rank = startai.Parafac2Tensor.validate_parafac2_tensor(parafac2_tensor)
        weights, factors, projections = parafac2_tensor
        ...
        for i, factor in enumerate(factors):
            ...
            scales_non_zero = startai.where(
                scales == 0, startai.ones(startai.shape(scales), dtype=factors[0].dtype), scales
            )
    ...
    def apply_parafac2_projections(parafac2_tensor):
        ...
        """
        startai.Parafac2Tensor.validate_parafac2_tensor(parafac2_tensor)
        weights, factors, projections = parafac2_tensor

        evolving_factor = [
            startai.dot(projection, factors[1]) for projection in projections
        ]
    ...
    def parafac2_to_slice(parafac2_tensor, slice_idx, validate=True):
        ...
        Ct = startai.permute_dims(C, (1, 0))

        B_i = startai.dot(projections[slice_idx], B)
        return startai.dot(B_i * a, Ct)
    ...
    def parafac2_to_slices(parafac2_tensor, validate=True):
        ...
        return [
            startai.Parafac2Tensor.parafac2_to_slice(decomposition, i, validate=False)
            for i in range(I)
        ]
    ...
    def parafac2_to_tensor(parafac2_tensor):
        ...
        slices = startai.Parafac2Tensor.parafac2_to_slices(parafac2_tensor)
        lengths = [projection.shape[0] for projection in projections]

        tensor = startai.zeros(
    ...
    def parafac2_to_unfolded(parafac2_tensor, mode):
        ...
        -------
            Full constructed tensor. Uneven slices are padded with zeros.
        """
        return startai.unfold(startai.Parafac2Tensor.parafac2_to_tensor(parafac2_tensor), mode)
    ...
    def parafac2_to_vec(parafac2_tensor):
        ...
        """
        return startai.reshape(startai.Parafac2Tensor.parafac2_to_tensor(parafac2_tensor), (-1))
