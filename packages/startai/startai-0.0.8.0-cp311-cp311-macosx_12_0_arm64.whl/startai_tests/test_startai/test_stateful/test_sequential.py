"""Collection of tests for Startai sequential."""
...
# local
import startai
from startai_tests.test_startai import helpers
from startai_tests.test_startai.helpers.testing_helpers import handle_method


class TrainableModule(startai.Module):
    def __init__(self, in_size, hidden_size, out_size):
        self._linear0 = startai.Linear(in_size, hidden_size)
        self._linear1 = startai.Linear(hidden_size, out_size)
...
def _copy_weights(v1, v2):
    ...
    v2.w = startai.copy_array(v1.w)
    v2.b = startai.copy_array(v1.b)
...
def _train(module, input_arr):
    def loss_fn(_v):
        return startai.abs(startai.mean(input_arr) - startai.mean(module(input_arr, v=_v)))

    # initial loss
    loss_tm1, grads = startai.execute_with_gradients(loss_fn, module.v)
    loss = None
    losses = []
    for i in range(5):
        loss, grads = startai.execute_with_gradients(loss_fn, module.v)
        module.v = startai.gradient_descent_update(module.v, grads, 1e-5)
        losses.append(loss)

    # loss is lower or very close to initial loss
    assert loss <= loss_tm1 or startai.abs(loss - loss_tm1) < 1e-5
...
def test_sequential_construction_and_value(
    ...
    with startai.utils.backend.ContextManager(backend_fw):
        ...
        layers = [
            startai.Linear(dims[i], dims[i + 1], device=on_device)
            for i in range(layer_count - 1)
        ]

        if use_activation:
            activations = [startai.GELU() for _ in range(layer_count - 1)]
            layers = itertools.chain.from_iterable(zip(layers, activations))

        module = startai.Sequential(*layers)

        input_array = startai.array(input_array, dtype="float32", device=on_device)
...
def test_sequential_same_as_class(input_array, dims, backend_fw):
    with startai.utils.backend.ContextManager(backend_fw):
        dims = [len(input_array)] + dims
        layer_count = len(dims)
        layers = [startai.Linear(dims[i], dims[i + 1]) for i in range(layer_count - 1)]

        m_sequential = startai.Sequential(*layers)
        ...
        input_array = startai.array(input_array, dtype="float32")
