...
from startai.functional.startai.layers import _deconv_length
from startai_tests.test_startai.helpers import handle_method
from startai_tests.test_startai.helpers.assertions import assert_same_type_and_shape
from startai_tests.test_startai.test_functional.test_experimental.test_nn import (
...
all_constant_initializers = (startai.Zeros, startai.Ones)
all_gaussian_initializers = (startai.KaimingNormal, startai.Siren)
all_uniform_initializers = (startai.GlorotUniform, startai.FirstLayerSiren, startai.Siren)
all_initializers = (
    ...
    with startai.utils.backend.ContextManager(backend_fw):
        ...
        if method_flags.as_variable[0]:
            x = _variable(
                startai.asarray(
                    startai.linspace(
                        startai.zeros(batch_shape), startai.ones(batch_shape), channels
                    ),
                    dtype=dtype,
                )
            )
        ...
        if with_v:
            ...
            v = Container(
                {
                    "submodules": {
                        "v0": {
                            ...
                            "b": _variable(
                                startai.zeros([channels], device=on_device, dtype=dtype)
                            ),
                        },
                        "v2": {
                            "w": _variable(
            ...
            v = None
        if seq_v:
            seq = startai.Sequential(
                startai.Linear(channels, channels, device=on_device, dtype=dtype),
                startai.Dropout(0.0, dtype=dtype),
                startai.Linear(channels, channels, device=on_device, dtype=dtype),
                device=on_device,
        ...
        ret = seq(x)
        # type test
        assert startai.is_startai_array(ret)
        # cardinality test
        assert ret.shape == startai.Shape(batch_shape + [channels])
        # value test
        if not with_v:
            return
        assert np.allclose(
            startai.to_numpy(seq(x)), np.array(target), rtol=tolerance_dict[dtype]
        )
