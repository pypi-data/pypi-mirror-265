...
def general_helpers_dtype_info_helper(backend, kind_dtype, dtype):
    with WithBackendContext(backend) as startai_backend:
        if kind_dtype == "float":
            return (
                startai_backend.finfo(dtype).max,
                startai_backend.finfo(dtype).min,
                getattr(startai_backend.finfo(dtype), "smallest_normal", None),
            )
        elif kind_dtype == "int":
            return (
                startai_backend.iinfo(dtype).max,
                startai_backend.iinfo(dtype).min,
                getattr(startai_backend.iinfo(dtype), "smallest_normal", None),
            )
...
def get_bounds(draw, *, dtype):
    ...
    if "int" in dtype:
        ...
        if startai.default(low, 0.0) >= startai.default(high, 1.0):
            return draw(get_bounds(dtype=dtype))
    return [low, high]
