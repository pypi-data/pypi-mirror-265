...
from startai_tests.test_startai.test_frontends.test_torch.test_miscellaneous_ops import (
    dtype_value1_value2_axis,
)
from startai_tests.test_startai.helpers.hypothesis_helpers.general_helpers import (
    matrix_is_stable,
)
from startai_tests.test_startai.test_functional.test_core.test_linalg import _matrix_rank_helper
...
# eig
# TODO: Test for all valid dtypes once startai.eig supports complex data types
@handle_frontend_test(
...
def test_torch_eig(
    *,
    ...
    if x.dtype == startai.float32:
        x = x.astype("float64")
        input_dtype = [startai.float64]
    ...
    ret = [startai.to_numpy(x).astype("float64") for x in ret]
...
def test_torch_eigvals(
    *,
    ...
    ret = startai.to_numpy(ret).astype(str(frontend_ret.dtype))
    ret = np.sort(ret)
