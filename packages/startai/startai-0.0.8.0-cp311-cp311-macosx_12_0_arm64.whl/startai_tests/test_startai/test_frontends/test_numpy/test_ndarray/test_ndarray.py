...
# local
import startai
from startai.functional.frontends.numpy import ndarray
...
import startai_tests.test_startai.test_frontends.test_numpy.helpers as np_frontend_helpers
...
# from startai.functional.frontends.numpy import ndarray
from startai_tests.test_startai.test_frontends.test_numpy.test_mathematical_functions.test_miscellaneous import (  # noqa
    _get_clip_inputs,
)
from startai_tests.test_startai.test_frontends.test_numpy.test_mathematical_functions.test_sums_products_differences import (  # noqa
    _get_castable_dtypes_values,
)

from startai_tests.test_startai.test_frontends.test_numpy.test_manipulation_routines.test_changing_number_of_dimensions import (  # noqa
    _squeeze_helper,
)

CLASS_TREE = "startai.functional.frontends.numpy.ndarray"
...
def test_numpy_T(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        x = startai_backend.functional.frontends.numpy.ndarray(shape, dtype[0])
        x.startai_array = data[0]
        ret = helpers.flatten_and_to_np(ret=x.T.startai_array, backend=backend_fw)
        ret_gt = helpers.flatten_and_to_np(
            ret=startai_backend.permute_dims(
                startai_backend.native_array(data[0]), list(range(len(shape)))[::-1]
            ),
            backend=backend_fw,
        )
        helpers.value_test(
...
def test_numpy_dtype(dtype_x, backend_fw, frontend):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        startai_backend.utils.assertions.check_equal(
            x.dtype, startai.Dtype(dtype[0]), as_array=False
        )
...
def test_numpy_flat(dtype_x, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        flat_startai = x.flat
        flat_startai = flat_startai.startai_array.to_numpy()
        flat_generated = startai_backend.to_numpy(data[0]).flatten()
        startai_backend.utils.assertions.check_equal(
            flat_startai, flat_generated, as_array=True
        )
...
def test_numpy_prod(
    ...
    where = startai.array(where, dtype="bool")
    helpers.test_frontend_method(
        init_input_dtypes=input_dtypes,
        backend_to_test=backend_fw,
        init_all_as_kwargs_np={
...
def test_numpy_property_ndim(dtype_x, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        x = startai_backend.functional.frontends.numpy.ndarray(shape, dtype[0])
        x.startai_array = data[0]
        startai_backend.utils.assertions.check_equal(x.ndim, data[0].ndim, as_array=False)
...
def test_numpy_shape(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        x.startai_array = data[0]
        startai_backend.utils.assertions.check_equal(
            x.shape, startai.Shape(shape), as_array=False
        )
...
def test_numpy_size(
    ...
    x.startai_array = data[0]
    startai.utils.assertions.check_equal(x.size, data[0].size, as_array=False)
