...
def test_all_nested_indices(nest):
    indices = startai.all_nested_indices(nest)
...
def test_all_nested_indices_w_nest_checks(nest):
    indices = startai.all_nested_indices(nest, True)
    assert indices[0] == ["a", 0, 0]
...
def test_copy_nest():
    nest = {
        "a": [startai.array([0]), startai.array([1])],
        "b": {"c": [startai.array([[2], [4]]), startai.array([[6], [8]])]},
    }
    nest_copy = startai.copy_nest(nest)
    ...
    nest = NAMEDTUPLE(x=startai.array([1.0]), y=startai.array([2.0]))
...
def test_duplicate_array_index_chains(nest, x, y):
    ...
    duplicate_index_chains = startai.duplicate_array_index_chains(nest)
...
def test_index_nest(nest, index):
    ret = startai.index_nest(nest, index)
    true_ret = nest
    for i in index:
        true_ret = true_ret[i]
    assert ret == true_ret
...
def test_insert_into_nest_at_indices(nest, indices, values):
    startai.insert_into_nest_at_indices(nest, indices, values)

    def indices_nest(nest, indices):
        ret = tuple(startai.index_nest(nest, index) for index in indices)
...
def test_insert_into_nest_index(nest, index, value):
    ...
    assert startai.index_nest(nest, index) == value
...
def test_map_nest_at_index(nest, index, fn, shallow):
    ...
    result = startai.map_nest_at_index(nest, index, fn, shallow=shallow)
...
def test_map_nest_at_indices(nest, indices, fn, shallow):
    nest_copy = copy.deepcopy(nest)
    result = startai.map_nest_at_indices(nest, indices, fn, shallow)
...
def test_multi_index_nest(nest, multi_indices):
    rets = startai.multi_index_nest(nest, multi_indices)
    true_rets = []
...
def test_nested_any(x, fn):
    ...
    x_bool = startai.nested_any(x, fn)
...
def test_nested_argwhere(nest):
    indices = startai.nested_argwhere(nest, lambda x: x < 5)
    assert indices[0] == ["a", 0, 0]
    assert indices[1] == ["a", 1, 0]
    assert indices[2] == ["b", "c", 0, 0, 0]
    assert indices[3] == ["b", "c", 0, 1, 0]
...
def test_nested_map(x, fn, shallow):
    x_copy = copy.deepcopy(x)
    result = startai.nested_map(fn, x, shallow=shallow)
    map_nested_dicts(x_copy, fn)
...
def test_nested_multi_map(func, nests):
    nests = startai.nested_map(
        lambda x: startai.array(x) if isinstance(x, np.ndarray) else x,
        nests,
        include_derived=True,
        shallow=False,
    )
    # without index_chains specification
    nested_multi_map_res = startai.nested_multi_map(func, nests)

    # modify this to test for other functions
    nests_without_multi_map_res = nests[0] - nests[1]

    assert startai.all_equal(nested_multi_map_res, nests_without_multi_map_res)


# prune_empty
@pytest.mark.parametrize("nest", [{"a": [{}, {}], "b": {"c": [1], "d": []}}])
def test_prune_empty(nest):
    ret = startai.prune_empty(startai.copy_nest(nest))
    assert ret == {"b": {"c": [1]}}
...
def test_prune_nest_at_index(nest, index):
    ...
    try:
        startai.prune_nest_at_index(nest, index)
...
def test_prune_nest_at_indices(nest, indices):
    nest_copy = copy.deepcopy(nest)
    startai.prune_nest_at_indices(nest_copy, indices)
    print(nest_copy)
    for idx in indices:
        try:
            ele_org = startai.index_nest(nest, idx)
            ele_new = startai.index_nest(nest_copy, idx)
        except startai.utils.exceptions.StartaiIndexError:
            return
        else:
...
def test_set_nest_at_index(nest, index, value, shallow):
    nest_copy = copy.deepcopy(nest)
    result = startai.set_nest_at_index(nest, index, value, shallow=shallow)
...
def test_set_nest_at_indices(nest, indices, values, shallow):
    nest_copy = copy.deepcopy(nest)
    result = startai.set_nest_at_indices(nest, indices, values, shallow=shallow)
