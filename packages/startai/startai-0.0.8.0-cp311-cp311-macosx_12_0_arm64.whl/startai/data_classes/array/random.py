# global
import abc
from typing import Optional, Union

# local
import startai


class _ArrayWithRandom(abc.ABC):
    def random_uniform(
        self: startai.Array,
        /,
        *,
        high: Union[float, startai.Array, startai.NativeArray] = 1.0,
        shape: Optional[Union[startai.Array, startai.Shape, startai.NativeShape]] = None,
        device: Optional[Union[startai.Device, startai.NativeDevice]] = None,
        dtype: Optional[Union[startai.Dtype, startai.NativeDtype]] = None,
        seed: Optional[int] = None,
        out: Optional[startai.Array] = None,
    ) -> startai.Array:
        """startai.Array instance method variant of startai.random_uniform. This method
        simply wraps the function, and so the docstring for startai.random_uniform
        also applies to this method with minimal changes.

        Parameters
        ----------
        self
            Lower boundary of the output interval. All values generated will be
            greater than or equal to ``low``. If array, must have same shape as
            ``high``.
        high
            Upper boundary of the output interval. All the values generated will be
            less than ``high``. If array, must have same shape as ``low``.
        shape
            If the given shape is, e.g ``(m, n, k)``, then ``m * n * k`` samples
            are drawn. Can only be specified when ``low`` and ``high`` are numeric
            values, else exception will be raised.
            Default is ``None``, where a single value is returned.
        device
            device on which to create the array 'cuda:0', 'cuda:1', 'cpu' etc.
            (Default value = None).
        dtype
            output array data type. If ``dtype`` is ``None``, the output array data
            type will be the default floating-point data type. Default ``None``
        seed
            A python integer. Used to create a random seed distribution
        out
            optional output array, for writing the result to. It must have a shape
            that the inputs broadcast to.

        Returns
        -------
        ret
            Drawn samples from the parameterized uniform distribution.

        Examples
        --------
        >>> x = startai.array([[9.8, 3.4], [5.8, 7.2]])
        >>> x.random_uniform(high=10.2)
        startai.array([[9.86, 4.89],
                   [7.06, 7.47]])

        >>> x.random_uniform(high=10.2, device='cpu')
        startai.array([[9.86, 4.89],
                   [7.06, 7.47]])

        >>> x.random_uniform(high=14.2, dtype='float16')
        startai.array([[9.86, 4.89],
                   [7.06, 7.47]])

        >>> x.random_uniform(high=10.8, device='cpu', dtype='float64')
        startai.array([[9.86, 4.89],
                   [7.06, 7.47]])

        >>> z = startai.ones((2,2))
        >>> x.random_uniform(high=11.2, device='cpu', dtype='float64', out=z)
        startai.array([[10.1 ,  6.53],
                   [ 7.94,  8.85]])

        >>> x = startai.array([8.7, 9.3])
        >>> y = startai.array([12.8, 14.5])
        >>> x.random_uniform(y)
        startai.array([12.1, 14. ])

        >>> x.random_uniform(high=y, device='cpu')
        startai.array([12.1, 14. ])

        >>> x.random_uniform(high=y, dtype='float16')
        startai.array([12.1, 14. ])

        >>> x.random_uniform(high=y, device='cpu', dtype='float64')
        startai.array([12.1, 14. ])

        >>> z = startai.ones((2,))
        >>> x.random_uniform(high=y, device='cpu', dtype='float64', out=z)
        startai.array([12.1, 14. ])
        """
        return startai.random_uniform(
            low=self._data,
            high=high,
            shape=shape,
            device=device,
            dtype=dtype,
            seed=seed,
            out=out,
        )

    def random_normal(
        self: startai.Array,
        /,
        *,
        std: Union[float, startai.Array, startai.NativeArray] = 1.0,
        shape: Optional[Union[startai.Shape, startai.NativeShape]] = None,
        device: Optional[Union[startai.Device, startai.NativeDevice]] = None,
        dtype: Optional[Union[startai.Dtype, startai.NativeDtype]] = None,
        seed: Optional[int] = None,
        out: Optional[startai.Array] = None,
    ) -> startai.Array:
        """startai.Array instance method variant of startai.random_normal. This method
        simply wraps the function, and so the docstring for startai.random_normal
        also applies to this method with minimal changes.

        Parameters
        ----------
        self
            The mean of the normal distribution to sample from. Default is ``0.0``.
        std
            The standard deviation of the normal distribution to sample from.
            Must be non-negative. Default is ``1.0``.
        shape
            If the given shape is, e.g ``(m, n, k)``, then ``m * n * k`` samples
            are drawn. Can only be specified when ``mean`` and ``std`` are numeric
            values, else exception will be raised.
            Default is ``None``, where a single value is returned.
        device
            device on which to create the array 'cuda:0', 'cuda:1', 'cpu' etc.
            (Default value = None).
        dtype
             output array data type. If ``dtype`` is ``None``, the output array data
             type will be the default floating-point data type. Default ``None``
        seed
            A python integer. Used to create a random seed distribution
        out
            optional output array, for writing the result to. It must have a shape
            that the inputs broadcast to.

        Returns
        -------
        ret
            Drawn samples from the parameterized normal distribution.

        Examples
        --------
        >>> x = startai.array([[9.8, 3.4], [5.8, 7.2]])
        >>> x.random_normal(std=10.2)
        startai.array([[19.   , -6.44 ],
                   [ 5.72 ,  0.235]])

        >>> x.random_normal(std=10.2, device='cpu')
        startai.array([[18.7 , 25.2 ],
                   [27.5 , -3.22]])

        >>> x.random_normal(std=14.2, dtype='float16')
        startai.array([[26.6 , 12.1 ],
                   [ 4.56,  5.49]])

        >>> x.random_normal(std=10.8, device='cpu', dtype='float64')
        startai.array([[ 1.02, -1.39],
                   [14.2 , -1.  ]])

        >>> z = startai.ones((2,2))
        >>> x.random_normal(std=11.2, device='cpu', dtype='float64', out=z)
        startai.array([[ 7.72, -8.32],
                   [ 4.95, 15.8 ]])

        >>> x = startai.array([8.7, 9.3])
        >>> y = startai.array([12.8, 14.5])
        >>> x.random_normal(std=y)
        startai.array([-10.8,  12.1])

        >>> x.random_normal(std=y, device='cpu')
        startai.array([ 13. , -26.9])

        >>> x.random_normal(std=y, dtype='float16')
        startai.array([14.3  , -0.807])

        >>> x.random_normal(std=y, device='cpu', dtype='float64')
        startai.array([21.3 ,  3.85])

        >>> z = startai.ones((2,))
        >>> x.random_normal(std=y, device='cpu', dtype='float64', out=z)
        startai.array([ 4.32, 42.2 ])
        """
        return startai.random_normal(
            mean=self._data,
            std=std,
            shape=shape,
            device=device,
            dtype=dtype,
            seed=seed,
            out=out,
        )

    def multinomial(
        self: startai.Array,
        population_size: int,
        num_samples: int,
        /,
        *,
        batch_size: int = 1,
        replace: bool = True,
        device: Optional[Union[startai.Device, startai.NativeDevice]] = None,
        seed: Optional[int] = None,
        out: Optional[startai.Array] = None,
    ) -> startai.Array:
        """startai.Array instance method variant of startai.multinomial. This method
        simply wraps the function, and so the docstring for startai.multinomial
        also applies to this method with minimal changes.

        Parameters
        ----------
        self
            The unnormalized probabilities for all elements in population,
            default is uniform *[batch_shape, population_size]*
        population_size
            The size of the population from which to draw samples.
        num_samples
            Number of independent samples to draw from the population.
        batch_size
            Number of tensors to generate. Default is 1.
        replace
            Whether to replace samples once they've been drawn. Default is ``True``.
        device
            device on which to create the array 'cuda:0', 'cuda:1', 'cpu' etc.
            (Default value = None)
        seed
            A python integer. Used to create a random seed distribution
        out
            optional output array, for writing the result to. It must have a shape
            that the inputs broadcast to.

        Returns
        -------
        ret
            Drawn samples from the parameterized normal distribution.
        """
        return startai.multinomial(
            population_size,
            num_samples,
            batch_size=batch_size,
            probs=self._data,
            replace=replace,
            device=device,
            seed=seed,
            out=out,
        )

    def randint(
        self: startai.Array,
        high: Union[int, startai.Array, startai.NativeArray],
        /,
        *,
        shape: Optional[Union[startai.Shape, startai.NativeShape]] = None,
        device: Optional[Union[startai.Device, startai.NativeDevice]] = None,
        dtype: Optional[Union[startai.Dtype, startai.NativeDtype]] = None,
        seed: Optional[int] = None,
        out: Optional[startai.Array] = None,
    ) -> startai.Array:
        """startai.Array instance method variant of startai.randint. This method simply
        wraps the function, and so the docstring for startai.randint also applies
        to this method with minimal changes.

        Parameters
        ----------
        self
            Lowest integer that can be drawn from the distribution.
        high
            One above the highest integer that can be drawn from the distribution.
        shape
            If the given shape is, e.g ``(m, n, k)``, then ``m * n * k`` samples
            are drawn. Can only be specified when ``low`` and ``high`` are numeric
            values, else exception will be raised.
            Default is ``None``, where a single value is returned.
        device
            device on which to create the array 'cuda:0', 'cuda:1', 'cpu' etc.
            (Default value = None).
        dtype
             output array data type. If ``dtype`` is ``None``, the output array data
             type will be the default integer data type. Default ``None``
        seed
            A python integer. Used to create a random seed distribution
        out
            optional output array, for writing the result to. It must have a shape
            that the inputs broadcast to.

        Returns
        -------
        ret
            Returns an array with the given shape filled with integers from
            the uniform distribution in the “half-open” interval [low, high)

        Examples
        --------
        >>> x = startai.array([[1, 2], [0, 5]])
        >>> x.randint(10)
        startai.array([[1, 5],
                   [9, 7]])

        >>> x.randint(8, device='cpu')
        startai.array([[6, 5],
                   [0, 5]])

        >>> x.randint(9, dtype='int8')
        startai.array([[1, 2],
                   [7, 7]])

        >>> x.randint(14, device='cpu', dtype='int16')
        startai.array([[6, 5],
                   [0, 5]])

        >>> z = startai.ones((2,2))
        >>> x.randint(16, device='cpu', dtype='int64', out=z)
        startai.array([[1, 2],
                   [7, 7]])

        >>> x = startai.array([1, 2, 3])
        >>> y = startai.array([23, 25, 98])
        >>> x.randint(y)
        startai.array([ 5, 14, 18])

        >>> x.randint(y, device='cpu')
        startai.array([20, 13, 46])

        >>> x.randint(y, dtype='int32')
        startai.array([ 9, 18, 33])

        >>> x.randint(y, device='cpu', dtype='int16')
        startai.array([ 9, 20, 85])

        >>> z = startai.ones((3,))
        >>> x.randint(y, device='cpu', dtype='int64', out=z)
        startai.array([20, 13, 46])
        """
        return startai.randint(
            self._data,
            high,
            shape=shape,
            device=device,
            dtype=dtype,
            seed=seed,
            out=out,
        )

    def shuffle(
        self: startai.Array,
        axis: Optional[int] = 0,
        /,
        *,
        seed: Optional[int] = None,
        out: Optional[startai.Array] = None,
    ) -> startai.Array:
        """startai.Array instance method variant of startai.shuffle. This method simply
        wraps the function, and so the docstring for startai.shuffle also applies
        to this method with minimal changes.

        Parameters
        ----------
        self
            Input array. Should have a numeric data type.
        axis
            The axis which x is shuffled along. Default is 0.
        seed
            A python integer. Used to create a random seed distribution
        out
            optional output array, for writing the result to. It must have a
            shape that the inputs broadcast to.

        Returns
        -------
        ret
            An array object, shuffled along the first dimension.

        Examples
        --------
        >>> x = startai.array([5, 2, 9])
        >>> y = x.shuffle()
        >>> print(y)
        startai.array([2, 5, 9])
        """
        return startai.shuffle(self, axis, seed=seed, out=out)
