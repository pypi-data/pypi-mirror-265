"""Collection of Startai neural network layers as stateful classes."""

# flake8: noqa
# local
import startai
from startai.func_wrapper import handle_nestable
from startai.stateful.initializers import GlorotUniform, Zeros
...
class Linear(Module):
    ...
    def _create_variables(self, device=None, dtype=None):
        ...
        dtype
            the desired data type of the internal variables to be created if not
             provided. Default is ``None``.
        """
        device = startai.default(device, self.device)
        dtype = startai.default(dtype, self.dtype)
        v = {
    ...
    def _forward(self, x):
        ...
        ret
            ...
            *[batch_shape, out]*
        """
        return startai.linear(x, self.v.w, bias=self.v.b if self._with_bias else None)
...
class MultiHeadAttention(Module):
    def __init__(
        ...
        *,
        ...
        self._scale = startai.default(scale, self._head_dim**-0.5)
...
class LSTM(Module):
    ...
    def get_initial_state(self, batch_shape, dtype=None):
        ...
        return (
            [
                startai.zeros((batch_shape + [self._output_channels]), dtype=dtype)
...
class Embedding(Module):
    ...
    def _pad_embd(self, indices, embd):
        mask = startai.expand_dims(indices == self._padding_idx, axis=-1)
        mask_val = startai.array(0.0, dtype=embd.dtype)
        return startai.where(mask, mask_val, embd)
    ...
    def _forward(self, indices):
        ...
        -------
            The output array of the layer.
        """
        emb = startai.embedding(self.v.w, indices, max_norm=self._max_norm)
