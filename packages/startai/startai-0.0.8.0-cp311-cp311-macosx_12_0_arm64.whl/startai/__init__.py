...
import startai.utils.backend.handler
from startai.utils import check_for_binaries
from startai._version import __version__ as __version__

_not_imported_backends = list(startai.utils.backend.handler._backend_dict.keys())
try:
...
warnings.filterwarnings("ignore", module="^(?!.*startai).*$")


# Local Startai

import_module_path = "startai.utils._importlib"


def is_local():
    return hasattr(startai, "_is_local_pkg")
...
class FrameworkStr(str):
    def __new__(cls, fw_str):
        startai.utils.assertions.check_elem_in_list(
            fw_str, startai.utils.backend.handler._backend_dict.keys()
        )
...
class Device(str):
    def __new__(cls, dev_str):
        if dev_str != "":
            startai.utils.assertions.check_elem_in_list(dev_str[0:3], ["gpu", "tpu", "cpu"])
            if dev_str != "cpu":
                # startai.assertions.check_equal(dev_str[3], ":")
...
class Dtype(str):
    def __new__(cls, dtype_str):
        ...
        if not isinstance(dtype_str, str):
            raise startai.utils.exceptions.StartaiException("dtype must be type str")
        if dtype_str not in _all_startai_dtypes_str:
            raise startai.utils.exceptions.StartaiException(
                f"{dtype_str} is not supported by startai"
            )
    ...
    def __ge__(self, other):
        ...
        return self == startai.promote_types(self, other)
    ...
    def info(self):
        if self.is_int_dtype or self.is_uint_dtype:
            ...
            raise startai.utils.exceptions.StartaiError(f"{self} is not supported by info")
...
class Shape(Sequence):
    def __init__(self, shape_tup):
        valid_types = (int, list, tuple, startai.Array, startai.Shape)
        if len(backend_stack) != 0:
            valid_types += (startai.NativeShape, startai.NativeArray)
        else:
            valid_types += (
                current_backend(shape_tup).NativeShape,
                current_backend(shape_tup).NativeArray,
            )
        startai.utils.assertions.check_isinstance(shape_tup, valid_types)
        if len(backend_stack) == 0:
            ...
            self._shape = startai.to_native_shape(shape_tup)
        else:
            self._shape = None

    @staticmethod
    def _shape_casting_helper(startai_shape, other):
        if isinstance(other, tuple) and not isinstance(startai_shape, tuple):
            return tuple(startai_shape)
        elif isinstance(other, list) and not isinstance(startai_shape, list):
            return list(startai_shape)
        else:
            return startai_shape
    ...
    def __repr__(self):
        ...
        return (
            f"startai.Shape({shape_repr})" if self._shape is not None else "startai.Shape(None)"
        )
    ...
    def __getitem__(self, key):
        try:
            return to_startai(self._shape[key])
    ...
    def index(self, index):
        ...
        if self._shape.rank is None:
            ...
            return to_startai(self._shape[index])

    def as_dimension(self):
        if isinstance(self._shape, Shape):
            return to_startai(self._shape)
        else:
...
class IntDtype(Dtype):
    def __new__(cls, dtype_str):
        if dtype_str is builtins.int:
            dtype_str = default_int_dtype()
        if not isinstance(dtype_str, str):
            raise startai.utils.exceptions.StartaiException("dtype_str must be type str")
...
warn_to_regex = {"all": "!.*", "startai_only": "^(?!.*startai).*$", "none": ".*"}
...
from .data_classes.array import Array, add_startai_array_instance_methods
...
from .data_classes.container import (
    ContainerBase,
    Container,
    add_startai_container_instance_methods,
)
...
from startai.utils.backend import (
    current_backend,
    compiled_backends,
    with_backend,
    set_backend,
...
from startai.utils.inspection import fn_array_spec, add_array_specs
...
try:
    ...
    from .engines import startai2xla
except:  # noqa: E722
    pass
try:
...
try:
    ...
    # Skip framework imports done by Startai compiler for now
    for backend_framework in _not_imported_backends.copy():
        if backend_framework in sys.modules:
...
# add instance methods to Startai Array and Container
from startai.functional.startai import (
    activations,
    creation,
    data_type,
    device,
...
add_startai_array_instance_methods(
    Array,
    [
        activations,
        arr_conversions,
...
# defines startai.globals attribute
...
def _assert_array_significant_figures_formatting(sig_figs):
    startai.utils.assertions.check_isinstance(sig_figs, int)
    startai.utils.assertions.check_greater(sig_figs, 0, as_array=False)
...
startai.array_significant_figures = (
...
def set_array_significant_figures(sig_figs):
    ...
    startai.__setattr__("array_significant_figures", sig_figs, True)
...
def _assert_array_decimal_values_formatting(dec_vals):
    startai.utils.assertions.check_isinstance(dec_vals, int)
    startai.utils.assertions.check_greater(dec_vals, 0, allow_equal=True, as_array=False)


startai.array_decimal_values = (
...
def set_array_decimal_values(dec_vals):
    ...
    startai.__setattr__("array_decimal_values", dec_vals, True)
...
startai.warning_level = warning_level_stack[-1] if warning_level_stack else "startai_only"
...
def set_warning_level(warn_level):
    ...
    ----------
    warn_level
        string for the warning level to be set, one of "none", "startai_only", "all"
    """
    global warning_level_stack
    warning_level_stack.append(warn_level)
    startai.__setattr__("warning_level", warn_level, True)
...
def unset_warning_level():
    ...
    if warning_level_stack:
        ...
        warn_level = warning_level_stack[-1] if warning_level_stack else "startai_only"
        startai.__setattr__("warning_level", warn_level, True)


def warn(warning_message, stacklevel=0):
    warn_level = startai.warning_level
    warnings.filterwarnings("ignore", module=warn_to_regex[warn_level])
    warnings.warn(warning_message, stacklevel=stacklevel)


# nan policy #
startai.nan_policy = nan_policy_stack[-1] if nan_policy_stack else "nothing"
...
def set_nan_policy(warn_level):
    ...
    startai.__setattr__("nan_policy", warn_level, True)
...
startai.dynamic_backend = dynamic_backend_stack[-1] if dynamic_backend_stack else True
...
def set_dynamic_backend(flag):  # noqa: D209
    ...
    startai.__setattr__("dynamic_backend", flag, True)
...
startai.cython_wrappers_mode = cython_wrappers_stack[-1] if cython_wrappers_stack else False
...
def set_cython_wrappers_mode(flag: bool = True) -> None:
    ...
    startai.__setattr__("cython_wrappers_mode", flag, True)
...
class DynamicBackendContext:
    ...
    def __enter__(self):
        self.original = startai.dynamic_backend
...
for backend_framework in _not_imported_backends:
    if backend_framework in sys.modules:
        warnings.warn(
            f"{backend_framework} module has been imported while startai doesn't "
...
INTERNAL_FILENAMES = [
    os.path.join("startai", "compiler"),
    os.path.join("startai", "functional"),
    os.path.join("startai", "data_classes"),
    os.path.join("startai", "stateful"),
    os.path.join("startai", "utils"),
    os.path.join("startai_tests", "test_startai"),
    os.path.join("startai", "func_wrapper.py"),
    os.path.join("startai", "__init__.py"),
]
...
class LoggingMode:
    ...
    def set_logging_mode(self, mode):
        """Set the current logging mode for Startai.
...
class StartaiWithGlobalProps(sys.modules[__name__].__class__):
...
if (
    "startai" in sys.modules
    and sys.modules["startai"].utils._importlib.IS_COMPILING_WITH_BACKEND
):
    # Required for startai.with_backend internal compilation
    sys.modules["startai"].utils._importlib.import_cache[
        __name__
    ].__class__ = StartaiWithGlobalProps
else:
    sys.modules[__name__].__class__ = StartaiWithGlobalProps
