...
class MaskedArray(np_frontend.ndarray):
    def __init__(
        ...
        # shrink
        if shrink and not startai.any(self._mask):
            self._mask = startai.array(False)
        # copy
        if copy:
            self._data = startai.copy_array(self._data)
            self._mask = startai.copy_array(self._mask)

    def _init_data(self, data, dtype, mask, keep_mask):
        if _is_masked_array(data):
            self._data = (
                startai.array(data.data, dtype=dtype)
                if startai.exists(dtype)
                else startai.array(data.data)
            )
            self._init_mask(mask)
            if keep_mask:
                if not isinstance(data.mask, bool):
                    startai.utils.assertions.check_equal(
                        startai.shape(self._mask),
                        startai.shape(data.mask),
                        message="shapes of input mask does not match current mask",
                        as_array=False,
                    )
                self._mask = startai.bitwise_or(self._mask, data.mask)
        else:
            self._data = (
                startai.array(data, dtype=dtype) if startai.exists(dtype) else startai.array(data)
            )
            self._init_mask(mask)
        self._dtype = self._data.dtype

    def _init_mask(self, mask):
        if isinstance(mask, list) or startai.is_array(mask):
            startai.utils.assertions.check_equal(
                startai.shape(self._data),
                startai.shape(startai.array(mask)),
                message="shapes of data and mask must match",
                as_array=False,
            )
            self._mask = startai.array(mask)
        elif mask.all():
            self._mask = startai.ones_like(self._data)
        else:
            self._mask = startai.zeros_like(self._data)
        self._mask = self._mask.astype("bool")

    def _init_fill_value(self, fill_value):
        if startai.exists(fill_value):
            self._fill_value = startai.array(fill_value, dtype=self._dtype)
        elif startai.is_bool_dtype(self._dtype):
            self._fill_value = startai.array(True)
        elif startai.is_int_dtype(self._dtype):
            self._fill_value = startai.array(999999, dtype="int64")
        else:
            self._fill_value = startai.array(1e20, dtype="float64")

    def _init_ndmin(self, ndmin):
        startai.utils.assertions.check_isinstance(ndmin, int)
        if ndmin > len(startai.shape(self._data)):
            self._data = startai.expand_dims(self._data, axis=0)
            self._mask = startai.expand_dims(self._mask, axis=0)

    def _init_hard_mask(self, hard_mask):
        startai.utils.assertions.check_isinstance(hard_mask, bool)
    ...
    def __getitem__(self, query):
        if self._mask.shape != self._data.shape:
            self._mask = startai.ones_like(self._data, dtype=startai.bool) * self._mask
        if self._fill_value.shape != self._data.shape:
            self._fill_value = startai.ones_like(self._data) * self._fill_value
    ...
    def __setitem__(self, query, val):
        ...
        if self._mask.shape != self._data.shape:
            self._mask = startai.ones_like(self._data, dtype=startai.bool) * self._mask
        val_mask = startai.ones_like(self._mask[query]) * getattr(val, "_mask", False)
        if self._hard_mask:
