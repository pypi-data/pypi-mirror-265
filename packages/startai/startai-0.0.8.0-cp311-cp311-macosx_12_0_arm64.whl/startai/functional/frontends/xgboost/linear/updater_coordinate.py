import startai
from startai.functional.frontends.xgboost.linear.coordinate_common import (
    get_bias_gradient,
...
def coordinate_updater(gpair, data, lr, weight, n_feat, n_iter, reg_alpha, reg_lambda):
    ...
    hess = startai.expand_dims(gpair[:, 1], axis=1)

    # don't update where hessian is less than zero
    mask = startai.where(hess < 0.0, 0.0, 1.0)
    sum_hess = startai.sum(startai.square(data) * hess * mask, axis=0, keepdims=True)
    sum_grad = startai.sum(data * grad * mask, axis=0, keepdims=True)
    ...
    return startai.vstack([feature_weights, bias_weights])
