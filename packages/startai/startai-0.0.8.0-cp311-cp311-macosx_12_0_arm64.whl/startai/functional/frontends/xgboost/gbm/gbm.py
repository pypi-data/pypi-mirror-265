import startai
from startai.functional.frontends.xgboost.objective.regression_loss import (
    LogisticRegression,
)
from startai.functional.frontends.xgboost.linear.updater_coordinate import (
    coordinate_updater,
)
...
class GBLinear:
    def __init__(self, params=None, compile=False, cache=None):
        ...
        self.weight = startai.zeros(
            (self.num_feature + 1, self.num_output_group), dtype=startai.float32
        )
        ...
        if self.compile:
            ...
            backend_compile = True if startai.current_backend_str() != "torch" else False
            self._comp_pred = startai.trace_graph(_pred, backend_compile=backend_compile)
            self._comp_get_gradient = startai.trace_graph(
                _get_gradient, backend_compile=backend_compile, static_argnums=(0,)
            )
            self._comp_updater = startai.trace_graph(
    ...
    def check_convergence(self):
        ...
        largest_dw = startai.max(startai.abs(self.weight - self.prev_weight))
...
def _get_gradient(obj, pred, label, scale_pos_weight):
    ...
    w_scaled = startai.where(label == 1.0, w * scale_pos_weight, w)

    return startai.hstack(
        [
...
def _pred(dt, w, base):
    return startai.matmul(dt, w[:-1]) + w[-1] + base
