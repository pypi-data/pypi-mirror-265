...
from startai.functional.frontends.tensorflow.func_wrapper import _to_startai_array
...
class EagerTensor:
    ...
    def __repr__(self):
        return (
            repr(self.startai_array).replace(
                "startai.array", "startai.frontends.tensorflow.EagerTensor"
            )[:-1]
            + ", shape="
            + str(self.shape)
            + ", dtype="
            + str(self.startai_array.dtype)
            + ")"
        )
    ...
    def dtype(self):
        return tf_frontend.DType(
            tf_frontend.tensorflow_type_to_enum[self.startai_array.dtype]
        )

    @property
    def shape(self):
        return TensorShape(self.startai_array.shape.shape)
    ...
    def __bool__(self, name="bool"):
        temp = startai.squeeze(self.startai_array, axis=None)
        if temp.shape != ():
            raise ValueError(
    ...
    def __getitem__(self, slice_spec, var=None, name="getitem"):
        startai_args = startai.nested_map(_to_startai_array, [self, slice_spec])
    ...
    def __setitem__(self, key, value):
        raise startai.utils.exceptions.StartaiException(
            "startai.functional.frontends.tensorflow.EagerTensor object "
...
class TensorShape:
    ...
    def startai_shape(self):
        return startai.Shape(self._dims)
    ...
    def num_elements(self):
        return startai.to_scalar(startai.prod(self._dims))
