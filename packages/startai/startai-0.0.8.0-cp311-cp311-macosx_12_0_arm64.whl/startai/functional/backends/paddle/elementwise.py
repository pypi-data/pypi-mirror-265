...
def positive(
    ...
    if not isinstance(x, paddle.Tensor):
        x = paddle.to_tensor(
            x, dtype=startai.default_dtype(item=x, as_native=True)
        ).squeeze()
    return x.clone()
...
def trapz(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        if y.shape[axis] < 2:
            return startai.zeros_like(startai.squeeze(y, axis=axis))
        ret = startai.sum(
            startai.divide(
                startai.multiply(
                    d,
                    startai.add(
                        startai.get_item(y, tuple(slice1)), startai.get_item(y, tuple(slice2))
                    ),
                ),
                2.0,
            ),
...
def abs(
    ...
    *,
    ...
    if not isinstance(x, paddle.Tensor):
        x = paddle.to_tensor(x, dtype=startai.default_dtype(item=x)).squeeze()
    return paddle.abs(x)
...
def logaddexp2(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        return startai.log2(startai.exp2(x1) + startai.exp2(x2))
    ...
    *,
    ...
    with startai.ArrayMode(False):
        if startai.is_int_dtype(x):
            if posinf is None:
                posinf = startai.iinfo(x).max
            if neginf is None:
                neginf = startai.iinfo(x).min
        elif startai.is_float_dtype(x) or startai.is_complex_dtype(x):
            if posinf is None:
                posinf = startai.finfo(x).max
            if neginf is None:
                neginf = startai.finfo(x).min
        ret = startai.where(startai.isnan(x), paddle.to_tensor(nan, dtype=x.dtype), x)
        ret = startai.where(
            startai.logical_and(startai.isinf(ret), ret > 0),
