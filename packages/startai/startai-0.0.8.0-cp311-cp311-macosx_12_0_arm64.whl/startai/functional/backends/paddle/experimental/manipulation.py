...
from startai.functional.startai.experimental.manipulation import (
    _check_paddle_pad,
    _to_paddle_padding,
)
...
def vstack(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        ...
        return startai.concat(arrays, axis=0)
...
def hstack(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        if arrays[0].ndim >= 2:
            return startai.concat(arrays, axis=1)
        else:
            return startai.concat(arrays, axis=0)
...
def flatten(
    ...
    *,
    ...
    if order == "F":
        with startai.ArrayMode(False):
            x = startai.permute_dims(x, list(reversed(range(x.ndim))))
            ret = _flatten(x, start_dim, end_dim)
            return startai.permute_dims(ret, list(reversed(range(ret.ndim))))
...
def vsplit(
    ...
    *,
    ...
    return startai.split(ary, copy=copy, num_or_size_splits=indices_or_sections, axis=0)
...
def dsplit(
    ...
    *,
    ...
    return startai.split(ary, num_or_size_splits=indices_or_sections, axis=2)
...
def atleast_1d(
    *arys: paddle.Tensor, copy: Optional[bool] = None
    ...
    res = []
    for ary in arys:
        ary = startai.array(ary, copy=copy).data
        if ary.ndim < 1:
            with startai.ArrayMode(False):
                res.append(startai.expand_dims(ary, axis=0))
        else:
            res.append(ary)
    if len(res) == 1:
        return res[0]
...
def dstack(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        arrays = startai.atleast_2d(*arrays)
        if not isinstance(arrays, list):
            arrays = [arrays]
        if arrays[0].ndim < 3:
            return startai.stack(arrays, axis=-1)
        else:
            return startai.concat(arrays, axis=2)
...
def atleast_2d(
    *arys: paddle.Tensor, copy: Optional[bool] = None
    ...
    for ary in arys:
        ...
        if ary.ndim < 2:
            with startai.ArrayMode(False):
                res.append(startai.expand_dims(ary, axis=list(range(2 - ary.ndim))))
        else:
            res.append(ary)
    if len(res) == 1:
        return res[0]
...
def take_along_axis(
    ...
    *,
    ...
    if mode == "clip":
        ...
        with startai.ArrayMode(False):
            indices = startai.clip(indices, 0, max_index)
        ...
        with startai.ArrayMode(False):
            indices = startai.where(
                (indices < 0) | (indices >= arr.shape[axis]), -1, indices
            )
            arr_shape = list(arr_shape)
            arr_shape[axis] = 1
            fill_arr = startai.full(arr_shape, fill_value, dtype=arr.dtype)
            arr = startai.concat([arr, fill_arr], axis=axis)
            indices = startai.where(indices < 0, arr.shape[axis] + indices, indices)
...
def hsplit(
    ...
    *,
    ...
    if ary.ndim == 1:
        return startai.split(ary, num_or_size_splits=indices_or_sections, axis=0)
    return startai.split(ary, num_or_size_splits=indices_or_sections, axis=1)
...
def concat_from_sequence(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        if new_axis == 0:
            return startai.concat(input_sequence, axis=axis)
        elif new_axis == 1:
            return startai.stack(input_sequence, axis=axis)
...
def unique_consecutive(
    ...
    *,
    ...
    output = paddle.concat(
        [
            startai.current_backend().unique_all(sub_array, axis=axis)[0]
...
def _take_with_axis(
    ...
    ret = startai.array(x)[tuple(slicer)]
