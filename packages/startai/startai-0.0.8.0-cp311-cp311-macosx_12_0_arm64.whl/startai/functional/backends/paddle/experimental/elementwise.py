...
import startai
from startai import promote_types_of_inputs
from startai.functional.backends.paddle.elementwise import _elementwise_helper
...
def sinc(x: paddle.Tensor, /, *, out: Optional[paddle.Tensor] = None) -> paddle.Tensor:
    y = startai.pi * paddle.where(x == 0, paddle.to_tensor(1.0e-20, dtype=x.dtype), x)
...
def ldexp(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        if startai.any(startai.less(x2, 0)):
            pos_exp = startai.greater_equal(x2, 0).astype(x2.dtype) * x2
            neg_exp = startai.less(x2, 0).astype(x2.dtype) * x2
            ret = startai.multiply(startai.pow(2, pos_exp), x1)
            ret = startai.divide(ret, startai.pow(2, -neg_exp))
        else:
            ret = startai.multiply(startai.pow(2, x2), x1)
        return startai.astype(ret, out_dtype, copy=False)
...
def nextafter(
    ...
    *,
    ...
    x1, x2 = startai.promote_types_of_inputs(x1, x2)
    with startai.ArrayMode(False):
        eps = startai.finfo(x1.dtype).eps
        return startai.where(
            startai.equal(x1, x2),
            x2,
            startai.where(startai.greater(x2, x1), startai.add(x1, eps), startai.subtract(x1, eps)),
        )
...
def zeta(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        s, a = startai.promote_types_of_inputs(x, q)
        ...
        k = paddle.unsqueeze(startai.arange(N, dtype=q.dtype), tuple(range(q.ndim)))
        S = paddle.sum((a_ + k) ** -s_, -1)
        Q = startai.divide((q + N) ** (1 - x), x - 1)
        T0 = (q + N) ** -x
        m = paddle.unsqueeze(startai.arange(2 * M, dtype=s.dtype), tuple(range(s.ndim)))
        s_over_a = (s_ + m) / (a_ + N)
        s_over_a = startai.where(
            s_over_a == 0, paddle.ones_like(s_over_a) * 1e-20, s_over_a
        )
        T1 = paddle.cumprod(s_over_a, -1)[..., ::2]
        # t=np.array(T1)
        T1 = paddle.clip(T1, max=startai.finfo(T1.dtype).max)
...
def xlogy(
    ...
    with startai.ArrayMode(False):
        x_ok = startai.not_equal(x, 0.0)
        safe_x = startai.where(x_ok, x, 1.0)
        safe_y = startai.where(x_ok, y, 1.0)
        return startai.where(
            x_ok, startai.multiply(safe_x, startai.log(safe_y)), startai.zeros_like(x)
        ).cast(ret_dtype)
...
def count_nonzero(
    ...
    *,
    ...
    with startai.ArrayMode(False):
        return startai.sum(startai.not_equal(a, 0), axis=axis, keepdims=keepdims, dtype=dtype)
