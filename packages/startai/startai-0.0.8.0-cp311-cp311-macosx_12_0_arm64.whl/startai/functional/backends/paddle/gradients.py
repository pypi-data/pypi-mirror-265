"""Collection of Paddle gradient functions, wrapped to fit Startai syntax and
signature."""
...
# local
import startai
from startai.func_wrapper import with_unsupported_device_and_dtypes
...
def _grad_func(y, xs, retain_grads):
    ...
    if isinstance(xs, paddle.Tensor):
    ...
    elif isinstance(xs, startai.Container):
        ...
        if isinstance(grads, startai.Container):
            ...
            grads = startai.add(grads, grads_)
        else:
            grads = grads_ if grads is None else grads
    else:
        ...
        grads = startai.nested_map(grad_, xs, include_derived=True, shallow=False)
        grads = startai.nested_multi_map(
...
def execute_with_gradients(
    ...
    if isinstance(xs, startai.Container):
        ...
        xs = startai.set_nest_at_indices(xs, duplicate_indices, None, shallow=False)
    ...
    if isinstance(y, startai.NativeArray):
        ...
        #
        y = _get_native_y(y)
        grad_arr_idxs = startai.nested_argwhere(y, lambda x: startai.is_native_array(x))
        grad_arr_values = startai.multi_index_nest(y, grad_arr_idxs)
        grads_ = [
...
def value_and_grad(func):
    def grad_fn(xs):
        return startai.to_native(func(xs))
    ...
    def callback_fn(xs):
        ...
        def autograd_fn(x):
            ...
            grad = startai.to_startai(grad)
            return grad

        grads = startai.nested_map(autograd_fn, xs, include_derived=True, shallow=False)
...
def _get_jac_one_arg_fn(grad_fn, xs, out_idx):
    nested_indices = iter(startai.all_nested_indices(xs))

    def one_arg_fn(x):
        idx = next(nested_indices)
        new_xs = startai.set_nest_at_index(xs, idx, x, shallow=False) if idx else x
        ret = grad_fn(new_xs)
        for i in out_idx:
            ret = ret[i]
        return ret
...
def _get_one_out_fn(grad_fn, xs, fn_ret):
    out_nested_indices = iter(startai.all_nested_indices(fn_ret))

    def one_out_fn(o):
        out_idx = next(out_nested_indices)
        out_shape = startai.index_nest(grad_fn(xs), out_idx).shape
        one_arg_fn = _get_jac_one_arg_fn(grad_fn, xs, out_idx)
        jacobian = startai.nested_map(
            lambda x: jacobian_to_startai(
                paddle.incubate.autograd.Jacobian(
                    one_arg_fn, startai.to_native(x.expand_dims())
                ),
                x.shape,
                out_shape,
            ),
...
def jacobian_to_startai(jacobian, in_shape, out_shape):
    jac_startai = startai.to_startai(jacobian[:])
    jac_shape = out_shape + in_shape
    jac_reshaped = jac_startai.reshape(jac_shape)
    return jac_reshaped
...
def jac(func: Callable):
    ...
    def callback_fn(xs):
        ...
        jacobian = startai.nested_map(one_out_fn, fn_ret)
        return jacobian
