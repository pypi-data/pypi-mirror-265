...
def bitwise_and(
    ...
    *,
    ...
    x1, x2 = startai.promote_types_of_inputs(x1, x2, array_api_promotion=True)
...
def divide(
    ...
    *,
    ...
    if startai.is_float_dtype(x1.dtype) or startai.is_complex_dtype(x1.dtype):
        ...
        ret = tf.cast(ret, dtype=startai.default_float_dtype(as_native=True))
    return ret
...
def isfinite(
    ...
    *,
    ...
    if startai.is_int_dtype(x):
        return tf.ones_like(x, tf.bool)
    elif startai.is_complex_dtype(x):
...
def isinf(
    ...
    *,
    ...
    if not startai.is_complex_dtype(x):
        if startai.is_int_dtype(x):
...
def logaddexp2(
    ...
    *,
    ...
    x1, x2 = promote_types_of_inputs(x1, x2)
    if not startai.is_float_dtype(x1):
        x1 = tf.cast(x1, startai.default_float_dtype(as_native=True))
        x2 = tf.cast(x2, startai.default_float_dtype(as_native=True))
    amax = startai.maximum(x1, x2)
    delta = x1 - x2
    return startai.where(
        startai.isnan(delta),
        x1 + x2,
        amax + startai.log1p(startai.exp2(-startai.abs(delta))) / startai.log(2.0).astype(amax.dtype),
    )
...
def pow(
    ...
    *,
    ...
    if startai.is_complex_dtype(x1) and startai.any(startai.isinf(x2)):
        ret = tf.experimental.numpy.power(x1, x2)
        return tf.where(
            startai.isinf(x2), startai.nan + startai.nan * 1j if x2 < 0 else -0 * 1j, ret
        )
    if startai.is_complex_dtype(x2) and startai.any(x1 == 0):
        ret = tf.experimental.numpy.power(x1, x2)
        return tf.where(x1 == 0, startai.nan + startai.nan * 1j, ret)
    x1, x2 = startai.promote_types_of_inputs(x1, x2)
    if startai.is_int_dtype(x1) and startai.any(x2 < 0):
        return tf.cast(
...
def remainder(
    ...
    *,
    ...
    if not modulus:
        ...
        diff, x2 = startai.promote_types_of_inputs(diff, x2)
...
def trunc(
    ...
    *,
    ...
    ret = x
    if not startai.is_array(x):
        raise startai.utils.exceptions.StartaiException("Input must be array")
