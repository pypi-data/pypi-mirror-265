...
def matmul(
    ...
    *,
    ...
    if (
        ...
        raise startai.utils.exceptions.StartaiException("Error,shapes not compatible")
    ...
    ret = startai.to_native(startai.astype(ret, dtype_from, copy=False))
    if x1_padded_2:
        ret = ret[0]
    elif x1_padded:
...
def matrix_norm(
    ...
    *,
    ...
    if dtype is not None:
        x = startai.astype(x, dtype).to_native()
    if ord == "nuc":
...
def pinv(
    ...
    *,
    ...
    if rtol is None:
        ...
        x, rtol = startai.promote_types_of_inputs(x, rtol)
