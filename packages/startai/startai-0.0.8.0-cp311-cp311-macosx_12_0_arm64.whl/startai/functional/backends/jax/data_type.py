...
def finfo(type: Union[jnp.dtype, str, JaxArray, np.ndarray], /) -> Finfo:
    ...
    return Finfo(jnp.finfo(startai.as_native_dtype(type)))
...
def iinfo(type: Union[jnp.dtype, str, JaxArray, np.ndarray], /) -> np.iinfo:
    if isinstance(type, np.ndarray):
        type = type.dtype.name
    return jnp.iinfo(startai.as_native_dtype(type))


def result_type(*arrays_and_dtypes: Union[JaxArray, jnp.dtype]) -> startai.Dtype:
...
def dtype(x: Union[JaxArray, np.ndarray], *, as_native: bool = False) -> startai.Dtype:
    if as_native:
