...
def argmax(
    ...
    *,
    ...
    >>> x = startai.array([-0., 1., -1.])
    >>> z = startai.zeros((1,3), dtype=startai.int64)
    >>> startai.argmax(x, out=z)
    >>> print(z)
    startai.array(1)

    >>> x = startai.array([[1., -0., -1.], [-2., 3., 2.]])
    >>> y = startai.argmax(x, axis=1)
    >>> print(y)
    startai.array([0, 1])

    >>> x = startai.array([[4., 0., -1.], [2., -3., 6]])
    >>> y = startai.argmax(x, axis=1, keepdims=True)
    >>> print(y)
    startai.array([[0], [2]])

    >>> x = startai.array([[4., 0., -1.], [2., -3., 6]])
    >>> y = startai.argmax(x, axis=1, dtype=startai.int64)
    >>> print(y, y.dtype)
    startai.array([0, 2]) int64

    >>> x = startai.array([[4., 0., -1.],[2., -3., 6], [2., -3., 6]])
    >>> z = startai.zeros((3,1), dtype=startai.int64)
    >>> y = startai.argmax(x, axis=1, keepdims=True, out=z)
    >>> print(z)
    startai.array([[0],[2],[2]])
...
def argmin(
    ...
    *,
    ...
    >>> x = startai.array([[0., 1., -1.],[-2., 1., 2.]])
    >>> y = startai.argmin(x, axis=1)
    >>> print(y)
    startai.array([2, 0])

    >>> x = startai.array([[0., 1., -1.],[-2., 1., 2.]])
    >>> y = startai.argmin(x, axis=1, keepdims=True)
    >>> print(y)
    startai.array([[2],
           [0]])
    ...
    >>> startai.argmin(x, axis=1, keepdims=True, out=y)
    >>> print(y)
    startai.array([[2],
           [0],
           [1]])
...
def nonzero(
    ...
    *,
    ...
    >>> x = startai.array([0, 10, 15, 20, -50, 0])
    >>> y = startai.nonzero(x)
    >>> print(y)
    ...
    >>> y = startai.nonzero(x)
    >>> print(y)
    (startai.array([0, 0, 1, 1]), startai.array([0, 1, 0, 1]))

    >>> x = startai.array([[0, 2], [-1, -2]])
    >>> y = startai.nonzero(x, as_tuple=False)
    >>> print(y)
    startai.array([[0, 1], [1, 0], [1, 1]])

    >>> x = startai.array([0, 1])
    >>> y = startai.nonzero(x, size=2, fill_value=4)
    >>> print(y)
    (startai.array([1, 4]),)

    With :class:`startai.NativeArray` input:

    >>> x = startai.native_array([[10, 20], [10, 0], [0, 0]])
    >>> y = startai.nonzero(x)
    >>> print(y)
    (startai.array([0, 0, 1]), startai.array([0, 1, 0]))

    >>> x = startai.native_array([[0], [1], [1], [0], [1]])
    >>> y = startai.nonzero(x)
    >>> print(y)
    (startai.array([1, 2, 4]), startai.array([0, 0, 0]))

    With :class:`startai.Container` input:

    >>> x = startai.Container(a=startai.array([0,1,2,3,0]), b=startai.array([1,1, 0,0]))
    >>> y = startai.nonzero(x)
    >>> print(y)
    [{
    ...
    >>> x = startai.Container(a=startai.array([1,1,1]), b=startai.native_array([0]))
    >>> y = x.nonzero()
    >>> print(y)
    [{
...
@handle_device
def where(
    condition: Union[startai.Array, startai.NativeArray],
    ...
    *,
    ...
    >>> res = startai.where(condition, x1, x2)
    >>> print(res)
    startai.array([[1, 6],
           [3, 4]])

    >>> x1 = startai.array([[6, 13, 22, 7, 12], [7, 11, 16, 32, 9]])
    >>> x2 = startai.array([[44, 20, 8, 35, 9], [98, 23, 43, 6, 13]])
    >>> res = startai.where(((x1 % 2 == 0) & (x2 % 2 == 1)), x1, x2)
    >>> print(res)
    startai.array([[44, 20,  8, 35, 12],
           [98, 23, 16,  6, 13]])
...
def argwhere(
    ...
    *,
    ...
    >>> x = startai.array([[0, 2], [3, 4]])
    >>> y = startai.zeros((3, 2), dtype=startai.int64)
    >>> res = startai.argwhere(x, out=y)
    >>> print(res)
    startai.array([[0, 1], [1, 0], [1, 1]])

    With a :class:`startai.Container` input:
