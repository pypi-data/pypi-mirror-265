    ...
    if hasattr(fn, "supported_dtypes") and hasattr(fn, "unsupported_dtypes"):
        ...
        if isinstance(fn_supported_dtypes, dict):
            if isinstance(fn_unsupported_dtypes, dict):
                backend_str = startai.current_backend_str()
                if (
...
def _handle_nestable_dtype_info(fn):
    ...
    def _handle_nestable_dtype_info_wrapper(type):
        if isinstance(type, startai.Container):
        ...
        if isinstance(node, ast.Call):
            ...
            if isinstance(nodef, ast.Name):
                ...
                if (
                    ...
                    and nodef.value.id not in ["startai", "self"]
...
def _get_functions_from_string(func_names, module):
    ret = set()
    # We only care about the functions in the startai or the same module
    for orig_func_name in func_names.keys():
        func_name = orig_func_name.split(".")[-1]
        if hasattr(startai, func_name) and callable(getattr(startai, func_name, None)):
            ret.add(getattr(startai, func_name))
...
# 1. The base functions must be defined in startai or the same module
...
def _nested_get(f, base_set, merge_fn, get_fn, wrapper=set):
    ...
    while to_visit:
        ...
        if is_backend_fn:
            ...
            if hasattr(fn, "partial_mixed_handler"):
                ...
                logging.warning(
                    ...
                    f" 'startai.{fn.__name__}'. Please note that the returned data types"
                    " may not be exhaustive. Please check the dtypes of"
                    f" `startai.{fn.__name__}` for more details"
                )
        ...
        if is_frontend_fn:
            frontends = {
                "jax_frontend": "startai.functional.frontends.jax",
                "jnp_frontend": "startai.functional.frontends.jax.numpy",
                "np_frontend": "startai.functional.frontends.numpy",
                "tf_frontend": "startai.functional.frontends.tensorflow",
                "torch_frontend": "startai.functional.frontends.torch",
                "paddle_frontend": "startai.functional.frontends.paddle",
            }
            for key in fl:
                if "frontend" in key:
                    frontend_fn = fl[key]
                    ...
                    if (
                        frontend_module == ""
                    ):  # single edge case: fn='frontend_outputs_to_startai_arrays'
                        continue
...
def _get_dtypes(fn, complement=True):
    supported = set(startai.valid_dtypes)
    ...
    if not is_backend_fn and not is_frontend_fn and not has_unsupported_dtypes_attr:
        if complement:
            supported = set(startai.all_dtypes).difference(supported)
        return supported
    ...
    basic = [
        ("supported_dtypes", set.intersection, startai.valid_dtypes),
        ("unsupported_dtypes", set.difference, startai.invalid_dtypes),
    ]
    ...
    for key, merge_fn, base in basic:
        if hasattr(fn, key):
            ...
            if isinstance(dtypes, dict):
                dtypes = dtypes.get(startai.current_backend_str(), base)
            startai.utils.assertions.check_isinstance(dtypes, tuple)
            if not dtypes:
                dtypes = base
...
def astype(
    ...
    dtype: Union[startai.Dtype, startai.NativeDtype],
    /,
    *,
    copy: bool = True,
    ...
    >>> y = startai.astype(x, startai.float64)
    >>> print(y)
    startai.array([1., 2.])

    >>> x = startai.array([3.141, 2.718, 1.618])
    >>> y = startai.zeros_like(x)
    >>> startai.astype(x, startai.int32, out=y)
    >>> print(y)
    startai.array([3., 2., 1.])

    >>> x = startai.array([[-1, -2], [0, 2]])
    >>> startai.astype(x, startai.float64, out=x)
    >>> print(x)
    startai.array([[-1., -2.],  [0.,  2.]])

    With :class:`startai.NativeArray` input:

    >>> x = startai.native_array([3.141, 2.718, 1.618])
    >>> y = startai.astype(x, startai.int32)
    >>> print(y)
    startai.array([3, 2, 1])

    With :class:`startai.Container` input:

    >>> x = startai.Container(a=startai.array([0,2,1]),b=startai.array([1,0,0]))
    >>> print(startai.astype(x, startai.bool))
    {
...
@handle_array_function
@handle_device
def broadcast_arrays(*arrays: Union[startai.Array, startai.NativeArray]) -> List[startai.Array]:
    ...
    >>> y = startai.broadcast_arrays(x1, x2)
    >>> print(y)
    [startai.array([1, 2, 3]), startai.array([4, 5, 6])]

    With :class:`startai.NativeArray` inputs:

    >>> x1 = startai.native_array([0.3, 4.3])
    >>> x2 = startai.native_array([3.1, 5])
    >>> x3 = startai.native_array([2, 0])
    >>> y = startai.broadcast_arrays(x1, x2, x3)
    [startai.array([0.3, 4.3]), startai.array([3.1, 5.]), startai.array([2, 0])]

    With mixed :class:`startai.Array` and :class:`startai.NativeArray` inputs:

    >>> x1 = startai.array([1, 2])
    >>> x2 = startai.native_array([0.3, 4.3])
    >>> y = startai.broadcast_arrays(x1, x2)
    >>> print(y)
    [startai.array([1, 2]), startai.array([0.3, 4.3])]

    With :class:`startai.Container` inputs:

    >>> x1 = startai.Container(a=startai.array([3, 1]), b=startai.zeros(2))
    >>> x2 = startai.Container(a=startai.array([4, 5]), b=startai.array([2, -1]))
    >>> y = startai.broadcast_arrays(x1, x2)
    >>> print(y)
    [{
        a: startai.array([3, 1]),
    ...
    With mixed :class:`startai.Array` and :class:`startai.Container` inputs:
...
def broadcast_to(
    ...
    *,
    ...
    >>> x = startai.native_array([0.1 , 0.3])
    >>> y = startai.broadcast_to(x, (3, 2))
    >>> print(y)
    startai.array([[0.1, 0.3],
            [0.1, 0.3],
            [0.1, 0.3]])
...
@handle_device
def can_cast(
    from_: Union[startai.Dtype, startai.Array, startai.NativeArray],
    to: startai.Dtype,
    /,
) -> bool:
    ...
    >>> print(startai.can_cast(startai.uint8, startai.int32))
    True

    >>> print(startai.can_cast(startai.float64, 'int64'))
    False

    With :class:`startai.Array` input:

    >>> x = startai.array([1., 2., 3.])
    >>> print(startai.can_cast(x, startai.float64))
    True

    With :class:`startai.NativeArray` input:

    >>> x = startai.native_array([[-1, -1, -1],
    ...                       [1, 1, 1]],
    ...                       dtype='int16')
    >>> print(startai.can_cast(x, 'uint8'))
    False
    ...
    >>> print(startai.can_cast(x, 'int64'))
    {
        a: False,
        b: True
    }
    """
    if isinstance(from_, (startai.Array, startai.NativeArray)):
        from_ = from_.dtype
    dtype = startai.promote_types(from_, to)
    return dtype == to
...
@handle_device
def finfo(
    type: Union[startai.Dtype, str, startai.Array, startai.NativeArray],
    /,
) -> Finfo:
    ...
    >>> y = startai.finfo(startai.float32)
    >>> print(y)
    finfo(resolution=1e-06, min=-3.4028235e+38, max=3.4028235e+38, dtype=float32)

    With :code:`str` input:

    >>> y = startai.finfo('float32')
    >>> print(y)
    finfo(resolution=1e-06, min=-3.4028235e+38, max=3.4028235e+38, dtype=float32)

    With :class:`startai.Array` input:

    >>> x = startai.array([1.3,2.1,3.4], dtype=startai.float64)
    >>> print(startai.finfo(x))
    finfo(resolution=1e-15, min=-1.7976931348623157e+308, \
    max=1.7976931348623157e+308, dtype=float64)

    >>> x = startai.array([0.7,8.4,3.14], dtype=startai.float16)
    >>> print(startai.finfo(x))
    finfo(resolution=0.001, min=-6.55040e+04, max=6.55040e+04, dtype=float16)

    With :class:`startai.Container` input:

    >>> c = startai.Container(x=startai.array([-9.5,1.8,-8.9], dtype=startai.float16),
    ...                   y=startai.array([7.6,8.1,1.6], dtype=startai.float64))
    >>> print(startai.finfo(c))
    {
...
def iinfo(
    ...
    >>> x = startai.array([13,21,34], dtype=startai.int8)
    >>> startai.iinfo(x)
    iinfo(min=-128, max=127, dtype=int8)

    With :class:`startai.NativeArray` input:

    >>> x = startai.native_array([7,84,314], dtype=startai.int64)
    >>> startai.iinfo(x)
    iinfo(min=-9223372036854775808, max=9223372036854775807, dtype=int64)

    With :class:`startai.Container` input:

    >>> c = startai.Container(x=startai.array([0,1800,89], dtype=startai.uint16),
    ...                   y=startai.array([76,81,16], dtype=startai.uint32))
    >>> startai.iinfo(c)
    {
...
def result_type(
    *arrays_and_dtypes: Union[startai.Array, startai.NativeArray, startai.Dtype]
    ...
    >>> y = startai.array([3., 4., 5.])
    >>> d = startai.result_type(x, y)
    >>> print(d)
    float32

    With :class:`startai.Dtype` input:

    >>> d = startai.result_type(startai.uint8, startai.uint64)
    >>> print(d)
    uint64

    With :class:`startai.Container` input:

    >>> x = startai.Container(a = startai.array([3, 4, 5]))
    >>> d = x.a.dtype
    >>> print(d)
    int32

    >>> x = startai.Container(a = startai.array([3, 4, 5]))
    >>> d = startai.result_type(x, startai.float64)
    >>> print(d)
    {
        a: float64
    }
...
class DefaultDtype:
    """Startai's DefaultDtype class."""

    def __init__(self, dtype: startai.Dtype):
        self._dtype = dtype
...
class DefaultFloatDtype:
    """Startai's DefaultFloatDtype class."""

    def __init__(self, float_dtype: startai.Dtype):
...
class DefaultIntDtype:
    """Startai's DefaultIntDtype class."""

    def __init__(self, int_dtype: startai.Dtype):
...
class DefaultUintDtype:
    """Startai's DefaultUintDtype class."""

    def __init__(self, uint_dtype: startai.UintDtype):
...
class DefaultComplexDtype:
    """Startai's DefaultComplexDtype class."""

    def __init__(self, complex_dtype: startai.Dtype):
...
@handle_exceptions
def dtype_bits(dtype_in: Union[startai.Dtype, startai.NativeDtype, str], /) -> int:
    ...
    Examples
    --------
    With :class:`startai.Dtype` inputs:

    >>> x = startai.dtype_bits(startai.float32)
    >>> print(x)
    32

    >>> x = startai.dtype_bits('int64')
    >>> print(x)
    64

    With :class:`startai.NativeDtype` inputs:

    >>> x = startai.dtype_bits(startai.native_bool)
    >>> print(x)
    1
    """
...
@handle_exceptions
def is_hashable_dtype(dtype_in: Union[startai.Dtype, startai.NativeDtype], /) -> bool:
...
@handle_exceptions
def as_startai_dtype(dtype_in: Union[startai.Dtype, str], /) -> startai.Dtype:
    ...
    """
    return current_backend(None).as_startai_dtype(dtype_in)


@handle_exceptions
def as_native_dtype(dtype_in: Union[startai.Dtype, startai.NativeDtype], /) -> startai.NativeDtype:
    ...
    -------
    ret
        data type e.g. startai.float32.
    """
    return current_backend(None).as_native_dtype(dtype_in)


def _check_float64(input) -> bool:
    if startai.is_array(input):
        return startai.dtype(input) == "float64"
...
def _check_complex128(input) -> bool:
    if startai.is_array(input):
        return startai.dtype(input) == "complex128"
...
@handle_exceptions
def closest_valid_dtype(type: Union[startai.Dtype, str, None], /) -> Union[startai.Dtype, str]:
    ...
    -------
    ret
        The closest valid data type as a native startai.Dtype

    Examples
    --------
    With :class:`startai.Dtype` input:

    >>> xType = startai.float16
    >>> yType = startai.closest_valid_dtype(xType)
    >>> print(yType)
    float16

    With :class:`startai.NativeDtype` inputs:

    >>> xType = startai.native_uint16
    >>> yType = startai.closest_valid_dtype(xType)
    >>> print(yType)
    uint16
...
def default_float_dtype(
    *,
    input: Optional[Union[startai.Array, startai.NativeArray]] = None,
    float_dtype: Optional[Union[startai.FloatDtype, startai.NativeDtype]] = None,
    as_native: bool = False,
) -> Union[startai.Dtype, str, startai.NativeDtype]:
    """
    Parameters
    ----------
    input
    ...
    Returns
    -------
        Return ``float_dtype`` as native or startai dtype if provided, else
    ...
    >>> startai.set_default_float_dtype(startai.FloatDtype("float64"))
    >>> startai.default_float_dtype()
    'float64'

    >>> startai.default_float_dtype(float_dtype=startai.FloatDtype("float16"))
    'float16'

    >>> startai.default_float_dtype(input=4294.967346)
    'float32'

    >>> x = startai.array([9.8,8.9], dtype="float16")
    >>> startai.default_float_dtype(input=x)
    'float16'
    """
    global default_float_dtype_stack
    if startai.exists(float_dtype):
        if as_native is True:
            return startai.as_native_dtype(float_dtype)
        return startai.FloatDtype(startai.as_startai_dtype(float_dtype))
    as_native = startai.default(as_native, False)
    if startai.exists(input):
        if startai.is_array(input):
            ret = startai.dtype(input)
        elif isinstance(input, np.ndarray):
            ret = str(input.dtype)
        elif isinstance(input, (list, tuple, dict)):
            if startai.nested_argwhere(
                ...
                if not default_float_dtype_stack:
                    ...
                    if startai.is_float_dtype(def_dtype):
                        ret = def_dtype
                    else:
                        ret = "float32"
                else:
        ...
        if not default_float_dtype_stack:
            ...
            ret = default_float_dtype_stack[-1]
    if as_native:
        return startai.as_native_dtype(ret)
    return startai.FloatDtype(startai.as_startai_dtype(ret))


@handle_exceptions
def infer_default_dtype(
    dtype: Union[startai.Dtype, startai.NativeDtype, str], as_native: bool = False
    ...
    Examples
    --------
    >>> startai.set_default_int_dtype("int32")
    >>> startai.infer_default_dtype("int8")
    'int8'

    >>> startai.set_default_float_dtype("float64")
    >>> startai.infer_default_dtype("float32")
    'float64'

    >>> startai.set_default_uint_dtype("uint32")
    >>> x = startai.array([0], dtype="uint64")
    >>> startai.infer_default_dtype(x.dtype)
    'uint32'
    """
    if startai.is_complex_dtype(dtype):
        default_dtype = startai.default_complex_dtype(as_native=as_native)
    elif startai.is_float_dtype(dtype):
        default_dtype = startai.default_float_dtype(as_native=as_native)
    elif startai.is_uint_dtype(dtype):
        default_dtype = startai.default_uint_dtype(as_native=as_native)
    elif startai.is_int_dtype(dtype):
        default_dtype = startai.default_int_dtype(as_native=as_native)
    elif as_native:
        default_dtype = startai.as_native_dtype("bool")
    else:
        default_dtype = startai.as_startai_dtype("bool")
...
def default_dtype(
    *,
    dtype: Optional[Union[startai.Dtype, str]] = None,
    item: Optional[Union[startai.Array, startai.NativeArray]] = None,
    as_native: bool = False,
) -> Union[startai.Dtype, startai.NativeDtype, str]:
    """
    Parameters
    ----------
    item
    ...
    Returns
    -------
        Return ``dtype`` as native or startai dtype if provided, else
        if ``item`` is given, return its dtype, otherwise return the
        global default dtype.

    Examples
    --------
    >>> startai.default_dtype()
    'float32'

    >>> startai.set_default_dtype(startai.bool)
    >>> startai.default_dtype()
    'bool'

    >>> startai.set_default_dtype(startai.int16)
    >>> startai.default_dtype()
    'int16'

    >>> startai.set_default_dtype(startai.float64)
    >>> startai.default_dtype()
    'float64'

    >>> startai.default_dtype(dtype="int32")
    'int32'

    >>> startai.default_dtype(dtype=startai.float16)
    'float16'

    >>> startai.default_dtype(item=53.234)
    'float64'

    >>> startai.default_dtype(item=[1, 2, 3])
    'int32'

    >>> x = startai.array([5.2, 9.7], dtype="complex128")
    >>> startai.default_dtype(item=x)
    'complex128'
    """
    if startai.exists(dtype):
        if as_native is True:
            return startai.as_native_dtype(dtype)
        return startai.as_startai_dtype(dtype)
    as_native = startai.default(as_native, False)
    if startai.exists(item):
        if hasattr(item, "override_dtype_check"):
            return item.override_dtype_check()
        elif isinstance(item, (list, tuple, dict)) and len(item) == 0:
            pass
        elif startai.is_complex_dtype(item):
            return startai.default_complex_dtype(input=item, as_native=as_native)
        elif startai.is_float_dtype(item):
            return startai.default_float_dtype(input=item, as_native=as_native)
        elif startai.is_uint_dtype(item):
            return startai.default_int_dtype(input=item, as_native=as_native)
        elif startai.is_int_dtype(item):
            return startai.default_int_dtype(input=item, as_native=as_native)
        elif as_native:
            return startai.as_native_dtype("bool")
        else:
            return "bool"
    ...
    if as_native:
        return startai.as_native_dtype(ret)
    return startai.as_startai_dtype(ret)
...
def default_int_dtype(
    *,
    input: Optional[Union[startai.Array, startai.NativeArray]] = None,
    int_dtype: Optional[Union[startai.IntDtype, startai.NativeDtype]] = None,
    as_native: bool = False,
) -> Union[startai.IntDtype, startai.NativeDtype]:
    """
    Parameters
    ----------
    input
    ...
    Returns
    -------
        Return ``int_dtype`` as native or startai dtype if provided, else
        if ``input`` is given, return its int dtype, otherwise return the
        global default int dtype.

    Examples
    --------
    >>> startai.set_default_int_dtype(startai.intDtype("int16"))
    >>> startai.default_int_dtype()
    'int16'

    >>> startai.default_int_dtype(input=4294967346)
    'int64'

    >>> startai.default_int_dtype(int_dtype=startai.intDtype("int8"))
    'int8'

    >>> x = startai.array([9,8], dtype="int32")
    >>> startai.default_int_dtype(input=x)
    'int32'
    """
    global default_int_dtype_stack
    if startai.exists(int_dtype):
        if as_native is True:
            return startai.as_native_dtype(int_dtype)
        return startai.IntDtype(startai.as_startai_dtype(int_dtype))
    as_native = startai.default(as_native, False)
    if startai.exists(input):
        if startai.is_array(input):
            ret = startai.dtype(input)
        elif isinstance(input, startai.Shape):
            ret = startai.default_int_dtype()
        elif isinstance(input, np.ndarray):
            ret = str(input.dtype)
        elif isinstance(input, (list, tuple, dict)):
            if startai.nested_argwhere(
                input,
                lambda x: (
                    startai.dtype(x) == "uint64"
                    if startai.is_array(x)
                    else x > 9223372036854775807 and x != startai.inf
                ),
                stop_after_n_found=1,
            ):
                ret = startai.uint64
            elif startai.nested_argwhere(
                input,
                lambda x: (
                    startai.dtype(x) == "int64"
                    if startai.is_array(x)
                    else x > 2147483647 and x != startai.inf
                ),
                stop_after_n_found=1,
            ):
                ret = startai.int64
                ...
                if not default_int_dtype_stack:
                    def_dtype = startai.default_dtype()
                    if startai.is_int_dtype(def_dtype):
                        ret = def_dtype
                    else:
                        ret = "int32"
                else:
            ...
            if (
                ...
                and input != startai.inf
                and startai.backend != "torch"
            ):
                ret = startai.uint64
            elif input > 2147483647 and input != startai.inf:
                ret = startai.int64
            else:
                if not default_int_dtype_stack:
                    def_dtype = startai.default_dtype()
    ...
    if as_native:
        return startai.as_native_dtype(ret)
    return startai.IntDtype(startai.as_startai_dtype(ret))
...
def default_uint_dtype(
    *,
    input: Optional[Union[startai.Array, startai.NativeArray]] = None,
    uint_dtype: Optional[Union[startai.UintDtype, startai.NativeDtype]] = None,
    as_native: bool = False,
) -> Union[startai.UintDtype, startai.NativeDtype]:
    """
    Parameters
    ----------
    input
    ...
    Returns
    -------
        Return ``uint_dtype`` as native or startai dtype if provided, else
        if ``input`` is given, return its uint dtype, otherwise return the
        global default uint dtype.

    Examples
    --------
    >>> startai.set_default_uint_dtype(startai.UintDtype("uint16"))
    >>> startai.default_uint_dtype()
    'uint16'

    >>> startai.default_uint_dtype(input=4294967346)
    'uint64'

    >>> startai.default_uint_dtype(uint_dtype=startai.UintDtype("uint8"))
    'uint8'

    >>> x = startai.array([9,8], dtype="uint32")
    >>> startai.default_uint_dtype(input=x)
    'uint32'
    """
    global default_uint_dtype_stack
    if startai.exists(uint_dtype):
        if as_native is True:
            return startai.as_native_dtype(uint_dtype)
        return startai.UintDtype(startai.as_startai_dtype(uint_dtype))
    as_native = startai.default(as_native, False)
    if startai.exists(input):
        if startai.is_array(input):
            ret = startai.dtype(input)
            ...
            def is_native(x):
                return startai.is_native_array(x)
            ...
            if startai.nested_argwhere(
                ...
                if default_uint_dtype_stack:
                    ...
                    def_dtype = startai.default_dtype()
                    if startai.is_uint_dtype(def_dtype):
                        ret = def_dtype
                    else:
                        ret = "uint32"
        elif isinstance(input, Number):
            if input > 4294967295 and input != startai.inf and startai.backend != "torch":
                ret = startai.uint64
            else:
                if default_uint_dtype_stack:
                    ret = default_uint_dtype_stack[-1]
    ...
    if as_native:
        return startai.as_native_dtype(ret)
    return startai.UintDtype(startai.as_startai_dtype(ret))
...
def default_complex_dtype(
    *,
    input: Optional[Union[startai.Array, startai.NativeArray]] = None,
    complex_dtype: Optional[Union[startai.ComplexDtype, startai.NativeDtype]] = None,
    as_native: bool = False,
) -> Union[startai.Dtype, str, startai.NativeDtype]:
    """
    Parameters
    ...
    Returns
    -------
        Return ``complex_dtype`` as native or startai dtype if provided, else
        if ``input`` is given, return its complex dtype, otherwise return the
        global default complex dtype.

    Examples
    --------
    >>> startai.default_complex_dtype()
    'complex64'

    >>> startai.set_default_complex_dtype(startai.ComplexDtype("complex64"))
    >>> startai.default_complex_dtype()
    'complex64'

    >>> startai.default_complex_dtype(complex_dtype=startai.ComplexDtype("complex128"))
    'complex128'

    >>> startai.default_complex_dtype(input=4294.967346)
    'complex64'

    >>> x = startai.array([9.8,8.9], dtype="complex128")
    >>> startai.default_complex_dtype(input=x)
    'complex128'
    """
    global default_complex_dtype_stack
    if startai.exists(complex_dtype):
        if as_native is True:
            return startai.as_native_dtype(complex_dtype)
        return startai.ComplexDtype(startai.as_startai_dtype(complex_dtype))
    as_native = startai.default(as_native, False)
    if startai.exists(input):
        if startai.is_array(input):
            ret = startai.dtype(input)
            ...
            if startai.nested_argwhere(
                ...
                ret = startai.complex128
            else:
                if not default_complex_dtype_stack:
                    def_dtype = default_dtype()
                    if startai.is_complex_dtype(def_dtype):
                        ret = def_dtype
                    else:
                        ret = "complex64"
                else:
    ...
    if as_native:
        return startai.as_native_dtype(ret)
    return startai.ComplexDtype(startai.as_startai_dtype(ret))
...
@handle_device
def dtype(
    x: Union[startai.Array, startai.NativeArray], *, as_native: bool = False
) -> Union[startai.Dtype, startai.NativeDtype]:
    ...
    >>> x1 = startai.array([1.0, 2.0, 3.5, 4.5, 5, 6])
    >>> y = startai.dtype(x1)
    >>> print(y)
    float32
    ...
    >>> x1 = startai.native_array([1, 0, 1, -1, 0])
    >>> y = startai.dtype(x1)
    >>> print(y)
    int32
    ...
    >>> x = startai.Container(a=startai.native_array([1.0, 2.0, -1.0, 4.0, 1.0]),
    ...                   b=startai.native_array([1, 0, 0, 0, 1]))
    >>> y = startai.dtype(x.a)
    >>> print(y)
    float32
    """
...
def function_supported_dtypes(fn: Callable, recurse: bool = True) -> Union[Tuple, dict]:
    ...
    recurse
        Whether to recurse into used startai functions. Default is ``True``.
    ...
    Examples
    --------
    >>> print(startai.function_supported_dtypes(startai.acosh))
...
def function_unsupported_dtypes(
    ...
    >>> print(startai.function_unsupported_dtypes(startai.acosh))
...
@handle_exceptions
def invalid_dtype(dtype_in: Union[startai.Dtype, startai.NativeDtype, str, None], /) -> bool:
    ...
    Examples
    --------
    >>> print(startai.invalid_dtype(None))
    False

    >>> print(startai.invalid_dtype("uint64"))
    False

    >>> print(startai.invalid_dtype(startai.float64))
    False

    >>> print(startai.invalid_dtype(startai.native_uint8))
    False
    """
    if dtype_in is None:
        return False
    return startai.as_startai_dtype(dtype_in) in startai.invalid_dtypes
...
@inputs_to_native_arrays
def is_bool_dtype(
    dtype_in: Union[startai.Dtype, str, startai.Array, startai.NativeArray, Number],
    /,
) -> bool:
    ...
    """
    if startai.is_array(dtype_in):
        dtype_in = startai.dtype(dtype_in)
        ...
        return bool(
            startai.nested_argwhere(
                dtype_in,
                lambda x: isinstance(x, (bool, np.bool_)) and x is not int,
            )
        )
    return "bool" in startai.as_startai_dtype(dtype_in)
...
def is_int_dtype(
    ...
    >>> x = startai.is_int_dtype(startai.float64)
    >>> print(x)
    False
    ...
    >>> x = startai.array([1., 2., 3.])
    >>> print(startai.is_int_dtype(x), x.dtype)
    False float32

    With :class:`startai.NativeArray` input:

    >>> x = startai.native_array([[-1, -1, -1], [1, 1, 1]], dtype=startai.int16)
    >>> print(startai.is_int_dtype(x))
    True
    ...
    if startai.is_array(dtype_in):
        dtype_in = startai.dtype(dtype_in)
    elif isinstance(dtype_in, startai.Shape):
        dtype_in = startai.default_int_dtype()
    elif isinstance(dtype_in, np.ndarray):
        ...
        def nested_fun(x):
            return (
                ...
                or (startai.is_array(x) and "int" in startai.dtype(x))
            ) and x is not bool

        return bool(startai.nested_argwhere(dtype_in, nested_fun))
    return "int" in startai.as_startai_dtype(dtype_in)
...
def is_float_dtype(
    ...
    Examples
    --------
    >>> x = startai.is_float_dtype(startai.float32)
    >>> print(x)
    True

    >>> arr = startai.array([1.2, 3.2, 4.3], dtype=startai.float32)
    >>> print(startai.is_float_dtype(arr))
    True
    """
    if startai.is_array(dtype_in):
        dtype_in = startai.dtype(dtype_in)
        ...
        return bool(
            startai.nested_argwhere(
                ...
                or (startai.is_array(x) and "float" in startai.dtype(x)),
            )
        )
    return "float" in as_startai_dtype(dtype_in)
...
def is_uint_dtype(
    ...
    Examples
    --------
    >>> startai.is_uint_dtype(startai.UintDtype("uint16"))
    True

    >>> startai.is_uint_dtype(startai.Dtype("uint8"))
    True

    >>> startai.is_uint_dtype(startai.IntDtype("int64"))
    False
    """
    if startai.is_array(dtype_in):
        dtype_in = startai.dtype(dtype_in)
        ...
        return startai.nested_argwhere(
            dtype_in,
            lambda x: isinstance(x, np.unsignedinteger)
            or (startai.is_array(x) and "uint" in startai.dtype(x)),
        )
    return "uint" in as_startai_dtype(dtype_in)
...
def is_complex_dtype(
    ...
    Examples
    --------
    >>> startai.is_complex_dtype(startai.ComplexDtype("complex64"))
    True

    >>> startai.is_complex_dtype(startai.Dtype("complex128"))
    True

    >>> startai.is_complex_dtype(startai.IntDtype("int64"))
    False
    """
    if startai.is_array(dtype_in):
        dtype_in = startai.dtype(dtype_in)
        ...
        return startai.nested_argwhere(
            ...
            or (startai.is_array(x) and "complex" in startai.dtype(x)),
        )
    return "complex" in as_startai_dtype(dtype_in)
...
def promote_types(
    ...
    *,
    ...
    query = [startai.as_startai_dtype(type1), startai.as_startai_dtype(type2)]
    query = tuple(query)
    if query not in startai.promotion_table:
        query = (query[1], query[0])

    def _promote(query):
        if array_api_promotion:
            return startai.array_api_promotion_table[query]
        return startai.promotion_table[query]

    return _promote(query)


@handle_exceptions
def set_default_dtype(dtype: Union[startai.Dtype, startai.NativeDtype, str], /):
    ...
    >>> startai.set_default_dtype(startai.bool)
    >>> startai.default_dtype_stack
    ['bool']
    >>> startai.unset_default_dtype()

    >>> startai.set_default_dtype("float64")
    >>> startai.default_dtype_stack
    ['float64']
    >>> startai.unset_default_dtype()

    With :class:`startai.NativeDtype` input:

    >>> startai.set_default_dtype(startai.native_uint64)
    >>> startai.default_dtype_stack
    ['uint64']
    """
...
@handle_exceptions
def set_default_float_dtype(float_dtype: Union[startai.Dtype, str], /):
    ...
    >>> startai.set_default_float_dtype(startai.floatDtype("float32"))
    >>> startai.default_float_dtype()
    'float32'
    """
    float_dtype = startai.FloatDtype(startai.as_startai_dtype(float_dtype))
    startai.utils.assertions._check_jax_x64_flag(float_dtype)
    global default_float_dtype_stack
    default_float_dtype_stack.append(float_dtype)


@handle_exceptions
def set_default_int_dtype(int_dtype: Union[startai.Dtype, str], /):
    ...
    >>> startai.set_default_int_dtype(startai.intDtype("int64"))
    >>> startai.default_int_dtype()
    'int64'

    >>> startai.set_default_int_dtype(startai.intDtype("int32"))
    >>> startai.default_int_dtype()
    'int32'
    """
    int_dtype = startai.IntDtype(startai.as_startai_dtype(int_dtype))
    startai.utils.assertions._check_jax_x64_flag(int_dtype)
    global default_int_dtype_stack
    default_int_dtype_stack.append(int_dtype)


@handle_exceptions
def set_default_uint_dtype(uint_dtype: Union[startai.Dtype, str], /):
    ...
    Examples
    --------
    >>> startai.set_default_uint_dtype(startai.UintDtype("uint8"))
    >>> startai.default_uint_dtype()
    'uint8'

    >>> startai.set_default_uint_dtype(startai.UintDtype("uint64"))
    >>> startai.default_uint_dtype()
    'uint64'
    """
    uint_dtype = startai.UintDtype(startai.as_startai_dtype(uint_dtype))
    startai.utils.assertions._check_jax_x64_flag(uint_dtype)
    global default_uint_dtype_stack
    default_uint_dtype_stack.append(uint_dtype)


@handle_exceptions
def set_default_complex_dtype(complex_dtype: Union[startai.Dtype, str], /):
    ...
    >>> startai.set_default_float_dtype(startai.ComplexDtype("complex128"))
    >>> startai.default_complex_dtype()
    'complex128'
    """
    complex_dtype = startai.ComplexDtype(startai.as_startai_dtype(complex_dtype))
    startai.utils.assertions._check_jax_x64_flag(complex_dtype)
    global default_complex_dtype_stack
...
def type_promote_arrays(
    ...
    """
    new_type = startai.promote_types(startai.dtype(x1), startai.dtype(x2))
    return startai.astype(x1, new_type), startai.astype(x2, new_type)
...
def unset_default_dtype():
    ...
    Examples
    --------
    >>> startai.set_default_dtype(startai.int32)
    >>> startai.set_default_dtype(startai.bool)
    >>> startai.default_dtype_stack
    ['int32', 'bool']
...
def unset_default_float_dtype():
    ...
    Examples
    --------
    >>> startai.set_default_float_dtype(startai.float32)
    >>> startai.set_default_float_dtype(startai.float64)
    >>> startai.default_float_dtype_stack
    ['float32','float64']
...
def unset_default_uint_dtype():
    ...
    >>> startai.unset_default_uint_dtype()
    >>> startai.default_uint_dtype()
    'uint32'
    """
    global default_uint_dtype_stack
...
def unset_default_complex_dtype():
    ...
    Examples
    --------
    >>> startai.set_default_complex_dtype(startai.complex64)
    >>> startai.set_default_complex_dtype(startai.complex128)
    >>> startai.default_complex_dtype_stack
    ['complex64','complex128']

    >>> startai.unset_default_complex_dtype()
    >>> startai.default_complex_dtype_stack
    ['complex64']
    """
    global default_complex_dtype_stack
...
@handle_exceptions
def valid_dtype(dtype_in: Union[startai.Dtype, startai.NativeDtype, str, None], /) -> bool:
    ...
    Examples
    --------
    >>> print(startai.valid_dtype(None))
    True

    >>> print(startai.valid_dtype(startai.float64))
    True

    >>> print(startai.valid_dtype('bool'))
    True

    >>> print(startai.valid_dtype(startai.native_float16))
    True
    """
    if dtype_in is None:
        return True
    return startai.as_startai_dtype(dtype_in) in startai.valid_dtypes


@handle_exceptions
def promote_types_of_inputs(
    x1: Union[startai.NativeArray, Number, Iterable[Number]],
    x2: Union[startai.NativeArray, Number, Iterable[Number]],
    /,
    *,
    array_api_promotion: bool = False,
) -> Tuple[startai.NativeArray, startai.NativeArray]:
    ...
    def _get_target_dtype(scalar, arr):
        ...
        if _special_case(scalar, arr):
            ...
            return None  # let startai infer a dtype
        elif isinstance(scalar, complex) and not startai.is_complex_dtype(arr):
            return "complex128"
        else:
            return arr.dtype

    if hasattr(x1, "dtype") and not hasattr(x2, "dtype"):
        device = startai.default_device(item=x1, as_native=True)
        x2 = startai.asarray(x2, dtype=_get_target_dtype(x2, x1), device=device)
    elif hasattr(x2, "dtype") and not hasattr(x1, "dtype"):
        device = startai.default_device(item=x2, as_native=True)
        x1 = startai.asarray(x1, dtype=_get_target_dtype(x1, x2), device=device)
    ...
    if x1.dtype != x2.dtype:
        ...
        x1 = startai.astype(x1, promoted, copy=False)
        x2 = startai.astype(x2, promoted, copy=False)

    startai.utils.assertions._check_jax_x64_flag(x1.dtype)
    return startai.to_native(x1), startai.to_native(x2)


@handle_exceptions
def is_native_dtype(dtype_in: Union[startai.Dtype, startai.NativeDtype], /) -> bool:
    ...
    >>> startai.is_native_dtype(np.int32)
    True

    >>> startai.set_backend('numpy')
    >>> startai.is_native_array(startai.float64)
    False
    """
