...
def _remove_so_log(trace):
    ...
    for idx, st in enumerate(old_stack_trace):
        ...
        if "<module>" in repr(st):
            ...
            and os.path.join("startai", "compiler") in st.filename
...
def _align_source(st, transpile_frame, module_frame, module_st, traced_lineno):
    from startai.compiler.utils.VVX import trace_obj
    from startai.compiler.utils.IIV import Graph
...
def _get_traces(curr_obj, area, local_dict, target_name):
    from startai.compiler.utils.VVX import trace_obj, get_source_code, CallVistior
...
def _configure_stack_trace(traceback):
    ...
    """
    tb = traceback
    trace_mode = startai.exception_trace_mode
    show_wrappers = startai.show_func_wrapper_trace_mode

    startai_path = os.path.join("startai", "functional", "startai")
    frontend_path = os.path.join("startai", "functional", "frontends")
    wrapper_path = os.path.join("startai", "func_wrapper.py")

    while tb.tb_next:
        frame = tb.tb_next.tb_frame
        file_path = frame.f_code.co_filename
        if trace_mode == "startai":
            if _check_if_path_found(startai_path, file_path):
                tb = tb.tb_next
            else:
                tb.tb_next = tb.tb_next.tb_next
        elif trace_mode == "frontend":
            if _check_if_path_found(frontend_path, file_path) or _check_if_path_found(
                startai_path, file_path
            ):
                tb = tb.tb_next
            else:
                tb.tb_next = tb.tb_next.tb_next
...
def _add_native_error(default):
    ...
    if isinstance(default[-1], Exception):
        if isinstance(default[-1], StartaiException):
            if default[-1].native_error is not None:
                ...
                # hence the last element is an StartaiException
...
def _combine_messages(*messages, include_backend=True):
    ...
    default = [
        "numpy" if startai.current_backend_str() == "" else startai.current_backend_str()
    ]
    delimiter = ": "
...
class StartaiException(Exception):
...
class StartaiBackendException(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiInvalidBackendException(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiNotImplementedException(StartaiException, NotImplementedError):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiError(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiIndexError(StartaiException, IndexError):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiAttributeError(StartaiException, AttributeError):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiValueError(StartaiException, ValueError):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiBroadcastShapeError(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiDtypePromotionError(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class StartaiDeviceError(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


class InplaceUpdateException(StartaiException):
    def __init__(self, *messages, include_backend=False):
        super().__init__(*messages, include_backend=include_backend)


_non_startai_exceptions_mapping = {
    IndexError: StartaiIndexError,
    AttributeError: StartaiAttributeError,
    ValueError: StartaiValueError,
    Exception: StartaiBackendException,
    NotImplementedError: StartaiNotImplementedException,
}
...
def handle_exceptions(fn: Callable) -> Callable:
    ...
    def _handle_exceptions(*args, **kwargs):
        """Catch all exceptions and raise them in StartaiException.
        ...
        except StartaiException as e:
            _handle_exceptions_helper(e, type(e))
        except Exception as e:
            startai_exception = _non_startai_exceptions_mapping.get(
                type(e), StartaiBackendException
            )
            _handle_exceptions_helper(e, startai_exception)
...
def _handle_inplace_mode(startai_pack=None):
    if not startai_pack:
        startai_pack = startai
    current_backend = startai_pack.current_backend_str()
    if (
        current_backend != ""
        and not _inplace_warning_cache.get(current_backend)
        and not startai_pack.native_inplace_support
        and startai_pack.inplace_mode == "lenient"
    ):
        warnings.warn(
            f"The current backend: '{current_backend}' does not support "
            "inplace updates natively. Startai would quietly create new arrays when "
...
def _check_inplace_update_support(x, ensure_in_backend):
    current_backend = startai.current_backend_str()
    is_tf_variable = current_backend == "tensorflow" and not startai.is_startai_array(
        x, exclusive=True
    )
    if (
        ensure_in_backend
        or startai.is_native_array(x)
        or (startai.inplace_mode == "strict" and not is_tf_variable)
    ):
        raise startai.utils.exceptions.InplaceUpdateException(
            f"{current_backend} does not support inplace updates "
            "and startai cannot support the operation in 'strict' mode\n"
            "To enable inplace update, use startai.set_inplace_mode('lenient')\n"
        )
