...
# adam_step
@handle_test(
    fn_tree="functional.startai.adam_step",
...
# adam_update
@handle_test(
    fn_tree="functional.startai.adam_update",
...
@handle_test(
    fn_tree="functional.startai.execute_with_gradients",
    ...
    *, dtype_and_xs, retain_grads, test_flags, backend_fw, fn_name, on_device
    ...
    def func(xs):
        with BackendHandler.update_backend(
            startai.current_backend(xs.to_native()).backend
        ) as startai_backend:
            if isinstance(xs, startai_backend.Container):
                array_idxs = startai_backend.nested_argwhere(xs, startai_backend.is_array)
                array_vals = startai_backend.multi_index_nest(xs, array_idxs)
                if len(array_vals) == 0:
                    final_array = None
                else:
                    final_array = startai_backend.stack(array_vals)
            else:
                final_array = xs
            ret = startai_backend.mean(final_array)
            return ret
...
@pytest.mark.parametrize(
    "func", [lambda x: startai.mean(startai.square(x)), lambda x: startai.mean(startai.cos(x))]
)
...
def test_grad(x, dtype, func, backend_fw, nth):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        _variable_fn = startai_backend.startai.functional.startai.gradients._variable
        var = _variable_fn(startai_backend.array(x, dtype=dtype))
        fn = startai_backend.grad(func)
        if nth > 1:
            for _ in range(1, nth):
                fn = startai_backend.grad(fn)
        grad = fn(var)
        grad_np = helpers.flatten_and_to_np(ret=grad, backend=backend_fw)

    with BackendHandler.update_backend("tensorflow") as gt_backend:
        _variable_fn = gt_backend.startai.functional.startai.gradients._variable
        var = _variable_fn(startai.array(x, dtype=dtype))
...
@handle_test(
    fn_tree="functional.startai.gradient_descent_update",
...
def test_jac(x, dtype, func_str, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        f = startai_backend.__dict__[func_str]

        def func(x):
            return startai_backend.mean(f(x))

        _variable_fn = startai_backend.startai.functional.startai.gradients._variable
        var = _variable_fn(startai_backend.array(x, dtype=dtype))
        fn = startai_backend.jac(func)
        jacobian = fn(var)
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        _variable_fn = startai_backend.startai.functional.startai.gradients._variable
        var1 = _variable_fn(startai_backend.array(x[0], dtype=dtype))
        var2 = _variable_fn(startai_backend.array(x[1], dtype=dtype))
        var = [var1, {"x2": var2}]
        fn = startai_backend.jac(func)
        jacobian = fn(var)
        jacobian_np = helpers.flatten_and_to_np(ret=jacobian, backend=backend_fw)
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        func = startai_backend.__dict__[func_str]
        _variable_fn = startai_backend.startai.functional.startai.gradients._variable
        var = _variable_fn(startai_backend.array(x, dtype=dtype))
        fn = startai_backend.jac(func)
        jacobian = fn(var)
...
# lamb_update
@handle_test(
    fn_tree="functional.startai.lamb_update",
    dtype_n_ws_n_dcdw_n_mwtm1_n_vwtm1_n_lr=get_gradient_arguments_with_lr(
        min_value=-1e5,
        max_value=1e5,
        num_arrays=4,
...
# lars_update
@handle_test(
    fn_tree="functional.startai.lars_update",
...
# optimizer_update
@handle_test(
    fn_tree="functional.startai.optimizer_update",
...
# stop_gradient
@handle_test(
    fn_tree="functional.startai.stop_gradient",
...
def test_value_and_grad(x, dtype, func, backend_fw):
    if backend_fw == "numpy":
        return
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        var = startai_backend.startai.functional.startai.gradients._variable(
            startai_backend.array(x, dtype=dtype)
        )
        fn = startai_backend.value_and_grad(func)
        value, grad = fn(var)
    ...
    with BackendHandler.update_backend("tensorflow") as gt_backend:
        var = gt_backend.startai.functional.startai.gradients._variable(
