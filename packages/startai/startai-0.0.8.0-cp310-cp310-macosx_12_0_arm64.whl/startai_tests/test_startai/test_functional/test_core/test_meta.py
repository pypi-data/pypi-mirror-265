...
@handle_test(
    fn_tree="functional.startai.fomaml_step",
...
def test_fomaml_step_overlapping_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        variable_fn = startai_backend.functional.startai._variable

        # create variables
        if batched:
            variables = startai_backend.Container(
                {
                    "latent": variable_fn(
                        startai_backend.repeat(
                            startai_backend.array([[0.0]], device=on_device),
                            num_tasks,
                            axis=0,
                        )
                    ),
                    "weight": variable_fn(
                        startai_backend.repeat(
                            startai_backend.array([[1.0]], device=on_device),
                            num_tasks,
                            axis=0,
                        )
                    ),
            ...
            variables = startai_backend.Container(
                {
                    "latent": variable_fn(startai_backend.array([0.0], device=on_device)),
                    "weight": variable_fn(startai_backend.array([1.0], device=on_device)),
                }
            )

        # batch
        batch = startai_backend.Container(
            {"x": startai_backend.arange(1, num_tasks + 1, dtype="float32")}
        )
        ...
        # numpy
        latent_np = startai_backend.to_numpy(variables.latent[0:1])
        weight_np = startai_backend.to_numpy(variables.weight[0:1])
        batch_np = batch.cont_map(lambda x, kc: startai_backend.to_numpy(x))
        ...
        # meta update
        rets = startai_backend.fomaml_step(
            batch,
            inner_cost_fn,
        ...
        calc_cost = rets[0]
        if stop_gradients:
            assert startai_backend.equal(
                startai_backend.functional.startai._is_variable(calc_cost, exclusive=True),
                False,
            )
        assert np.allclose(startai_backend.to_scalar(calc_cost), true_cost)
        outer_grads = rets[1]
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.weight[0]), np.array(true_weight_grad)
        )
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.latent[0]), np.array(true_latent_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, startai_backend.Container)
...
def test_fomaml_step_shared_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        if batched:
            ...
            variables = startai_backend.Container(
                {"latent": variable_fn(startai_backend.array([1.0], device=on_device))}
            )
        ...
        outer_grads = rets[1]
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.latent[0]), np.array(true_outer_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, startai_backend.Container)
...
def test_maml_step_overlapping_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        # numpy
        latent_np = startai_backend.to_numpy(variables.latent)
        weight_np = startai_backend.to_numpy(variables.weight)
        batch_np = batch.cont_map(lambda x, kc: startai_backend.to_numpy(x))
        ...
        # meta update
        rets = startai_backend.maml_step(
            batch,
            inner_cost_fn,
            outer_cost_fn if with_outer_cost_fn else None,
            variables,
        ...
        outer_grads = rets[1]
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.weight), np.array(true_weight_grad)
        )
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.latent), np.array(true_latent_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, startai_backend.Container)
...
def test_maml_step_shared_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        # numpy
        variables_np = variables.cont_map(lambda x, kc: startai_backend.to_startai(x))
        batch_np = batch.cont_map(lambda x, kc: startai_backend.to_startai(x))
        ...
        for sub_batch in batch_np.cont_unstack_conts(0, True, num_tasks):
            ...
            if average_across_steps:
                ...
                true_outer_grad = startai_backend.multiply(
        ...
        outer_grads = rets[1]
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.latent),
            startai_backend.to_numpy(true_outer_grad[0]),
            rtol=1e-1,
            atol=1e-1,
        )
        if return_inner_v:
...
def test_maml_step_unique_vars(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        outer_grads = rets[1]
        assert np.allclose(
            startai_backend.to_numpy(outer_grads.weight), np.array(true_outer_grad)
        )
        if return_inner_v:
            inner_v_rets = rets[2]
            assert isinstance(inner_v_rets, startai_backend.Container)
...
def test_reptile_step(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        # meta update
        rets = startai_backend.reptile_step(
            batch,
            inner_cost_fn,
            variables,
            inner_grad_steps,
