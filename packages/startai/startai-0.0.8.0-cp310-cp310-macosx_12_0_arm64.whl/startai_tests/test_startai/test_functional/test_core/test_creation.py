...
from startai_tests.test_startai.test_functional.test_core.test_dtype import astype_helper
...
def _asarray_helper(draw):
    ...
    with BackendHandler.update_backend(test_globals.CURRENT_BACKEND) as startai_backend:
        x_list = startai_backend.nested_map(lambda x: x.tolist(), x, shallow=False)
        sh = draw(helpers.get_shape(min_num_dims=1))
        sh = startai_backend.Shape(sh)
    # np_array = x[0]
...
def _fill_value(draw):
    ...
    with BackendHandler.update_backend(test_globals.CURRENT_BACKEND) as startai_backend:
        if startai_backend.is_uint_dtype(dtype):
            return draw(helpers.ints(min_value=0, max_value=5))
        if startai_backend.is_int_dtype(dtype):
...
# arange
@handle_test(
    fn_tree="functional.startai.arange",
...
@handle_test(
    fn_tree="functional.startai.asarray",
...
# copy array
@handle_test(
    fn_tree="functional.startai.copy_array",
    dtype_and_x=helpers.dtype_and_values(available_dtypes=helpers.get_dtypes("valid")),
    to_startai_array_bool=st.booleans(),
    ...
    *,
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        if test_flags.instance_method:
            ret = x.copy_array(to_startai_array=to_startai_array_bool)
        else:
            ret = startai_backend.copy_array(x, to_startai_array=to_startai_array_bool)
        # type test
        test_ret = ret
        test_x = x
        if test_flags.container[0]:
            assert startai_backend.is_startai_container(ret)
            test_ret = ret["a"]
            test_x = x["a"]
        if to_startai_array_bool:
            assert startai_backend.is_startai_array(test_ret)
        else:
            assert startai_backend.is_native_array(test_ret)
        # cardinality test
        assert test_ret.shape == test_x.shape
        # value test
        x, ret = startai_backend.to_startai(x), startai_backend.to_startai(ret)
...
# empty
@handle_test(
    fn_tree="functional.startai.empty",
...
# empty_like
@handle_test(
    fn_tree="functional.startai.empty_like",
...
@handle_test(
    ...
    fn_tree="functional.startai.eye",
    test_instance_method=st.just(False),
    test_gradients=st.just(False),
)
def test_eye(
...
# from_dlpack
@handle_test(
    fn_tree="functional.startai.from_dlpack",
    dtype_and_x=helpers.dtype_and_values(
...
def test_from_dlpack(*, dtype_and_x, backend_fw):
    ...
    native_array = startai.native_array(x[0])
    cap = startai.to_dlpack(native_array)
    array = startai.from_dlpack(cap)
    assert startai.is_native_array(array)


@handle_test(
    fn_tree="functional.startai.frombuffer",
...
# full
@handle_test(
    fn_tree="functional.startai.full",
    shape=helpers.get_shape(
        allow_none=False,
        min_num_dims=1,
        max_num_dims=5,
...
# full_like
@handle_test(
    fn_tree="functional.startai.full_like",
...
# linspace
@handle_test(
    fn_tree="functional.startai.linspace",
...
# logspace
@handle_test(
    fn_tree="functional.startai.logspace",
    dtype_and_start_stop_axis=helpers.dtype_values_axis(
        available_dtypes=helpers.get_dtypes("float"),
        num_arrays=2,
        min_value=-1e5,
...
# meshgrid
@handle_test(
    fn_tree="functional.startai.meshgrid",
...
# native_array
@handle_test(
    fn_tree="functional.startai.native_array",
...
# one_hot
@handle_test(
    fn_tree="functional.startai.one_hot",
...
# ones
@handle_test(
    fn_tree="functional.startai.ones",
    shape=helpers.get_shape(
        allow_none=False,
        min_num_dims=1,
        max_num_dims=5,
...
# ones_like
@handle_test(
    fn_tree="functional.startai.ones_like",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
    ),
)
...
# to_dlpack
@handle_test(
    fn_tree="functional.startai.to_dlpack",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes(kind="float", full=False, key="dtype"),
        min_num_dims=1,
        max_num_dims=5,
...
# tril
@handle_test(
    fn_tree="functional.startai.tril",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        min_num_dims=2,
        max_num_dims=5,
...
# triu
@handle_test(
    fn_tree="functional.startai.triu",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        min_num_dims=2,
        max_num_dims=5,
...
@handle_test(
    fn_tree="functional.startai.triu_indices",
...
# zeros
@handle_test(
    fn_tree="functional.startai.zeros",
    shape=helpers.get_shape(
        allow_none=False,
        min_num_dims=1,
        max_num_dims=5,
...
# zeros_like
@handle_test(
    fn_tree="functional.startai.zeros_like",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid"),
        min_num_dims=1,
        max_num_dims=5,
