...
def test_jax_eig(
    *,
    ...
    """Make symmetric positive-definite since startai does not support complex data
    dtypes currently."""
    x = np.matmul(x.T, x) + np.identity(x.shape[0]) * 1e-3
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ret = [startai_backend.to_numpy(x).astype(np.float64) for x in ret]
...
def test_jax_eigvals(
    *,
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        ret = np.sort(np.abs(startai_backend.to_numpy(ret))).astype(np.float64)
