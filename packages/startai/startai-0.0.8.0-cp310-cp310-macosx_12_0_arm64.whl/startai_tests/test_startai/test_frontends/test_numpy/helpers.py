...
from startai_tests.test_startai.helpers.pipeline_helper import (
    BackendHandler,
    get_frontend_config,
)
...
def _flatten_fw_return(ret, backend):
    with BackendHandler.update_backend(backend) as startai_backend:
        if not isinstance(ret, tuple):
            ret = (ret,)
        ret_idxs = startai_backend.nested_argwhere(
            ret, lambda x: startai_backend.is_startai_array(x) or startai_backend.is_native_array(x)
        )
        if len(ret_idxs) == 0:
        ...
        ret_np_flat = [startai_backend.to_numpy(x) for x in ret_flat]
        return ret_np_flat
...
def _get_safe_casting_dtype(draw, *, dtypes):
    ...
    with BackendHandler.update_backend(test_globals.CURRENT_BACKEND) as startai_backend:
        if startai_backend.is_float_dtype(target_dtype):
            dtype = draw(st.sampled_from(["float64", None]))
        elif startai_backend.is_uint_dtype(target_dtype):
            dtype = draw(st.sampled_from(["uint64", None]))
        elif startai_backend.is_int_dtype(target_dtype):
            dtype = draw(st.sampled_from(["int64", None]))
        elif startai_backend.is_complex_dtype(target_dtype):
...
def _test_frontend_function_ignoring_uninitialized(*args, **kwargs):
    ...
    if frontend_config.isscalar(frontend_ret):
        ...
        frontend_ret_idxs = startai.nested_argwhere(
