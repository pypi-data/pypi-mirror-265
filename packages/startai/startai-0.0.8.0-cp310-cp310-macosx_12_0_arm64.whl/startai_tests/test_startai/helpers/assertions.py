...
def assert_all_close(
    ...
    ----------
    ret_np
        Return from the framework to test. Startai Container or Numpy Array.
    ret_from_gt_np
        Return from the ground truth framework. Startai Container or Numpy Array.
    rtol
    ...
    # TODO enable
    # if startai.is_startai_container(ret_np) and startai.is_startai_container(ret_from_gt_np):
    #     startai.Container.cont_multi_map(assert_all_close, [ret_np, ret_from_gt_np])
    # else:
...
def check_unsupported_dtype(*, fn, input_dtypes, all_as_kwargs_np):
    ...
    unsupported_dtypes_fn = startai.function_unsupported_dtypes(fn)
    supported_dtypes_fn = startai.function_supported_dtypes(fn)
...
def check_unsupported_device(*, fn, input_device, all_as_kwargs_np):
    ...
    """
    test_unsupported = False
    unsupported_devices_fn = startai.function_unsupported_devices(fn)
    supported_devices_fn = startai.function_supported_devices(fn)
...
def check_unsupported_device_and_dtype(*, fn, device, input_dtypes, all_as_kwargs_np):
    ...
    otherwise.
    """
    unsupported_devices_dtypes_fn = startai.function_unsupported_devices_and_dtypes(fn)
