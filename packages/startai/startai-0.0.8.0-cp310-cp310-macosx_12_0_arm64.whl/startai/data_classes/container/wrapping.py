...
TO_IGNORE = ["is_startai_array", "is_native_array", "is_array", "shape"]
    ...
    def new_function(
        ...
        **kwargs
        ...
        if (
            ...
            and args
            and startai.is_startai_container(args[0])
            and not static
        ):
            ...
            if len(args) > data_idx[0][0]:
                ...
                startai.insert_into_nest_at_index(args, data_idx, self)
            else:
                kwargs = startai.copy_nest(kwargs, to_mutable=True)
                data_idx = [data_idx[0][1]] + [
                    0 if idx is int else idx for idx in data_idx[1:]
                ]
                startai.insert_into_nest_at_index(kwargs, data_idx, self)
...
def add_startai_container_instance_methods(
    cls: Type[startai.Container],
    modules: Union[List[ModuleType], startai.Container],
    static: Union[bool, startai.Container] = False,
    to_ignore: Union[Iterable, startai.Container] = (),
):
    """Loop over all startai modules such as activations, general, etc. and add the
    module functions to startai container as instance methods using _wrap_function.
    ...
    Examples
    --------
    As shown, `add_startai_container_instance_methods` adds all the appropriate functions
    from the statistical module as instance methods to our toy `ContainerExample` class:

    >>> from startai.functional.startai import statistical
    >>> class ContainerExample:
    ...     pass
    >>> startai.add_startai_container_instance_methods(ContainerExample, [statistical])
