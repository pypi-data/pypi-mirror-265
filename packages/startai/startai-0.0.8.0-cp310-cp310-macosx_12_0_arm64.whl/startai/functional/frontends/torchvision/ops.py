...
from startai.func_wrapper import with_supported_dtypes, with_unsupported_device_and_dtypes


@to_startai_arrays_and_back
def batched_nms(boxes, scores, idxs, iou_threshold):
    if boxes.size == 0:
        return startai.array([], dtype=startai.int64)
    else:
        max_coordinate = boxes.max()
        boxes_dtype = boxes.dtype
        offsets = idxs.astype(boxes_dtype) * (
            max_coordinate + startai.array(1, dtype=boxes_dtype)
        )
...
@to_startai_arrays_and_back
def box_area(boxes):
    return startai.prod(boxes[..., 2:] - boxes[..., :2], axis=-1)
...
def box_iou(boxes1, boxes2):
    ...
    lt = startai.maximum(boxes1[:, None, :2], boxes2[:, :2])
    rb = startai.minimum(boxes1[:, None, 2:], boxes2[:, 2:])
...
def clip_boxes_to_image(boxes, size):
    ...
    clipped_boxes = startai.stack([boxes_x, boxes_y], axis=-1)
    return clipped_boxes.reshape(boxes.shape).astype(boxes.dtype)


@to_startai_arrays_and_back
def nms(boxes, scores, iou_threshold):
    return startai.nms(boxes, scores, iou_threshold)


@to_startai_arrays_and_back
def remove_small_boxes(boxes, min_size):
    w, h = boxes[..., 2] - boxes[..., 0], boxes[..., 3] - boxes[..., 1]
    return startai.nonzero((w >= min_size) & (h >= min_size))[0]
