    ...
    def __getitem__(self, key):
        ...
        for k, item in enumerate(key):
            ...
            if isinstance(item, slice):
                ...
                if step is None:
                    step = 1
                if startai.is_complex_dtype(step):
                    size = int(abs(step))
                    newobj = np_frontend.linspace(start, stop, num=size).startai_array
                else:
                    newobj = np_frontend.arange(start, stop, step).startai_array
                if ndmin > 1:
                    newobj = np_frontend.array(
                        newobj, copy=False, ndmin=ndmin
                    ).startai_array
                    if trans1d != -1:
                        newobj = startai.swapaxes(newobj, -1, trans1d)
            ...
            elif (startai.isscalar(item)) or (startai.is_startai_array(item) and item.ndim == 0):
                scalar = True
                newobj = item
            else:
                item = startai.array(item)
                newobj = np_frontend.array(item, copy=False, ndmin=ndmin).startai_array
                if trans1d != -1 and item.ndim < ndmin:
                    ...
                    newobj = np_frontend.transpose(newobj, axes=axes).startai_array
        ...
        if len(result_type_objs) != 0:
            if len(result_type_objs) > 1:
                final_dtype = startai.result_type(*result_type_objs)
            else:
                final_dtype = startai.result_type(result_type_objs[0], result_type_objs[0])
...
def fill_diagonal(a, val, wrap=False):
    ...
    shape = a.shape
    temp = startai.flatten(a)
    temp[:end:step] = val
    a = startai.reshape(temp, shape)
