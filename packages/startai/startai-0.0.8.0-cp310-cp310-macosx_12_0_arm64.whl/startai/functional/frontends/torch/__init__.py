...
@handle_exceptions
def device(dev):
    return startai.default_device(dev)
...
def promote_types_torch(
    ...
    try:
        ret = torch_frontend.torch_promotion_table[
            (startai.as_startai_dtype(type1), startai.as_startai_dtype(type2))
        ]
...
def promote_types_of_torch_inputs(
    ...
    """
    if startai.isscalar(x1) and startai.is_int_dtype(x1):
        x1 = startai.asarray(x1, dtype="int64")
    elif startai.isscalar(x1):
        x1 = startai.asarray(x1)
    if startai.isscalar(x2) and startai.is_int_dtype(x2):
        x2 = startai.asarray(x2, dtype="int64")
    elif startai.isscalar(x2):
