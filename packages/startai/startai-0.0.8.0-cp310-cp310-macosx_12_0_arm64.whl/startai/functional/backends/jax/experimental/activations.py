...
def relu6(
    ...
    new_func = startai.bind_custom_gradient_function(relu6_func, custom_grad_func)
...
def selu(x: JaxArray, /, *, out: Optional[JaxArray] = None) -> JaxArray:
    ...
    if startai.exists(out):
        return startai.inplace_update(out, ret).astype(x.dtype)
    return ret
...
def threshold(
    ...
    *,
    ...
    if startai.exists(out):
        return startai.inplace_update(out, ret).astype(x.dtype)  # type: ignore
    return ret
