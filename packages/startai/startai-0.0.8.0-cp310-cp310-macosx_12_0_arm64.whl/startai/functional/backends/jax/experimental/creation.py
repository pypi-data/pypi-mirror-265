    ...
    *,
    ...
    return jnp.array(
        [
            round(
                math.sin(
                    (startai.pi / 2) * (math.sin(startai.pi * (i) / (window_length * 2)) ** 2)
                ),
                8,
            )
...
def unsorted_segment_min(
    ...
    startai.utils.assertions.check_unsorted_segment_valid_params(
...
def polyval(
    ...
    with startai.PreciseMode(True):
        promoted_type = startai.promote_types(startai.dtype(coeffs[0]), startai.dtype(x[0]))
    coeffs, x = startai.promote_types_of_inputs(coeffs, x)
    y = jnp.zeros_like(x)
    for pv in coeffs:
        y = y * x + pv
