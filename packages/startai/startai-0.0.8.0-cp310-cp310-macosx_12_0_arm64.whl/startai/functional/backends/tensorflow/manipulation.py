...
from startai.functional.startai.manipulation import _calculate_out_shape
    ...
    if axis is not None:
        try:
            ...
            if "(zero-based) was expected to be" in error.message:
                ...
                for i in xs:
                    highest_dtype = startai.promote_types(highest_dtype, i.dtype)
                highest_dtype = startai.as_native_dtype(highest_dtype)
                return tf.concat(
                    [
...
def expand_dims(
    ...
    *,
    ...
    try:
        ...
        raise startai.utils.exceptions.StartaiIndexError(error) from error
...
def stack(
    ...
    *,
    ...
    try:
        ...
        raise startai.utils.exceptions.StartaiIndexError(e) from e
    ...
    *,
    ...
    if x_min is not None:
        if not hasattr(x_min, "dtype"):
            x_min = startai.array(x_min).data
        promoted_type = startai.as_native_dtype(startai.promote_types(x.dtype, x_min.dtype))
    if x_max is not None:
        if not hasattr(x_max, "dtype"):
            x_max = startai.array(x_max).data
        promoted_type = startai.as_native_dtype(
            startai.promote_types(promoted_type, x_max.dtype)
        )
