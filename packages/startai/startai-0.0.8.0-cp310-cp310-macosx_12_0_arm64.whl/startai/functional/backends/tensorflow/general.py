...
Collection of TensorFlow general functions, wrapped to fit Startai syntax
and signature.
"""
...
from startai.utils.exceptions import _check_inplace_update_support
from . import backend_version
from ...startai.general import _broadcast_to
...
def array_equal(
    ...
    x0, x1 = startai.promote_types_of_inputs(x0, x1)
...
def get_item(
    ...
    *,
    ...
    if startai.is_array(query) and startai.is_bool_dtype(query) and not len(query.shape):
...
def to_numpy(x: Union[tf.Tensor, tf.Variable], /, *, copy: bool = True) -> np.ndarray:
    ...
    if (
        ...
        and startai.as_native_dtype(x.dtype) is tf.bfloat16
    ):
...
def gather(
    ...
    *,
    ...
    startai.utils.assertions.check_gather_input_valid(params, indices, axis, batch_dims)
...
def gather_nd(
    ...
    *,
    ...
    startai.utils.assertions.check_gather_nd_input_valid(params, indices, batch_dims)
    try:
...
def inplace_decrement(
    x: Union[startai.Array, tf.Tensor], val: Union[startai.Array, tf.Tensor]
) -> startai.Array:
    (x_native, val_native), _ = startai.args_to_native(x, val)
    if _is_variable(x_native):
        x_native.assign(x_native - val_native)
        if startai.is_startai_array(x):
            x.data = x_native
        else:
            x = startai.Array(x_native)
    elif startai.is_startai_array(x):
        x.data -= val_native
    else:
        x = startai.Array(val_native)
    return x
...
def inplace_update(
    x: Union[startai.Array, tf.Tensor],
    val: Union[startai.Array, tf.Tensor],
    /,
    *,
    ensure_in_backend: bool = False,
    keep_input_dtype: bool = False,
) -> startai.Array:
    if startai.is_array(x) and startai.is_array(val):
        _check_inplace_update_support(x, ensure_in_backend)
        if keep_input_dtype:
            val = startai.astype(val, x.dtype)
        (x_native, val_native), _ = startai.args_to_native(x, val)
        if _is_variable(x_native):
            ...
            x.data = val_native
            # Handle view updates
            if startai.exists(x._base):
                base = x._base
                base_idx = startai.arange(base.size).reshape(base.shape)
                for fn, args, kwargs, index in x._manipulation_stack:
                    kwargs["copy"] = True
                    base_idx = startai.__dict__[fn](base_idx, *args, **kwargs)
                    base_idx = base_idx[index] if startai.exists(index) else base_idx
                ...
                for ref in base._view_refs:
                    view = ref()
                    if startai.exists(view) and view is not x:
                        _update_view(view, base)
            else:
                for ref in x._view_refs:
                    view = ref()
                    if startai.exists(view):
                        _update_view(view, x)
        else:
            x = startai.to_startai(x_native)
        return x
    else:
        return val
...
def _update_view(view, base):
    for fn, args, kwargs, index in view._manipulation_stack:
        base = startai.__dict__[fn](base, *args, **kwargs)
        base = base[index] if startai.exists(index) else base
    view.data = base.data
    return view
...
def scatter_flat(
    ...
    *,
    ...
    target = out
    target_given = startai.exists(target)
    if startai.exists(size) and startai.exists(target):
        startai.utils.assertions.check_equal(len(target.shape), 1, as_array=False)
        startai.utils.assertions.check_equal(target.shape[0], size, as_array=False)
    if not target_given:
    ...
    return startai.inplace_update(out, res) if startai.exists(out) else res
...
def scatter_nd(
    ...
    *,
    ...
    if startai.exists(out):
        dtype = startai.promote_types(out.dtype, updates_dtype)
    updates = tf.cast(
        updates,
        (startai.as_native_dtype(dtype) if startai.exists(out) else updates_dtype),
    )
    ...
    target = out
    target_given = startai.exists(target)
    if startai.exists(shape) and target_given:
        startai.utils.assertions.check_equal(
            startai.Shape(target.shape), startai.Shape(shape), as_array=False
        )
    if not target_given:
        shape = list(shape) if startai.exists(shape) else list(out.shape)
    ...
    if reduction == "sum":
        ...
        updates = startai.multiply(startai.gather_nd(target, indices), updates).data
...
def shape(
    ...
    *,
    as_array: bool = False,
) -> Union[tf.Tensor, startai.Shape, startai.Array]:
    if as_array:
        return startai.array(tf.shape(x), dtype=startai.default_int_dtype())
    else:
        return startai.Shape(x.shape)
...
def vmap(
    ...
    @startai.output_to_native_arrays
    @startai.inputs_to_native_arrays
    def _vmap(*args, **kwargs):
        ...
        if isinstance(in_axes, (list, tuple)):
            startai.utils.assertions.check_any(
                [startai.exists(ax) for ax in in_axes],
