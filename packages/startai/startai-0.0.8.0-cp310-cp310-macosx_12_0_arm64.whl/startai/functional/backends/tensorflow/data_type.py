...
def finfo(type: Union[DType, str, tf.Tensor, tf.Variable, np.ndarray], /) -> Finfo:
    if isinstance(type, (tf.Tensor, np.ndarray)):
        type = type.dtype
    if startai.as_native_dtype(type) == tf.bfloat16:
        return Finfo(Bfloat16Finfo())
    return Finfo(tf.experimental.numpy.finfo(startai.as_native_dtype(type)))
...
def iinfo(type: Union[DType, str, tf.Tensor, tf.Variable, np.ndarray], /) -> np.iinfo:
    if isinstance(type, (tf.Tensor, np.ndarray)):
        type = type.dtype
    return tf.experimental.numpy.iinfo(startai.as_startai_dtype(type))
...
def is_native_dtype(dtype_in: Union[tf.DType, str], /) -> bool:
    if not startai.is_hashable_dtype(dtype_in):
        return False
    return bool(dtype_in in startai_dtype_dict and isinstance(dtype_in, tf.dtypes.DType))
