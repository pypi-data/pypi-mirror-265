...
# local
import startai
from startai.func_wrapper import with_unsupported_device_and_dtypes, with_supported_dtypes
...
def searchsorted(
    ...
    *,
    ...
    assert startai.is_int_dtype(ret_dtype), TypeError(
        "only Integer data types are supported for ret_dtype."
    )

    if sorter is not None:
        assert startai.is_int_dtype(sorter.dtype), TypeError(
            f"Only signed integer data type for sorter is allowed, got {sorter.dtype}."
        )
        if startai.as_native_dtype(sorter.dtype) not in [paddle.int32, paddle.int64]:
