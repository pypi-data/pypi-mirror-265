...
Collection of Paddle activation functions, wrapped to fit Startai syntax and
signature.
"""
...
def softplus(
    ...
    *,
    ...
    if beta is not None and beta != 1:
        ...
        res = (
            startai.add(
                startai.log1p(startai.exp(-startai.abs(x_beta))),
                startai.maximum(x_beta, 0),
            )
        ) / beta
    else:
        x_beta = x
    ...
    if threshold is not None:
        return startai.where(x_beta > threshold, x, res).astype(x.dtype)
