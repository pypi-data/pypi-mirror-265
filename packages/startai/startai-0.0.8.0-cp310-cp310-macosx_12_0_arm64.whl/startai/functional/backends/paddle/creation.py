...
from startai.functional.startai.creation import (
...
def arange(
    ...
    step: float = 1,
    *,
    dtype: Optional[Union[startai.Dtype, paddle.dtype]] = None,
    ...
    *,
    ...
    if isinstance(obj, paddle.Tensor):
        ...
        if ret.ndim != 0:  # for versions <2.5.0
            ...
            return ret
    obj = startai.nested_map(_remove_np_bfloat16, obj, shallow=False)
...
def full(
    ...
    *,
    ...
    if dtype is None:
        dtype = startai.default_dtype(item=fill_value)
    ...
    if isinstance(fill_value, complex):
        ...
        dtype_ = None if startai.as_native_dtype(dtype) == paddle.int8 else dtype
        ret = paddle.full(shape=shape, fill_value=fill_value, dtype=dtype_)
    if ret.dtype != startai.as_native_dtype(dtype):
...
def linspace(
    ...
    *,
    ...
    if (
        ...
        ans[0] = start
    if startai.is_startai_array(ans):
...
def copy_array(
    ...
    *,
    ...
    if 0 in x.shape:
        ...
        new_arr = x.clone()
    if to_startai_array:
        return startai.to_startai(new_arr)
    return new_arr
...
def frombuffer(
    ...
    dtype_bytes = int(startai.Dtype(dtype).dtype_bits / 8)
    if str(dtype) == "bool":
        dtype_bytes = 1
    dtype_str = str(dtype)
    struct_format = {
