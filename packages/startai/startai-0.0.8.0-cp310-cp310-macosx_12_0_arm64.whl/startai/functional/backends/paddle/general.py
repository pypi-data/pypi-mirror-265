"""Collection of Paddle general functions, wrapped to fit Startai syntax and
signature."""
...
# local
import startai
import startai.functional.backends.paddle as paddle_backend
from startai.func_wrapper import with_unsupported_device_and_dtypes, with_unsupported_dtypes
from startai.functional.startai.general import _broadcast_to
...
def to_numpy(
    ...
    if isinstance(x, (float, int, bool)):
        ...
        return [startai.to_numpy(u) for u in x]
    raise startai.utils.exceptions.StartaiException("Expected a Paddle Tensor.")
...
def inplace_decrement(
    x: Union[startai.Array, paddle.Tensor],
    val: Union[startai.Array, paddle.Tensor],
) -> startai.Array:
...
def scatter_flat(
    ...
    *,
    ...
    if startai.exists(size) and startai.exists(out):
        startai.utils.assertions.check_equal(out.ndim, 1, as_array=False)
        startai.utils.assertions.check_equal(out.shape[0], size, as_array=False)
...
def scatter_nd(
    ...
    *,
    ...
    updates = paddle.to_tensor(
        updates,
        dtype=(
            startai.promote_types(out.dtype, updates.dtype)
            if startai.exists(out)
            else startai.default_dtype(item=updates)
        ),
    )
    ...
    if indices.ndim > 1 and reduction != "sum":
        ...
        indices, unique_idxs = startai.unique_all(
            startai.flip(indices, axis=[0]), axis=0, by_value=True
        )[:2]
        ...
        if len(unique_idxs) < num_indices:
            ...
            updates = startai.gather(startai.flip(updates, axis=[0]), unique_idxs, axis=0).data
            expected_shape = (
                list(indices.shape[:-1]) + list(out.shape[indices.shape[-1] :])
                if startai.exists(out)
                else list(indices.shape[:-1]) + list(shape[indices.shape[-1] :])
    ...
    # implementation
    target_given = startai.exists(out)
    if target_given:
        target = out.data
    else:
        shape = list(shape) if startai.exists(shape) else out.shape
    ...
    if reduction == "min":
        updates = startai.minimum(startai.gather_nd(target, indices), updates).data
    elif reduction == "max":
        updates = startai.maximum(startai.gather_nd(target, indices), updates).data
    elif reduction == "sum":
        updates = startai.add(startai.gather_nd(target, indices), updates).data
    updates_ = _broadcast_to(startai.gather_nd(target, indices), expected_shape).data
...
def shape(
    x: paddle.Tensor, /, *, as_array: bool = False
) -> Union[startai.Shape, startai.Array]:
    if as_array:
        return startai.array(x.shape, dtype=startai.default_int_dtype())
    else:
