...
def ceil(x: torch.Tensor, /, *, out: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    if "int" in str(x.dtype):
        if startai.exists(out):
            return startai.inplace_update(out, x)
        return x
...
def divide(
    ...
    *,
    ...
    if startai.is_float_dtype(x1.dtype) or startai.is_complex_dtype(x1.dtype):
        ret = startai.astype(ret, x1.dtype, copy=False)
    else:
        ret = startai.astype(ret, startai.default_float_dtype(as_native=True), copy=False)
    return ret
...
def floor_divide(
    ...
    *,
    ...
    x1, x2 = startai.promote_types_of_inputs(x1, x2)
    if startai.exists(out):
        if not startai.is_float_dtype(out):
...
def pow(
    ...
    *,
    ...
    if startai.is_complex_dtype(x1) and startai.any(startai.isinf(x2)):
        ...
        return torch.where(
            startai.isinf(x2), torch.nan + torch.nan * 1j if x2 < 0 else -0 * 1j, ret
        )
    x1, x2 = startai.promote_types_of_inputs(x1, x2)
    if startai.any(x1 == 0):
        if startai.is_complex_dtype(x2):
            x2 = torch.broadcast_to(x2, x1.shape)
            ret = torch.pow(x1, x2)
            return torch.where(x1 == 0, torch.nan + torch.nan * 1j, ret)
        elif (
            startai.any(x2 < 0)
            and startai.is_int_dtype(x2)
            and all(dtype not in str(x1.dtype) for dtype in ["int16", "int8"])
        ):
            if startai.is_int_dtype(x1):
...
def logaddexp2(
    ...
    *,
    ...
    if not startai.is_float_dtype(x1):
        x1 = x1.type(startai.default_float_dtype(as_native=True))
        x2 = x2.type(startai.default_float_dtype(as_native=True))
...
def trunc_divide(
    ...
    *,
    ...
    if startai.is_float_dtype(x1.dtype):
        ret = ret.to(x1.dtype)
    else:
        ret = ret.to(startai.default_float_dtype(as_native=True))
    return ret
