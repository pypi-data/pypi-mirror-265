...
def unique_all(
    ...
    *,
    ...
    startai_torch = startai.current_backend()
    values = values.index_select(dim=axis, index=sort_idx)
    counts = startai_torch.gather(counts, sort_idx)
    indices = startai_torch.gather(indices, sort_idx)
    inv_sort_idx = startai_torch.invert_permutation(sort_idx)
