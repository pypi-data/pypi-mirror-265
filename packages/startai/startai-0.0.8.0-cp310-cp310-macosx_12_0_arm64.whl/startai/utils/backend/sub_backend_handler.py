...
for backend in os.listdir(
    ...
    sub_backends_dir = os.path.join(
        startai.__path__[0].rpartition(os.path.sep)[0],
        _backends_subpackage_path.replace(".", os.path.sep),
        backend,
        "sub_backends",
    )
...
def set_sub_backend(sub_backend_str: str):
    ...
    if startai.current_backend_str() not in _backend_to_sub_backends_dict:
        logging.warning(
            f"backend {startai.current_backend_str()} does not have any"
    ...
    if sub_backend_str not in _all_sub_backends:
        raise StartaiException(
            ...
            f" {_backend_to_sub_backends_dict[startai.current_backend_str()]}"
        )

    if sub_backend_str not in _backend_to_sub_backends_dict[startai.current_backend_str()]:
        logging.warning(
            f"{startai.current_backend_str()} does not support"
            f" {sub_backend_str} as a sub_backend"
        )
        return

    if sub_backend_str in startai.current_sub_backends:
        return

    global original_backend_dict
    if original_backend_dict is None:
        original_backend_dict = startai.__dict__.copy()
    sub_backend = importlib.import_module(_sub_backend_dict[sub_backend_str])
    set_sub_backend_to_specific_version(sub_backend)
    _set_sub_backend_as_startai(startai.__dict__.copy(), startai, sub_backend)
    startai.current_sub_backends.append(sub_backend_str)


# this is very similar to _set_backend_as_startai in handler.py, with a minor change
def _set_sub_backend_as_startai(
...
def unset_sub_backend(sub_backend_str: str):
    if sub_backend_str not in startai.current_sub_backends:
        return
    global original_backend_dict

    # The sub-backend is cached so this is fast
    sub_backend = importlib.import_module(_sub_backend_dict[sub_backend_str])
    _unset_sub_backend_from_startai(
        original_backend_dict, startai, sub_backend, sub_backend.name
    )
    startai.current_sub_backends.remove(sub_backend_str)


def _unset_sub_backend_from_startai(
...
def clear_sub_backends():
    if startai.current_sub_backends:
        startai.__dict__.update(original_backend_dict)
        startai.current_sub_backends.clear()
