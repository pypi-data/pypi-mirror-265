...
class TuckerTensor(FactorizedTensor):
    ...
    def n_param(self):
        ...
        total_params = sum(int(startai.prod(tensor.shape)) for tensor in [core] + factors)
        return total_params
    ...
    def validate_tucker_tensor(tucker_tensor):
        ...
        for i, factor in enumerate(factors):
            ...
            if current_rank != startai.shape(core)[i]:
                raise ValueError(
                    "Factor `n` of Tucker decomposition should"
                    " verify:\nfactors[n].shape[1] = core.shape[n].However,"
                    f" factors[{i}].shape[1]={startai.shape(factor)[1]} but"
                    f" core.shape[{i}]={startai.shape(core)[i]}."
                )
    ...
    def tucker_normalize(tucker_tensor):
        ...
        normalized_factors = []
        for i, factor in enumerate(factors):
            scales = startai.sqrt(startai.sum(startai.abs(factor) ** 2, axis=0))
            scales_non_zero = startai.where(
                scales == 0, startai.ones(startai.shape(scales), dtype=factor[0].dtype), scales
            )
            core = core * startai.reshape(
    ...
    def tucker_mode_dot(
        ...
        if contract:
            ...
            f = factors.pop(mode)
            core = startai.mode_dot(core, startai.dot(matrix_or_vector, f), mode=mode)
        else:
    ...
    def validate_tucker_rank(
        ...
        if isinstance(rank, float):
            ...
            n_param_tensor = int(startai.prod(tensor_shape))
            ...
            squared_dims = startai.sum([s**2 for s in tensor_shape])
    ...
    def tucker_n_param(shape, rank):
        core_params = startai.prod(rank)
        factors_params = startai.sum([r * s for (r, s) in zip(rank, shape)])
