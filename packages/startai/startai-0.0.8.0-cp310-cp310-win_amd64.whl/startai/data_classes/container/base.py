...
from startai.utils.exceptions import StartaiBackendException, StartaiException
...
class ContainerBase(dict, abc.ABC):
    def __init__(
        ...
        **kwargs,
        ...
        container_combine_method
            ...
            Default is startai.Container.cont_list_join
        queue_timeout
        ...
        startaih
            Handle to startai module to use for the calculations. Default is ``None``, which
            results in the global startai.
        default_key_color
        ...
        if startai.exists(self._queues):
            ...
            self._queue_timeout = startai.default(queue_timeout, startai.queue_timeout)
        ...
        if dict_in is None:
            ...
            raise startai.utils.exceptions.StartaiException(
                "dict_in and **kwargs cannot both be specified for startai.Container "
    ...
    def cont_multi_map_in_function(
        ...
        *args,
        ...
        out=None,
        **kwargs,
    ) -> Union[Tuple[startai.Container, startai.Container], startai.Container]:
        inspect_fn = fn
        if isinstance(fn, str):
            inspect_fn = startai.__dict__[fn]
        # retrieve indices where leaves of args are also nested
        arg_cont_idxs = startai.nested_argwhere(
            args, startai.is_startai_container, to_ignore=startai.Container
        )
        # retrieve indices where leaves of kwargs are also nested
        kwarg_cont_idxs = startai.nested_argwhere(
            kwargs, startai.is_startai_container, to_ignore=startai.Container
        )

        # retrieve all the containers in args
        arg_conts = startai.multi_index_nest(args, arg_cont_idxs)
        num_arg_conts = len(arg_conts)

        # retrieve all the containers in kwargs
        kwarg_conts = startai.multi_index_nest(kwargs, kwarg_cont_idxs)
        ...
        if with_out:
            ...
            num_out_conts = 1
            out_cont_idxs = []
            if not startai.is_array(out) and not startai.is_startai_container(out):
                out_cont_idxs = startai.nested_argwhere(
                    out, startai.is_startai_container, to_ignore=startai.Container
                )
                out_conts = startai.multi_index_nest(out, out_cont_idxs)
                num_out_conts = len(out_conts)
            conts = arg_conts + kwarg_conts + out_conts
        else:
            conts = arg_conts + kwarg_conts
        startai.utils.assertions.check_exists(
            conts, message="no containers found in arguments"
        )
        cont0 = conts[0]
        if isinstance(fn, str):
            fn = cont0.cont_startai.__dict__[fn]
        # Get the function with the name fn_name, enabling containers to specify
        # their backends irrespective of global startai's backend
        ...
        def map_fn(vals, _):
            ...
            a = startai.copy_nest(args)
            a = startai.set_nest_at_indices(a, arg_cont_idxs, arg_vals)
            kwarg_vals = vals[num_arg_conts:]
            kw = startai.copy_nest(kwargs)
            kw = startai.set_nest_at_indices(kw, kwarg_cont_idxs, kwarg_vals)
            if with_out:
        ...
        # as inputs
        ret = startai.Container.cont_multi_map(
            map_fn,
            conts,
            key_chains,
            to_apply,
        ...
        if startai.is_startai_container(ret):
            for values in ret.values():
                if isinstance(values, (tuple, list)):
                    for v in values:
                        if startai.is_startai_array(v):
                            return ret.cont_unstack_conts(0)

        if with_out:
            for out_cont_idx, out_cont in zip(out_cont_idxs, out_conts):
                out_cont.cont_inplace_update(startai.index_nest(ret, out_cont_idx))
            if len(out_conts) == 1:
    ...
    def cont_list_join(containers, config=None):
        ...
        if not startai.exists(config):
            config = (
                container0.cont_config if isinstance(container0, startai.Container) else {}
            )

        if isinstance(container0, startai.Container):
            return_dict = {}
            for key in container0.keys():
                new_list = []
                for container in containers:
                    new_list.append(container[key])
                return_dict[key] = startai.Container.cont_list_join(new_list, config)
            return startai.Container(return_dict, **config)
        else:
    ...
    def cont_list_stack(containers, dim, config=None):
        ...
        if isinstance(container0, startai.Container):
            return_dict = {}
            for key in container0.keys():
                return_dict[key] = startai.Container.cont_list_stack(
    ...
    def _cont_mean_unify(containers, device, _=None, _1=None):
        return startai.Container._cont_sum_unify(containers, device) / len(containers)
    ...
    def cont_unify(containers, device, mode, axis=0):
        ...
        """
        return {
            "concat": startai.Container._cont_concat_unify,
            "sum": startai.Container._cont_sum_unify,
            "mean": startai.Container._cont_mean_unify,
    ...
    def cont_combine(*containers, config=None):
        ...
        if not startai.exists(config):
            ...
            config = (
                ...
                if isinstance(container_rightmost, startai.Container)
                else {}
            )
        ...
        for key in all_keys:
            ...
            return_dict[key] = startai.Container.cont_combine(
    ...
    def cont_diff(
        ...
        *containers
        ...
        -------
            Compared containers
        """
        startai.utils.assertions.check_elem_in_list(mode, ["all", "same_only", "diff_only"])
        ...
        if not isinstance(container0, dict):
            equal_mat = startai.all_equal(*containers, equality_matrix=True)
            if not detect_value_diffs:
                equal_mat = startai.ones_like(equal_mat)
            if detect_shape_diffs:
                shape_equal_mat = startai.all_equal(
                    *[c.shape if startai.is_array(c) else None for c in containers],
                    equality_matrix=True,
                )
                equal_mat = startai.logical_and(equal_mat, shape_equal_mat)
            # noinspection PyTypeChecker
            if startai.min(startai.astype(equal_mat, "int32")) == 1:
                if mode == "diff_only":
                    return startai.Container(**config)
                return container0
            elif mode == "same_only":
                return startai.Container(**config)
            else:
                ...
                for idx in cont_range:
                    if idx not in idxs_added:
                        idxs_to_add = startai.argwhere(equal_mat[idx])
                        idxs_to_add_list = sorted(
                            startai.to_numpy(idxs_to_add).reshape(-1).tolist()
                        )
                ...
                return startai.Container(diff_dict, **config)
        ...
        for key in all_keys:
            ...
            if all_keys_present:
                res = startai.Container.cont_diff(
                    *[
                        (
                            cont[key]()
    ...
    def cont_structural_diff(
        ...
        *containers
        ...
        -------
            Compared containers
        """
        return startai.Container.cont_diff(
            *containers,
            mode=mode,
            diff_keys=diff_keys,
            detect_key_diffs=detect_key_diffs,
    ...
    def cont_multi_map(
        ...
        container0 = None
        for container in containers:
            if isinstance(container, startai.Container):
        ...
        startai.utils.assertions.check_exists(
            container0,
            message="No containers found in the inputs to startai.Container.cont_multi_map",
        )
        if not startai.exists(config):
            config = (
                container0.cont_config if isinstance(container0, startai.Container) else {}
        ...
        for key in keys:
            values = []
            for cont in containers:
                if isinstance(cont, (startai.Container, list, tuple)) and key in cont:
                    values.append(cont[key])
                elif not isinstance(cont, (startai.Container, list, tuple)):
                    values.append(cont)
            value0 = values[0]
            if len(values) >= 1:
                this_key_chain = key if key_chain == "" else (key_chain + "/" + key)
                is_container = [startai.is_startai_container(x) for x in values]
                ...
                if not assert_identical and not all(is_container) and any(is_container):
                    ...
                    if isinstance(value0, startai.Container):
                        ret = startai.Container.cont_multi_map(
                            func,
                            values,
                            key_chains,
                        ...
                        ret = startai.nested_multi_map(
                            lambda x, _: func(x, None), values, to_startai=False
                        )
    ...
    def cont_identical(
        ...
        """
        if partial:
            common_key_chains = startai.Container.cont_common_key_chains(containers)
        ...
        for key in keys:
            ...
            if not min(type_n is type_0 for type_n in types):
                if isinstance(value_0, startai.Container) or check_types:
                    return False
            if startai.is_array(value_0):
                if check_shapes:
                ...
                if same_arrays:
                    ...
                    if not startai.all_equal(*values):
                        return False
            ...
            if isinstance(value_0, startai.Container):
                ret = startai.Container.cont_identical(
                    values,
                    check_types,
                    check_shapes,
                    same_arrays,
    ...
    def cont_assert_identical(
        ...
        startai.utils.assertions.check_true(
            startai.Container.cont_identical(
                containers,
                check_types,
                check_shapes,
                same_arrays,
            ...
            f"Containers were not identical:\n\n{startai.Container.cont_diff(*containers)}",
        )
    ...
    def cont_identical_structure(
        ...
        -------
            Boolean
        """
        return startai.Container.cont_identical(
            containers,
            check_types,
            check_shapes,
            False,
    ...
    def cont_assert_identical_structure(
        ...
        """
        startai.utils.assertions.check_true(
            startai.Container.cont_identical_structure(
                containers,
                check_types,
                check_shapes,
                key_chains,
            ...
            f" structure:\n\n{startai.Container.cont_structural_diff(*containers)}",
        )
    ...
    def cont_identical_configs(containers):
        ...
        containers
            containers to check.
        """
        startai.utils.assertions.check_greater(len(containers), 1, as_array=False)
    ...
    def cont_load(filepath, format="h5py"):
        if format == "json":
            return startai.Container.cont_from_disk_as_json(filepath)
        elif format == "pickle":
            return startai.Container.cont_from_disk_as_pickled(filepath)
        elif format == "h5py":
            return startai.Container.cont_from_disk_as_hdf5(filepath)
        else:
            raise startai.utils.exceptions.StartaiException("Unsupported format")

    @staticmethod
    def cont_from_disk_as_hdf5(
        h5_obj_or_filepath, slice_obj=slice(None), alphabetical_keys=True, startaih=None
    ):
        ...
        for key, value in items:
            if isinstance(value, h5py.Group):
                container_dict[key] = startai.Container.cont_from_disk_as_hdf5(
                    value, slice_obj, startaih
                )
            elif isinstance(value, h5py.Dataset):
                container_dict[key] = startai.default(startaih, startai).array(
        ...
        return startai.Container(container_dict, startaih=startaih)

    @staticmethod
    def cont_from_disk_as_pickled(pickle_filepath, startaih=None):
    ...
    @staticmethod
    def cont_from_disk_as_json(json_filepath, startaih=None):
        ...
        with open(json_filepath) as json_data_file:
            return startai.Container(json.load(json_data_file), startaih=startaih)
    ...
    def h5_file_size(h5_obj_or_filepath):
        ...
        for value in h5_obj.values():
            if isinstance(value, h5py.Group):
                size_to_add, batch_size = startai.Container.h5_file_size(value)
                size += size_to_add
            elif isinstance(value, h5py.Dataset):
    ...
    def shuffle_h5_file(h5_obj_or_filepath, seed_value=0):
        ...
        for value in h5_obj.values():
            if isinstance(value, h5py.Group):
                startai.Container.shuffle_h5_file(value, seed_value)
            elif isinstance(value, h5py.Dataset):
    ...
    def cont_reduce(containers, reduction, config=None):
        ...
        if isinstance(container0, startai.Container):
            return_dict = {}
            for key in container0.keys():
                return_dict[key] = startai.Container.cont_reduce(
    ...
    def cont_trim_key(key, max_length):
        ...
        """
        key_len = len(key)
        if not startai.exists(max_length) or key_len <= max_length:
            return key
        idxs = (
            np.round(
                (key_len - 1)
    ...
    def _cont_get_shape(self):
        ...
        sub_shapes = [
            ...
            for k, v in self.cont_map(
                lambda x, kc: (
                    list(x.shape)
                    if self._cont_startai.is_native_array(x) or isinstance(x, startai.Array)
    ...
    def _cont_get_dev(self, as_native=False):
        sub_devs = [
            ...
            for k, v in self.cont_map(
                lambda x, kc: (
                    self._cont_startai.dev(x, as_native=as_native)
                    if self._cont_startai.is_native_array(x) or isinstance(x, startai.Array)
                    else None
                )
            ).cont_to_iterator()
    ...
    def _cont_at_key_chains_input_as_seq(self, key_chains, ignore_key_errors=False):
        return_cont = startai.Container({}, **self._config)
        for kc in key_chains:
            val = self.cont_at_key_chain(kc, ignore_key_errors=ignore_key_errors)
            if ignore_key_errors and not startai.exists(val):
                continue
        ...
        for k, v in key_chains.items():
            ...
            if isinstance(v, dict):
                ...
                if ignore_key_errors and not startai.exists(val):
                    continue
                return_dict[k] = val
        return startai.Container(return_dict, **self._config)
        ...
        for k, v in config.items():
            ...
            if k in self._config_in:
                if k == "types_to_iteratively_nest":
                    v = startai.default(lambda: tuple(v), (), catch_exceptions=True)
                elif k == "keyword_color_dict":
                    v = startai.default(v, {})
                elif k == "startaih":
                    att_name = "_local_startai"
                new_config[k] = v
    ...
    def cont_inplace_update(
        self, dict_in: Union[startai.Container, dict], **config
    ) -> startai.Container:
        ...
        **config
        ...
        if dict_in is None:
            return
        dict_types = tuple([dict] + startai.container_types())
        if isinstance(dict_in, dict_types):
            ...
            raise startai.utils.exceptions.StartaiException(f"invalid input {dict_in}")
        items = sorted(dict_in.items()) if self._alphabetical_keys else dict_in.items()
        for key, value in items:
            if (
                isinstance(value, dict_types)
                and (
                    not isinstance(value, startai.Container)
                    or self._rebuild_child_containers
                )
            ) or isinstance(value, tuple(self._types_to_iteratively_nest)):
                self[key] = startai.Container(value, **self._config)
            elif key in self and isinstance(self[key], startai.Container):
    ...
    def cont_slice_via_key(self, slice_key):
        ...
        for key, value in self.items():
            if key == slice_key:
                return value
            elif isinstance(value, startai.Container):
    ...
    def cont_as_bools(
        ...
        def _ret_bool(x):
            if assert_is_bool:
                startai.utils.assertions.check_isinstance(x, bool)
                return x
            return bool(x)
    ...
    def cont_num_arrays(self, exclusive=False):
        ...
        return sum(
            self.cont_map(
                lambda x, kc: startai.is_array(x, exclusive=exclusive)
    ...
    def cont_size_ordered_arrays(self, exclusive=False):
        ...
        """
        array_dict = {
            startai.Container.cont_flatten_key_chain(kc): v
            for kc, v in self.cont_to_iterator()
            if startai.is_array(v, exclusive=exclusive)
        }
        return startai.Container(
            dict(
                sorted(
    ...
    def cont_to_disk_as_hdf5(
        ...
        if isinstance(h5_obj_or_filepath, str):
            ...
            h5_obj = h5_obj_or_filepath
        for key, value in self.items():
            if isinstance(value, startai.Container):
                ...
                value_as_np = self._cont_startai.to_numpy(value)
    ...
    def cont_to_raw(self):
        ...
        for key, value in self.items():
            if isinstance(value, startai.Container):
                ...
                return_item = [
                    v.cont_to_raw() if isinstance(v, startai.Container) else v
    ...
    def cont_to_iterator(self, key_chain="", leaf_keys_only=False, include_empty=False):
        ...
        for key, value in self.items():
            ...
            if isinstance(value, startai.Container) and (not include_empty or value):
    ...
    def cont_from_flat_list(self, flat_list):
        ...
        return startai.Container(new_dict, **self._config)
    ...
    def cont_find_sub_container(self, sub_cont_to_find, partial=False):
        ...
        def _check_sub_cont(sub_cont, kc):
            ...
            if (
                ...
                and startai.Container.cont_identical(
    ...
    def cont_assert_contains_sub_container(self, sub_cont, partial=False):
        ...
        try:
            ...
            raise startai.utils.exceptions.StartaiException(
                ...
                f" values:\n\n{startai.Container.cont_diff(self[key_chain], sub_cont)}"
            )
    ...
    def cont_find_sub_structure(
        ...
        def _check_sub_cont(sub_cont, kc):
            ...
            if (
                kcs_in_sub_cont
                and min(kcs_in_sub_cont)
                and startai.Container.cont_identical_structure(
        ...
        try:
            ...
            raise startai.utils.exceptions.StartaiException(
                "Containers did not have identical structure:\n\n{}".format(
                    startai.Container.cont_structural_diff(
                        self[key_chain],
                        sub_cont,
        ...
        for key in keys:
            try:
                ...
                if ignore_key_errors:
                    return
                raise startai.utils.exceptions.StartaiException(repr(e))
        return ret
    ...
    def cont_at_key_chains(self, key_chains, ignore_none=True, ignore_key_errors=False):
        ...
        if isinstance(key_chains, (list, tuple)):
            ...
            raise startai.utils.exceptions.StartaiException(
                ...
                f" or startai.Container, but found type {type(key_chains)}"
            )
    ...
    def cont_set_at_keys(self, target_dict):
        ...
        for key, val in self.items():
            ...
            elif isinstance(val, startai.Container):
    ...
    def cont_set_at_key_chain(self, key_chain, val, inplace=False):
        ...
        for key in keys[:-1]:
            if key not in sub_cont:
                sub_cont[key] = startai.Container(**self._config)
        ...
        for key, value in self.items():
            if isinstance(value, startai.Container):
                ...
                if len(keys_in_chain) != 1 or key != keys_in_chain[0]:
                    out_dict[key] = value
        return startai.Container(out_dict, **self._config)
    ...
    def cont_format_key_chains(self, format_fn):
        ...
        """
        return startai.Container({format_fn(k): v for k, v in self.cont_to_iterator()})

    def cont_sort_by_key(self):
        new_dict = {}
        for k, v in self.items():
            if isinstance(v, startai.Container):
    ...
    def cont_prune_empty(self, keep_nones=False, base=True):
        ...
        for key, value in self.items():
            ...
            elif self._cont_startai.exists(value) or keep_nones:
                out_dict[key] = value
        if len(out_dict):
            return startai.Container(out_dict, **self._config)
        if base:
            return startai.Container(**self._config)
        return
    ...
    def cont_prune_key_from_key_chains(self, absolute=None, containing=None):
        ...
        out_cont = startai.Container(**self._config)
        for key, value in self.items():
            if (absolute and key == absolute) or (containing and containing in key):
                if isinstance(value, startai.Container):
                    out_cont = startai.Container.cont_combine(out_cont, value)
                else:
                    out_cont = value
            elif isinstance(value, startai.Container):
    ...
    def cont_restructure_key_chains(
        ...
        """
        new_cont = self.cont_copy() if keep_orig else startai.Container()
        for old_kc, new_kc in keychain_mapping.items():
            ...
            new_cont = startai.Container.cont_combine(
                new_cont, startai.Container({new_kc: self[old_kc]})
            )
        return new_cont
        ...
        for old_kc, new in mapping.items():
            ...
            if isinstance(new, dict):
                ...
                if "pattern" in new:
                    ...
                    if isinstance(val, startai.Container):
                        val = val.einops_rearrange(pattern, **axes_lengths)
                    else:
                        val = startai.einops_rearrange(val, pattern, **axes_lengths)
            else:
                new_kc = new
            new_cont = startai.Container.cont_combine(
                new_cont, startai.Container({new_kc: val})
            )
        return new_cont
    ...
    def cont_flatten_key_chains(
        ...
        return startai.Container(
            {
                startai.Container.cont_flatten_key_chain(
    ...
    def cont_copy(self):
        ...
        """
        return startai.Container(self.cont_to_dict(), **self._config)
    ...
    def cont_deep_copy(self):
        ...
        return self.cont_map(
            lambda x, kc: (
                startai.copy_array(x) if startai.is_array(x) and not isinstance(x, str) else x
            )
        )
    ...
    def cont_map_sub_conts(
        ...
        for key, value in self.items():
            ...
            if isinstance(value, startai.Container):
                ...
                return_dict[key] = value
        ret = return_dict if inplace else startai.Container(return_dict, **self._config)
    ...
    def cont_with_entries_as_lists(self):
        def to_list(x, _=""):
            try:
                return self._cont_startai.to_list(x)
            except (StartaiException, StartaiBackendException):
                return x
    ...
    def cont_reshape_like(self, target_dict, leading_shape=None, return_cont=None):
        ...
        """
        leading_shape = self._cont_startai.default(leading_shape, [])
        if return_cont is None:
            return_cont = self.cont_copy()
        for (_, v_shape), (k, v) in zip(target_dict.items(), return_cont.items()):
            if isinstance(v_shape, dict):
                ...
                return_cont[k] = self._cont_startai.reshape(
                    v, leading_shape + list(v_shape)
                )
        return startai.Container(return_cont, **self._config)
    ...
    def _cont_slice_keys(self, key_slice):
        ...
        if isinstance(key_slice, str):
            startai.utils.assertions.check_true(len(key_slice) == 3 and key_slice[1] == ":")
            startai.utils.assertions.check_true(self._alphabetical_keys)
    ...
    def cont_with_startai_backend(self, startai_backend: str, inplace=False):
        """Summary.
        ...
        """
        if inplace:
            self._cont_startai = startai_backend
            self._config["startaih"] = startai_backend
            return self
        else:
            return startai.Container(self, startaih=startai_backend)
    ...
    def cont_show_sub_container(self, sub_cont_or_keychain):
        ...
        this_cont[sub_cont_kc] = startai.Container({"SUB_CONT": None})
    ...
    def __repr__(self, as_repr=True):
        ...
        def _align_array(array_str_in):
            ...
            split_phrase = split_phrase_dict[self._cont_startai.current_backend_str()]
        ...
        for k, v in self.items():
            if isinstance(v, startai.Container):
                ...
                if (
                    (self._cont_startai.is_native_array(v) or isinstance(v, startai.Array))
                    and len(list(v.shape)) > 0
                    and startai.exists(self._print_limit)
                    ...
                    and v
                    and (
                        self._cont_startai.is_native_array(v[0])
                        or isinstance(v[0], startai.Array)
                    )
                ):
                    if (
                        isinstance(v, tuple)
        ...
        if as_repr:
            json_dumped_str = _align_arrays(
                json.dumps(
                    startai.Container(new_dict, **self._config)
                    .cont_map(
                        lambda x, kc: (
                            x
                            if _is_jsonable(x)
            ...
            if startai.backend_stack and startai.current_backend_str() == "tensorflow":
                ...
                [
                    (
                        ' "'.join(
                            ...
                            + [
                                termcolor.colored(
                                    startai.Container.cont_trim_key(
    ...
    def _get_queue_item(self, query):
        if isinstance(query, int):
            ...
            queue_queries = list(
                range(query.start, query.stop, startai.default(query.step, 1))
            )
        elif isinstance(query, (list, tuple)):
            queue_queries = list(
                range(query[0].start, query[0].stop, startai.default(query[0].step, 1))
            )
        else:
            raise startai.utils.exceptions.StartaiException(
    ...
    def __getitem__(self, query):
        ...
        if isinstance(query, str):
            ...
            return ret
        elif startai.exists(self._queues):
        ...
        for key, value in self.items():
            if isinstance(value, startai.Container):
                ...
                if isinstance(value, (list, tuple)):
                    ...
                    return_dict[key] = value[query]
        ret = startai.Container(return_dict, **self._config)
        return ret
    ...
    def __setitem__(self, query, val):
        ...
        val startai.Container, array, or other
    ...
    def __contains__(self, key):
        ...
        elif isinstance(key, startai.Container):
    ...
    def __getstate__(self):
        ...
        state_dict["_local_startai"] = (
            state_dict["_local_startai"].current_backend_str()
            if state_dict["_local_startai"] is not None
            else None
        )
        config_in = copy.copy(state_dict["_config_in"])
        config_in["startaih"] = (
            config_in["startaih"].current_backend_str()
            if config_in["startaih"] is not None
            else None
        )
        state_dict["_config_in"] = config_in
        config = copy.copy(state_dict["_config"])
        config["startaih"] = (
            config["startaih"].current_backend_str()
            if getattr(config, "startaih", None) is not None
            else None
        )
    ...
    def __setstate__(self, state_dict):
        if "_local_startai" in state_dict and startai.exists(state_dict["_local_startai"]):
            if len(state_dict["_local_startai"]) > 0:
                state_dict["_local_startai"] = startai.with_backend(state_dict["_local_startai"])
            else:
                state_dict["_local_startai"] = startai
        if "_config_in" in state_dict:
            config_in = copy.copy(state_dict["_config_in"])
            if "startaih" in config_in and startai.exists(config_in["startaih"]):
                if len(config_in["startaih"]) > 0:
                    config_in["startaih"] = startai.with_backend(config_in["startaih"])
                else:
                    config_in["startaih"] = startai
            state_dict["_config_in"] = config_in
        if "_config" in state_dict:
            config = copy.copy(state_dict["_config"])
            if "startaih" in config and startai.exists(config["startaih"]):
                if len(config["startaih"]) > 0:
                    config["startaih"] = startai.with_backend(config["startaih"])
                else:
                    config["startaih"] = startai
            state_dict["_config"] = config
    ...
    @property
    def _cont_startai(self):
        return startai.default(self._local_startai, startai)

    @_cont_startai.setter
    def _cont_startai(self, local_startai):
        self._local_startai = local_startai
    ...
    @property
    def cont_startai(self):
        return self._cont_startai
