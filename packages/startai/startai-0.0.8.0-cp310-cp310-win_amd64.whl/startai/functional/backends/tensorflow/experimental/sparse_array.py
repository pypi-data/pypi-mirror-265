...
# local
import startai
from startai.functional.startai.experimental.sparse_array import (
    ...
    *,
    ...
    if _is_data_not_indices_values_and_shape(
        ...
        startai.utils.assertions.check_true(
            startai.is_native_sparse_array(data), message="not a sparse array"
        )
        return data
    ...
    if format == "coo":
        ...
        for i in range(values.shape[0]):
            coordinate = startai.gather(coo_indices, startai.array([[i]]))
            coordinate = startai.reshape(coordinate, (coo_indices.shape[0],))
...
def native_sparse_array_to_indices_values_and_shape(x):
    ...
    raise startai.utils.exceptions.StartaiException("not a SparseTensor")
