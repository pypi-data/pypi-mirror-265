"""Collection of TensorFlow network layers, wrapped to fit Startai syntax and
signature."""
...
def conv_general_dilated(
    ...
    *,
    ...
    if data_format == "channel_first" and (
        startai.dev(x) == "cpu" or feature_group_count != 1
    ):
    ...
    *,
    ...
    if feature_group_count == 1:
        ...
        if filter_format == "channel_first":
            filters = tf.transpose(filters, (*range(2, dims + 2), 1, 0))
        permuted_x = False
        if data_format == "channel_first" and startai.dev(x) == "cpu":
            x = tf.transpose(x, (0, *range(2, dims + 2), 1))
            data_format = "channel_last"
            permuted_x = True
