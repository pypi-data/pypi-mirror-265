...
from startai.utils.exceptions import StartaiNotImplementedException, StartaiValueError
...
def fft(
    ...
    *,
    ...
    if not isinstance(dim, int):
        raise StartaiValueError(f"Expecting <class 'int'> instead of {type(dim)}")
    ...
    if n <= 1:
        raise StartaiValueError(f"Invalid number of data points {n}, expecting more than 1")
...
def stft(
    ...
    *,
    ...
    if not isinstance(frame_length, int):
        raise StartaiValueError(f"Expecting <class 'int'> instead of {type(frame_length)}")
    ...
    if not isinstance(frame_step, int):
        raise StartaiValueError(f"Expecting <class 'int'> instead of {type(frame_step)}")
