"""Collection of Jax gradient functions, wrapped to fit Startai syntax and
signature."""
...
def _forward_fn(
    ...
    x = startai.nested_map(startai.to_startai, x, include_derived=True)
    x_arr_idxs = startai.nested_argwhere(x, startai.is_array)
    x_arr_values = startai.multi_index_nest(x, x_arr_idxs)
    if xs_grad_idxs is not None:
        xs_grad_arr_idxs = []
        for grad_idx in xs_grad_idxs:
            xs_grad_arr_idx = startai.nested_argwhere(
                startai.index_nest(xs, grad_idx), startai.is_array
            )
            for idx in xs_grad_arr_idx:
                xs_grad_arr_idxs.append(list(grad_idx) + idx)
        startai.set_nest_at_indices(xs, xs_grad_arr_idxs, x_arr_values)
    elif startai.is_array(xs):
        xs = x
    else:
        xs_arr_idxs = startai.nested_argwhere(xs, lambda x: startai.is_array(x))
        startai.set_nest_at_indices(xs, xs_arr_idxs, x_arr_values)

    # Setting duplicates to ensure same references as in the original input
    if not startai.is_array(xs):
...
def value_and_grad(func):
    ...
    def callback_fn(xs):
        ...
        return startai.to_startai(value), startai.to_startai(grad)
...
def jac(func: Callable):
    ...
    def callback_fn(x_in):
        return startai.to_startai(
            jax.jacfwd(grad_fn)(startai.to_native(x_in, nested=True)),
            nested=True,
...
def grad(func: Callable, argnums: Union[int, Tuple[int]] = 0):
    def grad_fn(x_in):
        return startai.to_native(func(x_in))

    def callback_fn(x_in):
        return startai.to_startai(jax.grad(grad_fn, argnums)(startai.to_native(x_in)))
