"""Collection of Numpy general functions, wrapped to fit Startai syntax and
signature."""
...
from startai.functional.backends.numpy.helpers import _scalar_output_to_0d_array
...
def inplace_decrement(
    x: Union[startai.Array, np.ndarray], val: Union[startai.Array, np.ndarray]
) -> startai.Array:
...
def inplace_update(
    x: Union[startai.Array, np.ndarray],
    val: Union[startai.Array, np.ndarray],
    /,
    *,
...
def shape(
    ...
    *,
    ...
    if as_array:
        return startai.array(np.shape(x), dtype=startai.default_int_dtype())
    else:
