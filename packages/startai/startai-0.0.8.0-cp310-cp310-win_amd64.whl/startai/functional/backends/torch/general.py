...
# local
import startai
from startai.func_wrapper import _update_torch_views, with_unsupported_dtypes
...
def neg_step(query):
    return (
        ...
        and not startai.is_array(query)
...
def to_numpy(
    ...
    if isinstance(x, (float, int, bool)):
        ...
        if copy:
            ...
            if 0 in x.shape:
                ...
                return np.empty(x.shape, dtype=startai.as_startai_dtype(x.dtype))
            return np.array(x.tolist(), dtype=startai.as_startai_dtype(x.dtype))
        else:
    ...
    raise startai.utils.exceptions.StartaiException("Expected a pytorch tensor.")
...
def to_list(x: torch.Tensor, /) -> list:
    if isinstance(x, np.ndarray):
        ...
        if x.dtype is torch.bfloat16:
            default_dtype = startai.default_float_dtype(as_native=True)
...
def inplace_decrement(
    x: Union[startai.Array, torch.Tensor],
    val: Union[startai.Array, torch.Tensor],
) -> startai.Array:
...
def inplace_update(
    ...
    *,
    ...
    startai.utils.assertions.check_inplace_sizes_valid(x, val)
    ...
    *,
    ...
    updates = torch.tensor(
        ...
        dtype=(
            ...
            if startai.exists(out)
            else startai.default_dtype(item=updates, as_native=True)
        ),
    )
    ...
    if startai.exists(shape) and target_given:
        startai.utils.assertions.check_equal(
            startai.Shape(out.shape), startai.Shape(shape), as_array=False
        )
...
def vmap_v_1p13p1_and_below(
    ...
    def _vmap(*args):
        def new_fun(*args):
            return startai.to_native(func(*args))
