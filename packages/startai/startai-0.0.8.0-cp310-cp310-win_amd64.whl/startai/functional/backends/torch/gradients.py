"""Collection of PyTorch gradient functions, wrapped to fit Startai syntax and
signature."""
...
def variable(x, /):
    if startai.is_int_dtype(x.dtype):
        x = startai.astype(x, startai.default_float_dtype()).to_native()
    if not x.is_leaf:
    ...
    def callback_fn(xs):
        ...
        def autograd_fn(x):
            ...
            grad = (
                grad
                if grad is not None
                else startai.to_native(startai.zeros_like(startai.to_startai(x)))
            )
...
def jac(func: Callable):
    ...
    def callback_fn(x_in):
        return startai.to_startai(
            torch.func.jacfwd(grad_fn)(startai.to_native(x_in, nested=True)),
            nested=True,
            include_derived=True,
        )
