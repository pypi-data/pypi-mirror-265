...
# local
import startai
from startai.functional.startai.statistical import _get_promoted_type_of_operands
...
def min(
    ...
    *,
    ...
    if where is not None:
        max_val = (
            startai.iinfo(x.dtype).max
            if startai.is_int_dtype(x.dtype)
            else startai.finfo(x.dtype).max
        )
...
def _infer_dtype(dtype: torch.dtype) -> torch.dtype:
    default_dtype = startai.infer_default_dtype(dtype)
    if default_dtype in startai.valid_dtypes:
        if startai.dtype_bits(dtype) < startai.dtype_bits(default_dtype):
            return startai.as_native_dtype(default_dtype)
...
def std(
    ...
    *,
    ...
    if size - correction <= 0:
        ...
        ret = startai.full(ret.shape, float("nan"), dtype=ret.dtype)
        return ret
    ret = torch.mul(
...
def sum(
    ...
    *,
    ...
    dtype = startai.as_native_dtype(dtype)
    if dtype is None and not startai.is_bool_dtype(x):
        dtype = x.dtype
    if axis == ():
        return x.type(dtype)
...
def cumsum(
    ...
    *,
    ...
    if dtype is None:
        if startai.is_int_dtype(x.dtype):
            dtype = startai.promote_types(x.dtype, startai.default_int_dtype(as_native=True))
        dtype = _infer_dtype(x.dtype)
...
def einsum(
    ...
    *operands: torch.Tensor,
    ...
    return startai.astype(torch.einsum(equation, *operands), dtype, copy=False)
