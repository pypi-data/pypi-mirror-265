...
def _conv(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    ret = startai.conv_general_dilated(
        input,
        weight,
        stride,
        padding,
...
def _conv_transpose(
    ...
    dims = len(input.shape) - 2
    weight = startai.permute_dims(weight, axes=(*range(2, dims + 2), 0, 1))
    for i in range(dims):
        weight = startai.flip(weight, axis=i)
...
def conv_transpose1d(
    ...
    if startai.current_backend_str() in ["torch"]:
        # this backend supports explicit padding, no need for conv_general_dilated
        return startai.conv_general_transpose(
            input,
            weight,
            stride,
...
def conv_transpose2d(
    ...
    if startai.current_backend_str() in ["torch", "tensorflow"]:
...
def fold(input, output_size, kernel_size, dilation=1, padding=0, stride=1):
    ...
    output = startai.zeros((n_batches, n_channels, *output_size), dtype=input.dtype)
    output_padded = startai.zero_pad(
        output,
    ...
    if orig_ndim == 2:
        return startai.squeeze(ret, axis=0)
    return ret


@to_startai_arrays_and_back
def unfold(input, kernel_size, dilation=1, padding=0, stride=1):
    # TODO: refactor this function to use startai.sliding_window, but ensure that the
    # function is transpilable to all backends with varying batch size(see issue #25796)
    if input.ndim != 4:
        raise startai.utils.exceptions.StartaiException("only batched 4D inputs are supported")
    stride = [stride] * 2 if isinstance(stride, int) else stride
    dilation = [dilation] * 2 if isinstance(dilation, int) else dilation
    padding = [padding] * 2 if isinstance(padding, int) else padding
    kernel_size = [kernel_size] * 2 if isinstance(kernel_size, int) else kernel_size
    ...
    ret = startai.zeros((*input.shape[0:2], *kernel_size, *output_shape), dtype=input.dtype)
    input_padded = startai.zero_pad(
        input,
        ((0, 0), (0, 0), (padding[0],) * 2, (padding[1],) * 2),
    )
