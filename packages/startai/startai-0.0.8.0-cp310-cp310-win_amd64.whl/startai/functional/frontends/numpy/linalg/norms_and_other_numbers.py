...
def matrix_rank(A, tol=None, hermitian=False):
    return startai.matrix_rank(A, atol=tol, hermitian=hermitian)
...
def norm(x, ord=None, axis=None, keepdims=False):
    if axis is None and (ord is not None):
        if x.ndim not in (1, 2):
            ...
            if x.ndim == 1:
                ret = startai.vector_norm(x, axis=axis, keepdims=keepdims, ord=ord)
            else:
                ret = startai.matrix_norm(x, axis=axis, keepdims=keepdims, ord=ord)
    elif axis is None and ord is None:
        x = startai.flatten(x)
        ret = startai.vector_norm(x, axis=0, keepdims=keepdims, ord=2)
...
def trace(a, offset=0, axis1=0, axis2=1, out=None):
    ret = startai.trace(a, offset=offset, axis1=axis1, axis2=axis2, out=out)
    return ret
