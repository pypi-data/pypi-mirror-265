...
class ClassificationCriterion(Criterion):
    def __init__(self, n_outputs: int, n_classes: startai.Array):
        self.start = 0
        self.pos = 0
        self.end = 0
        ...
        self.n_classes = startai.empty(n_outputs, dtype=startai.int16)
        max_n_classes = 0
        ...
        if isinstance(max_n_classes, startai.Array):
            max_n_classes = startai.to_scalar(max_n_classes)
        self.max_n_classes = max_n_classes

        self.sum_total = startai.zeros((n_outputs, max_n_classes), dtype=startai.float64)
        self.sum_left = startai.zeros((n_outputs, max_n_classes), dtype=startai.float64)
        self.sum_right = startai.zeros((n_outputs, max_n_classes), dtype=startai.float64)
    ...
    def init(
        ...
        for k in range(self.n_outputs):
            n_cls = startai.to_scalar(self.n_classes[k])
    ...
    def init_sum_missing(self):
        self.sum_missing = startai.zeros(
            (self.n_outputs, self.max_n_classes), dtype=startai.float64
        )
    ...
    def update(self, new_pos):
        ...
        for k in range(self.n_outputs):
            for c in range(startai.to_scalar(self.n_classes[k])):
