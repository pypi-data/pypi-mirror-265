...
def rnn(
    ...
    def _new_step_function(*args, **kwargs):
        frontend_args = startai.nested_map(
            _startai_array_to_tensorflow, args, include_derived=True, shallow=False
        )
        frontend_kwargs = startai.nested_map(
            _startai_array_to_tensorflow, kwargs, include_derived=True, shallow=False
        )
        ret = step_function(*frontend_args, **frontend_kwargs)
        return startai.nested_map(_to_startai_array, ret, include_derived=True)
