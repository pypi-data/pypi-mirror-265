...
class DMatrix:
    def __init__(
        ...
        *,
        ...
        self.data = startai.array(data) if not isinstance(data, startai.Array) else data
        self.label = (
            startai.array(label) if (not isinstance(label, startai.Array) and label) else label
        )

        self.weight = (
            (
                startai.array(weight)
                if (not isinstance(weight, startai.Array) and weight)
                else weight
            ),
        )
        self.base_margin = (
            (
                startai.array(base_margin)
                if (not isinstance(base_margin, startai.Array) and base_margin)
                else base_margin
            ),
        )
        ...
        self.group = (
            startai.array(group) if (not isinstance(group, startai.Array) and group) else group,
        )
        self.qid = (
            startai.array(qid) if (not isinstance(qid, startai.Array) and qid) else qid,
        )
        self.label_lower_bound = (
            (
                startai.array(label_lower_bound)
                if (not isinstance(label_lower_bound, startai.Array) and label_lower_bound)
                else label_lower_bound
            ),
        )
        self.label_upper_bound = (
            (
                startai.array(label_upper_bound)
                if (not isinstance(label_upper_bound, startai.Array) and label_upper_bound)
                else label_upper_bound
            ),
        )
        self.feature_weights = (
            (
                startai.array(feature_weights)
                if (not isinstance(feature_weights, startai.Array) and feature_weights)
    ...
    def num_row(self):
        return startai.shape(self.data)[0]

    @with_unsupported_dtypes(
        {"1.7.6 and below": ("bfloat16", "complex64", "complex128")}, "xgboost"
    )
    def num_col(self):
        return startai.shape(self.data)[1]
...
class Booster:
    def __init__(self, params=None, cache=None, model_file=None, compile=False):
        ...
        n_output_group = startai.unique_values(cache[1]).shape[0]
        ...
        if self.compile:
            self._comp_binary_prediction = startai.trace_graph(
...
def _binary_prediction(obj, raw_pred):
    ...
    return startai.where(pred >= 0.5, 1.0, 0.0)
