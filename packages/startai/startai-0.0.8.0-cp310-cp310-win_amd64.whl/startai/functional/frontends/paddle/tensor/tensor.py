...
from startai.functional.frontends.paddle.func_wrapper import _to_startai_array


class Tensor:
    def __init__(self, array, dtype=None, place="cpu", stop_gradient=True):
        self._startai_array = (
            startai.array(array, dtype=dtype, device=place)
            if not isinstance(array, startai.Array)
            else array
        )
        self._dtype = dtype
        self._place = place
    ...
    def __repr__(self):
        return (
            f"startai.frontends.paddle.Tensor(shape={self.shape}, dtype={self.dtype}, "
            + str(self.startai_array.__repr__()).replace("startai.array(", "")
        )
    ...
    @property
    def dtype(self):
        return self._startai_array.dtype

    @property
    def shape(self):
        return list(self.startai_array.shape.shape)
    ...
    def __getitem__(self, item):
        startai_args = startai.nested_map(_to_startai_array, [self, item])
        ret = startai.get_item(*startai_args)
        return paddle_frontend.Tensor(ret)

    def __setitem__(self, item, value):
        raise startai.utils.exceptions.StartaiException(
            "startai.functional.frontends.paddle.Tensor object doesn't support assignment"
        )
    ...
    def __float__(self):
        return float(self._startai_array)
    ...
    def __int__(self):
        return int(self._startai_array)
    ...
    def __mod__(self, y, /, name=None):
        return paddle_frontend.Tensor(startai.fmod(self._startai_array, _to_startai_array(y)))
    ...
    def reshape_(self, *args, shape=None):
        if args and shape:
            raise TypeError("reshape() got multiple values for argument 'shape'")
        if shape is not None:
            self.startai_array = paddle_frontend.reshape(
                self._startai_array, shape=shape
            ).startai_array
            return self
        if args:
            if isinstance(args[0], (tuple, list)):
    ...
    def ceil_(self):
        self.startai_array = self.ceil().startai_array
        return self
    ...
    def lerp_(self, y, weight, name=None):
        self.startai_array = paddle_frontend.lerp(self, y, weight).startai_array
        return self
    ...
    def unsqueeze(self, axis=None, name=None):
        return paddle_frontend.Tensor(startai.expand_dims(self._startai_array, axis=axis))
    ...
    def zero_(self):
        self.startai_array = paddle_frontend.zeros_like(self).startai_array
        return self
    ...
    def subtract_(self, y, name=None):
        self.startai_array = self.subtract(y).startai_array
        return self

    @with_unsupported_dtypes({"2.6.0 and below": ("float16", "bfloat16")}, "paddle")
    def log10(self, name=None):
        return paddle_frontend.Tensor(startai.log10(self._startai_array))
    ...
    def round_(self, name=None):
        self.startai_array = paddle_frontend.round(self).startai_array
        return self
    ...
    def clip(self, min=None, max=None, name=None):
        ...
        if min is None:
            ret = startai.minimum(self._startai_array, max)
        elif max is None:
            ret = startai.maximum(self._startai_array, min)
        else:
            ret = startai.clip(self._startai_array, min, max)
        return paddle_frontend.Tensor(ret)
    ...
    def clip_(self, min=None, max=None, name=None):
        self._startai_array = self.clip(min, max).startai_array
        return self
    ...
    def add(self, y, name=None):
        return paddle_frontend.Tensor(startai.add(self._startai_array, _to_startai_array(y)))

    @with_supported_dtypes({"2.6.0 and below": ("float32", "float64")}, "paddle")
    def add_(self, y, name=None):
        self.startai_array = paddle_frontend.add(self, y).startai_array
        return self
    ...
    def unsqueeze_(self, axis=None, name=None):
        self.startai_array = self.unsqueeze(axis=axis).startai_array
        return self
    ...
    def remainder_(self, y, name=None):
        self.startai_array = paddle_frontend.remainder(self, y).startai_array
        return self
    ...
    def squeeze_(self, axis=None, name=None):
        self.startai_array = paddle_frontend.squeeze(self, axis=axis).startai_array
        return self
    ...
    def all(self, axis=None, keepdim=False, name=None):
        return paddle_frontend.Tensor(
            startai.all(self.startai_array, axis=axis, keepdims=keepdim)
        )
    ...
    def astype(self, dtype):
        return paddle_frontend.Tensor(startai.astype(self._startai_array, dtype))
    ...
    def cumsum(self, axis=None, dtype=None, name=None):
        return paddle_frontend.Tensor(
            startai.cumsum(self._startai_array, axis=axis, dtype=dtype)
        )
    ...
    def tolist(self):
        return paddle_frontend.Tensor(startai.to_list(self._startai_array))
    ...
    def prod(self, axis=None, keepdim=False, dtype=None, name=None):
        return paddle_frontend.Tensor(
            startai.prod(self._startai_array, axis=axis, keepdims=keepdim, dtype=dtype)
        )
    ...
    def as_real(self, name=None):
        if not startai.is_complex_dtype(self._startai_array):
            raise startai.exceptions.StartaiError(
                "as_real is only supported for complex tensors"
            )
        re_part = startai.real(self._startai_array)
        im_part = startai.imag(self._startai_array)
        return paddle_frontend.Tensor(startai.stack((re_part, im_part), axis=-1))
    ...
    def trace(self, offset=0, axis1=0, axis2=1, name=None):
        return paddle_frontend.Tensor(
            startai.trace(self._startai_array, offset=offset, axis1=axis1, axis2=axis2)
        )
    ...
    def flatten(self, start_axis=0, stop_axis=-1, name=None):
        ...
        return paddle_frontend.Tensor(
            startai.flatten(self.startai_array, start_dim=start_axis, end_dim=stop_axis)
        )
    ...
    def tanh_(self, name=None):
        y = self.tanh(self)
        return startai.inplace_update(self, y)
    ...
    def numpy(self):
        return self.startai_array.to_numpy()
    ...
    def acos(self, name=None):
        return paddle_frontend.Tensor(startai.acos(self._startai_array))
    ...
    def as_complex(self, name=None):
        if self.startai_array.shape[-1] != 2:
            raise startai.exceptions.StartaiError(
                "The size of the last dimension of tensor does not equals 2"
            )
        dtype = (
            startai.complex64 if startai.dtype(self.startai_array) == "float32" else startai.complex128
        )
        re_part = self.startai_array[..., 0]
        im_part = startai.multiply(1j, self.startai_array[..., 1])
        value = paddle_frontend.Tensor(startai.add(re_part, im_part).astype(dtype))
        return value
    ...
    def not_equal(self, y, name=None):
        return paddle_frontend.not_equal(self._startai_array, y)

    @with_supported_dtypes(
        {"2.6.0 and below": ("float32", "float64", "int32", "int64")}, "paddle"
    )
    def less_equal(self, y, name=None):
        return paddle_frontend.less_equal(self._startai_array, y)

    @with_supported_dtypes({"2.6.0 and below": ("complex64", "complex128")}, "paddle")
    def real(self, name=None):
        return paddle_frontend.real(self._startai_array)

    @with_unsupported_dtypes({"2.6.0 and below": ("float16", "bfloat16")}, "paddle")
    def t(self, name=None):
        axes = list(range(len(self.startai_array.shape)))[::-1]
        return startai.permute_dims(self.startai_array, axes=axes)
    ...
    def fill_(self, value):
        ...
        return startai.inplace_update(self, filled_tensor)
    ...
    def unbind(self, axis=0):
        return paddle_frontend.unbind(self._startai_array, axis=axis)
    ...
    def unique_consecutive(self, axis=0):
        return paddle_frontend.unique_consecutive(self._startai_array, axis=axis)

    def cpu(self):
        self.startai_array = startai.to_device(self.startai_array, startai.as_startai_dev("cpu"))
        return self
    ...
    def split(self, num_or_sections, axis=0, name=None):
        return paddle_frontend.split(self._startai_array, num_or_sections, axis, name)

    @with_supported_dtypes(
        {"2.6.0 and below": ("float32", "float64", "int32", "int64")}, "paddle"
    )
    def frac(self, name=None):
        return paddle_frontend.frac(self._startai_array)
    ...
    def gather_(self, y, name=None):
        ...
        return startai.inplace_update(self, res)
    ...
    def expand(self, shape, name=None):
        return paddle_frontend.expand(self._startai_array, shape)
    ...
    def tile(self, repeat_times):
        return paddle_frontend.Tensor(startai.tile(self._startai_array, repeats=repeat_times))
    ...
    def chunk(self, chunks, axis=0, name=None):
        return paddle_frontend.split(self._startai_array, num_or_sections=chunks, axis=axis)
