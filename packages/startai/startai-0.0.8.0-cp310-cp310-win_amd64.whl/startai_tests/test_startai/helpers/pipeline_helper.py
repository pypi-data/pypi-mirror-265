...
class WithBackendContext:
    ...
    def __enter__(self):
        return startai.with_backend(self.backend, cached=self.cached)

    def __exit__(self, exc_type, exc_val, exc_tb):
        return


update_backend: Callable = startai.utils.backend.ContextManager
...
class BackendHandler:
    ...
    def _update_context(cls, mode: BackendHandlerMode):
        if mode == BackendHandlerMode.WithBackend:
            ...
            cls._context = startai.utils.backend.ContextManager
            cls._ctx_flag = 1
        else:
...
def get_frontend_config(frontend: str):
    config_module = importlib.import_module(
        f"startai_tests.test_startai.test_frontends.config.{frontend}"
    )
