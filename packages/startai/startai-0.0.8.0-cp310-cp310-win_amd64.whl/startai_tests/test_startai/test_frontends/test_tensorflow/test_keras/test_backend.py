...
from startai.func_wrapper import output_to_native_arrays

# local
import startai_tests.test_startai.helpers as helpers
from startai_tests.test_startai.helpers import handle_frontend_test
from startai_tests.test_startai.test_functional.test_experimental.test_core.test_linalg import (
    _generate_dot_dtype_and_arrays,
)
from startai_tests.test_startai.test_frontends.test_tensorflow.test_nn import (
    _generate_bias_data,
)
from startai_tests.test_startai.test_functional.test_experimental.test_nn.test_layers import (
    _lstm_helper,
)
import startai
...
def test_tensorflow_keras_backend_rnn(
    *,
    ...
    # so can't be inferred through startai.function_unsupported_devices_and_dtypes
    ...
    def _lstm_step(cell_inputs, cell_states):
        ...
        kernel = startai.array(kernel_orig)
        recurrent_kernel = startai.array(recurrent_kernel_orig)
        bias = startai.array(bias_orig)
        recurrent_bias = startai.array(recurrent_bias_orig)

        h_tm1 = cell_states[0]  # previous memory state
        c_tm1 = cell_states[1]  # previous carry state

        z = startai.dot(cell_inputs, kernel) + bias
        z += startai.dot(h_tm1, recurrent_kernel) + recurrent_bias

        z0, z1, z2, z3 = startai.split(z, num_or_size_splits=4, axis=-1)

        i = startai.sigmoid(z0)  # input
        f = startai.sigmoid(z1)  # forget
        c = f * c_tm1 + i * startai.tanh(z2)
        o = startai.sigmoid(z3)  # output
    ...
    with startai.utils.backend.ContextManager(backend_fw):
        _lstm_step_backend = outputs_to_frontend_arrays(
            inputs_to_startai_arrays(_lstm_step)
        )
        vals = [startai.array(val) for val in np_vals]
        if len(vals) > 3:
        ...
        startai_ret = startai.nested_map(lambda x: x.startai_array, ret, shallow=False)
        startai_idxs = startai.nested_argwhere(startai_ret, startai.is_startai_array)
        startai_vals = startai.multi_index_nest(startai_ret, startai_idxs)
        ret_np_flat = [x.to_numpy() for x in startai_vals]

    with startai.utils.backend.ContextManager(frontend):
        _lstm_step_gt = output_to_native_arrays(inputs_to_startai_arrays(_lstm_step))
        import tensorflow as tf

        vals = [startai.array(val).data for val in np_vals]
        if len(vals) > 3:
            inputs, init_h, init_c, mask = vals
        else:
            inputs, init_h, init_c = vals
        ...
        native_idxs = startai.nested_argwhere(ret, lambda x: isinstance(x, startai.NativeArray))
        native_vals = startai.multi_index_nest(ret, native_idxs)
