...
def _get_type_dict_helper(framework, kind, is_frontend_test):
    ...
    elif startai.current_backend_str() == framework:
        framework_module = startai
    else:
        with BackendHandler.update_backend(framework) as startai_backend:
            framework_module = startai_backend
...
def get_dtypes(
    ...
    # The function may be called from a frontend test or an Startai API test
    # In the case of an Startai API test, the function should make sure it returns a valid
...
def array_dtypes(
    ...
    *,
    ...
    if num_arrays == 1:
        ...
        unwanted_types = set(startai.all_dtypes).difference(set(available_dtypes))
        if array_api_dtypes:
            pairs = startai.array_api_promotion_table.keys()
        else:
            pairs = startai.promotion_table.keys()
...
def cast_filter_helper(d, dtype, x, current_backend):
    with BackendHandler.update_backend(current_backend) as startai_backend:

        def bound_dtype_bits(d):
            return (
                startai_backend.dtype_bits(d) / 2
                if startai_backend.is_complex_dtype(d)
                else startai_backend.dtype_bits(d)
            )

        if startai_backend.is_int_dtype(d):
            max_val = startai_backend.iinfo(d).max
            min_val = startai_backend.iinfo(d).min
        elif startai_backend.is_float_dtype(d) or startai_backend.is_complex_dtype(d):
            max_val = startai_backend.finfo(d).max
            min_val = startai_backend.finfo(d).min
        else:
            max_val = 1
            min_val = -1
        if x is None:
            if startai_backend.is_int_dtype(dtype):
                max_x = startai_backend.iinfo(dtype).max
                min_x = startai_backend.iinfo(dtype).min
            elif startai_backend.is_float_dtype(dtype) or startai_backend.is_complex_dtype(
                dtype
            ):
                max_x = startai_backend.finfo(dtype).max
                min_x = startai_backend.finfo(dtype).min
            else:
                max_x = 1
                min_x = -1
        else:
        ...
        return (
            ...
            and (
                startai_backend.is_complex_dtype(d)
                or not startai_backend.is_complex_dtype(dtype)
            )
            and (min_x > 0 or not startai_backend.is_uint_dtype(dtype))
        )
