...
def test_numpy_copyto(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        src_startai = startai_backend.functional.frontends.numpy.array(xs[0])
        dst_startai = startai_backend.functional.frontends.numpy.array(xs[1])
        startai_backend.functional.frontends.numpy.copyto(
            dst_startai, src_startai, where=where, casting=casting
        )
        ...
        assert dst_np.shape == dst_startai.shape
        # value test
        dst_ = startai_backend.to_numpy(dst_startai.startai_array)
        helpers.assert_all_close(
            dst_, dst_np, backend=backend_fw, ground_truth_backend=frontend
        )
        assert id(src_startai) != id(dst_startai)
...
def test_numpy_shape(
    *,
    ...
    # Manually compare the shape here because startai.shape doesn't return an array, so
    # startai.to_numpy will narrow the bit-width, resulting in different dtypes. This is
