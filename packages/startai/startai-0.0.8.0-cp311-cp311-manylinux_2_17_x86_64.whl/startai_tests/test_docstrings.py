...
def check_docstring_examples_run(
    *, fn, from_container=False, from_array=False, num_sig_fig=2
    ...
    from_container
        if True, check docstring of the function as a method of an Startai Container.
    from_array
        if True, check docstring of the function as a method of an Startai Array.
    num_sig_fig
    ...
    to_skip = [
        ...
        "multinomial",
        "num_cpu_cores",
        "get_all_startai_arrays_on_dev",
        "num_startai_arrays_on_dev",
        "total_mem_on_dev",
        "used_mem_on_dev",
    ...
    skip_list_temp = [
        ...
        "pool",  # Maximum recursion depth exceeded startai.pool
    ...
    if not hasattr(fn, "__name__"):
        return True
    fn_name = fn.__name__
    if fn_name not in startai.utils.backend.handler.startai_original_dict:
        return True

    if from_container:
        docstring = getattr(
            startai.utils.backend.handler.startai_original_dict["Container"], fn_name
        ).__doc__
    elif from_array:
        docstring = getattr(
            startai.utils.backend.handler.startai_original_dict["Array"], fn_name
        ).__doc__
    else:
        docstring = startai.utils.backend.handler.startai_original_dict[fn_name].__doc__
    ...
    with redirect_stdout(f):
        for line in executable_lines:
            ...
            try:
                ...
                print(e, " ", startai.current_backend_str(), " ", line)
    ...
    num_output = output.replace("startai.array", "").replace("startai.Shape", "")
    num_parsed_output = parsed_output.replace("startai.array", "").replace("startai.Shape", "")
...
def test_docstrings(backend):
    ...
    for k, v in startai.__dict__.copy().items():
        if k == "Array":
            for method_name in dir(v):
                method = getattr(startai.Array, method_name)
                if hasattr(startai.functional, method_name):
                    if helpers.gradient_incompatible_function(
                        fn=getattr(startai.functional, method_name)
            ...
            for method_name in dir(v):
                method = getattr(startai.Container, method_name)
                if hasattr(startai.functional, method_name):
                    if helpers.gradient_incompatible_function(
                        fn=getattr(startai.functional, method_name)
