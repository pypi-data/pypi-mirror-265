"""Collection of Startai's losses as stateful classes."""
...
class LogPoissonLoss(Module):
    ...
    def _forward(
        ...
        return startai.log_poisson_loss(
            true,
            pred,
            compute_full_loss=startai.default(compute_full_loss, self._compute_full_loss),
            axis=startai.default(axis, self._axis),
            reduction=startai.default(reduction, self._reduction),
        )
...
class CrossEntropyLoss(Module):
    ...
    def _forward(self, true, pred, *, axis=None, epsilon=None, reduction=None):
        ...
        return startai.cross_entropy(
            ...
            pred,
            axis=startai.default(axis, self._axis),
            epsilon=startai.default(epsilon, self._epsilon),
            reduction=startai.default(reduction, self._reduction),
        )
    ...
    def _forward(
        ...
        *,
        ...
        return startai.binary_cross_entropy(
            true,
            pred,
            from_logits=startai.default(from_logits, self._from_logits),
            epsilon=startai.default(epsilon, self._epsilon),
            reduction=startai.default(reduction, self._reduction),
