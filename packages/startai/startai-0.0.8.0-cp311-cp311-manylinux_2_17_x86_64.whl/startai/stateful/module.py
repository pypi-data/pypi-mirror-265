...
from startai.stateful.helpers import ModuleHelpers
from startai.stateful.converters import ModuleConverters
...
class Module(ModuleHelpers, ModuleConverters, ModuleMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize Startai layer, which is a stateful object consisting of
        trainable variables.
        ...
        v
            Startai container of trainable variables. Created internally by default.
        buffers
            Startai container of buffers/non-trainable arrays in the state_dict.
        build_mode
        ...
        startai.utils.assertions.check_elem_in_list(build_mode, valid_build_modes)
        ...
        self._buffers = Container(startai.default(buffers, {}))
        ...
        self._device = startai.default(device, startai.default_device())
        self._dtype = startai.default(dtype, startai.default_dtype())
    ...
    def build(
        ...
        **kwargs,
        ...
        """
        self._device = startai.default(device, self._device)
        self._dtype = startai.default(dtype, self._dtype)
        self._dynamic_backend = startai.default(dynamic_backend, self._dynamic_backend)
        ...
        # remain un-generated
        built = startai.default(self._build(*args, **kwargs), True)
        ...
        if startai.exists(self._v_from_constructor):
        ...
        if not built and from_call:
            ...
            for v in self.__dict__.values():
                if isinstance(v, startai.Module):
            ...
            if not startai.exists(self._v_from_constructor):
                created_n_found = Container(
                    dict(
    ...
    def trace_graph(
        ...
        **trace_kwargs,
        ...
        """Trace the `startai.Module`'s `_unified_startai_graph` or `_call` method to
        the target backend.
        ...
        args = startai.default(args, ())
        kwargs = startai.default(kwargs, {})

        # shallow copy the kwargs dict
        kwargs = copy.copy(kwargs)
        kwargs["v"] = self.v

        fn_to_trace = startai.default(self._module_graph, self._call)

        self._module_graph = startai.trace_graph(
    ...
    def train(self, mode: bool = True):
        ...
        for module in self.v:
            ...
            if isinstance(module, startai.Module):
    ...
    def to_device(self, device):
        ...
        for obj in self.state_dict.values():
            ...
            elif startai.is_array(obj) or startai.is_startai_container(obj):
                startai.to_device(obj, device, out=obj)
        return self
    ...
    def show_graph(
        ...
        if not startai.exists(self._module_graph):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self._lazy_traced:
            ...
            if self._target:
                startai.set_backend(self._target)
...
class _HaikuStartaiModule(Module):
    ...
    def _build(self, params_hk, *args, **kwargs):
        startai.set_jax_backend()
        args, kwargs = startai.args_to_native(*args, **kwargs)
        # noinspection PyUnresolvedReferences
        params_dict = self._hk_flat_map_to_dict(params_hk)
        self._hk_params = startai.Container(params_dict, dynamic_backend=False)
        param_iterator = self._hk_params.cont_to_iterator()
        _, param0 = next(param_iterator, ["_", 0])
        if hasattr(param0, "device"):
            self._device = startai.as_startai_dev(param0.device())
        else:
            self._device = startai.as_startai_dev("cpu")
        startai.previous_backend()

    def _forward(self, *a, **kw):
        a, kw = startai.args_to_native(*a, **kw)
        ...
        return startai.to_native(ret, nested=nested)
...
class _FlaxStartaiModule(Module):
    ...
    def _build(self, params_fx, *args, **kwargs):
        ...
        self._fx_params = startai.Container(params_dict, dynamic_backend=False)
        param_iterator = self._fx_params.cont_to_iterator()
        _, param0 = next(param_iterator, ["_", 0])
        self._device = startai.as_startai_dev(startai.dev(param0))
...
class _KerasStartaiModule(Module):
    def __init__(self, *args, native_module, device, devices, **kwargs):
        self._native_module = native_module
        self._args = args
        self._kwargs = kwargs

        startai.Module.__init__(self, *args, device=device, devices=devices, **kwargs)

    def _create_variables(self, device=None, dtype=None):
        return self._native_params

    def _build(self, *args, **kwargs):
        self._native_params = startai.Container(
            OrderedDict(
...
class _PaddleStartaiModule(Module):
    def __init__(self, *args, native_module, device, devices, **kwargs):
        self._native_module = native_module
        self._args = args
        self._kwargs = kwargs
...
class _TorchStartaiModule(Module):
    ...
    def _inplace_update_v(self, new_v):
        startai.Container.cont_multi_map(
    ...
    def _replace_update_v(self, new_v, native=None):
        ...
        native = startai.default(native, self._native_module)
        for k, v in new_v.items():
            if isinstance(v, startai.Container):
                ...
                raise startai.utils.exceptions.StartaiException(
                    ...
                    " startai.Container nor a variable."
                )
        return native
