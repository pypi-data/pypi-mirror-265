...
EPSILON = startai.finfo(startai.double).eps
INFINITY = startai.inf
INTPTR_MAX = startai.iinfo(startai.int32).max
TREE_LEAF = -1
TREE_UNDEFINED = -2
...
class Tree:
    def __init__(self, n_features, n_classes, n_outputs):
        ...
        self.n_classes = startai.zeros(n_outputs, dtype=startai.int32)

        self.max_n_classes = startai.max(n_classes)
    ...
    def predict(self, X):
        ...
        out = startai.take(self.value, X_applied, axis=0)
        if self.n_outputs == 1:
