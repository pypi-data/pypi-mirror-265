import startai
import startai.functional.frontends.pandas.series as series
...
class Index:
    def __init__(self, data, dtype=None, copy=False, name=None, tupleize_cols=True):
        self.index = data
        self.tokens = None
        if not isinstance(data, startai.Array):
            try:
                self.index_array = startai.array(data, dtype=dtype)
            except startai.utils.exceptions.StartaiBackendException:
                # labels as strings
    ...
    def _tokenize_1d(x: Iterable):
        return startai.array([v for v, _ in enumerate(x)])
    ...
    def isin(self, values, level=None):
        # todo handle level with mutliindexer
        return startai.isin(self.index_array, values)
