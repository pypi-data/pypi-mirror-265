...
def _from_startai_array_to_jax_frontend_array(x, nested=False, include_derived=None):
    if nested:
        return startai.nested_map(
            _from_startai_array_to_jax_frontend_array, x, include_derived, shallow=False
        )
    elif isinstance(x, startai.Array):
        return jax_frontend.Array(x)
    return x


def _from_startai_array_to_jax_frontend_array_weak_type(
    x, nested=False, include_derived=None
):
    if nested:
        return startai.nested_map(
            _from_startai_array_to_jax_frontend_array_weak_type,
            x,
            include_derived,
            shallow=False,
        )
...
def _from_jax_frontend_array_to_startai_array(x):
    if isinstance(x, jax_frontend.Array) and x.weak_type and x.startai_array.shape == ():
        setattr(x.startai_array, "weak_type", True)
...
def _to_startai_array(x):
    return _from_jax_frontend_array_to_startai_array(_native_to_startai_array(x))
...
def inputs_to_startai_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _inputs_to_startai_arrays_jax(*args, **kwargs):
    ...
    _inputs_to_startai_arrays_jax.inputs_to_startai_arrays_jax = True
    return _inputs_to_startai_arrays_jax
...
def outputs_to_frontend_arrays(fn: Callable) -> Callable:
    ...
    def _outputs_to_frontend_arrays_jax(*args, **kwargs):
        weak_type = not any(
            ...
            or (isinstance(arg, startai.Array) and arg.weak_type is False)
        ...
        if weak_type:
            return _from_startai_array_to_jax_frontend_array_weak_type(
                ret,
                nested=True,
                include_derived={"tuple": True},
            )
        return _from_startai_array_to_jax_frontend_array(
