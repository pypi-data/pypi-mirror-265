...
class Array:
    ...
    def __repr__(self):
        main = (
            str(self.startai_array.__repr__())
            .replace("startai.array", "startai.frontends.jax.Array")
            .replace(")", "")
            + ", dtype="
    ...
    @property
    def at(self):
        return jax_frontend._src.numpy.lax_numpy._IndexUpdateHelper(self.startai_array)

    @property
    def T(self):
        return self.startai_array.T
    ...
    def copy(self, order=None):
        return jax_frontend.numpy.copy(self._startai_array, order=order)

    def diagonal(self, offset=0, axis1=0, axis2=1):
        return jax_frontend.numpy.diagonal(
            self._startai_array, offset=offset, axis1=axis1, axis2=axis2
        )

    def all(self, *, axis=None, out=None, keepdims=False):
        return jax_frontend.numpy.all(
            self._startai_array, axis=axis, keepdims=keepdims, out=out
        )
    ...
    def conj(self, /):
        return jax_frontend.numpy.conj(self._startai_array)

    def conjugate(self, /):
        return jax_frontend.numpy.conjugate(self._startai_array)
    ...
    def any(self, *, axis=None, out=None, keepdims=False, where=None):
        return jax_frontend.numpy.any(
            self._startai_array, axis=axis, keepdims=keepdims, out=out, where=where
        )
    ...
    def __setitem__(self, idx, val):
        raise startai.utils.exceptions.StartaiException(
            "startai.functional.frontends.jax.Array object doesn't support assignment"
        )
