...
# local
import startai
import startai.functional.frontends.tensorflow as frontend
...
def _startai_array_to_tensorflow(x):
...
def _tf_frontend_array_to_startai(x):
...
def _to_startai_array(x):
    return _tf_frontend_array_to_startai(_native_to_startai_array(x))
...
def inputs_to_startai_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _inputs_to_startai_arrays_tf(*args, **kwargs):
        """Convert all `TensorFlow.Tensor` instances in both the positional and
        keyword arguments into `startai.Array` instances, and then call the
        ...
        startai_args = startai.nested_map(
            _to_startai_array, args, include_derived=True, shallow=False
        )
        startai_kwargs = startai.nested_map(
            _to_startai_array, kwargs, include_derived=True, shallow=False
        )
        if has_out:
            startai_kwargs["out"] = out
        return fn(*startai_args, **startai_kwargs)

    _inputs_to_startai_arrays_tf.inputs_to_startai_arrays_tf = True
    return _inputs_to_startai_arrays_tf
...
def outputs_to_frontend_arrays(fn: Callable) -> Callable:
    ...
    def _outputs_to_frontend_arrays_tf(*args, **kwargs):
        ...
        instances in the function return into `startai.Array` instances.
        ...
        Returns
        -------
            The return of the function, with startai arrays as tensorflow.Tensor arrays.
        """
        ...
        return startai.nested_map(
            _startai_array_to_tensorflow, ret, include_derived={"tuple": True}
        )
...
def to_startai_dtype(dtype):
    if not dtype or isinstance(dtype, str):
        return dtype
    if isinstance(dtype, np_frontend.dtype):
        return dtype.startai_dtype
    return frontend.as_dtype(dtype).startai_dtype
