...
import startai.functional.frontends.tensorflow.nn as tf_nn
...
def fused_batch_norm(
    ...
    if data_format[1] == "C":
        if dims == 4:
            ...
            x = startai.permute_dims(x, axes=(0, 2, 3, 4, 1))
        else:
            raise startai.utils.exceptions.StartaiException(
                f"input tensor must be of 4 or 5 dimensions, got {dims}"
            )

    scale = scale.astype(startai.float32)
    offset = offset.astype(startai.float32)
    old_mean = mean.astype(startai.float32)
    old_var = variance.astype(startai.float32)
    x = x.astype(startai.float32)

    if is_training:
        depth = x.shape[-1]
        rest_size = startai.prod(x.shape) // depth
        x_rest_by_depth = startai.reshape(x, [rest_size, depth])
        mean = startai.mean(x_rest_by_depth, axis=0, keepdims=True)
        variance = startai.var(x_rest_by_depth, axis=0, keepdims=True)
        y = startai.reshape(
            scale * (x_rest_by_depth - mean) / startai.sqrt(variance + epsilon) + offset,
            x.shape,
        )
        float_rest_size = startai.astype(rest_size, x.dtype)
        variance = (
            variance * float_rest_size / (float_rest_size - 1)
            if rest_size > 1
            else variance
        )
        mean = startai.reshape(
            mean * exponential_avg_factor + old_mean * (1 - exponential_avg_factor),
            old_mean.shape,
        )
        variance = startai.reshape(
            variance * exponential_avg_factor + old_var * (1 - exponential_avg_factor),
            old_var.shape,
        )
    else:
        y = scale * (x - old_mean) / startai.sqrt(old_var + epsilon) + offset

    # permute dimensions back
    if data_format[1] == "C":
        if dims == 4:
            y = startai.permute_dims(y, axes=(0, 3, 1, 2))
        elif dims == 5:
            y = startai.permute_dims(y, axes=(0, 4, 1, 2, 3))
