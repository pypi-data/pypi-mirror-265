...
from startai.functional.frontends.torch import promote_types_of_torch_inputs
...
def atleast_1d(*tensors):
    return startai.atleast_1d(*tensors)


@to_startai_arrays_and_back
def atleast_2d(*tensors):
    return startai.atleast_2d(*tensors)


@to_startai_arrays_and_back
def atleast_3d(*tensors):
    return startai.atleast_3d(*tensors)


# TODO: Add Startai function for block_diag but only scipy.linalg and \
# and torch supports block_diag currently
@to_startai_arrays_and_back
def block_diag(*tensors):
    shapes_list = [startai.shape(t) for t in tensors]
    # TODO: Add startai function to return promoted dtype for multiple tensors at once
    promoted_dtype = startai.as_startai_dtype(tensors[0].dtype)
    for idx in range(1, len(tensors)):
        promoted_dtype = torch_frontend.promote_types_torch(
            tensors[idx - 1].dtype, tensors[idx].dtype
        )

    inp_tensors = [startai.asarray(t, dtype=promoted_dtype) for t in tensors]
    tensors_2d = []
    ...
    for idx, t_shape in enumerate(shapes_list):
        ...
        if len(t_shape) > 2:
            ...
            dim_1 = t_shape[0]
            tensors_2d.append(startai.reshape(inp_tensors[idx], shape=(dim_0, dim_1)))
        else:
            tensors_2d.append(startai.reshape(inp_tensors[idx], shape=(dim_0, dim_1)))
    ...
    ret = startai.zeros((result_dim_0, result_dim_1), dtype=promoted_dtype)
    ret_dim_0 = 0
    ret_dim_1 = 0
    for idx, t_shape in enumerate(shapes_list):
        dim_0, dim_1 = t_shape
        ...
        ] = startai.copy_array(tensors_2d[idx])
        ret_dim_0 += dim_0
        ret_dim_1 += dim_1
...
def broadcast_to(tensor, shape):
    return startai.broadcast_to(tensor, shape)
...
def cartesian_prod(*tensors):
    ...
    ret = startai.meshgrid(*tensors, indexing="ij")
    ret = startai.stack(ret, axis=-1)
    ret = startai.reshape(ret, shape=(-1, len(tensors)))
...
def cdist(x1, x2, p=2.0, compute_mode="use_mm_for_euclid_dist_if_necessary"):
    if len(x1.shape) == 2 and len(x2.shape) == 2:
        ...
        if (
            ...
            return startai.vector_norm(x1[:, None, :] - x2[None, :, :], axis=-1, ord=p)
        else:
            distances = startai.zeros((x1_first_dim, x2_first_dim), dtype=x1.dtype)
            for i in range(x1_first_dim):
                for j in range(x2_first_dim):
                    distances[i, j] = startai.vector_norm(x1[i, :] - x2[j, :], ord=p)
            return distances
    if p == 2:
        B, P, M = x1.shape
        _, R, _ = x2.shape
        if (
            ...
            distances = startai.zeros((B, P, R), dtype=x1.dtype)
            for b in range(B):
                for i in range(P):
                    for j in range(R):
                        distances[b, i, j] = startai.vector_norm(
                            x1[b, i, :] - x2[b, j, :], ord=p
                        )
            return distances
    else:
...
@to_startai_arrays_and_back
def corrcoef(input):
    if len(startai.shape(input)) > 2:
        raise startai.exceptions.StartaiError(
            "corrcoef(): expected input to have two or fewer dimensions but got an"
            f" input with {startai.shape(input)} dimensions"
        )
    return startai.corrcoef(input, y=None, rowvar=True)


@to_startai_arrays_and_back
def cov(input, /, *, correction=1, fweights=None, aweights=None):
    return startai.cov(input, ddof=correction, fweights=fweights, aweights=aweights)
...
def cross(input, other, dim=None, *, out=None):
    ...
    return startai.cross(input, other, axisa=-1, axisb=-1, axisc=-1, axis=dim, out=out)
...
def cummax(input, dim, *, out=None):
    ...
    result_values, result_indices = startai.cummax(input, axis=dim, out=out)
...
def cumprod(input, dim, *, dtype=None, out=None):
    if not dtype and "int" in input.dtype:
        dtype = startai.int64
    return startai.cumprod(input, axis=dim, dtype=dtype, out=out)
...
def cumsum(input, dim, *, dtype=None, out=None):
    if not dtype and "int" in input.dtype:
        dtype = startai.int64
    return startai.cumsum(input, axis=dim, dtype=dtype, out=out)


@to_startai_arrays_and_back
def diag(input, diagonal=0, *, out=None):
    return startai.diag(input, k=diagonal)
...
def diag_embed(
    ...
    input_type = startai.dtype(input)
    rank = input.ndim + 1
    ...
    if offset != 0:
        ...
        z = startai.zeros(t_shape, dtype=input.dtype, device=input.device)
        pair = (z, input) if offset > 0 else (input, z)
        input = startai.concat(pair, axis=-1)
        last_dim += abs(offset)
    input = input.expand_dims(axis=dim1).moveaxis(-1, dim2)
    # generate ranges shifting indices based on offset
    a_range = startai.arange(last_dim, device=input.device, dtype=startai.int64)
    b_range = startai.arange(
        offset, last_dim + offset, device=input.device, dtype=startai.int64
    )
    # broadcast
    ...
    if input.dtype == startai.bool:
        ret = cond.logical_and(input)
    else:
        ret = startai.where(cond, input, 0)
...
def diagonal(input, offset=0, dim1=0, dim2=1):
    return startai.diagonal(input, offset=offset, axis1=dim1, axis2=dim2)
...
def diff(input, n=1, dim=-1, prepend=None, append=None):
    return startai.diff(input, n=n, axis=dim, prepend=prepend, append=append, out=None)
...
def einsum(equation, *operands):
    if len(operands) == 1 and isinstance(operands[0], (list, tuple)):
        operands = operands[0]
    return startai.einsum(equation, *operands)
...
def flatten(input, start_dim=0, end_dim=-1):
    return startai.flatten(input, start_dim=start_dim, end_dim=end_dim)


@to_startai_arrays_and_back
def flip(input, dims):
    return startai.flip(input, axis=dims, copy=True)
...
def fliplr(input):
    ...
    return startai.fliplr(input, copy=True)
...
def flipud(input):
    ...
    return startai.flipud(input, copy=True)


@to_startai_arrays_and_back
def gcd(input, other, *, out=None):
    return startai.gcd(input, other, out=out)


@to_startai_arrays_and_back
def kron(input, other, *, out=None):
    return startai.kron(input, other, out=out)


@to_startai_arrays_and_back
@with_unsupported_dtypes({"2.2 and below": ("int8",)}, "torch")
def lcm(input, other, *, out=None):
    return startai.lcm(input, other, out=out)
...
def logcumsumexp(input, dim, *, out=None):
    if len(input.shape) == 0:
        ...
        exp_input = startai.exp(input.astype("float64"))
        summed_exp_input = startai.cumsum(exp_input, axis=dim)
        ret = startai.log(summed_exp_input).astype(original_dtype)
...
def meshgrid(*tensors, indexing=None):
    ...
    if len(tensors) == 1 and isinstance(tensors[0], (list, tuple)):
        tensors = tensors[0]
    return tuple(startai.meshgrid(*tensors, indexing=indexing))


@to_startai_arrays_and_back
def ravel(input):
    return startai.reshape(input, (-1,))
...
def renorm(input, p, dim, maxnorm, *, out=None):
    ...
    input_swapped = startai.swapaxes(input, 0, dim)
    ...
    for individual_tensor in individual_tensors:
        ...
        tensor_flattened = startai.flatten(individual_tensor)

        # Don't scale up to the maximum norm, only scale down to it.
        norm = startai.vector_norm(tensor_flattened, axis=0, ord=p)
        multiplier = startai.minimum(maxnorm / (norm + epsilon), startai.ones_like(norm))

        # Store the result in its original shape
        ret.append(
            startai.reshape(startai.multiply(tensor_flattened, multiplier), original_shape)
        )

    # We must undo our axis swap from the start.
    ret = startai.asarray(ret, dtype=ret[0].dtype)
    ret = startai.swapaxes(ret, 0, dim)
    ret = startai.reshape(ret, input.shape)
...
def repeat_interleave(input, repeats, dim=None, *, output_size=None):
    return startai.repeat(input, repeats, axis=dim)


@to_startai_arrays_and_back
def roll(input, shifts, dims=None):
    return startai.roll(input, shifts, axis=dims)


@to_startai_arrays_and_back
def rot90(input, k, dims):
    total_dims = startai.get_num_dims(input)
    ...
    if k == 1:
        flipped = startai.flip(input, axis=dims[1])
        return startai.permute_dims(flipped, axes=new_axes, copy=True)
    elif k == 2:
        return startai.flip(input, axis=dims, copy=True)
    elif k == 3:
        flipped = startai.flip(input, axis=dims[0])
        return startai.permute_dims(flipped, axes=new_axes, copy=True)
    else:
        return input
...
def searchsorted(
    ...
    *,
    ...
    ret = startai.searchsorted(sorted_sequence, values, side=side, out=out, sorter=sorter)
    if out_int32:
...
def tensordot(a, b, dims=2, out=None):
    ...
    return startai.tensordot(a, b, axes=dims, out=out)
...
def trace(input):
    ...
    return startai.astype(startai.trace(input), target_type)


@with_supported_dtypes({"2.5.0 and below": ("int8", "int16", "bfloat16")}, "paddle")
@to_startai_arrays_and_back
def tril(input, diagonal=0, *, out=None):
    return startai.tril(input, k=diagonal, out=out)


@with_unsupported_dtypes({"2.2 and below": ("int8", "uint8", "int16")}, "torch")
@to_startai_arrays_and_back
def tril_indices(row, col, offset=0, *, dtype=startai.int64, device="cpu", layout=None):
    sample_matrix = startai.tril(startai.ones((row, col), device=device), k=offset)
    return startai.stack(startai.nonzero(sample_matrix)).astype(dtype)
...
def triu(input, diagonal=0, *, out=None):
    return startai.triu(input, k=diagonal, out=out)


@to_startai_arrays_and_back
def triu_indices(row, col, offset=0, dtype="int64", device="cpu", layout=None):
    # TODO: Handle layout flag when possible.
    sample_matrix = startai.triu(startai.ones((row, col), device=device), k=offset)
    return startai.stack(startai.nonzero(sample_matrix)).astype(dtype)


@to_startai_arrays_and_back
def unflatten(input, dim, sizes):
    return startai.unflatten(input, dim=dim, shape=sizes, out=None)


@to_startai_arrays_and_back
def vander(x, N=None, increasing=False):
    # if N == 0:
    #     return startai.array([], dtype=x.dtype)
    # else:
    return startai.vander(x, N=N, increasing=increasing, out=None)


@to_startai_arrays_and_back
@with_supported_dtypes({"2.2 and below": ("float32", "float64")}, "torch")
def view_as_complex(input):
    if startai.shape(input)[-1] != 2:
        raise startai.exceptions.StartaiError("The last dimension must have a size of 2")

    real, imaginary = startai.split(
        startai.stop_gradient(input, preserve_type=False),
        num_or_size_splits=2,
        axis=startai.get_num_dims(input) - 1,
    )
    dtype = startai.complex64 if input.dtype == startai.float32 else startai.complex128
    real = startai.squeeze(real, axis=startai.get_num_dims(real) - 1).astype(dtype)
    imag = startai.squeeze(imaginary, axis=startai.get_num_dims(imaginary) - 1).astype(dtype)
    complex_ = real + imag * 1j
    return startai.array(complex_, dtype=dtype)
...
def view_as_real(input):
    if not startai.is_complex_dtype(input):
        raise startai.exceptions.StartaiError(
            "view_as_real is only supported for complex tensors"
        )
    re_part = startai.real(input)
    im_part = startai.imag(input)
    return startai.stack((re_part, im_part), axis=-1)
