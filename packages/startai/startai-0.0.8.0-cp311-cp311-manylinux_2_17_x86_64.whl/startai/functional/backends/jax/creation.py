...
# local
import startai
from startai import as_native_dtype
from startai.functional.backends.jax import JaxArray
from startai.functional.backends.jax.device import dev
...
def arange(
    ...
    *,
    ...
    if dtype:
        ...
        startai.utils.assertions._check_jax_x64_flag(dtype.name)
...
def full(
    ...
    *,
    dtype: Optional[Union[startai.Dtype, jnp.dtype]] = None,
    device: jaxlib.xla_extension.Device = None,
    out: Optional[JaxArray] = None,
) -> JaxArray:
    dtype = startai.default_dtype(dtype=dtype, item=fill_value, as_native=True)
...
def linspace(
    ...
    *,
    ...
    if dtype is None:
        dtype = startai.promote_types(start.dtype, stop.dtype)
...
def copy_array(
    x: JaxArray, *, to_startai_array: bool = True, out: Optional[JaxArray] = None
) -> JaxArray:
    x = (
