"""Collection of Jax device functions, wrapped to fit Startai syntax and
signature."""
...
def dev(
    ...
    *,
...
) -> Union[startai.Device, jaxlib.xla_extension.Device]:
    ...
    if hasattr(x, "device_buffer"):
        ...
        dv = jax.devices()[0]
    return dv if as_native else as_startai_dev(dv)
...
# since if we use to_device, it will return startai.array which is not desirable
...
def as_startai_dev(device, /):
    ...
    if p == "cpu":
        return startai.Device(p)
    return startai.Device(p + ":" + str(dev_id))


def as_native_dev(device, /):
    if not isinstance(device, str):
        return device
    dev_split = startai.Device(device).split(":")
    device = dev_split[0]
    if len(dev_split) > 1:
        idx = int(dev_split[1])
    else:
