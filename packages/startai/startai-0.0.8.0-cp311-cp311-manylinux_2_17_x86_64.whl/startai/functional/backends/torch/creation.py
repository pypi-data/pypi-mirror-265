...
def arange(
    ...
    step: float = 1,
    *,
    dtype: Optional[Union[startai.Dtype, torch.dtype]] = None,
    ...
    if dtype is None:
        ...
        dtype = startai.as_native_dtype(startai.default_dtype(dtype=dtype))
...
def asarray(
    ...
    *,
    ...
    if isinstance(obj, Sequence) and len(obj) != 0:
        contain_tensor = startai.nested_any(obj, lambda x: isinstance(x, torch.Tensor))
...
def copy_array(
    ...
    *,
    ...
    if to_startai_array:
        return startai.to_startai(x.clone())
    return x.clone()
