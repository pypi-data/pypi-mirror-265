...
def cummax(
    ...
    *,
    ...
    if not (exclusive or reverse):
        ...
        x, indices = __find_cummax(startai.flip(x, axis=(axis,)), axis=axis)
        x, indices = startai.swapaxes(x, axis, -1), startai.swapaxes(indices, axis, -1)
        x = startai.concat((startai.zeros_like(x[..., -1:]), x[..., :-1]), axis=-1)
        indices = startai.concat(
            (startai.zeros_like(indices[..., -1:]), indices[..., :-1]), axis=-1
        )
        x, indices = startai.swapaxes(x, axis, -1), startai.swapaxes(indices, axis, -1)
        return startai.flip(x, axis=(axis,)), startai.flip(indices, axis=(axis,))
...
def __find_cummax(
    ...
    if (
        ...
        and len(x[0].shape) >= 1
        and (isinstance(x[0], (paddle.Tensor, startai.Array)))
    ):
        if axis >= 1:
    ...
    if isinstance(x, paddle.Tensor):
        ...
        return startai.array(values, dtype=x_dtype), startai.array(indices, dtype="int64")
