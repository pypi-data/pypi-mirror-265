...
from ...startai.manipulation import _calculate_out_shape
    ...
    *,
    ...
    if len(dtypes_list) > 0:
        for d in dtypes_list:
            dtype = startai.promote_types(dtype, d)
    ...
    if all(0 in x.shape for x in xs):
        ...
        if any(len(s) != len(shapes[0]) for s in shapes):
            raise startai.exceptions.StartaiValueError(
...
def constant_pad(
    ...
    *,
    ...
    for item in pad_width:
        if len(item) != 2:
            raise startai.utils.exceptions.StartaiException("Length of each item should be 2")
        else:
