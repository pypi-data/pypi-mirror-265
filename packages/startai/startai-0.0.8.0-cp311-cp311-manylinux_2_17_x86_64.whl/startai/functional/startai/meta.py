# global
import startai
from startai.func_wrapper import handle_array_function
from startai.functional.startai.gradients import gradient_descent_update
...
def _compute_cost_and_update_grads(
    ...
    if order == 1:
        ...
        cost, inner_grads = startai.execute_with_gradients(
        ...
        inner_grads = startai.Container(
            {
                k: startai.zeros_like(v) if k not in inner_grads else inner_grads[k]
                for k, v in var.cont_to_iterator()
            }
        )

        if batched:
            inner_grads = startai.multiply(inner_grads, num_tasks)
...
def _train_task(
    ...
    for i in range(inner_grad_steps):
        ...
        cost, inner_update_grads = startai.execute_with_gradients(
        ...
        inner_update_grads = startai.Container(
            {
                k: (
                    startai.zeros_like(v)
        ...
        if batched:
            inner_update_grads = startai.multiply(inner_update_grads, num_tasks)
...
def _train_tasks_batched(
    ...
    grads = grads.mean(axis=0) if isinstance(grads, startai.Container) else grads
    if order == 1:
    ...
    if order == 1:
        if return_inner_v:
            return (
                ...
                startai.concat(updated_ivs_to_return, axis=0),
            )
        return total_cost / num_tasks, sum(all_grads) / num_tasks
    if return_inner_v:
        return total_cost / num_tasks, startai.concat(updated_ivs_to_return, axis=0)
...
def fomaml_step(
    ...
    *,
    ...
    batched: bool = True,
    inner_v: Optional[startai.Container] = None,
    keep_inner_v: bool = True,
    outer_v: Optional[startai.Container] = None,
    keep_outer_v: bool = True,
    return_inner_v: Union[str, bool] = False,
    num_tasks: Optional[int] = None,
    stop_gradients: bool = True,
) -> Tuple[startai.Array, startai.Container, Any]:
    ...
    inner_optimization_step
        ...
        Default is startai.gradient_descent_update.
    inner_batch_fn
    ...
    cost = rets[0]
    if stop_gradients:
        cost = startai.stop_gradient(cost, preserve_type=False)
    grads = rets[1]
    if return_inner_v:
...
def reptile_step(
    ...
    *,
    ...
    >>> num_tasks = 2
    >>> batch = startai.Container({"x": startai.arange(1, num_tasks + 1, dtype="float32")})
    >>> variables = startai.Container({
    ...     "latent": _variable(startai.repeat(startai.array([[1.0]]), num_tasks, axis=0))
    ... })

    >>> cost, gradients = startai.reptile_step(batch, inner_cost_fn, variables, 5, 0.01,
    ...                                    num_tasks=num_tasks)
    >>> print(cost)
    startai.array(1.4485182)
    >>> print(gradients)
    {
        latent: startai.array([-139.9569855])
    }

    >>> batch = startai.Container({"x": startai.arange(1, 4, dtype="float32")})
    >>> variables = startai.Container({
    ...     "latent": _variable(startai.array([1.0, 2.0]))
    ... })

    >>> cost, gradients, firsts = startai.reptile_step(batch, inner_cost_fn, variables, 4,
    ...                                            0.025, batched=False, num_tasks=2,
    ...                                            return_inner_v='first')
    >>> print(cost)
    startai.array(0.9880483)
    >>> print(gradients)
    {
        latent: startai.array([-13.01766968, -13.01766968])
    }
    >>> print(firsts)
    {
        latent: startai.array([[1.02197957, 2.02197981]])
    }
    """
    if num_tasks is None:
        num_tasks = batch.cont_shape[0]
...
def maml_step(
    ...
    *,
    ...
    >>> cost = startai.maml_step(batch, inner_cost_fn, outer_cost_fn, variables, 5, 0.01)
    >>> print(cost)
    (startai.array(1.40069818), {
    latent: startai.array([-1.13723135])
    }, ())
    """
    if num_tasks is None:
        num_tasks = batch.cont_shape[0]
    unique_outer = outer_v is not None
    func_ret, grads = startai.execute_with_gradients(
