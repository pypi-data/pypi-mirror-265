...
def _conv(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    if bias is not None:
        return startai.add(ret, startai.expand_dims(bias, axis=(0, *range(2, dims + 2))))
    return ret
...
def adaptive_avg_pool2d(input, output_size):
    return startai.adaptive_avg_pool2d(input, output_size, data_format="NCHW")
...
def avg_pool2d(
    ...
    if all(pad == startai.ceil((kernel - 1) / 2) for kernel, pad in kernel_pads):
        padding_str = "SAME"
    else:
        padding_str = "VALID"
...
def dropout(input, p=0.5, training=True, seed=None):
    return startai.dropout(input, p, training=training, seed=seed)
...
def dropout2d(input, p=0.5, training=True):
    return startai.dropout2d(input, p, training=training, data_format="NCHW")
...
def dropout3d(input, p=0.5, training=True):
    return startai.dropout3d(input, p, training=training, data_format="NCDHW")
...
def fast_gelu(input_x):
    return (input_x / (1 + startai.exp(-1.702 * startai.abs(input_x)))) * startai.exp(
        0.851 * (input_x - startai.abs(input_x))
    )


@to_startai_arrays_and_back
def flatten(input, order="C", *, start_dim=1, end_dim=-1):
    return startai.flatten(input, order=order, start_dim=start_dim, end_dim=end_dim)
...
def kl_div(logits, labels, reduction="mean"):
    ...
    """
    assert startai.shape(logits) == startai.shape(
        labels
    ), "logits and labels must have the same shape."
    L = labels * (startai.log(labels) - logits)
    if reduction == "none":
        return L
    elif reduction == "mean":
        return startai.mean(L)
    elif reduction == "batchmean":
        return startai.mean(L, axis=0)
    elif reduction == "sum":
        return startai.sum(L)
    else:
        raise ValueError(
...
def log_softmax(input, axis=-1):
    return startai.log_softmax(input)
...
def pad(input, pad_width, mode="constant", constant_values=0):
    return startai.pad(input, pad_width, mode=mode, constant_values=constant_values)


@with_supported_dtypes({"2.0.0 and below": ("float16", "float32")}, "mindspore")
@to_startai_arrays_and_back
def selu(input_x):
    return startai.selu(input_x)
