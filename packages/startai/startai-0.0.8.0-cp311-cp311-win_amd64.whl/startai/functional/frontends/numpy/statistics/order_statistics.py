...
def _cpercentile(N, percent, key=lambda x: x):
    ...
    N.sort()
    k = (len(N) - 1) * percent
    f = startai.math.floor(k)
    c = startai.math.ceil(k)
    if f == c:
        return key(N[int(k)])
    d0 = key(N[int(f)]) * (c - k)
    d1 = key(N[int(c)]) * (k - f)
...
def nanpercentile(
    ...
    *,
    ...
    if axis is None:
        ...
        try:
            a = startai.swapaxes(a, 0, 1)
        except startai.utils.exceptions.StartaiError:
            startai.logging.warning("axis is 0 but couldn't swap")
