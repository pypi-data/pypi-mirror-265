...
class Splitter:
    ...
    def init(
        ...
        *args,
        ...
        n_samples = X.shape[0]
        self.samples = startai.empty(n_samples, dtype=startai.int32)
        ...
        self.features = startai.arange(n_features, dtype=startai.int32)
        self.n_features = n_features
        self.feature_values = startai.empty(n_samples, dtype=startai.float32)
        self.constant_features = startai.empty(n_features, dtype=startai.int32)
        self.y = y
...
class DensePartitioner:
    ...
    def sort_samples_and_feature_values(self, current_feature):
        ...
        if (
            ...
            while i <= current_end:
                if startai.isnan(X[samples[current_end], current_feature]):
                    n_missing += 1
                    current_end -= 1
                    continue
                if startai.isnan(X[samples[i], current_feature]):
    ...
    def partition_samples_final(
        ...
        if best_n_missing != 0:
            while p < partition_end:
                if startai.isnan(X[samples[end], best_feature]):
                    end -= 1
                    continue
                current_value = X[samples[p], best_feature]
                if startai.isnan(current_value):
...
def _init_split(split_record, start_pos):
    split_record.impurity_left = startai.inf
    split_record.impurity_right = startai.inf
    split_record.pos = start_pos
    split_record.feature = 0
    split_record.threshold = 0.0
    split_record.improvement = -startai.inf
...
def node_split_best(
    ...
    best_proxy_improvement = -startai.inf
    ...
    while f_i > n_total_constants and (
        ...
        f_j = startai.randint(n_drawn_constants, f_i - n_found_constants)
        ...
        if has_missing:
            ...
            if not ((n_left < min_samples_leaf) or (n_right < min_samples_leaf)):
                ...
                if not (
                    ...
                    if current_proxy_improvement > best_proxy_improvement:
                        best_proxy_improvement = current_proxy_improvement
                        current_split.threshold = startai.inf
                        current_split.missing_go_to_left = missing_go_to_left
                        current_split.n_missing = n_missing
                        current_split.pos = p
...
def sort(feature_values, samples, n):
    if n == 0:
        return
    idx = startai.argsort(feature_values)
