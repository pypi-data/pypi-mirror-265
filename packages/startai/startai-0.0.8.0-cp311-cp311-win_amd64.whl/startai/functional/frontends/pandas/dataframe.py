...
class DataFrame(NDFrame):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.orig_data, dict):
            ...
            self.array = startai.array([array_data for _ in range(len(self.index))])
        elif isinstance(self.orig_data, Series):
            self.array = self.array.expand_dims()
            self.columns = [0]
        elif columns is None:
            self.columns = startai.arange(self.array.shape[1]).tolist()
        else:
    ...
    def sum(self, axis=None, skipna=True, level=None, numeric_only=None, min_count=0):
        ...
        if axis is None or axis == "index":
            ...
            axis = 1
        if min_count > 0:
            if startai.has_nans(_array):
                number_values = _array.size - startai.sum(startai.isnan(_array))
            else:
                number_values = _array.size
            if min_count > number_values:
                return startai.nan
        if skipna:
            ret = startai.nansum(_array, axis=axis)
        else:
    ...
    def mean(self, axis=0, skipna=True, numeric_only=None, **kwargs):
        _array = startai.astype(self.array, startai.default_float_dtype())
        axis = 0 if axis == "index" else 1 if axis == "columns" else axis
        if skipna:
            ret = startai.nanmean(_array, axis=axis)
        else:
