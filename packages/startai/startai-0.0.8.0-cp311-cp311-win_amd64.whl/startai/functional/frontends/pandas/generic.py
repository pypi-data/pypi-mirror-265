...
from startai.functional.frontends.pandas.func_wrapper import outputs_to_self_class
import startai.functional.frontends.pandas.series as series
from startai.functional.frontends.pandas.index import Index
...
class NDFrame:
    def __init__(self, data, index, columns, dtype, name, copy, *args, **kwargs):
        ...
        if startai.is_native_array(data):
            self.array = startai.array(data)

        # repeatedly used checks
        data_is_array = isinstance(data, (startai.Array, np.ndarray))
        ...
        if index is None:
            if data_is_array_or_like:
                index = startai.arange(orig_data_len)
        ...
        if data_is_array_or_like:
            ...
            self.index = index
            self.array = startai.array(list(data.values()))
