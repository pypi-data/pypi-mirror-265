...
def finfo(type: Union[paddle.dtype, str, paddle.Tensor], /) -> Finfo:
    ...
    if startai.as_native_dtype(type) == paddle.bfloat16:
...
def result_type(*arrays_and_dtypes: Union[paddle.Tensor, paddle.dtype]) -> startai.Dtype:
    return startai.promote_types_of_inputs(*arrays_and_dtypes)[0].dtype
...
def as_startai_dtype(dtype_in: Union[paddle.dtype, str, bool, int, float], /) -> startai.Dtype:
    if dtype_in is int:
    ...
    return startai.Dtype(startai_dtype_dict[dtype_in])
...
def dtype(x: paddle.Tensor, *, as_native: bool = False) -> startai.Dtype:
    if as_native:
