...
# use numpy implementation with startai functions
    ...
    *,
    ...
    with startai.ArrayMode(False):
        if training:
            ...
            n = (x_size if x.ndim == 1 else startai.divide(x_size, x_shape[-1])).cast(
                x.dtype
            )
            ...
            runningmean = startai.add(
                startai.multiply(startai.subtract(1, momentum), runningmean),
                startai.multiply(momentum, mean),
            )
            # runningvariance = (
            #    1 - momentum
            # ) * runningvariance + momentum * variance * n / (n - 1)
            runningvariance = startai.add(
                startai.multiply(startai.subtract(1, momentum), runningvariance),
                startai.divide(startai.multiply(startai.multiply(momentum, variance), n), n - 1),
            )
