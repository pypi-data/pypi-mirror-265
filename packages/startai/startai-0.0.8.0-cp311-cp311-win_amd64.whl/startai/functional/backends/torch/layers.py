"""Collection of PyTorch network layers, wrapped to fit Startai syntax and
signature."""
...
from startai.functional.startai.layers import _get_embed_dim, _handle_padding, _deconv_length
...
def multi_head_attention(
    ...
    *,
    ...
    ret = torch.nn.functional.multi_head_attention_forward(
        ...
        use_separate_proj_weight=not startai.exists(in_proj_weights),
...
multi_head_attention.partial_mixed_handler = (
    lambda *args, scale=None, out_proj_weights=None, is_causal=False, attention_mask=None, return_attention_weights=False, in_proj_weights=None, q_proj_weights=None, k_proj_weights=None, v_proj_weights=None, **kwargs: not startai.exists(  # noqa: E501
        scale
    )
    and startai.exists(out_proj_weights)
    and (not is_causal or startai.exists(attention_mask))
    and (not is_causal or not return_attention_weights)
    and (
        startai.exists(in_proj_weights)
        or all(startai.exists(x) for x in [q_proj_weights, k_proj_weights, v_proj_weights])
    )
    and len(
        set(
            _get_embed_dim(
    ...
    if x_dilations_idxs:
        for i in x_dilations_idxs:
            ...
            h = torch.eye(
                new_height,
                dtype=x.dtype,
                device=startai.as_native_dev(startai.default_device()),
            )[:: x_dilations[i]]
...
def conv1d_transpose(
    ...
    /,
    *,
    output_shape: Optional[Union[startai.NativeShape, Sequence[int]]] = None,
    filter_format: str = "channel_last",
    data_format: str = "NWC",
    dilations: Union[int, Tuple[int]] = 1,
    bias: Optional[torch.Tensor] = None,
...
def depthwise_conv2d(
    ...
    *,
    ...
    if data_format == "NHWC":
        x = x.permute(0, 3, 1, 2)
    filters = startai.squeeze(filters, axis=3).to_native() if filters.ndim == 4 else filters
...
def lstm(
    ...
    if initial_states[0].dim() == 2:
        initial_states[0] = startai.expand_dims(initial_states[0])
    if initial_states[1].dim() == 2:
        initial_states[1] = startai.expand_dims(initial_states[1])
