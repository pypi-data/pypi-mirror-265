...
def matrix_norm(
    ...
    *,
    ...
    # startai forces out to be the same type as the input
    # but it has to be the same type as ret.
    if (
        "complex" in startai.as_startai_dtype(dtype) or "complex" in startai.as_startai_dtype(x.dtype)
    ) and startai.exists(out):
        out = startai.astype(out, "float32").to_native()
...
def tensordot(
    ...
    *,
    ...
    dtype = startai.as_native_dtype(startai.promote_types(x1.dtype, x2.dtype))
...
def vecdot(
    ...
    *,
    ...
    if startai.exists(out):
        if startai.as_startai_dtype(out.dtype) == startai.as_startai_dtype(x1.dtype):
