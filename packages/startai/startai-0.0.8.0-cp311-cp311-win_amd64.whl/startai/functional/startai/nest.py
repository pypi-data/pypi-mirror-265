"""Collection of Startai functions for nested objects."""
...
@handle_exceptions
def index_nest(
    nest: Union[List, Tuple, Dict, startai.Array, startai.NativeArray, startai.Container],
...
def set_nest_at_index(
    nest: Union[startai.Array, startai.NativeArray, startai.Container, Dict, List, Tuple],
    index: Sequence[Union[str, int]],
    value: Any,
    /,
    shallow: bool = True,
    _result: Union[startai.Array, startai.NativeArray, startai.Container, Dict, List, Tuple] = None,
) -> Union[startai.Array, startai.NativeArray, startai.Container, Dict, List, Tuple]:
    ...
    >>> y = (1, 1)
    >>> z = 5.
    >>> startai.set_nest_at_index(x, y, z)
    >>> print(x)
    startai.array([[1., 2.], [3., 5.]])
...
def insert_into_nest_at_index(nest: Iterable, index: Tuple, value) -> None:
    ...
    >>> index = (1, 1)
    >>> value = 99
    >>> startai.insert_into_nest_at_index(nest, index, value)
    >>> print(nest)
    [[1, 2], [3, 99, 4]]
    """
    if isinstance(nest, (dict, startai.Container)):
        if len(index) == 1:
            key = index[0]
...
def map_nest_at_index(
    nest: Union[startai.Array, startai.NativeArray, startai.Container, Dict, List],
    index: Sequence[Union[str, int]],
    fn: Callable[[Any], Any],
    /,
    shallow: bool = True,
    _result: Union[startai.Array, startai.NativeArray, startai.Container, Dict, List] = None,
) -> Union[startai.Array, startai.NativeArray, startai.Container, Dict, List, Tuple]:
    ...
    >>> y = (1, 1)
    >>> z = lambda a: a + 1.
    >>> startai.map_nest_at_index(x, y, z)
    >>> print(x)
    startai.array([[1., 2.], [3., 5.]])
...
def multi_index_nest(
    nest: Union[List, Dict, Tuple, startai.Array, startai.NativeArray, startai.Container],
    ...
    >>> x = (1, 2)
    >>> y = [[0]]
    >>> z = startai.multi_index_nest(x, y)
    >>> print(z)
    [1]
...
def set_nest_at_indices(
    nest: Union[List, Tuple, Dict, startai.Array, startai.NativeArray],
    ...
    >>> indices = [[0, 4], [1, 3]]
    >>> values = [111, 'x']
    >>> startai.set_nest_at_indices(nest, indices, values)
    >>> print(nest)
    ...
    >>> nest = startai.array([[1., 2., 3.],[4., 5., 6.]])
    >>> indices = ((0, 1),(1, 2))
    >>> values = (11., 22.)
    >>> startai.set_nest_at_indices(nest, indices, values)
    >>> print(nest)
    startai.array([[1., 11., 3.], [4., 5., 22.]])
    """
    is_tuple = isinstance(nest, tuple)
    nest_type = type(nest) if is_tuple else lambda x: x
    if shallow:
...
def map_nest_at_indices(
    ...
    >>> startai.map_nest_at_indices(nest, indices, function)
    >>> print(nest)
    ...
    >>> nest = startai.array([[-9., 8., -17.],[11., -3., 5.]])
    >>> indices = ((0, 1),(1, 1),(1, 2))
    >>> function = lambda x : x ** 2
    >>> startai.map_nest_at_indices(nest, indices, function)
    >>> print(nest)
    startai.array([[ -9.,  64., -17.],
           [ 11.,   9.,  25.]])
    """
    is_tuple = isinstance(nest, tuple)
    nest_type = type(nest) if is_tuple else lambda x: x
...
def nested_argwhere(
    ...
    >>> fun = startai.abs
    >>> nested_indices = startai.nested_argwhere(nest, fn=fun)
    ...
    >>> nest = ([-5, 9, 2], [0.3, 4.])
    >>> fun = startai.abs
    >>> nested_indices = startai.nested_argwhere(nest, fn=fun, stop_after_n_found=4)
    >>> print(nested_indices)
    [[0, 0], [0, 1], [0, 2], [1, 0]]
...
def all_nested_indices(
    nest: Union[List, Tuple, Dict, startai.Array, startai.NativeArray, startai.Container] = None,
    /,
    ...
    >>> y = startai.all_nested_indices(x)
    >>> print(y)
    [[0], [1, 0], [1, 1], [2, 0], [2, 1]]
    ...
    >>> x = (189, (863, 672), (264, 384))
    >>> y = startai.all_nested_indices(x, include_nests=True)
    >>> print(y)
    ...
    >>> x = startai.array([[True, False], [False, False]])
    >>> y = startai.all_nested_indices(x)
    >>> print(y)
    [[]]
    ...
    >>> x = startai.Container(a=startai.array([412, 948, 482]), b=startai.array([168, 674, 341]))
    >>> y = startai.all_nested_indices(x)
    >>> print(y)
    [['a'], ['b']]
    """
...
def map(
    ...
    >>> results = startai.map(fn = special_square,
    ...                   constant = None,
    ...                   unique = {'x' : [1,2,3]},
    ...                   mean = False)
    >>> print(results)
    ...
    >>> results = startai.map(fn = special_pow,
    ...                   constant = {'y':[0,1]},
    ...                   unique = {'x':[1,2,3]},
    ...                   mean = False)
    >>> print(results)
    ...
    >>> results = startai.map(fn = linear_model,
    ...                   constant = {'w':10., 'b':1.},
    ...                   unique = {'x':[0.,1.,2.]},
    ...                   mean = False)
    >>> print(results)
    ...
    >>> results = startai.map(fn = linear_model,
    ...    constant = {'w':startai.array([1.,0.,1.]), 'b':startai.array([0.,10.,100.])},
    ...    unique = {'x':[startai.array([0.,1.,0.]), startai.array([1.,1.,1.])]},
    ...    mean = False)
    >>> print(results)
    [startai.array([0., 10., 100.]),
    startai.array([1., 10., 101.])]
    ...
    ...    mean = True)
    >>> print(results)
    startai.array([  0.5,  10. , 100. ])
    """
    c = startai.default(constant, {})
    u = startai.default(unique, {})
    rets = [
        r
        for r in _map(
...
def nested_map(
    ...
    shallow: bool = True,
) -> Union[startai.Array, startai.NativeArray, Iterable, Dict]:
    ...
    >>> x = startai.Container(
    ...   a=startai.array([[1, 2, 3], [9, 8, 7]]) , b=startai.array([[4, 5, 6], [12, 13, 14]])
    ... )
    >>> function = lambda a : a  + 1
    >>> startai.nested_map(function, x)
    >>> print(x)
    ...
    >>> function = lambda a :  a * 2
    >>> startai.nested_map(function, nest, to_ignore=list)
    ([1, 2, 1, 2], [3, 4, 3, 4], [5, 6, 5, 6], {'a': 2, 'b': 4, 'c': 6})

    >>> nest = ([23, 25, 1337], [63, 98, 6])
    >>> function = lambda a :  a + 1
    >>> startai.nested_map(function, nest, to_mutable=True)
    [[24, 25, 1338], [64, 99, 7]]
    """
    to_ignore = startai.default(to_ignore, ())
    ...
    tuple_check_fn = startai.default(
        _tuple_check_fn,
        (
...
@handle_exceptions
def copy_nest(
    nest: Union[startai.Array, startai.NativeArray, Iterable],
    /,
    ...
    >>> nest = startai.array([[1.,2.,3.],[7.,8.,9.]])
    >>> copied_nest = startai.copy_nest(nest)
    >>> print(copied_nest)
    startai.array([[1., 2., 3.],
            [7., 8., 9.]])
    ...
    >>> copied_nest = startai.copy_nest(nest, include_derived = True)
    >>> print(copied_nest)
    [[1, 2, 3, 4, 5], [23, 24, 25, 26, 27]]

    >>> nest = ([23, 25, 1337], [63, 98, 6])
    >>> copied_nest = startai.copy_nest(nest, to_mutable = True)
    >>> print(copied_nest)
    [[23, 25, 1337], [63, 98, 6]]
...
def nested_multi_map(
    ...
    to_startai
        convert the output to startai_arrays. Default is ``True``
    ...
    if nest0 is not None:
        ...
        for index, val in enumerate(nest0):
            ...
            if ret is not None:
                if to_startai and isinstance(nest, (startai.Array, startai.NativeArray)):
                    ret = startai.array(startai.to_list(ret))
                (
        ...
        if index_chains is not None:
            ...
            if (found and not to_apply) or (not found and to_apply):
                if prune_unapplied:
                    return return_nest
                if startai.is_array(value0):
                    if not to_startai:
                        value0 = startai.array(value0)
                (
                ...
                return (
                    ...
                    if isinstance(nest, tuple)
                        ...
                        startai.Container(return_nest)
                        if startai.is_startai_container(nest)
                        else return_nest
                    )
                )
        ret = func(values, this_index_chain)
        if to_startai and ret is not None:
            if isinstance(nest, (startai.Array, startai.NativeArray)):
                return ret
            else:
                return startai.array(ret)
        else:
            return ret
    ...
    return (
        ...
        else startai.Container(return_nest) if startai.is_startai_container(nest0) else return_nest
    )


@handle_exceptions
def duplicate_array_index_chains(nest: Union[startai.Array, startai.NativeArray, Iterable]):
    ...
    """
    all_index_chains = startai.nested_argwhere(nest, lambda _: True)
    duplicates = []
    duplicate_index_chains = {}
    for index_chain in all_index_chains:
        val = startai.index_nest(nest, index_chain)
        if startai.is_array(val):
...
def prune_empty(nest):
    ...
    if isinstance(nest, dict):
        ...
        for i in range(len(nest) - 1, -1, -1):
            if nest[i] is None:
                del nest[i]
    if not valid and not startai.is_array(nest) and not isinstance(nest, (int, float, str)):
        return None
    return nest
