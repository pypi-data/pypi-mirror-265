...
class NestedArrayBase(abc.ABC):
    ...
    def __init__(self, data, nested_rank, inner_shape, dtype, device, internal=False):
        ...
        self._dtype = dtype
        self._device = device
        self._pre_repr = "startai.NestedArray"

    @classmethod
    def nested_array(
        cls, data, nested_rank=None, inner_shape=None, dtype=None, device=None
    ):
        dtype = startai.default_dtype(dtype=dtype, item=data)
        device = startai.default_device(device, item=data)

        # convert all the leaf lists to startai arrays, determine inner_shape and depth
        det_inner_shape = []

        # ToDo: add check for depth being the same for all nests
        def _seq_to_startai(x, depth=0):
            if nested_rank is not None and depth >= nested_rank:
                x = startai.array(x, dtype=dtype, device=device)
                depth += x.ndim - 1
                if x.ndim > 1:
                ...
                for i, item in enumerate(x):
                    ...
                    x[i], depth_ret = _seq_to_startai(item, depth=depth + 1)
        ...
        if isinstance(data, (list, tuple)):
            data, depth = _seq_to_startai(data)
            depth += 1
    ...
    def ragged_multi_map_in_function(fn, *args, **kwargs):
        arg_nest_idxs = startai.nested_argwhere(
            args, startai.is_startai_nested_array, to_ignore=startai.NestedArray
        )
        kwarg_nest_idxs = startai.nested_argwhere(
            kwargs, startai.is_startai_nested_array, to_ignore=startai.NestedArray
        )
        # retrieve all the nested_array in args and kwargs
        arg_nest = startai.multi_index_nest(args, arg_nest_idxs)
        kwarg_nest = startai.multi_index_nest(kwargs, kwarg_nest_idxs)
        ...
        def map_fn(vals):
            ...
            a = startai.copy_nest(args, to_mutable=True)
            startai.set_nest_at_indices(a, arg_nest_idxs, arg_vals)
            kwarg_vals = vals[num_arg_nest:]
            kw = startai.copy_nest(kwargs, to_mutable=True)
            startai.set_nest_at_indices(kw, kwarg_nest_idxs, kwarg_vals)
        ...
        ret = startai.NestedArray.ragged_multi_map(map_fn, nests)
        return ret
    ...
    def ragged_multi_map(fn, ragged_arrays):
        ...
        for ragged in ragged_arrays:
            args.append(startai.copy_nest(ragged.data))
        ret = startai.nested_multi_map(lambda x, _: fn(x), args)
        # infer dtype, shape, and device from the first array in the ret data
        broadcasted_shape = startai.NestedArray.broadcast_shapes(
        ...
        arr0_id = startai.nested_argwhere(ret, startai.is_startai_array, stop_after_n_found=1)[0]
        arr0 = startai.index_nest(ret, arr0_id)
        ragged_ret = startai.NestedArray.nested_array(
            ret, nested_rank, inner_shape, arr0.dtype, arr0.device
        )
        return ragged_ret

    @staticmethod
    def replace_startai_arrays(ragged_array, arrays):
        data = ragged_array.data
        startai_idxs = startai.nested_argwhere(data, startai.is_startai_array)
        arr0 = arrays[0]
        inner_shape, dev, dtype = arr0.shape.as_list(), arr0.device, arr0.dtype
        ret = startai.set_nest_at_indices(data, startai_idxs, arrays, shallow=False)
        return startai.NestedArray.nested_array(
    ...
    def ragged_map(self, fn):
        arg = startai.copy_nest(self._data)
        startai.nested_map(lambda x: fn(x), arg, shallow=True)
        # infer dtype, shape, and device from the first array in the ret data
        arr0_id = startai.nested_argwhere(arg, startai.is_startai_array, stop_after_n_found=1)[0]
        arr0 = startai.index_nest(arg, arr0_id)
    ...
    def unbind(self):
        return tuple(startai.copy_nest(self._data))
    ...
    def __repr__(self):
        rep = self._data.__repr__().replace("[startai.array", "[")
        rep = rep.replace("startai.array", "\n\t").replace("(", "").replace(")", "")
        ret = self._pre_repr + "(\n\t" + rep + "\n)"
        return ret
