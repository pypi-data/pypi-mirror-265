...
importlib_module_path = "startai.utils._importlib"
...
_local_import_template = Template(
    "$name = "
    "startai.utils.backend.handler._compiled_backends_ids[$startai_id].utils._importlib.$name"
)
_unmodified_startai_path = sys.modules["startai"].__path__[0].rpartition(os.path.sep)[0]
_compiled_modules_cache = {}


def _retrive_local_modules():
    ret = ["startai"]  # TODO temporary hacky solution for finder
    # Get Startai package root
    wd = sys.modules["startai"].__path__[0]
...
class ImportTransformer(ast.NodeTransformer):
    def __init__(self):
        ...
        self.include_startai_import = False

    def visit_Import(self, node):
        ret, should_impersonate = _parse_import(node)
        if should_impersonate and not self.include_startai_import:
            self.include_startai_import = True
        return ret
    ...
    def impersonate_import(self, tree: ast.Module, local_startai_id=None):
        if not self.include_startai_import:
            return tree
        ...
        if local_startai_id is None:
            ...
            insert_import(
                _local_import_template.substitute(
                    name=importlib_abs_import_fn, startai_id=local_startai_id
                )
            )
            insert_import(
                _local_import_template.substitute(
                    name=importlib_from_import_fn, startai_id=local_startai_id
                )
            )
            insert_import("import startai")

        return tree


class StartaiPathFinder(MetaPathFinder):
    def find_spec(self, fullname, path, target=None):
        if fullname.partition(".")[0] not in local_modules:
            return None
        # We're local
        if path is None or path == "":
            path = [_unmodified_startai_path]
        if "." in fullname:
        ...
        for entry in path:
            ...
            return spec_from_file_location(
                fullname,
                filename,
                loader=StartaiLoader(filename),
                submodule_search_locations=submodule_locations,
            )
        return None


class StartaiLoader(Loader):
    def __init__(self, filename):
        self.filename = filename

    def exec_module(self, module, local_startai_id=None):
        if self.filename in _compiled_modules_cache:
            ...
            transformer.impersonate_import(ast_tree, local_startai_id)
