...
# multinomial
@handle_test(
    fn_tree="functional.startai.multinomial",
...
def test_multinomial(*, everything, seed, test_flags, backend_fw, fn_name, on_device):
    ...
    if seed:
        ...
        assert startai.any(ret_np == ret_np1)
...
# randint
@handle_test(
    fn_tree="functional.startai.randint",
...
def test_randint(*, dtype_low_high, seed, test_flags, backend_fw, fn_name, on_device):
    ...
    for u, v in zip(ret, ret_gt):
        assert startai.all(u >= low)
        assert startai.all(u < high)
        assert startai.all(v >= low)
        assert startai.all(v < high)


# random_normal
@handle_test(
    fn_tree="functional.startai.random_normal",
...
# random_uniform
@handle_test(
    fn_tree="functional.startai.random_uniform",
...
# seed
@handle_test(
    fn_tree="functional.startai.seed",
...
def test_seed(seed_val, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        startai_backend.seed(seed_value=seed_val)


# shuffle
@handle_test(
    fn_tree="functional.startai.shuffle",
...
def test_shuffle(
    *, dtype_and_x, seed, axis, test_flags, backend_fw, fn_name, on_device
    ...
    for u, v in zip(ret, ret_gt):
        assert startai.all(startai.sort(u, axis=0) == startai.sort(v, axis=0))
