...
from startai_tests.test_startai.helpers import (
    handle_test,
    BackendHandler,
    handle_example,
)
from startai_tests.test_startai.helpers.assertions import assert_all_close
from startai_tests.test_startai.test_functional.test_core.test_elementwise import pow_helper
...
try:
    import startai.functional.backends.jax
except ImportError:
    startai.functional.backends.jax = SimpleNamespace()

try:
    import startai.functional.backends.tensorflow
except ImportError:
    startai.functional.backends.tensorflow = SimpleNamespace()

try:
    import startai.functional.backends.torch
except ImportError:
    startai.functional.backends.torch = SimpleNamespace()
...
def _composition_1():
    return startai.relu().argmax()


def _composition_2():
    return startai.ceil() or startai.linspace()


def _fn1(x, y):
    return startai.matmul(x, y)


def _fn2(x, y):
    return startai.vecdot(x, y)
...
def _supports_inplace_update(startai_backend, test_flags) -> bool:
    supports_array_inplace_update = (
        not test_flags.as_variable and startai_backend.inplace_arrays_supported()
    )
    supports_variable_inplace_update = (
        test_flags.as_variable and startai_backend.inplace_variables_supported()
    )
...
# fourier_encode
# @given(
#     x=helpers.dtype_and_values(startai_np.valid_float_dtypes, min_num_dims=1),
#     max_freq=helpers.dtype_and_values(startai_np.valid_float_dtypes),
...
def _vector_norm_helper(draw):
    ...
    if startai.is_int_dtype(dtype[0]):
        max_val = startai.iinfo(dtype[0]).max
    else:
        max_val = startai.finfo(dtype[0]).max
...
# all_equal
@handle_test(
    fn_tree="functional.startai.all_equal",
...
@given(
    x_n_value=st.sampled_from(
        [
            [startai.value_is_nan, ["x", "include_infs"]],
            [startai.clip_matrix_norm, ["x", "max_norm", "p", "out"]],
        ]
    )
)
def test_arg_names(x_n_value):
    x, value = x_n_value
    ret = startai.arg_names(x)
    assert ret == value


# array_equal
@handle_test(
    fn_tree="functional.startai.array_equal",
    dtypes_and_xs=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid"),
        num_arrays=2,
    ),
...
@handle_test(
    fn_tree="functional.startai.assert_supports_inplace",
...
def test_cache_fn():
    ...
    cached_fn = startai.cache_fn(func)
    ret0 = cached_fn()
    ret0_again = cached_fn()
    ret1 = func()

    assert startai.to_numpy(ret0).item() == startai.to_numpy(ret0_again).item()
    assert startai.to_numpy(ret0).item() != startai.to_numpy(ret1).item()
    assert ret0 is ret0_again
    assert ret0 is not ret1

    # call startai.cache_fn repeatedly, the new cached functions
    # each use the same global dict
    ret0 = startai.cache_fn(func)()
    ret0_again = startai.cache_fn(func)()
    ret1 = func()
...
def test_cache_fn_with_args():
    ...
    # call startai.cache_fn repeatedly, the new cached functions
    # each use the same global dict
    ret0 = startai.cache_fn(func)(0)
    ret0_again = startai.cache_fn(func)(0)
    ret1 = startai.cache_fn(func)(1)
...
# clip_matrix_norm
@handle_test(
    fn_tree="functional.startai.clip_matrix_norm",
...
# clip_vector_norm
@handle_test(
    fn_tree="functional.startai.clip_vector_norm",
...
def test_container_types():
    cont_types = startai.container_types()
...
def test_current_backend_str(fw):
    startai.set_backend(fw)
    assert startai.current_backend_str() == fw
    startai.previous_backend()


# default
@handle_test(
    fn_tree="functional.startai.default",
    x=st.one_of(
        st.none(),
        helpers.dtype_and_values(
...
def test_default(x, default_val, test_flags, backend_fw):
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        truth_val = startai_backend.to_native(x if x is not None else default_val)
        if with_callable:
            assert startai_backend.default(x, default_val) == truth_val
        else:
            assert_all_close(
                np.asarray(startai_backend.default(x, default_val)),
...
# ToDo: re-add this test once startai.get_backend is working correctly, with the returned
#  startai handle having no dependence on the globally set startai
# @handle_cmd_line_args
#
# def test_class_startai_handles(device, call):
#
...
#
#     class ArrayGen:
#         def __init__(self, startaih):
#             self._startai = startaih
#
#         def get_array(self):
#             return self._startai.array([0.0, 1.0, 2.0], dtype="float32", device=device)
#
#     # create instance
#     ag = ArrayGen(startai.get_backend())
#
...
# einops_rearrange
@handle_test(
    fn_tree="functional.startai.einops_rearrange",
    dtype_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        allow_inf=False,
        min_num_dims=4,
        ...
        lambda x: (startai.array([x[1][0]], dtype="float32").shape[2] % 2 == 0)
        and (startai.array([x[1][0]], dtype="float32").shape[3] % 2 == 0)
...
# einops_reduce
@handle_test(
    fn_tree="functional.startai.einops_reduce",
    dtype_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        allow_inf=False,
        min_num_dims=4,
...
# einops_repeat
@handle_test(
    fn_tree="functional.startai.einops_repeat",
    dtype_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        allow_inf=False,
        min_num_dims=2,
...
# exists
@handle_test(
    fn_tree="functional.startai.exists",
    x=st.one_of(
        st.none(),
        helpers.dtype_and_values(
            available_dtypes=helpers.get_dtypes("numeric"),
...
def test_exists(x):
    if x is not None:
        if not callable(x):
            dtype, x = x
    ret = startai.exists(x)
    assert isinstance(ret, bool)
    y_true = x is not None
    assert ret == y_true


def test_explicit_startai_framework_handles(backend_fw):
    if backend_fw == "numpy":
        # Numpy is the conflicting framework being tested against
        pytest.skip()

    # set with explicit handle caught
    startai_exp = startai.with_backend(backend_fw)
    assert startai_exp.current_backend_str() == backend_fw

    # assert backend implemented function is accessible
    assert "array" in startai_exp.__dict__
    assert callable(startai_exp.array)

    # assert joint implemented function is also accessible
    assert "cache_fn" in startai_exp.__dict__
    assert callable(startai_exp.cache_fn)

    # set global startai to numpy
    startai.set_backend("numpy")

    # assert the explicit handle is still unchanged
    assert startai.current_backend_str() == "numpy"
    assert startai_exp.current_backend_str() == backend_fw

    # unset global startai from numpy
    startai.previous_backend()
...
def test_function_supported_device_and_dtype(func, backend_fw):
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        res = startai_backend.function_supported_devices_and_dtypes(func, recurse=True)
        exp = {"cpu": func.test_unsupported_devices_and_dtypes.copy()["cpu"]}
        for dev in exp:
            exp[dev] = tuple(
                set(startai.valid_dtypes).difference(exp[dev][startai.current_backend_str()])
            )
...
def test_function_unsupported_devices(func, backend_fw):
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        res = startai_backend.function_unsupported_devices_and_dtypes(func)
...
# gather
@handle_test(
    fn_tree="functional.startai.gather",
...
# gather_nd
@handle_test(
    fn_tree="functional.startai.gather_nd",
...
@handle_test(
    fn_tree="functional.startai.get_item",
    ground_truth_backend="numpy",
...
def test_get_item(
    ...
    except startai.utils.exceptions.StartaiBackendException as e:
...
def test_get_min_base():
    assert startai.min_base == 1e-5


# get_min_denominator
def test_get_min_denominator():
    assert startai.min_denominator == 1e-12


# get_num_dims
@handle_test(
    fn_tree="functional.startai.get_num_dims",
...
def test_get_queue_timeout(x):
    startai.set_queue_timeout(x)
    ret = startai.queue_timeout
    assert ret == x
...
def test_get_referrers_recursive():
    ...
    some_obj = SomeClass()
    refs = startai.get_referrers_recursive(some_obj.x)
    ref_keys = refs.keys()
...
# has_nans
@handle_test(
    fn_tree="functional.startai.has_nans",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("float"),
        allow_nan=True,
        allow_inf=True,
...
def test_inplace_arrays_supported(backend_fw):
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        if backend_fw in ["numpy", "torch"]:
            assert startai_backend.inplace_arrays_supported()
        elif backend_fw in ["jax", "tensorflow", "paddle"]:
            assert not startai_backend.inplace_arrays_supported()
        else:
...
# inplace_decrement
@handle_test(
    fn_tree="functional.startai.inplace_decrement",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        allow_inf=False,
        min_num_dims=1,
...
def test_inplace_decrement(x_val_and_dtypes, test_flags, on_device, backend_fw):
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        x = startai_backend.array(x, dtype=dtype, device=on_device)
        val = startai_backend.array(val, dtype=dtype, device=on_device)
        new_val = x - val
        supports_update = _supports_inplace_update(startai_backend, test_flags)
        if supports_update:
            x_inplace = startai_backend.inplace_decrement(x, val)
...
# inplace_increment
@handle_test(
    fn_tree="functional.startai.inplace_increment",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        allow_inf=False,
        min_num_dims=1,
...
def test_inplace_increment(x_val_and_dtypes, test_flags, on_device, backend_fw):
    dtype = x_val_and_dtypes[0][0]
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        if dtype in startai_backend.function_unsupported_dtypes(
            startai_backend.inplace_increment
        ):
            return
        x, val = x_val_and_dtypes[1]
        x, val = x.tolist(), val.tolist()
        ...
        supports_update = _supports_inplace_update(startai_backend, test_flags)
        if supports_update:
            x_inplace = startai_backend.inplace_increment(x, val)
            assert id(x_inplace) == id(x)
            x = helpers.flatten_and_to_np(ret=x, backend=backend_fw)
            new_val = helpers.flatten_and_to_np(ret=new_val, backend=backend_fw)
            helpers.value_test(
...
# inplace_update
@handle_test(
    fn_tree="functional.startai.inplace_update",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        num_arrays=2,
        shared_dtype=True,
...
def test_inplace_update(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        dtype = x_val_and_dtypes[0][0]
        if dtype in startai_backend.function_unsupported_dtypes(startai_backend.inplace_update):
            return
        x, val = x_val_and_dtypes[1]
        x = startai_backend.array(x.tolist(), dtype=dtype, device=on_device)
        val = startai_backend.array(val.tolist(), dtype=dtype, device=on_device)

        startai_backend.set_inplace_mode(inplace_mode)
        supports_update = _supports_inplace_update(startai_backend, test_flags)
        if supports_update or startai_backend.inplace_mode == "lenient":
            if keep_x_dtype:
                x_dtype = x.dtype
                x_inplace = startai_backend.inplace_update(x, val, keep_input_dtype=True)
                assert x_dtype == x_inplace.dtype
            else:
                x_inplace = startai_backend.inplace_update(x, val)
            assert id(x_inplace) == id(x)
        ...
        elif not supports_update and startai_backend.inplace_mode == "strict":
            with pytest.raises(startai.utils.exceptions.InplaceUpdateException):
                startai_backend.inplace_update(x, val)


def test_inplace_variables_supported(backend_fw):
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        if backend_fw in ["numpy", "torch", "tensorflow"]:
            assert startai_backend.inplace_variables_supported()
        elif backend_fw in ["jax", "paddle"]:
            assert not startai_backend.inplace_variables_supported()
        else:
            raise RuntimeError("Unrecognized framework")
...
# is_array
@handle_test(
    fn_tree="functional.startai.is_array",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid")
    ),
...
# is_startai_array
@handle_test(
    fn_tree="functional.startai.is_startai_array",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid")
    ),
    exclusive=st.booleans(),
...
# is_startai_container
@handle_test(
    fn_tree="functional.startai.is_startai_container",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid")
    ),
    test_with_out=st.just(False),
...
def test_is_startai_container(x_val_and_dtypes, test_flags, backend_fw, fn_name, on_device):
    dtype, x = x_val_and_dtypes
    helpers.test_function(
        input_dtypes=dtype,
        test_flags=test_flags,
...
# is_native_array
@handle_test(
    fn_tree="functional.startai.is_native_array",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid")
    ),
    exclusive=st.booleans(),
...
@handle_test(
    fn_tree="functional.startai.isin",
...
@handle_test(
    fn_tree="functional.startai.itemsize",
...
def test_match_kwargs(allow_duplicates):
    ...
    kwfa, kwfb, kwca = startai.match_kwargs(
...
# scatter_flat
@handle_test(
    fn_tree="functional.startai.scatter_flat",
...
# scatter_nd
@handle_test(
    fn_tree="functional.startai.scatter_nd",
...
def test_set_inplace_mode(mode):
    startai.set_inplace_mode(mode)
    assert startai.inplace_mode == mode


# set_item
# TODO: add container and array instance methods
@handle_test(
    fn_tree="functional.startai.set_item",
    ground_truth_backend="numpy",
...
def test_set_min_base(x):
    startai.set_min_base(x)
    assert startai.min_base == x


# set_min_denominator
@given(x=st.floats(allow_nan=False, allow_infinity=False))
def test_set_min_denominator(x):
    startai.set_min_denominator(x)
    assert startai.min_denominator == x
...
@handle_test(
    fn_tree="functional.startai.shape",
    x0_n_x1_n_res=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid")
    ),
    as_array=st.booleans(),
...
# size
@handle_test(
    fn_tree="functional.startai.size",
...
# stable_divide
@handle_test(
    fn_tree="functional.startai.stable_divide",
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("numeric"),
        num_arrays=3,
        shared_dtype=True,
...
# stable_pow
@handle_test(
    fn_tree="functional.startai.stable_pow",
...
@handle_test(
    fn_tree="functional.startai.strides",
    x_and_dtype=helpers.dtype_and_values(available_dtypes=helpers.get_dtypes("valid")),
    test_instance_method=st.just(False),
    test_with_out=st.just(False),
    test_gradients=st.just(False),
...
@handle_test(
    fn_tree="functional.startai.supports_inplace_updates",
    x_val_and_dtypes=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid")
    ),
    test_with_out=st.just(False),
...
# to_list
@handle_test(
    fn_tree="functional.startai.to_list",
    x0_n_x1_n_res=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid"),
...
# to_numpy
@handle_test(
    fn_tree="functional.startai.to_numpy",
    dtype_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid"),
    ),
    copy=st.booleans(),
...
def test_to_numpy(*, dtype_x, copy, test_flags, backend_fw, fn_name, on_device):
    ...
    if startai.current_backend_str() == "torch" and not copy:
        return
    helpers.test_function(
        input_dtypes=dtype,
        test_flags=test_flags,
...
# to_scalar
@handle_test(
    fn_tree="functional.startai.to_scalar",
    x0_n_x1_n_res=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypes("valid"),
        min_num_dims=1,
        max_num_dims=1,
...
def test_try_else_none(x):
    if x:
        fn = startai.try_else_none(lambda: True)
        assert fn() is True
    else:
        fn = startai.try_else_none(lambda x: x)
        assert fn is None
...
def test_unset_inplace_mode(mode):
    startai.set_inplace_mode(mode)
    startai.unset_inplace_mode()
    assert startai.inplace_mode == "lenient"


def test_use_within_use_framework():
    with startai.functional.backends.numpy.use:
        pass
    with startai.functional.backends.jax.use:
        pass
    with startai.functional.backends.tensorflow.use:
        pass
    with startai.functional.backends.torch.use:
        pass


# value_is_nan
@handle_test(
    fn_tree="functional.startai.value_is_nan",
...
# vmap
@handle_test(
    fn_tree="functional.startai.vmap",
...
def test_vmap(func, dtype_and_arrays_and_axes, in_axes_as_cont, backend_fw):
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        arrays = [startai_backend.native_array(array) for array in generated_arrays]
        assume(
            startai_backend.as_startai_dtype(dtype[0])
            not in startai_backend.function_unsupported_dtypes(startai_backend.vmap)
        )

        if in_axes_as_cont:
            vmapped_func = startai_backend.vmap(func, in_axes=in_axes, out_axes=0)
        else:
            vmapped_func = startai_backend.vmap(func, in_axes=0, out_axes=0)
...
_composition_1.test_unsupported_devices_and_dtypes = {
    ...
    "gpu": {
        "numpy": startai.all_dtypes,
        "jax": ("complex64", "complex128"),
        "tensorflow": ("complex64", "complex128"),
        "torch": ("complex64", "float16", "uint16", "complex128", "uint64", "uint32"),
        "paddle": startai.all_dtypes,
    },
    "tpu": {
        "numpy": startai.all_dtypes,
        "jax": startai.all_dtypes,
        "tensorflow": startai.all_dtypes,
        "torch": startai.all_dtypes,
        "paddle": startai.all_dtypes,
    },
}
