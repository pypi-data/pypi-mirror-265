...
from startai.functional.startai.layers import _deconv_length, _pack_padded_sequence
...
def _dropout_helper(draw):
    ...
    is_torch_backend = startai.current_backend_str() == "torch"
...
def _lstm_helper(draw):
    ...
    if weights_transposed:
        all_weights = [
            startai.swapaxes(w, 0, 1) if w.dims() == 2 else w for w in all_weights
        ]
    ...
    if packed:
        ...
        input, batch_sizes = (
            startai.to_numpy(p) for p in _pack_padded_sequence(input, batch_sizes)
        )
    else:
        batch_sizes = None
...
def _mha_helper(draw, same_pre_embed_dim=False, batch_second=False):
    ...
    ret_dtypes = [str(r.dtype) for r in ret if startai.is_array(r)]
...
# conv
@handle_test(
    fn_tree="functional.startai.conv",
...
# conv1d
@handle_test(
    fn_tree="functional.startai.conv1d",
...
# conv1d_transpose
@handle_test(
    fn_tree="functional.startai.conv1d_transpose",
    x_f_d_df=_x_and_filters(
        dim=1,
        transpose=True,
        bias=True,
...
# conv2d
@handle_test(
    fn_tree="functional.startai.conv2d",
    x_f_d_df=_x_and_filters(
        dim=2,
        bias=True,
    ),
...
# conv2d_transpose
@handle_test(
    fn_tree="functional.startai.conv2d_transpose",
    x_f_d_df=_x_and_filters(
        dim=2,
        transpose=True,
        bias=True,
...
# conv3d
@handle_test(
    fn_tree="functional.startai.conv3d",
    x_f_d_df=_x_and_filters(
        dim=3,
        bias=True,
    ),
...
# conv3d_transpose
@handle_test(
    fn_tree="functional.startai.conv3d_transpose",
    x_f_d_df=_x_and_filters(
        dim=3,
        transpose=True,
        bias=True,
...
@handle_test(
    fn_tree="functional.startai.conv_general_dilated",
    dims=st.shared(st.integers(1, 3), key="dims"),
    x_f_d_df=_x_and_filters(
        dim=st.shared(st.integers(1, 3), key="dims"),
        general=True,
...
@handle_test(
    fn_tree="functional.startai.conv_general_transpose",
...
# depthwise_conv2d
@handle_test(
    fn_tree="functional.startai.depthwise_conv2d",
    x_f_d_df=_x_and_filters(
        dim=2,
        depthwise=True,
    ),
...
# dropout
@handle_test(
    fn_tree="functional.startai.dropout",
...
# linear
@handle_test(
    fn_tree="functional.startai.linear",
...
# lstm
# @handle_test(
#     fn_tree="functional.startai.lstm",
...
# lstm_update
@handle_test(
    fn_tree="functional.startai.lstm_update",
...
@handle_test(
    fn_tree="functional.startai.multi_head_attention",
...
@handle_test(
    fn_tree="functional.startai.nms",
    inputs=_nms_helper(),
...
def test_nms(
    *,
    ...
    helpers.test_function(
        input_dtypes=[startai.float32, startai.float32],
        test_flags=test_flags,
        backend_to_test=backend_fw,
        fn_name=fn_name,
        on_device=on_device,
...
@handle_test(
    fn_tree="functional.startai.roi_align",
...
@handle_test(
    fn_tree="functional.startai.scaled_dot_product_attention",
