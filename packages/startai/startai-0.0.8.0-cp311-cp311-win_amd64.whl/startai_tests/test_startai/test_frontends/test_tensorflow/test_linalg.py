...
from startai_tests.test_startai.helpers import handle_frontend_test, assert_all_close
...
def tridiagonal_compact_filter(x):
    ...
    matrix = startai.scatter_nd(
        indices, diagonals, startai.array([dim, dim]), reduction="replace"
    )
...
def tridiagonal_matrix_filter(x):
    dim = x.shape[0]
    if startai.abs(startai.det(x)) < 1e-3:
        return False
    for i in range(dim):
        for j in range(dim):
            cell = x[i][j]
