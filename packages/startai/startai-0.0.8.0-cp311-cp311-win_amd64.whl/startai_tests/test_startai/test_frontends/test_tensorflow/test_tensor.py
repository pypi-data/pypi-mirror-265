...
from startai_tests.test_startai.test_frontends.test_tensorflow.test_raw_ops import (
    _pow_helper_shared_dtype,
)
from startai.functional.frontends.tensorflow import EagerTensor

CLASS_TREE = "startai.functional.frontends.tensorflow.EagerTensor"
...
def _check_query(query):
    return not isinstance(query, list) and (
        not (startai.is_array(query) and startai.is_bool_dtype(query) ^ bool(query.ndim > 0))
    )
...
def test_tensorflow___array__(
    ...
    with BackendHandler.update_backend(backend_fw) as startai_backend:
        ...
        function_module = local_importer.import_module(
            "startai.functional.frontends.tensorflow"
        )
...
def test_tensorflow_device(
    ...
    _, data = dtype_x
    data = startai.native_array(data[0])
    x = EagerTensor(data)
    startai.utils.assertions.check_equal(x.device, startai.dev(data), as_array=False)
...
def test_tensorflow_dtype(
    ...
    startai.utils.assertions.check_equal(x.dtype, startai.Dtype(dtype[0]), as_array=False)
    startai.previous_backend()
...
def test_tensorflow_startai_array(
    dtype_x,
    backend_fw,
):
    startai.set_backend(backend_fw)
