...
def _binary_focal_args(draw):
    ...
    dtype_axis = draw(
        helpers.dtype_and_values(
            available_dtypes=[startai.int32, startai.int64],
...
def test_tensorflow_categorical_crossentropy(
    *,
    ...
    y_true = startai.array(y_true, dtype=startai.float32)
    dtype, y_pred = dtype_y_pred

    # Perform softmax on prediction if it's not a probability distribution.
    if not from_logits:
        y_pred = startai.exp(y_pred) / startai.sum(startai.exp(y_pred))

    helpers.test_frontend_function(
        input_dtypes=[startai.float32, dtype],
        backend_to_test=backend_fw,
        frontend=frontend,
        test_flags=test_flags,
        fn_tree=fn_tree,
...
@handle_frontend_test(
    ...
    y_true=helpers.array_values(
        dtype=startai.int32, shape=(1, 5), min_value=1, max_value=5
    ),
    y_pred=helpers.array_values(
        dtype=startai.int32, shape=(1, 5), min_value=5, max_value=10
    ),
    test_with_out=st.just(False),
)
...
def test_tensorflow_sparse_categorical_crossentropy(
    *,
    ...
    y_true = startai.array(y_true, dtype=startai.int32)
    dtype, y_pred = dtype_y_pred
    y_pred = y_pred[0]
    # Perform softmax on prediction if it's not a probability distribution.
    if not from_logits:
    ...
    helpers.test_frontend_function(
        input_dtypes=[startai.int32] + dtype,
        backend_to_test=backend_fw,
        frontend=frontend,
        test_flags=test_flags,
        fn_tree=fn_tree,
