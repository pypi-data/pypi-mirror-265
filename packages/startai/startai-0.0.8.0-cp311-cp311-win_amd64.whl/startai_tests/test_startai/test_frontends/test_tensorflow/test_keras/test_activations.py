...
def simple_test_two_function(
    *,
    ...
    atol_: float = 1e-06,
    startai_submodules: list = [],
    framework_submodules: list = [],
    backend: str,
):
    startai.set_backend(frontend)
    fn_startai = startai.functional.frontends.__dict__[frontend]
    for startai_submodule in startai_submodules:
        fn_startai = fn_startai.__dict__[startai_submodule]
    fn_startai = fn_startai.__dict__[fn_str]
    ...
    x = startai.array(x).to_native()

    ret_startai = fn_startai(fn_name)(x)
    ret = fn_framework(fn_name)(x)

    ret_startai = startai.array(ret_startai, dtype=dtype_data)
    ret = startai.array(ret, dtype=dtype_data)

    ret_np_flat = helpers.flatten_and_to_np(backend=backend, ret=ret)
    frontend_ret_np_flat = helpers.flatten_and_to_np(backend=backend, ret=ret_startai)
...
def test_tensorflow_deserialize(
    *,
    ...
    simple_test_two_function(
        ...
        rtol_=1e-01,
        atol_=1e-01,
        startai_submodules=["keras", "activations"],
