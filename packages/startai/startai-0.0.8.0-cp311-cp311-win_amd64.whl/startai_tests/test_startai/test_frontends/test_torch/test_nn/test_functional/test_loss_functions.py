...
import startai
from startai.functional.frontends.torch.nn.functional.loss_functions import (
    cosine_similarity,
)
...
def test_torch_cosine_embedding_loss(
    *,
    ...
    if input1.ndim == input2.ndim == 1:
        tar = startai.array(1.0)
    else:
        third = input1.shape[0] // 3
        ones = startai.ones(input1.shape[0] - (third * 2))
        minus_ones = startai.ones(third) * -1
        randoms = startai.random_uniform(shape=[third])
        tar = startai.hstack((ones, minus_ones, randoms)).shuffle()
