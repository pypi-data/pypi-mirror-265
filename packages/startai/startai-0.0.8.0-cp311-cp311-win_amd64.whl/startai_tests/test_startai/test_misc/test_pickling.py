...
def test_pickle_to_and_from_disk(dtype_and_x, on_device, backend_fw):
    ...
    x = startai.array(x[0], dtype=input_dtype[0], device=on_device)
    ...
    save_filepath = "startai_array.pickle"
    ...
    # check for equality
    assert np.allclose(startai.to_numpy(x), startai.to_numpy(unpickled_arr))
