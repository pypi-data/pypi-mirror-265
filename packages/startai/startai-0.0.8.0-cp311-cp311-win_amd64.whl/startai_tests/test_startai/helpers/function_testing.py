...
import startai
from startai_tests.test_startai.helpers.test_parameter_flags import FunctionTestFlags
import startai_tests.test_startai.helpers.test_parameter_flags as pf
import startai_tests.test_startai.helpers.globals as t_globals
from startai.functional.startai.data_type import _get_function_list, _get_functions_from_string
from startai_tests.test_startai.test_frontends import NativeClass
from startai_tests.test_startai.helpers.structs import FrontendMethodData
from startai_tests.test_startai.helpers.testing_helpers import _create_transpile_report
...
def traced_if_required(
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        try:
            if test_trace:
                if (
                    ...
                    t_globals.CURRENT_TRACED_DATA[
                        t_globals.CURRENT_RUNNING_TEST.fn_name
                    ][backend] = startai_backend.trace_graph(
            ...
            if test_trace_each:
                fn = startai_backend.trace_graph(
                    fn, args=args, kwargs=kwargs, backend_compile=True
                )
        except Exception:
            import logging
...
def _find_instance_in_args(backend: str, args, array_indices, mask):
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        instance = startai_backend.index_nest(args, instance_idx)
        new_args = startai_backend.copy_nest(args, to_mutable=False)
        startai_backend.prune_nest_at_index(new_args, instance_idx)
...
def test_function_backend_computation(
    ...
    if test_flags.test_cython_wrapper:
        startai.set_cython_wrappers_mode(True)
    else:
        startai.set_cython_wrappers_mode(False)

    with BackendHandler.update_backend(fw) as startai_backend:
        # Update variable flags to be compatible with float dtype and with_out args
        test_flags.as_variable = [
            v if startai_backend.is_float_dtype(d) and not test_flags.with_out else False
            for v, d in zip(test_flags.as_variable, input_dtypes)
        ]

    # update instance_method flag to only be considered if the
    # first term is either an startai.Array or startai.Container
    ...
    with BackendHandler.update_backend(fw) as startai_backend:
        ...
        if instance_method:
            ...
            target_fn = startai_backend.__dict__[fn_name]
        ...
        assert startai_backend.nested_map(
            lambda x: startai_backend.is_startai_array(x) if startai_backend.is_array(x) else True,
            ret_from_target,
        ), f"Startai function returned non-startai arrays: {ret_from_target}"
        ...
        if test_flags.with_out and not (
            ...
            test_ret = (
                ret_from_target[getattr(startai_backend.__dict__[fn_name], "out_index")]
                if hasattr(startai_backend.__dict__[fn_name], "out_index")
                else ret_from_target
            )
            out = startai_backend.nested_map(
                startai_backend.zeros_like, test_ret, to_mutable=True, include_derived=True
            )
            if instance_method:
                (
                    ret_from_target,
                ...
                (
                    ...
                    fw,
                    startai_backend.__dict__[fn_name],
                    *args,
                    **kwargs,
                    out=out,
                    precision_mode=test_flags.precision_mode,
            ...
            assert not startai_backend.nested_any(
                startai_backend.nested_multi_map(
                    lambda x, _: x[0] is x[1], [test_ret, out]
                ),
                lambda x: not x,
            ), "the array in out argument does not contain same value as the returned"
            if not max(test_flags.container) and startai_backend.native_inplace_support:
        ...
        if test_flags.with_copy:
            array_fn = startai_backend.is_array
            if "copy" in list(inspect.signature(target_fn).parameters.keys()):
                kwargs["copy"] = True
            if instance_method:
                first_array = instance
            else:
                first_array = startai_backend.func_wrapper._get_first_array(
            ...
            first_array = startai_backend.stop_gradient(first_array).to_numpy()
            ret_ = startai_backend.stop_gradient(ret_).to_numpy()
            assert not np.may_share_memory(first_array, ret_)

    ret_device = None
    if isinstance(ret_from_target, startai_backend.Array):  # TODO use str for now
        ret_device = startai_backend.dev(ret_from_target)
    return (
        ret_from_target,
        ret_np_flat_from_target,
        ret_device,
...
def test_function_ground_truth_computation(
    ...
    with BackendHandler.update_backend(ground_truth_backend) as gt_backend:
        ...
        assert gt_backend.nested_map(
            lambda x: gt_backend.is_startai_array(x) if gt_backend.is_array(x) else True,
            ret_from_gt,
        ), f"Ground-truth function returned non-startai arrays: {ret_from_gt}"
        if test_flags.with_out and not (
            test_flags.test_trace or test_flags.test_trace_each
        ):
            test_ret_from_gt = (
...
def test_function(
    *,
    input_dtypes: Union[startai.Dtype, List[startai.Dtype]],
    ...
    **all_as_kwargs_np,
    ...
    if (
        ...
        if backend_to_test not in fw_list or not startai.nested_argwhere(
            all_as_kwargs_np,
            lambda x: (
...
def _assert_frontend_ret(ret, for_fn=True):
    ...
    if is_ret_tuple:
        non_frontend_idxs = startai.nested_argwhere(
            ret, lambda _x: not _is_frontend_array(_x) if startai.is_array(_x) else False
        )
        assert not non_frontend_idxs, (
            ...
            f" {startai.multi_index_nest(ret, non_frontend_idxs)}"
        )
    elif startai.is_array(ret):
...
def _transpile_if_required_backend(backend: str, fn_name: str, args=None, kwargs=None):
    iterations = 1
    with BackendHandler.update_backend(backend) as startai_backend:
        args, kwargs = startai_backend.args_to_startai(*args, **kwargs)
        backend_fn = startai.__dict__[fn_name]
...
def test_frontend_function(
    ...
    **all_as_kwargs_np,
    ...
    with BackendHandler.update_backend(backend_to_test) as startai_backend:
        ...
        local_importer = startai_backend.utils.dynamic_import

        # strip the decorator to get an Startai array
        # TODO, fix testing for jax frontend for x32
        if frontend == "jax":
            local_importer.import_module("startai.functional.frontends.jax").config.update(
                "jax_enable_x64", True
            )
        ...
        create_frontend_array = local_importer.import_module(
            f"startai.functional.frontends.{frontend}"
        )._frontend_array
        ...
        if test_flags.with_out and "out" in list(
            ...
            if is_ret_tuple:
                ...
                for ret_array, out_array in zip(flatten_ret, flatten_out):
                    if startai_backend.native_inplace_support and not any(
                        (startai_backend.isscalar(ret), startai_backend.isscalar(out))
                    ):
                        assert ret_array.startai_array.data is out_array.startai_array.data
                    assert ret_array is out_array
            else:
                if startai_backend.native_inplace_support and not any(
                    (startai_backend.isscalar(ret), startai_backend.isscalar(out))
                ):
                    assert ret.startai_array.data is out.startai_array.data
                assert ret is out
            ...
            if test_flags.generate_frontend_arrays:
                first_array = first_array.startai_array
            ret_ = ret_.startai_array
            if "bfloat16" in str(ret_.dtype):
                ret_ = startai_backend.astype(ret_, startai_backend.float64)
            if "bfloat16" in str(first_array.dtype):
                first_array = startai_backend.astype(first_array, startai_backend.float64)
            if not startai_backend.is_native_array(first_array):
            ...
            if test_flags.generate_frontend_arrays:
                ...
                startai_backend.is_native_array(first_array)
                and startai_backend.inplace_arrays_supported()
            ):
                assert first_array is ret_.startai_array.data
            elif startai_backend.is_startai_array(first_array):
                assert first_array.data is ret_.startai_array.data
        ...
        if not test_values:
            ret = startai_backend.nested_map(
                _frontend_array_to_startai, ret, include_derived={"tuple": True}
            )

    # create frontend framework args
    frontend_config = get_frontend_config(frontend)
    args_frontend = startai.nested_map(
        lambda x: (
    ...
    # change startai device to native devices
...
def test_gradient_backend_computation(
    ...
    with BackendHandler.update_backend(backend_to_test) as startai_backend:
        ...
        def _grad_fn(all_args):
            ...
            call_fn = startai_backend.__dict__[fn] if isinstance(fn, str) else fn[i]
            ...
            return startai_backend.nested_map(startai_backend.mean, ret, include_derived=True)

        with startai_backend.PreciseMode(test_flags.precision_mode):
            _, grads = startai_backend.execute_with_gradients(
                _grad_fn,
                [args, kwargs, 0],
                xs_grad_idxs=xs_grad_idxs,
                ret_grad_idxs=ret_grad_idxs,
...
def test_method_backend_computation(
    ...
    init_input_dtypes = startai.default(init_input_dtypes, [])

    # Constructor arguments #
    init_all_as_kwargs_np = startai.default(init_all_as_kwargs_np, {})
    # split the arguments into their positional and keyword components
    ...
    with BackendHandler.update_backend(backend_to_test) as startai_backend:
        init_flags.as_variable = [
            v if startai_backend.is_float_dtype(d) else False
    ...
    # method arguments #
    method_input_dtypes = startai.default(method_input_dtypes, [])
    ...
    # Run testing
    with BackendHandler.update_backend(backend_to_test) as startai_backend:
        ins = startai_backend.__dict__[class_name](*args_constructor, **kwargs_constructor)
        # TODO remove when the handle_method can properly compute unsupported dtypes
        if any(
            dtype
            in startai_backend.function_unsupported_dtypes(
            ...
            return
        v_np = None
        if isinstance(ins, startai_backend.Module):
            if init_with_v:
                v = startai_backend.Container(
                    ins._create_variables(
                        device=on_device, dtype=method_input_dtypes[0]
                    )
                )
                ins = startai_backend.__dict__[class_name](
                    *args_constructor, **kwargs_constructor, v=v
                )
            v = ins.__getattribute__("v")
            v_np = v.cont_map(
                lambda x, kc: startai_backend.to_numpy(x) if startai_backend.is_array(x) else x
            )
            if method_with_v:
        ...
        if isinstance(ret, startai_backend.Array):
            ret_device = startai_backend.dev(ret)
        else:
            ret_device = None
...
def test_method_ground_truth_computation(
    ...
    with BackendHandler.update_backend(ground_truth_backend) as gt_backend:
        ...
        assert gt_backend.nested_map(
            lambda x: gt_backend.is_startai_array(x) if gt_backend.is_array(x) else True,
            ret_from_gt,
        ), f"Ground-truth method returned non-startai arrays: {ret_from_gt}"
...
def test_method(
    *,
    init_input_dtypes: Optional[List[startai.Dtype]] = None,
    method_input_dtypes: Optional[List[startai.Dtype]] = None,
    ...
    init_as_variable_flags
        ...
        should be treated as an startai.Array.
    ...
    method_as_variable_flags
        ...
        be treated as an startai.Array.
    ...
    method_container_flags
        dictates whether the corresponding input argument passed to the method should
        be treated as an startai Container.
    method_all_as_kwargs_np:
    ...
    init_with_v
        if the class being tested is an startai.Module, then setting this flag as True will
    ...
    #     and not any(startai.is_complex_dtype(d) for d in method_input_dtypes)
    # ):
    #     if fw in fw_list:
...
def test_frontend_method(
    *,
    init_input_dtypes: Union[startai.Dtype, List[startai.Dtype]] = None,
    method_input_dtypes: Union[startai.Dtype, List[startai.Dtype]],
    init_flags,
    method_flags,
    init_all_as_kwargs_np: Optional[dict] = None,
    ...
    init_as_variable_flags
        dictates whether the corresponding input argument passed to the constructor
        should be treated as an startai.Variable.
    init_num_positional_args
        number of input arguments that must be passed as positional arguments to the
        constructor.
    init_native_array_flags
    ...
    with BackendHandler.update_backend(backend_to_test) as startai_backend:
        if frontend == "jax":
            importlib.import_module("startai.functional.frontends.jax").config.update(
                "jax_enable_x64", True
            )
        ...
        args_constructor_startai, kwargs_constructor_startai = startai_backend.args_to_startai(
            *args_constructor, **kwargs_constructor
        )
        args_method_startai, kwargs_method_startai = startai_backend.args_to_startai(
            *args_method, **kwargs_method
        )

        args_constructor_np = startai_backend.nested_map(
            lambda x: (
                startai_backend.to_numpy(x._data) if isinstance(x, startai_backend.Array) else x
            ),
            args_constructor_startai,
            shallow=False,
        )
        kwargs_constructor_np = startai_backend.nested_map(
            lambda x: (
                startai_backend.to_numpy(x._data) if isinstance(x, startai_backend.Array) else x
            ),
            kwargs_constructor_startai,
            shallow=False,
        )
        args_method_np = startai_backend.nested_map(
            lambda x: (
                startai_backend.to_numpy(x._data) if isinstance(x, startai_backend.Array) else x
            ),
            args_method_startai,
        ...
        kwargs_method_np = startai_backend.nested_map(
            lambda x: (
                startai_backend.to_numpy(x._data) if isinstance(x, startai_backend.Array) else x
            ),
            kwargs_method_startai,
            shallow=False,
        )

        frontend_fw_module = startai_backend.utils.dynamic_import.import_module(
            frontend_method_data.startai_init_module
        )
        startai_frontend_creation_fn = getattr(
            frontend_fw_module, frontend_method_data.init_name
        )

        ins = startai_frontend_creation_fn(*args_constructor, **kwargs_constructor)
        ...
        if method_flags.inplace:
            ...
            copy_ins = startai_frontend_creation_fn(*args_constructor, **kwargs_constructor)
    ...
    frontend_config = get_frontend_config(frontend)
    args_constructor_frontend = startai.nested_map(
        lambda x: frontend_config.native_array(x) if isinstance(x, np.ndarray) else x,
        args_constructor_np,
        shallow=False,
    )
    kwargs_constructor_frontend = startai.nested_map(
        lambda x: frontend_config.native_array(x) if isinstance(x, np.ndarray) else x,
        kwargs_constructor_np,
        shallow=False,
    )
    args_method_frontend = startai.nested_map(
        lambda x: (
            frontend_config.native_array(x)
            if isinstance(x, np.ndarray)
            else (
    ...
    kwargs_method_frontend = startai.nested_map(
        lambda x: frontend_config.native_array(x) if isinstance(x, np.ndarray) else x,
        kwargs_method_np,
        shallow=False,
    )
...
def _get_nested_np_arrays(nest):
    ...
    """
    indices = startai.nested_argwhere(nest, lambda x: isinstance(x, np.ndarray))

    ret = startai.multi_index_nest(nest, indices)
...
def create_args_kwargs(
    *,
    ...
    # create args
    with BackendHandler.update_backend(backend) as startai_backend:
        args = startai_backend.copy_nest(args_np, to_mutable=False)
        startai_backend.set_nest_at_indices(
            args,
            args_idxs,
        ...
        # create kwargs
        kwargs = startai_backend.copy_nest(kwargs_np, to_mutable=False)
        startai_backend.set_nest_at_indices(
            kwargs,
            kwargs_idxs,
            test_flags.apply_flags(
...
def wrap_frontend_function_args(argument):
    ...
    with BackendHandler.update_backend(t_globals.CURRENT_FRONTEND_STR) as startai_frontend:
        if startai_frontend.nested_any(
            argument,
            lambda x: hasattr(x, "__module__")
            and x.__module__.startswith("startai.functional.frontends"),
        ):
            return startai_frontend.output_to_native_arrays(
                startai_frontend.frontend_outputs_to_startai_arrays(argument)
            )
    if startai_frontend.nested_any(argument, lambda x: isinstance(x, startai_frontend.Shape)):
...
def flatten(*, backend: str, ret):
    ...
    if not isinstance(ret, tuple):
        ret = (ret,)
    with BackendHandler.update_backend(backend) as startai_backend:
        ret_idxs = startai_backend.nested_argwhere(ret, startai_backend.is_startai_array)
        # no startai array in the returned values, which means it returned scalar
        if len(ret_idxs) == 0:
            ret_idxs = startai_backend.nested_argwhere(ret, startai_backend.isscalar)
            ret_flat = startai_backend.multi_index_nest(ret, ret_idxs)
            ret_flat = [
                startai_backend.asarray(
                    x, dtype=startai_backend.Dtype(str(np.asarray(x).dtype))
                )
                for x in ret_flat
            ]
        else:
...
def flatten_frontend(*, ret, backend: str, frontend_array_fn=None):
    ...
    if not isinstance(ret, tuple):
        ret = (ret,)
    with BackendHandler.update_backend(backend) as startai_backend:
        ret_idxs = startai_backend.nested_argwhere(ret, _is_frontend_array)
...
def flatten_frontend_fw_to_np(
    ...
    frontend_ret_idxs = startai.nested_argwhere(frontend_ret, is_native_array_func)
    if len(frontend_ret_idxs) == 0:  # handle scalars
        frontend_ret_idxs = startai.nested_argwhere(frontend_ret, isscalar_func)
        frontend_ret_flat = startai.multi_index_nest(frontend_ret, frontend_ret_idxs)
    else:
        frontend_ret_flat = startai.multi_index_nest(frontend_ret, frontend_ret_idxs)
...
def flatten_and_to_np(*, backend: str, ret):
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        ret = [startai_backend.to_numpy(x) for x in ret_flat]
    return ret
...
def flatten_frontend_to_np(*, backend: str, ret):
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        ...
        if len(ret_idxs) == 0:  # handle scalars
            ret_idxs = startai_backend.nested_argwhere(ret, startai_backend.isscalar)
            ret_flat = startai_backend.multi_index_nest(ret, ret_idxs)
            return [startai_backend.to_numpy(x) for x in ret_flat]
        else:
            ret_flat = startai_backend.multi_index_nest(ret, ret_idxs)
            return [startai_backend.to_numpy(x.startai_array) for x in ret_flat]
...
def get_ret_and_flattened_np_array(
    ...
    **kwargs,
    ...
    with BackendHandler.update_backend(backend_to_test) as startai_backend:
        with startai_backend.PreciseMode(precision_mode):
            ret = fn(*args, **kwargs)
            if test_trace or test_trace_each:
                ret = startai_backend.to_startai(ret, True)

        def map_fn(x):
            if _is_frontend_array(x):
                return x.startai_array
            elif startai_backend.is_native_array(x) or isinstance(x, np.ndarray):
                return startai_backend.to_startai(x)
            return x

        ret = startai_backend.nested_map(map_fn, ret, include_derived={"tuple": True})
...
def get_frontend_ret(
    ...
    **kwargs,
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        if test_trace or test_trace_each:
            args, kwargs = startai_backend.nested_map(
                _frontend_array_to_startai, (args, kwargs), include_derived={"tuple": True}
            )
        with startai_backend.PreciseMode(precision_mode):
...
def _get_transpiled_data_if_required(
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        if generate_frontend_arrays:
            args_for_test, kwargs_for_test = startai.nested_map(
                _frontend_array_to_startai,
                (args_for_test, kwargs_for_test),
                include_derived={"tuple": True},
            )
        else:
            args_for_test, kwargs_for_test = startai_backend.args_to_startai(
                *args_for_test, **kwargs_for_test
            )
    ...
    # compile to get startai nodes
    with BackendHandler.update_backend(backend) as startai_backend:
        traced_fn_to_startai = startai_backend.trace_graph(
            frontend_fn, to="startai", args=args_for_test, kwargs=kwargs_for_test
        )
    ...
    startai_nodes = len(traced_fn_to_startai._functions)
...
def args_to_container(array_args):
    array_args_container = startai.Container({str(k): v for k, v in enumerate(array_args)})
...
def gradient_incompatible_function(*, fn):
    return (
        not startai.supports_gradients
...
def _is_frontend_array(x):
    return hasattr(x, "startai_array")


def _frontend_array_to_startai(x):
    if _is_frontend_array(x):
        return x.startai_array
    else:
        return x
...
def args_to_frontend(
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        frontend_args = startai_backend.nested_map(
            arrays_to_frontend(backend=backend, frontend_array_fn=frontend_array_fn),
            args,
            include_derived,
            shallow=False,
        )
        frontend_kwargs = startai_backend.nested_map(
            arrays_to_frontend(backend=backend, frontend_array_fn=frontend_array_fn),
            kwargs,
            include_derived,
            shallow=False,
...
def arrays_to_frontend(backend: str, frontend_array_fn):
    with BackendHandler.update_backend(backend) as startai_backend:
        ...
        def _new_fn(x):
            if _is_frontend_array(x):
                return x
            elif startai_backend.is_array(x):
                if tuple(x.shape) == ():
                    try:
                        ret = frontend_array_fn(
                            x, dtype=startai_backend.Dtype(str(x.dtype))
                        )
                    except startai_backend.utils.exceptions.StartaiException:
                        ret = frontend_array_fn(x, dtype=startai_backend.array(x).dtype)
                else:
