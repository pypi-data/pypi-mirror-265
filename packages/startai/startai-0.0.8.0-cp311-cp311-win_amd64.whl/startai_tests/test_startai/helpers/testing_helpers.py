...
from startai_tests.test_startai.helpers.test_parameter_flags import (
    DynamicFlag,
...
from startai_tests.test_startai.helpers.available_frameworks import available_frameworks
from startai_tests.test_startai.helpers.hypothesis_helpers.dtype_helpers import (
    _dtype_kind_keys,
    _get_type_dict,
)
...
def num_positional_args_helper(fn_name, backend):
    ...
    with BackendHandler.update_backend(backend) as startai_backend:
        startai_backend.utils.dynamic_import.import_module(fn_name.rpartition(".")[0])
        for i, fn_name_key in enumerate(fn_name.split(".")):
            if i == 0:
                fn = startai_backend.__dict__[fn_name_key]
            else:
...
def _import_fn(fn_tree: str):
    ...
    ----------
    fn_tree
        Full function tree without "startai" root
    ...
    try:
        ...
        raise ImportError(
            ...
            " details, please"
            " visit:\nhttps://lets-khulnasoft.com/startai/contributing/open_tasks.html\n"
        )
...
def _get_method_supported_devices_dtypes(
    ...
    """Get supported devices and data types for a method in Startai API.
...
def _get_supported_devices_dtypes(fn_name: str, fn_module: str):
    """Get supported devices and data types for a function in Startai API.
    ...
    if fn_module == "startai.functional.frontends.numpy":
...
def handle_test(
    ...
    **_given_kwargs,
    ...
    """Test wrapper for Startai functions.
    ...
    def test_wrapper(test_fn):
        ...
        if is_hypothesis_test:
            ...
            def wrapped_test(*args, **kwargs):
                try:
                    ...
                    # exception object in with_backend is different from global Startai
                    if e.__class__.__qualname__ == "StartaiNotImplementedException":
        ...
        wrapped_test._startai_test = True
...
def handle_frontend_test(
    ...
    **_given_kwargs,
    ...
    """Test wrapper for Startai frontend functions.
    ...
    generate_frontend_arrays
        ...
        be frontend array
    """
    fn_tree = f"startai.functional.frontends.{fn_tree}"
    if aliases is not None:
        for i in range(len(aliases)):
            aliases[i] = f"startai.functional.frontends.{aliases[i]}"
    is_hypothesis_test = len(_given_kwargs) != 0
...
def handle_method(
    ...
    **_given_kwargs,
    ...
    """Test wrapper for Startai methods.
    ...
    if is_method_tree_provided:
        method_tree = f"startai.{method_tree}"
    is_hypothesis_test = len(_given_kwargs) != 0
    possible_arguments = {
        "ground_truth_backend": st.just(ground_truth_backend),
        "test_gradients": _get_runtime_flag_value(test_gradients),
...
def handle_frontend_method(
    ...
    **_given_kwargs,
    ...
    """Test wrapper for Startai frontends methods.
    ...
    startai_init_module = f"startai.functional.frontends.{init_tree[:split_index]}"
    init_name = init_tree[split_index + 1 :]
    init_tree = f"startai.functional.frontends.{init_tree}"
    is_hypothesis_test = len(_given_kwargs) != 0
    ...
    if os.path.isfile(file_name):
        with open(file_name, "r") as outfile:
            ...
            if not is_backend:
                ...
                for key in ["startai_nodes", "fw_time"]:
