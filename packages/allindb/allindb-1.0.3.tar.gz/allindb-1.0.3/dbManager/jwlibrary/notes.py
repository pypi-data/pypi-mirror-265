...
import os
import re
from .utils import askForSec as ask
...
def mergeAndRefill(conn: Connection, conn2: Connection, outpath, close):
    "Merge Notes from JW Library and reconstruct to .jwlibrary"
    session = createSession(conn, "Note")
    session2 = createSession(conn2, "Note")
    file = ask("output filename", lambda x: re.search(r"[^A-Za-z0-9_\-\\]", x))
