...
def cleanUp(b, gotRels=None, condition=lambda a: False):
    for a in b:
        if condition(a) or (gotRels and not bool(inRels(a.key, gotRels))):
            b.remove(a)


def generator(tables, relations, structure: Structure = None):
    cleanUp(relations, structure.getRels(0) if structure else None)
    for table in tables:
        ...
        def search(table, rels):
            ...
            for col in table.__table__.columns:
                if (not structure or col.name in structure.getOwn()) and len(
                    col.foreign_keys
                ) == 0:
            ...
            def getSearch(res2, pastLevel=0, isList: bool = False):
                if res2 is None:
                    raise IndexError()
                gotRels = structure.getRels(pastLevel) if structure else None
            ...
            for col in [x.key for x in relations]:
                ...
                try:
                    ...
                    pastLevel = 1
                    irr = inRels(col, structure.getRels(0)) if structure else None
