...
config = ServiceConfig(name="my-service", applications=[{"import_path": "main:app"}])
anyscale.service.deploy(config)
"""
...
class ServiceVersionStatus(ModelBase):
    ...
    state: Union[str, ServiceVersionState] = field(
        metadata={"docstring": "Current state of the service version."}
    )

    def _validate_state(
        self, state: Union[str, ServiceVersionState]
    ) -> ServiceVersionState:
    ...
    config: Union[Dict, ServiceConfig] = field(
...
class ServiceStatus(ModelBase):
    ...
    state: Union[str, ServiceState] = field(
        metadata={"docstring": "Current state of the service."}
    )

    def _validate_state(self, state: Union[str, ServiceState]) -> ServiceState:
        if isinstance(state, str):
            # This will raise a ValueError if the state is unrecognized.
