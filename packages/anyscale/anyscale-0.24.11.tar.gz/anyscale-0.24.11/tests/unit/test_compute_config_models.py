...
class TestHeadNodeConfig:
    ...
    def test_invalid_advanced_instance_config(self):
        with pytest.raises(
            ...
            HeadNodeConfig(advanced_instance_config=["foobar"])
    ...
    def test_options(self):
        ...
        for option, val in options.items():
            assert config.options(**{option: val}) == HeadNodeConfig(**{option: val})

        # Test setting fields all at once.
        assert config.options(**options) == HeadNodeConfig(**options)
...
class TestComputeConfig:
    ...
    def test_invalid_max_resources(self):
        ...
        with pytest.raises(TypeError, match="'max_resources' keys must be strings"):
            ComputeConfig(max_resources={123: 1})

    def test_invalid_head_node(self):
        ComputeConfig(head_node=HeadNodeConfig())
