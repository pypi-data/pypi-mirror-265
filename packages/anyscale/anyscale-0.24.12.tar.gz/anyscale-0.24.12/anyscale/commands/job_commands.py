...
def submit(  # noqa: PLR0912
    ...
    $ anyscale job submit config.yaml # Specify the job config as a positional argument.

    # anyscale job submit --config config.yaml  # Use the --config-file flag to specify the job config.

    # anyscale job submit --config config.yaml --name my-job # Override arguments in the config file.

    $ anyscale job submit -- python main.py # Submit from an inlined entrypoint without a config file.
    ...
    if len(entrypoint) == 1 and entrypoint[0].endswith(".yaml"):
        ...
        log.info("Submitting job from config file {entrypoint_config_file}.")
        ...
        if config_file is not None:
            config = JobConfig.from_yaml(config_file, override=args)
        else:
