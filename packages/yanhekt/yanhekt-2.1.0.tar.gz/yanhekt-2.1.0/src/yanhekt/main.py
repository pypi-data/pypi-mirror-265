'''
Project      :
FilePath     : \PypiPkg\src\yanhekt\main.py
Descripttion :
Author       : GDDG08
Date         : 2022-11-08 02:07:44
LastEditors  : GDDG08
LastEditTime : 2024-04-02 18:22:18
'''
import requests
        ...
        if args.vga or args.dual:
            if 'vga' in videos:  # 检查是否存在vga链接
                if args.skip and os.path.exists(f"{dirName}/{fileName}-VGA.mp4"):
                    ...
                    print("VGA -->")
                    M3u8Download(videos['vga'], dirName, fileName + '-VGA', max_workers=args.max_workers)
            else:
        ...
        if args.video or args.dual:
            if 'main' in videos:  # 检查是否存在main链接
                if args.skip and os.path.exists(f"{dirName}/{fileName}-Video.mp4"):
                    ...
                    print("Video -->")
                    M3u8Download(videos['main'], dirName, fileName + '-Video', max_workers=args.max_workers)
            else:
