...
class Nav(object):
    ...
    @staticmethod
    def create_leo_navblock(tle:TLE, sat_clock:Clock) -> str:
        """
        ...
        try:
            ...
            logger.warning(f'Could not infer constellation from [ {tle.label} ]')
            return ""

        constellation = rnxid.get_constellation_letter()
        satid = rnxid.get_sat_number(tle.id)
        out = f'> EPH {constellation}{satid:05d}\n'

        # EPOCH - CLK epoch
        out = out + tle.toe.strftime('    %Y %m %d %H %M %S')
        out = out + f'{sat_clock.bias_s:19.12e}{sat_clock.drift_s_per_s:19.12e}{sat_clock.drift_rate_s_per_s2:19.12e}\n'
        ...
        out = out + create_orbit_line(adot, crs, delta_n0, M0)
        ...
        out = out + create_orbit_line(cuc, e, cus, sqrta)
        ...
        cis = 0.0
        out = out + create_orbit_line(void, cic, OMEGA0, cis)
        ...
        OMEGA_DOT = 0.0
        out = out + create_orbit_line(i0, crc, omega, OMEGA_DOT)

        # ORBIT - 5
        idot = 0.0
        delta_n_dot = tle.n_dot_rad_per_s
        out = out + create_orbit_line(idot, delta_n_dot, void, void)
