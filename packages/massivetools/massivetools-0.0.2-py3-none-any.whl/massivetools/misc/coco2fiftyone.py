...
coco = COCO('/home/mim/project/CVutils/train_coco/labels.json')
preds_path = None
...
# image 정규화
def min_max(bbox ):
    x, w = [x / 1280 for x in bbox[::2]]
    y, h = [x / 720 for x in bbox[1::2]]
    return [x, y, w, h]
...
for img_id in tqdm(coco.getImgIds()):
    ...
    sample = fo.Sample(filepath='/home/mim/project/CVutils/train_coco/data/' +
                       filename)
    pattern = r'/C[^/]+/'

    # 경로에서 정규 표현식에 해당하는 부분 검색
    match = re.search(pattern, filename)
    # sample['video_name'] = match.group(0)
    detections = []
