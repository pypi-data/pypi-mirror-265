    ...
    def insert_link_contact_group_with_groups_local(
        ...
        for group_num, group in enumerate(groups):
            ...
            if not group_id:
                ...
                # i.e. If we have in the 1st block "Penetration Tests (PT)" We should create group "Penetration Tests" and add abbreviation/synonym "PT".
                # If not, can we such private function and call it from process_first_block_phrase() and process_organization_name()?
                group_dict = self.__create_group_dict(
                    group_title=group,
                    parent_group_id=parent_group_id, is_interest=is_interest, image=image)
                data_json_compare = {
                    "title": group_dict.get("title"),
                    "lang_code": title_lang_code.value if title_lang_code else LangCode.detect_lang_code_str(groups[group_num]),
                }
