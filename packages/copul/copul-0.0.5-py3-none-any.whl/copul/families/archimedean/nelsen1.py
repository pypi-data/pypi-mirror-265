...
from copul.families.other.frechet import IndependenceCopula, LowerFrechet
...
class Clayton(ArchimedeanCopula):
    ...
    def inv_generator(self):
        ind = sympy.Piecewise((1, (self.y < -1 / self.theta) | (self.theta > 0)), (0, True))
    ...
    def cond_distr_2(self):
        ...
        u = self.u
        theta = self.theta
        cond_distr = sympy.Heaviside((-1 + v ** (-theta) + u ** (-theta)) ** (-1 / theta)) / (
            v
            * v**theta
            * (-1 + v ** (-theta) + u ** (-theta))
