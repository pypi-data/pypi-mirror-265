...
from copul.families.other.frechet import IndependenceCopula, LowerFrechet, UpperFrechet
...
class Gaussian(EllipticalCopula):
    ...
    def cond_distr_1(self):
        ...
        return lambda u, v: norm.cdf(norm.ppf(v), loc=self.rho * norm.ppf(u), scale=scale)

    def cond_distr_2(self):
        scale = sympy.sqrt(1 - self.rho**2)
        return lambda u, v: norm.cdf(norm.ppf(u), loc=self.rho * norm.ppf(v), scale=scale)
...
if __name__ == "__main__":
    gauss = Gaussian(rho=0.5)
    cdf = gauss.cdf()(0.3, 0.4)
    pdf = gauss.pdf()(0.3, 0.4)
    cd1 = gauss.cond_distr_1()(0.3, 0.4)
    print("Done!")
