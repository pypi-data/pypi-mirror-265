...
class SchurOrderVerifier:
    ...
    def verify(self, range_min=None, range_max=None):
        ...
        for theta in thetas:
            ...
            rearranged_ccop = SchurRearranger().rearrange_checkerboard(my_ccop)
        ...
        # for i, j in np.ndindex(my_ccop.shape):
        #     my_ccop[i, j] = round(16*my_ccop[i, j], 1)
        # print(sympy.latex(my_ccop))
        # print(sympy.latex(rearranged_ccop))
        # print(sympy.latex(reversed_cols))
        # print(sympy.latex(cond_dens))
        # print(sympy.latex(cond_distr))
        ...
        for i in range(len(cond_distributions) - 1):
            ...
            if not positively_ordered and not self._is_pointwise_lower_equal(
                ...
                counterexample = [
                    f"{i} {j}, diff: {larger_cop[i, j] - smaller_cop[i, j]}"
                    for i, j in itertools.product(range(larger_cop.rows), range(larger_cop.cols))
