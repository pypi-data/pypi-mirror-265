...
class multivariate_laplace_gen(multi_rv_generic):
    ...
    def __call__(self, mean=None, cov=1, allow_singular=False, seed=None):
        return multivariate_laplace_frozen(mean, cov, allow_singular=allow_singular, seed=seed)
    ...
    def rvs(self, mean=None, cov=1, size=1, random_state=None):
        ...
        # Standard laplace
        x = random_state.laplace(loc=0.0, scale=1.0, size=final_shape).reshape(-1, mean.shape[0])
        dim, mean, cov = self._process_parameters(None, mean, cov)
        (u, s, v) = svd(cov)
...
if __name__ == "__main__":
    laplace = Laplace(rho=0.5)
    cdf = laplace.cdf()(0.3, 0.4)
    pdf = laplace.pdf()(0.3, 0.4)
    cd1 = laplace.cond_distr_1()(0.3, 0.4)
    print("Done!")
