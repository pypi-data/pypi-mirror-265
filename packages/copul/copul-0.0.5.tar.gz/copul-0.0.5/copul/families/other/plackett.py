...
from copul.families.other.frechet import LowerFrechet
...
class Plackett(AbstractCopula):
    ...
    def spearmans_rho(self):
        return (self.theta + 1) / (self.theta - 1) - 4 * self.theta * sympy.log(self.theta) / (
            self.theta - 1
        ) ** 2
    ...
    def cond_distr_1(self) -> SymPyFunctionWrapper:
        ...
        cond_distr_1 = (
            theta
            - (-2 * theta * v * (theta - 1) + (2 * theta - 2) * ((theta - 1) * (u + v) + 1) / 2)
            / sympy.sqrt(-4 * theta * u * v * (theta - 1) + ((theta - 1) * (u + v) + 1) ** 2)
            - 1
        ) / (2 * (theta - 1))
