...
def download_template_model(windmill_client: WindmillClient, model_store_name: str, model_name: str, output_uri: str):
    """
    Download template model.
    """
    model_object_name = get_model_name(parse_modelstore_name(model_store_name).workspace_id,
                                       parse_modelstore_name(model_store_name).local_name,
                                       model_name)

    logit.info(f"Download model {model_object_name} to {output_uri}")
    windmill_client.download_artifact(object_name=model_object_name, version="latest", output_uri=output_uri)
