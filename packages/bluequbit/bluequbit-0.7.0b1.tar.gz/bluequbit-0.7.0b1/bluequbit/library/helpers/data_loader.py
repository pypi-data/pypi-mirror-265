    ...
    def adjoint_kl(params):
        ...
        for op in reversed(ops):
            ...
            if op.num_params != 0:
                ...
                use_async = False
                dev_gpu._gpu_state.DeviceToDevice(dev_ket._gpu_state, use_async)
                dev_gpu.apply([_qu])
                dm = (
                    -2
                    * bra_norm
                    * dev_gpu._gpu_state.dotWithBraReal(dev_bra._gpu_state)
                )
                # A more reliable way to reset _gpu_state
                # TODO raise a bug report to Pennylane
                dev_gpu._gpu_state = _gpu_dtype(complex_dtype)(dev_gpu._num_local_wires)
                grads.append(dm)
