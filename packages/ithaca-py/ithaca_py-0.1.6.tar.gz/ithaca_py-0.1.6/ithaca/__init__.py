...
class IthacaSDK:
    ...
    def post(self, endpoint, json=None):
        ...
        res = self.session.post(self.base_url + endpoint, json=json, verify=verify, timeout=30)
        try:
            return res.json()
        except requests.JSONDecodeError as e:
            logging.error(f"JSON decode error for POST endpoint {endpoint}, res: {res}, {e}")
            return res
    ...
    def get(self, endpoint):
        ...
        res = self.session.get(self.base_url + endpoint, params=headers, verify=verify, timeout=30)
        try:
            return res.json()
        except requests.JSONDecodeError as e:
            logging.error(f"JSON decode error for GET endpoint {endpoint}, res: {res}, {e}")
            return res
        except Exception as e:
