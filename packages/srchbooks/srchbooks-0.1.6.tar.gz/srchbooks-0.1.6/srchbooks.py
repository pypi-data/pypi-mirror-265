...
def get_sql(args):
    query = args.terms
    queries = ["{}:{}^{}".format("title", "\"" + query + "\"", 1.0)]
    ...
    if filter_type == 'all':
        sql = prepare_sql(
            "select {} title ,author ,year ,extension ,filesize ,ipfs_cid ,language ,publisher from library.ebook where query_string_recency(%s) ".format(
                highlight), [query_str])
    else:
        query_str = [query_str, filter_type]
        sql = prepare_sql(
            "select {} title ,author ,year ,extension ,filesize ,ipfs_cid ,language ,publisher from library.ebook where query_string_recency(%s) and extension=%s ".format(
                highlight), query_str)
...
def main():
    ...
    for row in rst['result']:
        ...
        content = Tree(row["title"]["value"].replace("<mark>", "[red]").replace("</mark>", "[/red]"))
