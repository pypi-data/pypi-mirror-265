...
class DsTcResnetQatNBitTest(tf.test.TestCase):
  """Test ds_tc_resnet model in non streaming and streaming modes."""

  def setUp(self):
    super(DsTcResnetQatNBitTest, self).setUp()
    ...
    self.params.use_quantize_nbit = 1
    self.params.nbit_activation_bits = 8
    # TODO(guynich) change nbit_weight_bits to 4 when TFLite conversion works.
    self.params.nbit_weight_bits = 6
    self.params.nbit_8bit_until_block = 1
    self.params.nbit_8bit_last = 1
