# PYTHONPATH=$(pwd) python tools/ft_train.py
import numpy as np
import random
import sys
import time
...
from quant.io.logging import setup_logging, print_log
...
def train(model, device, train_loader, criterion, optimizer, epoch, log_interval):
    ...
    for batch_idx, (data, target) in enumerate(train_loader, 1):
        ...
        if batch_idx % log_interval == 0:
            print_log(
                f"Train Epoch: {epoch} [{batch_idx}/{dataloader_size}] Loss: {loss.item():.6f}")
...
def main(config):
    ...
    print_log("[best model]")
    print_log(f"\n{best_acc=:.4f}, {best_epoch=:03d}\n")

    print_log("[check train dataset]")
...
config = dict(
    data=dict(
        ...
        data_root="/workspace/xlab/datasets/table20240326_03281154_torch_norm",
