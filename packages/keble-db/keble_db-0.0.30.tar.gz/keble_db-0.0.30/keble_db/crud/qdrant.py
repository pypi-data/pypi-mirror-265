...
class QdrantCRUDBase(Generic[ModelType, VectorModelType]):
    ...
    def search(self, client: QdrantClient, *, vector: List[float], vector_key: str, query: Optional[QueryBase] = None,
        ...
        return [Record(**sp.model_dump(include={"score", "shard_key", "id"}), payload=self.model(**sp.payload) if sp.payload is not None else None, vector=self.vector_model(**sp.vector) if sp.vector is not None is not None else None) for sp in
                scored_points]
