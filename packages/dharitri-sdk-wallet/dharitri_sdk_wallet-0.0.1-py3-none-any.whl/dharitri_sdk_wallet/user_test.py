...
import pytest
from dharitri_sdk_core import (Address, AddressConverter, MessageV1,
                                 Transaction)

from dharitri_sdk_wallet.crypto.randomness import Randomness
from dharitri_sdk_wallet.user_keys import UserSecretKey
...
def test_user_signer_from_pem_file():
    ...
    pubkey = UserSigner.from_pem_file(Path("./dharitri_sdk_wallet/testdata/alice.pem"), 0).get_pubkey()
    assert converter.pubkey_to_bech32(pubkey.buffer) == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"

    pubkey = UserSigner.from_pem_file(Path("./dharitri_sdk_wallet/testdata/bob.pem"), 0).get_pubkey()
    assert converter.pubkey_to_bech32(pubkey.buffer) == "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk"

    pubkey = UserSigner.from_pem_file(Path("./dharitri_sdk_wallet/testdata/carol.pem"), 0).get_pubkey()
    assert converter.pubkey_to_bech32(pubkey.buffer) == "moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h"
...
def test_user_wallet_to_keyfile_object_using_known_test_wallets_with_their_randomness():
    ...
    alice_saved_path = Path("./dharitri_sdk_wallet/testdata/alice.saved.json")
    bob_saved_path = Path("./dharitri_sdk_wallet/testdata/bob.saved.json")
    carol_saved_path = Path("./dharitri_sdk_wallet/testdata/carol.saved.json")

    alice_wallet.save(alice_saved_path, "moa")
    bob_wallet.save(bob_saved_path, "moa")
    carol_wallet.save(carol_saved_path, "moa")

    assert alice_saved_path.read_text().strip() == Path("./dharitri_sdk_wallet/testdata/alice.json").read_text().strip()
    assert bob_saved_path.read_text().strip() == Path("./dharitri_sdk_wallet/testdata/bob.json").read_text().strip()
    assert carol_saved_path.read_text().strip() == Path("./dharitri_sdk_wallet/testdata/carol.json").read_text().strip()
...
def test_sign_transaction():
    """
    Also see: https://github.com/Dharitri-org/drt-chain-go/blob/master/examples/construction_test.go
    """
    ...
    signer = UserSigner.from_pem_file(Path("./dharitri_sdk_wallet/testdata/alice.pem"))
    verifier = UserVerifier.from_address(Address.from_bech32("moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"))
...
def test_user_pem_save():
    path = Path("./dharitri_sdk_wallet/testdata/alice.pem")
...
def test_load_secret_key_but_without_kind_field():
    keystore_path = Path("./dharitri_sdk_wallet/testdata/withoutKind.json")
    secret_key = UserWallet.load_secret_key(keystore_path, "password")
    actual_address = Address(secret_key.generate_public_key().buffer, "moa").bech32()
...
def test_load_secret_key_with_unecessary_address_index():
    keystore_path = Path("./dharitri_sdk_wallet/testdata/alice.json")
...
def test_create_keystore_with_mnemonic_with_randomness():
    expected_dummy_wallet_json = Path("./dharitri_sdk_wallet/testdata/withDummyMnemonic.json").read_text()
...
def test_load_secret_key_with_mnemonic():
    keystore_path = Path("./dharitri_sdk_wallet/testdata/withDummyMnemonic.json")

    assert UserWallet.load_secret_key(keystore_path, "password", 0).generate_public_key().to_address("moa").bech32() == "moa1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssfq94h8"
    assert UserWallet.load_secret_key(keystore_path, "password", 1).generate_public_key().to_address("moa").bech32() == "moa1spyavw0956vq68xj8y4tenjpq2wd5a9p2c6j8gsz7ztyrnpxrruq0yu4wk"
    assert UserWallet.load_secret_key(keystore_path, "password", 2).generate_public_key().to_address("moa").bech32() == "moa1k2s324ww2g0yj38qn2ch2jwctdy8mnfxep94q9arncc6xecg3xaqhr5l9h"
