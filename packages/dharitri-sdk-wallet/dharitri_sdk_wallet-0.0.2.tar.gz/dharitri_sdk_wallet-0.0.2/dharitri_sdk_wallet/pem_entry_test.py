...
from dharitri_sdk_wallet.constants import USER_SEED_LENGTH
from dharitri_sdk_wallet.pem_entry import PemEntry


def test_from_text_all():
    text = Path("./dharitri_sdk_wallet/testdata/alice.pem").read_text()
    ...
    text = Path("./dharitri_sdk_wallet/testdata/multipleUserKeys.pem").read_text()
    entries = PemEntry.from_text_all(text)
    entry = entries[0]
...
def test_from_text_all_for_validators():
    text = Path("./dharitri_sdk_wallet/testdata/validatorKey00.pem").read_text()
    ...
    text = Path("./dharitri_sdk_wallet/testdata/multipleValidatorKeys.pem").read_text()
    entries = PemEntry.from_text_all(text)
