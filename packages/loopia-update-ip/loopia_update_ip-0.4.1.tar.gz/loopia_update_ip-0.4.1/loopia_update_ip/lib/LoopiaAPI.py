...
class LoopiaAPI:
    ...
    def add_zone_record(self, ip: str, subdomain: str, ttl: int = 3600, priority: int = 0) -> str:
        """
        Add a new zone record for a subdomain
        :param ip: IP address for zone record of A-type
        ...
        :return:  Loopia API endpoint status report
        """
        record_object = LoopiaAPI.create_record_dict(record_type="A",
                                                     ttl=ttl,
                                                     priority=priority,
                                                     rdata=ip,
                                                     record_id=0)
    ...
    def update_zone_record(self, ip: str, subdomain: str, ttl=3600) -> str:
        """
        Update zone record for subdomain. I.e. only a single record is supported!
        :param ip: New IP address of zone record
    ...
    def remove_zone_records(self, subdomain: str) -> List[str]:
        """
    ...
    def get_zone_record_ip_address(self, subdomain: str) -> str:
        """
        Get current IP address from zone record for subdomain. Note: Only a single record is supported!
        :param subdomain: The subdomain for which to update the zone record
        ...
        # Get IP address from current zone record
        zone_record = record_list[0]
        zone_record_ip_address = zone_record['rdata']

        return zone_record_ip_address
