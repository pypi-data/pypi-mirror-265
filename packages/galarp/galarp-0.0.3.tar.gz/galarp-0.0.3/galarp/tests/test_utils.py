...
class TestRotate:
    ...
    def test_rotate(self):
        ...
        rotated = rotate(vec, alpha=np.pi/4, beta=np.pi/4, gamma=np.pi/4)
    ...
    def test_rotate_about_axis(self):
        ...
        rotated_x0 = rotate(np.array([zero_vec, rand, rand]), alpha=np.pi/4)
        assert np.max(np.abs(rotated_x0[0])) < 1e-12

        rotated_y0 = rotate(np.array([rand, zero_vec, rand]), beta=np.pi/4)
        assert np.max(np.abs(rotated_y0[1])) < 1e-12

        rotated_z0 = rotate(np.array([rand, rand, zero_vec]), gamma=np.pi/4)
