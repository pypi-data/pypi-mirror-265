...
from ..exc import raise_detailed_error, convert_list_to_pyre, convert_to_pyre, RequiredError
...
class Database:
    """ Database Service """
    def __init__(self, credentials: Union[ServiceAccountCredentials, None], api_key: str, database_url: str, requests: Union[Session, httpx.Client]):
    ...
    def get(self, token=None, json_kwargs={}):
        ...
        # do request
        #print(request_ref); exit()
        request_object = self.requests.get(request_ref, headers=headers, timeout=30)
    ...
    def push(self, data, token=None, json_kwargs={}):
        ...
        self.path = ""
        headers = self.build_headers(token)
        request_object = self.requests.post(request_ref, headers=headers, data=json.dumps(data, **json_kwargs).encode("utf-8"), timeout=30)
        raise_detailed_error(request_object)
    ...
    def set(self, data, token=None, json_kwargs={}):
        ...
        self.path = ""
        headers = self.build_headers(token)
        request_object = self.requests.put(request_ref, headers=headers, data=json.dumps(data, **json_kwargs).encode("utf-8"), timeout=30)
        raise_detailed_error(request_object)
        return request_object.json()
    ...
    def update(self, data, token=None, json_kwargs={}):
        ...
        self.path = ""
        headers = self.build_headers(token)
        request_object = self.requests.patch(request_ref, headers=headers, data=json.dumps(data, **json_kwargs).encode("utf-8"), timeout=30)
        raise_detailed_error(request_object)
        return request_object.json()
    ...
    def remove(self, token=None):
        ...
        self.path = ""
        headers = self.build_headers(token)
        request_object = self.requests.delete(request_ref, headers=headers, timeout=30)
        raise_detailed_error(request_object)
        return request_object.json()
