...
def cli(log: str, patchflow: str, opts: list[str], config: str | None, output: str | None, data_format: str):
    patchflow_path = Path(__file__).parent / "patchflows"
    try:
        module = importlib.import_module(f".{patchflow_path.stem}", "patchwork")
    ...
    try:
        ...
        logger.debug(f"Patchflow {patchflow} not found as a class in {patchflow_path}")
        exit(1)
