...
def quick_strangle_buy(
    ...
    notification_url: Optional[str] = None,
    strategy_tag: Optional[str] = "Quick Strangle Buy",
):
    # Entering the main function
    ...
    while current_time().time() < time(*scan_exit_time):
        ...
        above_strangle_prices = above_strangle.fetch_ltp(for_type="BUY")
        below_strangle_price = below_strangle.fetch_ltp(for_type="BUY")
        ...
        cheapest_strangle = min(ivs, key=ivs.get)
        lowest_iv = ivs[cheapest_strangle]
        total_price = np.sum(cheapest_strangle.fetch_ltp(for_type="BUY"))
        logger.info(
            f"{underlying.name} lowest IV is {lowest_iv} for {cheapest_strangle} "
            f"with total price {total_price}"
        )

        if lowest_iv <= iv_threshold:
            logger.info(
                f"{underlying.name} {strategy_tag} buy triggered because {lowest_iv} <= {iv_threshold}. "
                f"Initiating trade on {cheapest_strangle}"
            )

            # Calculation of quantity
            shares_to_buy = investment / total_price
            shares_to_buy = round_shares_to_lot_size(shares_to_buy, underlying.lot_size)
            ...
            entry_price = call_avg_price + put_avg_price
            logger.info(
                f"Entered {underlying.name} {cheapest_strangle} with avg price {call_avg_price + put_avg_price}"
            )
            trade_triggered = True
            target_strangle = cheapest_strangle
            break
    ...
    if trade_triggered:
        while current_time().time() < time(*trade_exit_time):
            current_call_price, current_put_price = target_strangle.fetch_ltp()
            total_current_price = current_call_price + current_put_price
            logger.info(
                f"{underlying.name} {strategy_tag} current price: {total_current_price}. "
                f"Target price: {entry_price * (1 + take_profit)}"
            )
            if (current_call_price + current_put_price) >= entry_price * (
                1 + take_profit
            ):
                logger.info(
