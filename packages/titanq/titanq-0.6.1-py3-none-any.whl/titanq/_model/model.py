...
from .numpy_util import reshape_to_2d
...
class Model:
    ...
    def add_set_partitioning_constraint(self, constraint_mask: np.ndarray):
        """
        Adds set partitioning constraints vector to the model.
        ...
        self._constraints.add_constraint(
            constraint_mask=reshape_to_2d(constraint_mask),
            cardinalities=None
        )
    ...
    def add_cardinality_constraints_matrix(self, constraint_mask: np.ndarray, cardinalities: np.ndarray):
        ...
        :raise MissingVariableError: If no variable have been added to the model.
        :raise ValueError: If the constraint_mask shape or the constraint_rhs shape does not fit the expected shape of this model.
    ...
    def add_cardinality_constraint(self, constraint_mask: np.ndarray, cardinality: int):
        ...
        :type cardinality:  non-zero unsigned integer
        ...
        if constraint_mask.ndim > 1:
            raise ValueError(
                "Cannot use this add_cardinality_constraints_matrix() function with a matrix, " \
                "please use add_cardinality_constraint() insead")
