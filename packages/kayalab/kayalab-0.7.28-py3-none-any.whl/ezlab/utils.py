...
def get_new_vm_name(name_pattern: str, count: int):
    ...
    multivm = count > 0  # only for single VM we will get 0
    lastchar = name_pattern[-1]
    if lastchar.isdigit():
        count = int(lastchar) + count - 1  # if input is vm33, we would start from vm34, so reducing 1
        newname = name_pattern[:-1]

    return newname + str(count if multivm else "")
