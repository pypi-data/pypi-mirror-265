...
def bridges(proxmox, fromtemplatename):
    ...
    try:
        template = [t for t in vms(proxmox) if t["template"] and t["name"] == fromtemplatename].pop()
    except IndexError:
        return []

    return proxmox.nodes(template["node"]).network.get(type="any_bridge") if template else []
...
def clone(
    ...
    response[
        "task"
    ] = f"""Clone {vm_name} with disks in volume: {volume["storage"]}:
    ...
    if first_ip == "dhcp":
        ...
        vm_ip = str(ip_address(first_ip) + vm_number - 1 if multivm else ip_address(first_ip))  # adjust for single VM
