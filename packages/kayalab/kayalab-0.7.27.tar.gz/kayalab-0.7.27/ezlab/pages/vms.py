    ...
    with ui.dialog() as dialog, ui.card():
        ...
        bridge_selector = (
            ui.select(
                options=[
                    x["iface"]
                    for x in pve.bridges(ezapp.connection, app.storage.general[PVE]["template"])
                ],
                label="Bridge",
            )
            .classes("w-full")
...
async def new_vm_ui():
    ...
    if resources and all(resources):
        ...
        for count in range(vmcount):
            try:
                result = await run.io_bound(clone,
                ...
                if dryrun:
                    ...
                    logger.info("[ %s ] ready: %s", resources[4] + str(count+1), result)
