...
from ezlab.utils import get_doc
...
async def prepare_action():
    ...
    for host in hosts:
        result = await run.io_bound(prepare,
            hostname=host["name"],
            hostip=host["ip"],
...
def prepare_menu():
    ...
    with ui.row().classes("w-full items-center items-stretch") as prep_view:
        ...
        with ui.row():
            ...
            ui.button(icon="add_box", color=None, on_click=add_host).props(
                "flat"
            )
            ui.button(icon="delete", color=None, on_click=remove_host).props(
                "flat"
            )
...
async def install_action(hosts_string: str):
    ...
    result = await run.io_bound(cluster_install,
...
async def client_action():
    ...
    result = await run.io_bound(client_install,
