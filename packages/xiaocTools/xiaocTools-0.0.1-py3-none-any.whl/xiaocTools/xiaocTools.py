...
def get_today_last_datetime():
    """
        获取今天最后一秒的datetime
    """
    lastToday = get_today_zero_datetime() + datetime.timedelta(hours=23, minutes=59, seconds=59)
    return lastToday
...
def get_today_zero_timestamp():
    ...
    now = datetime.datetime.now()
    zeroToday = now - datetime.timedelta(hours=now.hour, minutes=now.minute, seconds=now.second, microseconds=now.microsecond)
    return datetime_2_unixtime(zeroToday)
...
def get_today_last_datetime():
    ...
    """
    lastToday = get_today_zero_datetime() + datetime.timedelta(hours=23, minutes=59, seconds=59)
    return datetime_2_unixtime(lastToday)
...
def get_html(url, headers=None, encode=None, maxError=3, timeout=10):
    error = 0
...
def version_code_compare(version1, way, version2):
