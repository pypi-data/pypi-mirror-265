        ...
        <div v-for="(item, index) in items" :key="item._links.view" v-else>
          <a class="dropdown-item text-default p-2" :href="item._links.view">
            ...
            <div class="d-flex justify-content-between">
              <strong class="font-title wb-break-all elevated" :title="item.title">
                {{ kadi.utils.truncate(item.title, 50) }}
              </strong>
              <div class="d-none d-xl-block">
                <span class="badge badge-light border font-weight-normal ml-3">{{ item.pretty_type }}</span>
              </div>
            </div>
            <span class="font-identifier wb-break-all">@{{ item.identifier }}</span>
            <br>
