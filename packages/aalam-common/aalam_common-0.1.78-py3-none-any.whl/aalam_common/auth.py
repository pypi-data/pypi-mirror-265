...
class HTTPAuthenticationFailed(webob.exc.HTTPUnauthorized):
    ...
    def __init__(self, *args, **kwargs):
        super(HTTPAuthenticationFailed, self).__init__(*args, **kwargs)
        super(HTTPAuthenticationFailed, self).delete_cookie(AUTH_COOKIE_NAME)
...
class Auth(wsgi.Middleware):
    ...
    def _validate_user_signature(self, path, val, ts, signature, retry=False):
        ret = False
        try:
        ...
        if not ret and retry:
            ...
            if (time.time() - st.st_mtime) > (5*60):  # more than 5 minutes
                ...
                ret = self._validate_user_signature(path, val, ts, signature)

        if ret:
            return ret

        raise HTTPAuthenticationFailed()
