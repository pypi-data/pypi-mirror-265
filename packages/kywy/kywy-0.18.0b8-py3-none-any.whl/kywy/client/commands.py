...
class KawaCommands:
    ...
    def replace_user_role(self, user_id, new_role):
        user_tech_id = self._k.entities.principals().find_first_entity_by_attribute('uniqueId', user_id)
        return self._run_command(command_name='ReplaceUserRole',
                                 command_parameters={
                                     'principalId': self._entity_id(user_tech_id),
                                     'role': new_role
                                 })

    def change_user_status(self, user_id, active=True):
        user_tech_id = self._k.entities.principals().find_first_entity_by_attribute('uniqueId', user_id)
    ...
    def replace_access_policy_for_datasource(self, datasource, user_id, access_policy):
        user_tech_id = self._k.entities.principals().find_first_entity_by_attribute('uniqueId', user_id)
    ...
    def assign_user_to_perimeters(self, user_id, perimeter_names):
        principal = self._k.entities.principals().find_first_entity_by_attribute('uniqueId', user_id)
        return self._run_command(command_name='AssignPrincipalToPerimeters',
                                 command_parameters={
                                     'principalEntityId': self._entity_id(principal),
                                     'memberOfPerimeterNames': perimeter_names or []
                                 })

    def create_users(self, user_id_list):
    ...
    def change_user_password(self, email, new_password):
        ...
        return self._run_command(command_name='AdminChangeUserPassword',
                                 command_parameters={
                                     'principalId': self._entity_id(user_tech_id[0]),
    ...
    def toggle_users_status(self, user_id_list, active=False):
        user_tech_id = self._k.entities.principals().find_entities_by_attribute('uniqueId', user_id_list)
        for user in user_tech_id:
            return self._run_command(command_name='ReplaceUserStatus',
                                     command_parameters={
                                         'principalId': self._entity_id(user),
                                         'active': active
                                     })

    def replace_forbidden_data_types(self, user_id_list, forbidden_types):
        user_tech_id = self._k.entities.principals().find_entities_by_attribute('uniqueId', user_id_list)
        for user in user_tech_id:
            return self._run_command(command_name='ReplaceForbiddenDataTypes',
                                     command_parameters={
                                         'principalId': self._entity_id(user),
    ...
    def add_user_to_white_list(self, email_address):
        return self._run_command(command_name='AddUserToWhiteList',
                                 command_parameters={
                                     'userId': email_address.lower(),
                                 })

    def remove_user_from_white_list(self, email_address):
        return self._run_command(command_name='RemoveUserFromWhiteList',
                                 command_parameters={
                                     'userId': email_address.lower(),
                                 })

    def create_users_in_workspace(self, workspace_name, user_list):
        """
        Create users, create the workspace if it does not exist and put the users
        in the workspace
        :param workspace_name: The workspace name to create/load
