...
import numpy as np
from tutils.new.utils.core_utils import _tfilename, _tprint
...
class CSVLogger(object):
    ...
    def __init__(self, logdir, name="record.csv", mode="w"):
        ...
        self.filename = _tfilename(self.logdir, name)
        self.mode = mode
        ...
        if mode == "w":
            if os.path.isfile(self.filename):
                ...
                _tprint(f"[CSVLogger] backup excel file `{self.filename}` to `{backup_name}`")
        elif mode == "a+":
            _tprint("[CSVLogger] Add data on existing CSV file!")
            if os.path.isfile(self.filename):
...
class ExcelLogger(object):
    def __init__(self, logdir, name="record.xls"):
        ...
        if os.path.isfile(self.filename):
            ...
            shutil.move(self.filename, backup_name)
            _tprint(f"[ExcelLogger] backup excel file `{self.filename}` to `{backup_name}`")
    ...
    def save(self):
        ...
        filename = self.fname
        filename = _tfilename(self.logdir, filename)
