...
from ... import async_hooks, exceptions, h, types
...
def bootstrap(
    ...
    pg_hba = manifest.pg_hba(instance.version, settings).splitlines()
    pg_ident = manifest.pg_ident(instance.version, settings).splitlines()
    return dict(
...
async def postgresql(
    ...
    **args: Any,
    ...
    if "pg_hba" not in args:
        args["pg_hba"] = manifest.pg_hba(instance.version, settings).splitlines()
    if "pg_ident" not in args:
        args["pg_ident"] = manifest.pg_ident(instance.version, settings).splitlines()
