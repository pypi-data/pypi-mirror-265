...
class Instance(CompositeModel):
    ...
    version: Annotated[
        Optional[pgs.PostgreSQLVersion],
        Field(
    ...
    def pg_hba(self, version: str, settings: s.Settings) -> str:
        ...
        return postgresql.template(version, "pg_hba.conf").format(
            auth=auth,
            surole=surole.name,
    ...
    def pg_ident(self, version: str, settings: s.Settings) -> str:
        surole = self.surole(settings)
        replrole = self.replrole(settings)
        replrole_name = replrole.name if replrole else None
        return postgresql.template(version, "pg_ident.conf").format(
            surole=surole.name,
            backuprole=settings.postgresql.backuprole.name,
            replrole=replrole_name,
