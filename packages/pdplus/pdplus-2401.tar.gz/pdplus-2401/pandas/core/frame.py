...
class DataFrame(NDFrame, OpsMixin):
    ...
    #     # lambda df,str1:sqldf(str1,locals())
    #     from pandasql import sqldf
    #     this=self
    #     return sqldf(str1,locals())
    #
    # # wangnz v2
    # def sql(
    #         self,
    ...
    def sql(
        ...
        if str1 is None:
            return duckdb.from_df(self.reset_index())

        kwargs['self']=self.reset_index()
        for k, v in kwargs.items():
            exec (f"{k}=v")
        return duckdb.sql(str1)
...
"""
    ...
    def resample(
        ...
        group_keys: bool | lib.NoDefault = no_default,
    ) -> Resampler | Series | DataFrame | None:
