    ...
    async for chunk in runnable:
        ...
        if operation:
            if type(operation) == str:
                ...
                if filled_chunk:
                    yield token_stream(filled_chunk, redactions=redactions)
            else:
                ...
                if generations:
                    ...
                    if tool:
                        yield token_stream(tool, 'tool', redactions=redactions)
                    ...
                    if args:
                        ...
                        yield token_stream(f"Invoking: `{tool}` with `{tool_args}`", 'log', redactions=redactions)
    yield token_stream()
