...
class C_Widgits():
    ...
    def section(self, section_parent, Title=None):
...
###############################################################################################################################################################################################
    ...
    def frame_template(self, title):     # Parent frame to use when calling the add tab func
        parent_f = ctk.CTkFrame(self.parent, fg_color="transparent")

        title = ctk.CTkLabel(parent_f, text=f"{title}", font=(FONT_B, 25), fg_color="transparent", text_color=(LIGHT_MODE["text"], DARK_MODE["text"]), anchor="w")
    ...
    def add_tab(self, frame, text=None, image=None, size=None, image_size=250): # (frame >> frame_template, image input is of a normal image)
        if image != None:        
            im = Image.open(image)
        ...
        if size == "l":
            self.large(frame, text, im_ctk)
        elif size == "s":
            self.small(frame, text, im_ctk)
