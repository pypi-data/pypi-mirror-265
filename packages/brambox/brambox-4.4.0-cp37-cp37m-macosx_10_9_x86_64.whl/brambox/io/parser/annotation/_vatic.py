...
class VaticParser(AnnotationParser):
    ...
    def serialize(self, row):
        idval = int(row.id) if not np.isnan(row.id) else -1
        occ = int(row.occluded >= self.occluded_cutoff)
        return (
            f'{idval} {row.x_top_left} {row.y_top_left} {row.x_top_left + row.width} {row.y_top_left + row.height} '
            f'{row.image} {int(row.lost)} {occ} 0 "{row.class_label}"'
        )
