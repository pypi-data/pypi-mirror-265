...
class Camera:
    ...
    def detect_object(self,myt,color_area,round_square_threshold,big_small_round_threshold, times=10):
        ...
        print("本次识别结果为：",str)
        if(target_object['shape']==target_object['target_shape'] and target_object["target_color"]==target_object["shape"]):
...
#识别物体颜色
    def detect_object_color(self,times,target:int):
        rs=[]
            ...
            for i in range(3):
                ...
                for c in cnts:
                    ...
                    if area>target:
                        ...
                        if(center_x>40 and center_x<280 and center_y>30 and center_y<260):
                            print("颜色面积:",area)
    ...
    #识别物体形状
    def detect_object_shape(self,color_flag,round_square_threshold,big_small_round_threshold,times=10,target_area=5000):
        rs=[]
        for i in range(0,times):
            ...
            for c in cnts:
                ...
                if area>target_area:
                    ...
                    cv2.putText(img,"Point:"+str(len(approx)),(x+w+10,y+h+10),cv2.FONT_HERSHEY_COMPLEX,0.7,(0,255,0),2)
                    print("面积",area)
                    print("百分比:",per)
                    #print("点数",len(approx))
                    if(area<=big_small_round_threshold):
                        cv2.circle(img,(int((minx+maxx)/2),int((miny+maxy)/2)),int((maxy-miny)/2),(255,0,0),3)
