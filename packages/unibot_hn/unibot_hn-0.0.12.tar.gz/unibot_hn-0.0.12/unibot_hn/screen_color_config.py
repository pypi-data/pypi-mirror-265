...
from unibot_hn.unibot import Unibot

robotB = Unibot()
chassis = robotB.chassis
camera = robotB.camera
robotic_arm = robotB.robotic_arm
gripper = robotB.gripper
conf=Device_Config('./unibot/robot_param_init.txt')
flag=0
colorflag=0
lh = 43
ls = 64
...
def obj_onmouse(event, x, y, flags, param):  # 标准鼠标交互函数
...
def screen_onmouse(event, x, y, flags, param):  # 标准鼠标交互函数
    global lh,ls,lv,uh,us,uv,conf,flag,colorflag
    if event == cv2.EVENT_LBUTTONDOWN:  # 当鼠标移动时
        if(x<100 and y>250  and x>0 and y<320):
            #红色
        ...
        if(x<480 and y>240  and x>320 and y<340):
            ...
            print(colorflag)
            if(colorflag==0):
                lls=conf.readconfsectionvalue("P_RED")
            if(colorflag==1):
                lls=conf.readconfsectionvalue("P_GREEN")
            if(colorflag==2):
                lls=conf.readconfsectionvalue("P_YELLOW")
            cv2.setTrackbarPos('L_min', 'camera', lls[0])
            cv2.setTrackbarPos('L_max', 'camera', lls[3])
            cv2.setTrackbarPos('A_min', 'camera', lls[1])
            cv2.setTrackbarPos('A_max', 'camera', lls[4])
        ...
        if(x<480 and y>0  and x>320 and y<100):
            print(lh,uh,ls,us,lv,uv)
            if(colorflag==0):
                conf.setconf('P_RED','lh',lh)
                conf.setconf('P_RED','ls',ls)
                conf.setconf('P_RED','lv',lv)
                conf.setconf('P_RED','uh',uh)
                conf.setconf('P_RED','us',us)
                conf.setconf('P_RED','uv',uv)
            if(colorflag==1):
                conf.setconf('P_GREEN','lh',lh)
                conf.setconf('P_GREEN','ls',ls)
                conf.setconf('P_GREEN','lv',lv)
                conf.setconf('P_GREEN','uh',uh)
                conf.setconf('P_GREEN','us',us)
                conf.setconf('P_GREEN','uv',uv)
            if(colorflag==2):
                conf.setconf('P_YELLOW','lh',lh)
                conf.setconf('P_YELLOW','ls',ls)
                conf.setconf('P_YELLOW','lv',lv)
                conf.setconf('P_YELLOW','uh',uh)
                conf.setconf('P_YELLOW','us',us)
                conf.setconf('P_YELLOW','uv',uv)
            
        print(x, y)  # 显示鼠标所在像素坐标和数值，注意像素表示方法和坐标位置的不同
def screen_detect_config():
    cv2.setMouseCallback("camera", screen_onmouse)  # 回调绑定窗口
    height_max = 320
    width_min = 480
...
def obj_detect_config():
    cv2.setMouseCallback("camera", obj_onmouse)  # 回调绑定窗口
    height_max = 320
    width_min = 480
    canvas1 = np.zeros((height_max, width_min,3), dtype=np.uint8)
    canvas2 = np.zeros((height_max, width_min), dtype=np.uint8)
