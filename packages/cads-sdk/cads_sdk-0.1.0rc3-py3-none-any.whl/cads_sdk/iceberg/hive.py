...
from cads_sdk.iceberg.catalog import (
    EXTERNAL_TABLE,
    ICEBERG,
    LOCATION,
    METADATA_LOCATION,
...
from cads_sdk.iceberg.io import FileIO, load_file_io
from pyiceberg.partitioning import UNPARTITIONED_PARTITION_SPEC, PartitionSpec
from pyiceberg.schema import Schema, SchemaVisitor, visit
from pyiceberg.serializers import FromInputFile
from cads_sdk.iceberg.table import Table
from pyiceberg.table import CommitTableRequest, CommitTableResponse, TableProperties, update_table_metadata
...
class _HiveClient:
    ...
    def __init__(self, uri: str, ugi: Optional[str] = None):
        ...
        def sasl_factory():
            from cads_sdk.conf import HADOOP_HOST
            if HADOOP_HOST:
                parsed_url = urlparse(HADOOP_HOST)
                host = parsed_url.netloc
                return PureSASLClient(host=host, service="hive")
            else:
                raise Exception("There is no HADOOP_HOST in environment variable")

        self._transport = TSaslClientTransport(sasl_factory, "GSSAPI", socket)
    ...
    def __enter__(self) -> Client:
        try:
            ...
            raise Exception("You are not login kerberos yet, run kinit to login")
        if self._ugi:
