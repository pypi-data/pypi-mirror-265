...
from cads_sdk.iceberg.io import (
...
class PyArrowFile(InputFile, OutputFile):
    ...
    def parse_uri(uri):
        ...
        host_port = parsed_uri.netloc.split(':')
        host = host_port[0]
        port = host_port[1] if len(host_port) > 1 else None

        # Extract routes
        routes = parsed_uri.path
    ...
    def _file_info(self) -> FileInfo:
        ...
        """
        try:
            host, port, _path = PyArrowFile.parse_uri(self._path)
            file_info = self._filesystem.get_file_info(_path)
        except OSError as e:
