...
int main(int argc, const char *argv[])
{
    ...
    const std::vector<ltd_topology_f_0_integral::nested_series_t<ltd_topology_f_0_integral::cuda_integrand_t>> sector_integrands =
        ltd_topology_f_0_integral::make_cuda_integrands(real_parameters, complex_parameters);

    // Add integrands of sectors (together flag)
    std::cerr << "Summing integrands" << std::endl;
    const ltd_topology_f_0_integral::nested_series_t<ltd_topology_f_0_integral::cuda_together_integrand_t> all_sectors =
        std::accumulate(++sector_integrands.begin(), sector_integrands.end(), ltd_topology_f_0_integral::cuda_together_integrand_t()+*sector_integrands.begin());

    // Integrate
    std::cerr << "Integrating" << std::endl;
    secdecutil::integrators::Qmc<
                                    ltd_topology_f_0_integral::integrand_return_t,
                                    ltd_topology_f_0_integral::maximal_number_of_integration_variables,
                                    integrators::transforms::Korobov<3>::type,
                                    ltd_topology_f_0_integral::cuda_together_integrand_t
                                > integrator;
