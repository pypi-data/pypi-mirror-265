...
class NDLLM(LLM):
    ...
    def invoke(
        ...
        **kwargs,
        ...
        if not isinstance(prompt_template, NDPromptTemplate) or not isinstance(
        ...
        if not best_llm:
            ...
            if best_llm is None:
                raise ApiError(
                    "ND couldn't find a suitable model to call. To avoid disruptions, we recommend setting a default fallback model or make max depth larger."
                )
        ...
        try:
            ...
            if (
                ...
                print(
                    "If you see this message, means the NotDiamond API returned a Google model as the best option, but the LLM call will fail. So we will automatically fall back to a non-Google model, if possible."
                )
    ...
    def _llm_from_provider(provider: NDLLMProvider) -> Any:
        ...
        if provider.provider == "cohere":
            return ChatCohere(
                cohere_api_key=provider.api_key, model=provider.model, **provider.kwargs
            )
        ...
        if provider.provider == "togetherai":
            ...
            print(f"MODEL: {model}")
            return Together(
                together_api_key=provider.api_key, model=model, **provider.kwargs
            )
    ...
    def _prepare_prompt_template(
        ...
        if isinstance(prompt_template, NDPromptTemplate):
            return prompt_template
        if isinstance(prompt_template, NDChatPromptTemplate):
            return prompt_template
