...
class JeffersonStreetBaseIngestDag():
    ...
    def run(self):
        ...
        with DAG(self.name, default_args=dag_default_args.to_dict(), schedule_interval=self.schedule_interval, catchup=False) as dag:
            volume_mount = k8s.V1VolumeMount(name='persist-pods-disk-volume',
                           mount_path='/files',
                           sub_path=None,
                           read_only=False)
            volume = k8s.V1Volume(name='persist-pods-disk-volume', ephemeral=k8s.V1EphemeralVolumeSource(claim_name="ephemeral-deployment-6b69b4d57f-xpwxq-ephemeral-volume"))
