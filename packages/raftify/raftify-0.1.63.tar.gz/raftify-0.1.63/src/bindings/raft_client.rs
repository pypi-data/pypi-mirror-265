...
use raftify::{create_client, raft_service, Channel, RaftServiceClient};
...
impl PyRaftServiceClient {
    ...
    pub fn join_cluster<'a>(&'a mut self, tickets: &PyList, py: Python<'a>) -> PyResult<&'a PyAny> {
        let mut client = self.inner.clone();
        let tickets = tickets
            .iter()
            .map(|ticket| {
                let ticket = ticket.extract::<PyClusterJoinTicket>().unwrap().inner;
                let peers = ticket
                    .peers
                    .iter()
                    .map(|(k, v)| raft_service::Peer {
                        node_id: *k,
                        addr: v.to_string(),
                    })
                    .collect::<Vec<_>>();

                raft_service::ClusterJoinTicket {
                    reserved_id: ticket.reserved_id,
                    raft_addr: ticket.raft_addr,
                    leader_addr: ticket.leader_addr,
                    peers: Some(raft_service::Peers { peers }),
                }
            })
            .collect::<Vec<_>>();
        ...
        future_into_py(py, async move {
            let _ = client
                .join_cluster(raft_service::ClusterJoinTickets { tickets })
                .await
                .unwrap()
                .into_inner();
