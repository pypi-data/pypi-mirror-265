...
from os import system as mkdir, getcwd as pwd, chdir, unlink, chmod
...
"""
    ...
    def _make(self, content: str, outfile: str, _setup: str, _setupargs=(), sha256=False, compress='gz', quiet=False, label=None, extension='.dsfx'):
        """Create SFX
        ...
        _copy_package_files(_pkg, content, _setup, in_content)
        _tar, sha256_sum = _archive_package(_pkg, compress, sha256)
        self._midfile = self._pkg_to_exe(_tar, outfile, _setup, _setupargs, in_content, sha256_sum, label)
    ...
    def _pkg_to_exe(self, _tar: str, outfile: str, _setup: str, _setupargs: tuple, in_content: bool, sha256_sum: str, label: str):
...
def _copy_package_files(_pkg: str, _install_src: str, _setup: str, in_content: bool):
    if not there(_pkg):
        mkdir(_pkg)
...
def _archive_package(_pkg: str, compress: str, sha256: bool):
...
def cli():
    color()
    # define shortargs
    shortargs = ['i', 'o', 's', 'c', 'l', 'e', 'cs', 'q', 'v', 'h']
    # define longargs
    longargs = ['infile', 'outfile', 'setup', 'compress-method', 'label', 'extension', 'checksum', 'quiet', 'version', 'help']
    # get option control
    ...
    sfxCTRL = _makesfx()
    # call class method
    sfxCTRL._make(infile, outfile, setup, setupargs, checksum, compress_method, quiet, label, extension)
