...
class MarketHandle():
    ...
    def fin_save_ohlc_list(self, instrument_id: str, range: str, df, compress: str = 'xz', level: int = -1, pack: str = 'msgpack') -> Tuple[bool, str]:
        ...
        b, m, params = self.__create_fin_persists_save_param_data(instrument_id, range, df, compress, level, pack)
        if b == False:
            return [b, m]
    ...
    def fin_save_basetick_list(self, instrument_id, df, compress='zip', level: int = -1, pack: str = 'msgpack') -> Tuple[bool, str]:
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]
