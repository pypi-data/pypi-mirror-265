...
class MakeSvgInstance:
    ...
    def __init__(
            ...
            unknown_time_first_object: bool = False,
            unknown_time_second_object: bool = False
    ):

        self.unknown_time_first_object = unknown_time_first_object
        self.unknown_time_second_object = unknown_time_second_object
    ...
    def __makeHouses(self, r):
        ...
        for i in range(xr):
            ...
            linecolor = self.colors_settings['houses_radix_line'] if not self.unknown_time_first_object else 'white'
            ...
            if self.chart_type == "Transit" or self.chart_type == "Composite":
                ...
                t_linecolor = self.colors_settings['houses_radix_line'] if not self.unknown_time_second_object else 'white'
                text_k = 25
                text_x_k = -3
                text_y_k = +6
                ...
                path_opacity = "0" if self.chart_type == "Transit" or self.unknown_time_second_object else ".4"
                ...
                text_opacity = "0" if self.chart_type == "Transit" or self.unknown_time_second_object else ".8"
            ...
            path_data = f"M {x1},{y1} L {x2},{y2} A {r-roff},{r-roff} 0 0,0 {x4},{y4} L {x3},{y3} A {r-dropin},{r-dropin} 0 0,1 {x1},{y1} Z"
            line_opacity = "1" if self.unknown_time_first_object else ".4"
            text_opacity = "0" if self.unknown_time_first_object else ".8"
            path = path + f'<path d="{path_data}" class="house" id="{house_by_index[i+1]} House" fill="white" style="stroke: {linecolor}; stroke-width: 1px; stroke-opacity:{line_opacity}; stroke-linejoin:{stroke_linejoin}"/>'
            path = path + f'<text class="houseLabel" id="{house_by_index[i+1]} House Label" style="fill: #000000; fill-opacity: ' + text_opacity + '; font-size: .6em; font-family: Cinzel; font-weight: bold"><tspan x="' + str(xtext) + '" y="' + str(ytext) + '">' + str(house_number) + '</tspan></text>'
        return path
    ...
    def makeTemplate(self):
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            td['transitRingAxes'] = self.__makeAxis(r, r-self.c1+54.5, self.t_user.axis_list, 9.7, 'black', transit_axis, composite_axis) if not self.unknown_time_second_object else ''
            ...
            td['makePatterns'] = ''
            td['makeAxis'] = self.__makeAxis(r, r-self.c1+18, self.user.axis_list, 8.4, '#ff0000', False, False) if not self.unknown_time_first_object else ''
            ...
            td['makeAxis'] = self.__makeAxis(r, (r-self.c1+30), self.user.axis_list, 8.4, '#ff0000', False, False) if not self.unknown_time_first_object else ''
