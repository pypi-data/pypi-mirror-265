...
// Class used to acquire Attribute level lock (only in ATTR_BY_KERNEL mode)
// Used to protect the attribute buffer in multi-threaded environment
...
public:
    AttributeMonitor(Tango::Attribute* attr){
        lock = attr->get_attr_mutex();
        serial_model = attr->get_attr_serial_model();
