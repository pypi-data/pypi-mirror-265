...
namespace PyAttribute
{
    ...
    inline void set_value(Tango::Attribute &att, Tango::EncodedAttribute *data)
    {
        if(data == NULL)
        {
            ...
            description << "Data pointer for attribute " << att.get_name() << " is NULL! Most probably set_value was called with None ";

            TangoSys_OMemStream origin;
            origin << TANGO_EXCEPTION_ORIGIN << std::ends;

            Tango::Except::throw_exception(
                    "PyDs_AttributeDataIsNULL",
                    description.str(),
                    origin.str());
        }
        ...
        if( *f==NULL )
        {
            ...
            Tango::Except::throw_exception(
                    "PyDs_DevEncodedFormatNotSpecified",
                    o.str(),
                    "set_value()");
        }
        ...
        if( size==0 || !d )
        {
            ...
            Tango::Except::throw_exception(
                    "PyDs_DevEncodedDataNotSpecified",
                    o.str(),
                    "set_value()");
        }
