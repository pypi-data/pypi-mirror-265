...
class FilePathUtils:
    '''文件路径工具类'''
    @staticmethod
    def get_directory_path(directory: str,packageName: str = Constants.DEMO_PACKAGE_NAME) -> str:
        directory = os.path.normpath(directory)  # 规范化路径
        if os.path.exists(directory):  # 开发模式
            return directory
        else:  # 安装后运行
            return resource_filename(packageName, directory)
        
    @staticmethod
    def get_filenames(directory: str,packageName: str = Constants.DEMO_PACKAGE_NAME) -> List[str]:
        '''
        获取指定目录下的所有文件名
        '''
        directory = FilePathUtils.get_directory_path(directory, packageName)
        if os.path.exists(directory):  # 开发模式
            filenames = os.listdir(directory)
        else:  # 安装后运行
            filenames = resource_listdir(Constants.DEMO_PACKAGE_NAME, directory)
        return filenames