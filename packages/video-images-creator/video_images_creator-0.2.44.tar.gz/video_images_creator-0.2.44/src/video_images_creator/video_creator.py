...
def synthesize_speech_with_ssml(intro_text, feature_descriptions, subscription_key, service_region, output_file):
    """
...
def build_v3(feature_images_urls, feature_names, feature_descriptions, intro_text, subscription_key, service_region):
    ...
    directory_name = f'images/{folder_name}'
    os.mkdir(directory_name)
    tts_output_filename = f'{directory_name}/tts_outfile_name'
    tts_output_2_channel_filename = f'{directory_name}/tts_2_chnl_outfile_name'
    ...
    result_code, message = synthesize_speech_with_ssml(intro_text, feature_desciption_stripped, subscription_key, service_region, tts_output_filename)
        ...
        with open(temp_text_file, 'w') as file:
            for image in image_files:
                ...
                if "_feature_frames" in image:
                    ...
                    seconds = math.ceil( len(feature_desciption_) * 0.0645 ) 
...
def run_ffmpeg_v4(directory_name, uniq_code, temp_text_file, tts_output_2_channel_filename, total_duration):
    ...
    delay_for_thanks = ( floor( total_duration - 2.0 ) ) * 1000
    ...
    "-safe", "0",     # Allow use of absolute paths
    "-i", temp_text_file,  # Specify the input text file
    "-i", tts_output_2_channel_filename,
    "-i", "thanks.wav",
    "-filter_complex", f"[2:a]adelay={delay_for_thanks}|{delay_for_thanks}[a2];[1:a][a2]amix=inputs=2:duration=longest[aout]",
    "-map", "0:v",  # Map video from the first input (image sequence)
    "-map", "[aout]",  # Map the audio output from the filtergraph
