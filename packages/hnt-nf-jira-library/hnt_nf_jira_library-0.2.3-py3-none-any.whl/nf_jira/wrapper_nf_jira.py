    ...
    def _get_nf_jira(self, issue_id: str):
        try:
            ...
            attachment["data_procmto"] = (
                form_data["state"]["answers"]["14"]["date"]
                if "14" in form_data["state"]["answers"]
                else attachment.get("data_procmto")
            )
            attachment["hora_procmto"] = (
                form_data["state"]["answers"]["15"]["time"]
                if "15" in form_data["state"]["answers"]
                else attachment.get("hora_procmto")
            )
            attachment[nf_type]["DataLeituraAnterior"] = (
                form_data["state"]["answers"]["16"]["date"]
                if "16" in form_data["state"]["answers"]
