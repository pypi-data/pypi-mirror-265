...
def _is_arm_or_gait(exp_id: int) -> str:
    ...
    search = lambda arm_or_gait: github.listdir(
        "SimiPixel", "ikarus", "main", f"dataset/{arm_or_gait}/exp{exp_id}"
    )
    if len(search("arm")) > 0:
        return "arm"
...
def _load_data(exp_id: int, motion: str):
    ...
    downloader = lambda file: github.download(
        path + file, "SimiPixel", "ikarus", "main"
    )
