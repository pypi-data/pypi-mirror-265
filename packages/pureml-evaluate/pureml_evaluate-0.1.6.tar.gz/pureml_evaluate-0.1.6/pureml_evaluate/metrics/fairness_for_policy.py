...
class Fairness(BaseModel):
    ...
    def calculate_group_metric(self, metric_func, y_true, y_pred, sensitive_features):
        ...
        if isinstance(sensitive_features, (list, np.ndarray)):
            if isinstance(sensitive_features, list) and all(isinstance(i, dict) for i in sensitive_features):
                # If it's a list of dictionaries, directly create a DataFrame
                sensitive_features = pd.DataFrame(sensitive_features)
            elif isinstance(sensitive_features, np.ndarray) and sensitive_features.dtype == object and isinstance(sensitive_features[0], dict):
                ...
                sensitive_features = pd.DataFrame(sensitive_features, columns=['feature'])
            elif isinstance(sensitive_features, np.ndarray) and sensitive_features.ndim == 2:
                # If it's a 2D ndarray, convert it to a DataFrame with multiple columns
                sensitive_features = pd.DataFrame(sensitive_features, columns=[f'feature_{i}' for i in range(sensitive_features.shape[1])])
        ...
        if isinstance(sensitive_features, pd.DataFrame):
            ...
            raise TypeError("sensitive_features must be a list, a numpy array, or a pandas DataFrame.")
    ...
    def predictive_value_parity(self, y_true, y_pred, sensitive_features):
        ...
        precisions = self.calculate_group_metric(metric_func = precision_score, y_true = y_true, y_pred = y_pred, sensitive_features= sensitive_features)
        return precisions
    ...
    def false_positive_parity(self, y_true, y_pred, sensitive_features):
        ...
        fpr_rates = self.calculate_group_metric(metric_func=fpr, y_true = y_true, y_pred = y_pred, sensitive_features=sensitive_features)
        return fpr_rates
    ...
    def true_positive_parity(self, y_true, y_pred, sensitive_features):
        ...
        group_tpr_rates = self.calculate_group_metric(metric_func= recall_score, y_true=y_true, y_pred = y_pred, sensitive_features=sensitive_features)
    ...
    def disparate_impact(self, y_pred, sensitive_features, y_true):
        ...
        if isinstance(sensitive_features, pd.DataFrame):
            sensitive_features = sensitive_features.apply(lambda row: '_'.join(row.values.astype(str)), axis=1)
        elif isinstance(sensitive_features, list) and all(isinstance(i, dict) for i in sensitive_features):
            sensitive_features = pd.Series(['_'.join(str(val) for val in item.values()) for item in sensitive_features])
        ...
        for metric_name, metric_func in self.fairness_metrics.items():
            try:
                ...
                metrics[metric_name] = {'value':metric_func(
                    y_true=self.references, y_pred=self.predictions,sensitive_features=self.sensitive_features)}
            except Exception as e:
