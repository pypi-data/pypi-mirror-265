...
class FeatureLabelCorrelation(DataMetricBase):
    ...
    def compute(self, data, label_column, **kwargs):
        ...
        for column in data.columns:
            ...
            if data[column].dtype == 'object':
                pps_score = self.calculate_categorical_correlation(data[column], data[label_column])
            else:
                pps_score = self.calculate_numerical_correlation(data[column], data[label_column])
    ...
    def calculate_categorical_correlation(self, feature_column, label_column):
        feature_label_df = pd.DataFrame({'Feature': feature_column, 'Label': label_column})
        contingency_table = pd.crosstab(feature_label_df['Feature'], feature_label_df['Label'])
    ...
    def calculate_chi2(self, contingency_table):
        chi2 = contingency_table.apply(lambda r: np.sum((r - np.mean(r))**2 / np.mean(r)))
        chi2 = chi2.sum()
