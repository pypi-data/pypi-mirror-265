...
class PopulationStabilityIndex(DriftMetricBase):
        ...
        def compute(self, reference, production=None,columns = None, bins=10,**kwargs):
            ...
            if len(reference) == 0 or len(production) == 0:
                raise ValueError("Neither the reference nor the production data should be empty.")
            
            # Exception: Mismatched Shapes
            if len(reference) != len(production):
                raise ValueError("The reference and production data should have the same length for a meaningful PSI computation.")
            
            # Exception: Negative Values in Data
            if np.any(reference < 0) or np.any(production < 0):
                raise ValueError("Both datasets should have non-negative values for a meaningful PSI computation.")
            ...
            small_value = 1e-10
            psi_bins = (prop_ref - prop_prod) * np.log((prop_ref + small_value) / (prop_prod + small_value))
