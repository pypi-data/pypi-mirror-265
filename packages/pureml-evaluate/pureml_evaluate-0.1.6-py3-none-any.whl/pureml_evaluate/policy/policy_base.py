...
class PolicyBase(BaseModel):
    ...
    def compute(self, references=None, predictions=None, prediction_scores=None, sensitive_features=None,
        ...
        if type == 'performance' and references is not None and predictions is not None:
            for m in list_metrics_objects:
                try:
                    ...
                    #print(e)
                    logger.error(f"Error in computing {m} from performance metrics: {e}")
                    pass
        ...
        if kwargs.get('subset_name', 'None') != 'None':
            self.all_metrics_results = disparate_filter_metrics_result(self.all_metrics_results, keys_to_keep = kwargs.get('subset_name', 'None'))
                ...
                if category == 'fairness' and 'disparate_impact' in metrics:
                    for metric_name, metric_values in metrics.items():
                        if metric_name == 'disparate_impact':
                            filtered_metric_values = {k: v for k, v in metric_values.items() if k in keys_to_keep}
                            if filtered_metric_values:
                                new_item = {
                                    category: {
                                        metric_name: filtered_metric_values
                                    }
                                }
