...
class PositionEditor_builder(PartitionManager):
    ...
    def handleInterpolation(self, values, file_location=None):
        ...
        new_interpolated_datga = self.interpolate_with_splines(interpolation_data, M=values['images'], degree=values['degree'])
        ...
        for container_index, container in enumerate(self.containers[1:]):
            for n in range(values['images']+1):
                ...
                container_copy.AtomPositionManager.set_atomPositions_fractional( new_interpolated_datga[:,:,container_index*(values['images']+1) + n] ) 
