...
def create_material(name,  base_color, metallic=0.7, specular=0.9, roughness=0.4 ):
...
def setup_lighting():
    ...
    sun.data.energy = 1 # Adjust as needed
    sun.data.angle = math.radians(15) 

    bpy.ops.object.light_add(type='SUN', location=(0, 0, 10))
    sun = bpy.context.object
    sun.data.energy = 1 # Adjust as needed
    sun.data.angle = math.radians(165) 


    bpy.ops.object.light_add(type='SUN', location=(0, 0, 10))
    sun = bpy.context.object
    sun.data.energy = 1 # Adjust as needed
    sun.data.angle = math.radians(90) 
...
def setup_orthographic_cameras(lattice_vectors, look_at_point):
    ...
    ortho_scales = {{
        'x': np.max([np.ptp(corners, axis=0)[1], np.ptp(corners, axis=0)[2]]) * 2.0,  # Y-Z plane
        'y': np.max([np.ptp(corners, axis=0)[0], np.ptp(corners, axis=0)[2]]) * 2.0,  # X-Z plane
        'z': np.max([np.ptp(corners, axis=0)[0], np.ptp(corners, axis=0)[1]]) * 2.0,  # X-Y plane
    }}

    camera_positions = {{
        'Camera_X': (center[0] + distance, center[1], center[2]),
        'Camera_Y': (center[0], center[1] + distance, center[2]),
        'Camera_Z': (center[0], center[1], center[2] + distance),
    }}
...
def draw_lattice_edges_with_material(lattice_vectors, edge_radius=0.05, edge_color=(0.5, 0.5, 0.5, 1)):
            ...
            for container_index, container in enumerate(self.containers):
                if plot.upper() == 'XYZ':
                    blender_str = self.get_blender_str(
                        ...
                        radii = container.AtomPositionManager.atomic_radii, 
