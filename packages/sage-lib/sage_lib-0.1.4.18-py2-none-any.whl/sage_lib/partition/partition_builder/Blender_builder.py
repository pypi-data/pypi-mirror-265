...
def setup_orthographic_cameras(lattice_vectors, look_at_point):
    ...
    ortho_scales = {{
        'x': np.max([np.ptp(corners, axis=0)[1], np.ptp(corners, axis=0)[2]]) * 1.05,  # Y-Z plane
        'y': np.max([np.ptp(corners, axis=0)[0], np.ptp(corners, axis=0)[2]]) * 1.05,  # X-Z plane
        'z': np.max([np.ptp(corners, axis=0)[0], np.ptp(corners, axis=0)[1]]) * 1.05,  # X-Y plane
    }}
