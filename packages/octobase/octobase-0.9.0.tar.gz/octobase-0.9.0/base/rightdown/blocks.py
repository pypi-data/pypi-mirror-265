...
class Link(Block):
  ...
  def AsText(self):
    ...
    if self.text:
      return (self.image or '') + '[' + (self.text or '') + '](' + (self.url or '') + ')'
    if self.url:
      return '[(' + self.url + ')]'

  def PrintOpen(self, printer, first, last):
    imprints        = []
    if printer.markup and self.url:
      if self.image:
        printer.MuteUntilClose(self)
        text        = self.children and rightdown.printers.Printer().Print(self.children[0])
        if text:
          text      = base.utils.UnifyQuotes(text, "'")
          imprints.append(Imprint(IMPRINT_MARKUP, '<img alt="' + text + '" src="' + self.url + '">', inline=True))
        else:
          imprints.append(Imprint(IMPRINT_MARKUP, '<img src="' + self.url + '">', inline=True))
      else:
        imprints.append(Imprint(IMPRINT_MARKUP, '<a href="' + self.url + '">', inline=True))
    return imprints
...
class Text(Block):
  ...
  FIRST_SPECIAL_CHAR  = chr(rightdown.patterns.FIRST_SPECIAL_CHAR)
  ...
  def align(self):
    ...
    lpushr          = self.text[0] == rightdown.patterns.COMINGLE_PUSH_RIGHT.tag
    lpushl          = self.text[0] == rightdown.patterns.COMINGLE_PUSH_LEFT.tag
    rpushr          = self.text[-1] == rightdown.patterns.COMINGLE_PUSH_RIGHT.tag
    rpushl          = self.text[-1] == rightdown.patterns.COMINGLE_PUSH_LEFT.tag
    if lpushr and rpushl:
      return ALIGN_CENTER
    if lpushl and rpushr:
  ...
  def __init__(self, **kwargs):
    super().__init__(**kwargs)
    self.subchars   = self.subchars or SpecialCharGen(start=rightdown.patterns.FIRST_SPECIAL_CHAR)
    ...
    for result in self._IterChars(printer):
      ...
      if isinstance(result, Imprint):
        if chunk:
          flush     = ''.join(chunk)
          chunk     = []
          yield Imprint(imprinttype=IMPRINT_NARRATIVE, text=flush, inline=True)
        yield result
  ...
  def _IterChars(self, printer):
    ''' iterates through our raw text one character at a time, returning either that char,
        or that special char's expansion, which may be text, or may be a full imprint '''
    # one char at a time
    for c in self.text:
      # normal character?
      ...
      # comingle indicator?
      if c < self.FIRST_SPECIAL_CHAR:
        c           = self._Comingle(c, printer)
      ...
      if isinstance(c, list) or isinstance(c, Imprint):
        yield c
        continue
      ...
      if isinstance(c, Snip):
        yield c.ToString(printer)
        continue

      if isinstance(c, Block):
        yield printer.Dupe().Print(c)
        continue

      yield str(c)

  def _ResetComingles(self):
    self._strike    = False

  def _Comingle(self, c, printer):
    imprints        = []
    if c == rightdown.patterns.COMINGLE_PUSH_LEFT and printer.formatting:
      imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='<-', inline=True))
    if c == rightdown.patterns.COMINGLE_PUSH_RIGHT and printer.formatting:
      imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='->', inline=True))

    if c == rightdown.patterns.COMINGLE_STAR_3_UP:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='<b><i>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='***', inline=True))
    if c == rightdown.patterns.COMINGLE_STAR_3_DOWN:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='</i></b>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='***', inline=True))

    if c == rightdown.patterns.COMINGLE_STAR_2_UP:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='<b>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='**', inline=True))
    if c == rightdown.patterns.COMINGLE_STAR_2_DOWN:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='</b>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='**', inline=True))

    if c == rightdown.patterns.COMINGLE_STAR_1_UP:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='<i>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='*', inline=True))
    if c == rightdown.patterns.COMINGLE_STAR_1_DOWN:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='</i>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='*', inline=True))

    if c == rightdown.patterns.COMINGLE_UNDER_UP:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='<u>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='_', inline=True))
    if c == rightdown.patterns.COMINGLE_UNDER_DOWN:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='</u>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='_', inline=True))

    if c == rightdown.patterns.COMINGLE_LIGHT_UP:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='<mark>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='=', inline=True))
    if c == rightdown.patterns.COMINGLE_LIGHT_DOWN:
      if printer.markup:
        imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='</mark>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='=', inline=True))

    if c == rightdown.patterns.COMINGLE_STRIKE:
      if printer.markup:
        if self._strike:
          imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='</s>', inline=True))
        else:
          imprints.append(Imprint(imprinttype=IMPRINT_MARKUP, text='<s>', inline=True))
      if printer.formatting:
        imprints.append(Imprint(imprinttype=IMPRINT_FORMATTING, text='~', inline=True))
