...
# This is a workarround for python3
# TODO: fix it
StringType = type("a")
...
class CNF(CFGrammar):
    """ Chomsky Normal Form.
    No useless nonterminals or eepsipsilon rules are ALLOWED... Given a CFG grammar description generates one in CNF
    Then its possible to random generate words of a given size. Before some pre-calculations are needed."""
        ...
        if not m:
            ...
            if m.end() == len(rule):
                ...
                if rest == "[]":
                    ...
                    multi = str.split(rest, rulesep)
                    rhs = []
                    for i in multi:
                        l = str.split(i, rhssep)
                        if len(l) > 1:
                            l = tuple(str.split(i, rhssep))
                        else:
                            l = l[0]
                        gr.append((lhs, l))
    return gr
...
reGrammar = {'g_regular_base': ["Tr -> Tr + Tc | Tc", "Tc -> Tc Ts |  Ts",
             ...
             'g_regular_wredund': ["Tr -> Trs | Tf ", "Trs -> Trs + Tf | Tf + Tf",
                                   ...
                                   "Ts -> Te | Ti | ( Trs ) ",
                                   "Te -> ( Trs ) * | ( Tc ) * | Ti *"],
             'g_regular_uncollaps': ["Ts ->  Trs | Tcc  | Tee | Ti | {0:s} | {1:s}".format(common.Epsilon,
                                                                                           common.EmptySet),
                                     "Tcc -> Tcc Tr | Tr Tr",
                                     "Tr -> ( Trs ) | Tee | Ti ",
                                     "Tee -> ( Trs ) * | ( Tcc ) * | Ti *",
             ...
             'g_rpn_snf_option': ["Tx -> {0:s} | Tp | Tnp".format(common.Epsilon),
                                  "Tp -> . Tp Tp | + Tp Tnp | + Tnp Tp | + Tp Tp |* Tnp| {0:s} Tnp".format(
                                      common.Option),
                                  "Tnp -> Ti | + Tnp Tnp | . Tp Tnp | . Tnp Tp | . Tnp Tnp"],
             ...
             'grse_rpn': ["Tx -> {0:s} | Ti | +  Tx  Tx  | {1:s} Tx Tx | . Tx Tx  | * Tx".format(common.Epsilon,
                                                                                                 common.Shuffle)],
             ...
             'grcn_rpn': [
                 "Tx ->  Ti | +  Tx  Tx  | {0:s} Tx Tx | . Tx Tx  | {1:s} Tx | * Tx".format(common.Conj, common.Not)],
             'grche_rpn': ["Ts -> {0:s} Tx Tx".format(common.Conj),
                           "Tx ->  {0:s} |  Ti | +  Tx  Tx  | . Tx Tx  | * Tx".format(common.Epsilon)],
             'grcv_rpn': ["Tn -> Ti | + Tn Tx | + Tv Tn | * Tn | . Tn Tx | . Tv Tn | {0:s} Tn Tn".format(common.Conj),
                          "Tv -> {0:s} | + Tv Tv | * Tv | . Tv Tv | {1:s} Tv Tx | {1:s} Tx Tv".format(common.Epsilon,
                                                                                                      common.Conj),
                          "Tx -> {0:s} | Ti | +  Tx  Tx  | {1:s} Tx Tx | . Tx Tx  | * Tx".format(common.Epsilon,
                                                                                                 common.Conj)],
             'gpt_rpn': ["Tz ->  + Tz  Tf | Tf",
