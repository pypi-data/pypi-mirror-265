...
def create_threadpool_message(
    _type: str, content: Any, retry: int = 3, timeout: int = 5, hook: Callable = None
) -> ThreadPoolMessage:
    """
    ...
    """
    return ThreadPoolMessage(_type, retry, timeout, content, hook)
    ...
    def callback(
        ...
        for message in iter(input_queue.get, "##STOP##"):
            ...
            for _ in range(message.retry):
                with TimerThread(message.timeout, threading.get_ident()):
                    ...
                    try:
                        ...
                        print(f"##TIMEOUT## - {message.content}")
                    except Exception as e:
                        print(f"##EXCEPTION## - {message.content} - {e}")
            ...
            if result not in (None, 0):
                ...
                print(f"##FAILURE## - {message.content}")
                output_queue.put(("##FAILURE##", message.content))
            else:
