...
def initialize_theta_grid_points(df, theta, grid_df):
    ...
    # The np.where function returns the indices of True values in the boolean array. The output is a tuple of arrays, where the first array contains the row indices and the second array contains the corresponding column indices.
...
def impute_spots_with_theta(df, theta, grid_step=0.5, miss_spot_threshold=1, add_edge=False, sigma=0.4, phi=1, diagnosis=False):
    '''
    ...
    sigma : float, optional
        standard deviation of Gaussian kernel W. The default is 0.4.
    phi : float, optional
    ...
    if diagnosis:
        from utils import plot_imputation
        if tech_mode == 'st':
            plot_imputation(df_label_edge, impute_df, contours, hierarchy, f'imputation_stepsize_{grid_step}.png', figsize=(6.4, 4.8))
...
def do_imputation(loc_file, theta_file, spatial_file, grid_step=0.5, miss_spot_threshold=1, add_edge=False, sigma=0.4, phi=1, diagnosis=False):
    '''
...
runImputation [option][value]...
    ...
    For each specified grid step size d, there will be three output files: 1) imputed spot locations "impute_stepsize_d_spot_loc.csv", 2) imputed spot cell type proportions "impute_stepsize_d_spot_celltype_prop.csv", 3) imputed spot gene expressions (already normalized by sequencing depth of spots) "impute_stepsize_d_spot_gene_norm_exp.csv".
    
    
    --------------- imputation-related options --------------
    
    --impute_stepsize       Step size for generating grid at a higher resolution. It equals the distance of two neighboring imputed spots, given the distance of two original neighboring spatial spots is defined as 1 for ST or 2 for 10x Visium techniques. This step size must be within the range [0.1, 0.9]. Either one number or an array of numbers separated by "," are supported. Default value is 0.5.
    --hole_threshold        The minimum number of uncaptured spots required to validate a detected hole in tissue map. Holes with spots less than or equal to this threshold are ignored during imputation, treating the area as if no hole exists, i.e. we will impute smaller spots within this hole. Default value is 1, meaning single-spot holes are ignored.
    --keep_shape            During imputation, the higher-resolution tissue map may become distorted. By setting this option to true, we retain all border points, ensuring the tissue shape remains accurate, though it results in an irregular imputed grid. Default value is false.
    --diagnosis             If true, provide the scatter plot showing spatial spots and imputed spots for diagnosis. Default value is false.
''')
...
def parseOpt():
    ...
    paramdict : Dict
        parsed command line parameters for model, including:
            ...
            impute_stepsize : Step size for generating grid at a higher resolution\n
            hole_threshold : threshold of number of uncaptured spots to validate holes\n
            keep_shape : whether to preserve the exact shape of tissue map\n
    ...
    shortargs = 'hq:l:p:v'
    longargs = ['help', 'query=', 'loc=', 'prop=', 'impute_stepsize=', 'hole_threshold=', 'keep_shape=', 'diagnosis=', 'version']
    ...
    paramdict = {'spatial_file': None, 'loc_file': None, 'prop_file': None,
                 'impute_stepsize': [0.5], 'hole_threshold': 1, 'keep_shape': False, 'diagnosis': False}
    ...
    for opt,val in opts:
        ...
        if opt in ('--impute_stepsize'):
            ...
            for x in ori_list:
                try:
                    tmp_val = float(x.strip())
                    
                    if tmp_val < 0.1 or tmp_val > 0.9:
                        print(f'WARNING: option value `{tmp_val}` for for impute_stepsize outside range [0.1, 0.9]! Please use numeric value within range [0.1, 0.9]. Currently this value will be ignored!')
                    else:
                        tmp_list.append(tmp_val)
                except:
                    print(f'WARNING: unrecognized option value `{x.strip()}` for impute_stepsize! Please use numeric value. Currently this value will be ignored!')
                    
            if len(tmp_list) == 0:
                print(f'WARNING: no valid value can be extracted from option value `{val}` for impute_stepsize! Please use one numeric value within range [0.1, 0.9] or an array of numeric values separated by ",". Currently impute_stepsize is set to be default value `0.5`!')
            else:
        ...
        if opt in ('--hole_threshold'):
            try:
                paramdict['hole_threshold'] = int(float(val))
                
                if paramdict['hole_threshold'] < 0:
                    print(f'WARNING: negative option value `{paramdict["hole_threshold"]}` for hole_threshold! Please use non-negative integer. Currently hole_threshold is set to be default value `1`!')
                    paramdict['hole_threshold'] = 1
            except:
                print(f'WARNING: unrecognized option value `{val}` for hole_threshold! Please use numeric value. Currently hole_threshold is set to be default value `1`!')
            continue
        
        
        if opt in ('--keep_shape'):
            if val.casefold() == 'true'.casefold():
                paramdict['keep_shape'] = True
            elif val.casefold() == 'false'.casefold():
                paramdict['keep_shape'] = False
            else:
                print(f'WARNING: unrecognized option value `{val}` for keep_shape! Please use string of true or false. Currently keep_shape is set to be default value `False`!')
            continue
        ...
        if opt in ('--diagnosis'):
            if val.casefold() == 'true'.casefold():
                ...
                print(f'WARNING: unrecognized option value `{val}` for diagnosis! Please use string of true or false. Currently diagnosis is set to be default value `False`!')
            continue
...
def main():
    ...
    for x in paramdict['impute_stepsize']:
        impute_start = time()
        # we now totally discard the transforming from integer coordinates to pixels
        result = do_imputation(paramdict['loc_file'], paramdict['prop_file'], paramdict['spatial_file'], x, paramdict['hole_threshold'], paramdict['keep_shape'], diagnosis=paramdict['diagnosis'])
        # return imputed spot locations, cell type proportions and gene expressions
        result[0].to_csv(os.path.join(output_path, f'impute_stepsize_{x}_spot_loc.csv'))
        result[1].to_csv(os.path.join(output_path, f'impute_stepsize_{x}_spot_celltype_prop.csv'))
        result[2].to_csv(os.path.join(output_path, f'impute_stepsize_{x}_spot_gene_norm_exp.csv'))
        print(f'imputation for step size {x} finished. Elapsed time: {(time()-impute_start)/60.0:.2f} minutes')
