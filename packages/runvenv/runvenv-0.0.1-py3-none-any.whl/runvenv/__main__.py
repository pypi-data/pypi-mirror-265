...
if platform.system() == "Linux":
    venv_dir_base = os.path.join(pathlib.Path.home(),       ".venv_wrap_cache")
else:  # Windows
    venv_dir_base = os.path.join(os.environ["USERPROFILE"], ".venv_wrap_cache")
...
def get_venv_context(venv_dir, force_recreation=False):
    ...
    if not os.path.exists(venv_dir):
        ...
        if pkgutil.find_loader("venv") and pkgutil.find_loader("ensurepip"):
            cmd = f"{python_cmd} -m venv {venv_dir}"
        elif pkgutil.find_loader("virtualenv"):
    ...
    return (
        ...
        dict(
            os.environ,
            PATH=os.path.join(venv_dir, bin_subdir) + \
                 os.pathsep + os.getenv("PATH","")       # Environ
        )
    )
...
if "--help" in sys.argv:
    ...
    Usage:
        ...
        python -m runvenv [--clean]  [--reqs <requirements-frozen.txt>] [arg1, [arg2, ...]]

            This runs 'python arg1 arg2 ...' inside a virtual environment which
            contains all the modules listed in <requirements-frozen.txt>

            <requirements-frozen.txt> is a file listing all the python modules
            (and their versions) you want to have inside the virtual
            environment. You can generate such a file calling this script with
            '--freeze' (read the corresponding documentation below to understand
            how).

            If option '--reqs <requirements-frozen.txt>' is *not* present, then
            a file called "{DEFAULT_REQUERIMENTS_FROZEN_FILE}" in the current
            folder will be used instead (in case it exists)

            Some ways in which you can use it:

                1. python -m runvenv --reqs requirements.txt my_script.py 103 203
                2. python -m runvenv --reqs requirements.txt -m webbrowser -n 'https://www.python.org'
                3. python -m runvenv --reqs requirements.txt 

            (The last one will simply open a python interpreter inside the
            virtual environment)
            ...
            The folder itself will be named after a hash of the frozen
            requirements file.
            This makes it possible not to have to re-install the virtual
            environment each time as long as the frozen requirements do not
            change.

            If the '--clean' flag is provided the virtual environment will be
            re-created no matter what.

        python -m runvenv --freeze <requirements.txt>

            This creates <requirements-frozen.txt>. It works like this:

                1. You first need to specify which python modules you want in
                   your virtual environment (no need to specify a fixed
                   version), and save them to a file, like this:
                ...
                2. Then call 'python -m runvenv --freeze'.
                   Example:

                       $ python -m runvenv --freeze my_requirements.txt

                3. The script will create a new virtual environment and install
                   each of the listed modules. Then it will generate a new file
                   called like the input file plus an appended suffix
    ...
    Example:
        ...
        # Run script inside the just created virtual environment (the first time
        # it will take some time because it needs to create the virtual
        # environment)

        $ python -m runvenv --reqs my_requirements-frozen-{platform.system()}.txt my_python_script.py


        # Run script inside the just created virtual environment again (the
        # second time will be very fast because it will reuse the already
    ...
    Notes:
        ...
        A. This script uses "pip install ..." under the hood. If you want to
           pass extra options to pip you can do so by setting environment
           variable "VENVWRAP_EXTRA_OPTS" before calling this script. This is
           useful, for example, to deal with certificate problems when running
           behind a corporate network that man-in-the-middles traffic. Example:

               $ export VENVWRAP_EXTRA_OPTS="--trusted-host pypi.org --trusted-host pypi.python.org --trusted-host files.pythonhosted.org"

        B. The argument to "--reqs" is expected to be a list of frozen
           requirements (ie. a list which you have generated calling "--freeze"
           which contains a complete list of packages and the exact desired
           version), *however*, despinte not being recommended, for convenience,
           you can also use a non frozen list directly such as in this example:

               $ echo "termcolor"  > my_requirements.txt
               $ echo "pexpect"   >> my_requirements.txt
               $ echo "pandas"    >> my_requirements.txt
               $ echo "pyserial"  >> my_requirements.txt
               $ python -m runvenv --reqs my_requirements.txt my_python_script.py

           The main drawbacks of doing this are:

               - The latest version of each package will be installed, which
                 changes depending on when you run the script for the first
                 time.

               - The next time you run it, because the hash of the txt file has
                 not changed, dependencies will not be updated (ie. they will
                 remain "frozen" at some arbitrary point in the past, more
                 specifically when you first executed it)

               - Dependencies of packages are also installed (this does *not*
                 happen when you use a real "frozen" file), which is also a
                 source of noise.

           ...however it can be useful as a quick an dirty way of running a
           script with some dependencies in one step.
...
if "--freeze" in sys.argv:
    ...
    # folder.
    #
    new_hash = hashlib.md5(open(frozen_reqs, "r").read().encode("utf8")).hexdigest()[0:8]
    ...
    venv_dir = os.path.join(
        venv_dir_base,
        hashlib.md5(open(frozen_reqs, "r").read().encode("utf8")).hexdigest()[0:8]
    )
    ...
    if not os.path.exists(os.path.join(venv_dir, "_updated_mark_")):
        ...
        ret, output = run_in_venv(venv_ctx,  
        ...
        open(os.path.join(venv_dir, "_updated_mark_"), 
             "w", 
             encoding="utf-8").write("YES")
