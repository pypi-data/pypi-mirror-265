...
def pawn_capture(
    ...
    match style:
        ...
        case PawnCapture.PxN:
            return f"{pawn}x{captured_piece.upper()}{rest}"
    ...
    match style:
        ...
        case PieceCapture.NxN:
            ...
            if san[1] == "x": # no disambiguation
                [N, x, d, n, *tail] = san
                rest = "".join(tail)
                return f"{N}x{captured_piece.upper()}{rest}"
            elif san[2] == "x":
                [N, e, x, d, n, *tail] = san
                rest = "".join(tail)
                return f"{N}{e}x{captured_piece.upper()}{rest}"
            else:
...
def applies(san: str, language: Language, styles: Iterable[Style] = [], captured_piece: str = None) -> str:
