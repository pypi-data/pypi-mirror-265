            ...
            if 'params' in component and isinstance(component['params'], dict):
                for key, value in component['params'].items():
                    ...
                    if isinstance(value, dict) and 'valueFrom' in value:
                        print(f"Component '{component.get('name', 'Unknown')}' has a param '{key}' with 'valueFrom'.")
                        if 'secretKeyRef' in value['valueFrom']:
                            logger.info(f"Component '{component.get('name', 'Unknown')}' has a param '{key}' with 'value from secretKeyRef'.")
                            secret = kubernetes.client.CoreV1Api().read_namespaced_secret(value['valueFrom']['secretKeyRef']['name'], namespace)
                            secret_data_encoded = secret.data[value['valueFrom']['secretKeyRef']['key']]
                            secret_data_decoded = base64.b64decode(secret_data_encoded).decode('utf-8')
                            component['params'][key] = secret_data_decoded
                        if 'configMapKeyRef' in value['valueFrom']:
                            logger.info(f"Component '{component.get('name', 'Unknown')}' has a param '{key}' with 'value from configMapKeyRef'.")
    else:
        raise errors.UserFailure("No `.components` array found or the structure is incorrect.")
