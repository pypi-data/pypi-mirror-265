    ...
    async def _get_account(request: Request, volume_id: str) -> AWSAccount:
        ...
        try:
            ...
            if code != 'AccessDeniedException':
                ...
                raise e
            logger.exception('Account %s is not authorized to access contact information', aid)
        ...
        try:
            ...
            logger.exception('Account %s is not part of an organization', aid)
        except ClientError as e:
            code = e.response['Error']['Code']
            if code != 'AccessDeniedException':
                logger.exception('Client error %s', code)
                raise e
            logger.exception('Account %s is not authorized to access organization-level account information', aid)
