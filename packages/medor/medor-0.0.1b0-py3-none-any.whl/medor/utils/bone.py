...
from validators import url as valid_url, domain as valid_domain, ipv4, ipv6, validator
...
from medor.utils.globals_ import tor


class Bone:
    def __init__(
        self, ptype: str, item: str, onion: bool = False, proxy: str or None = None
    ) -> None:
        medor_home()
        self.ptype = ptype
        self.item = item
        ...
        if onion:
            ...
            self.proxy = tor["tor_proxy"]
            self.onion_pattern = r"^https?://[a-z2-7]{16,56}\.onion/?$"
            self.tor = Tor()

        else:
            self.net = net.Net(proxy=proxy)
    ...
    def find_post(self, url: str) -> str:
        ...
        exit(
            ...
            """   If you use medor find, find a blog post manually and use post_url"""
        )
    ...
    def valid_site_url(self, url: str) -> None:
        if self.onion and not self.valid_onion(url):
            spinner.stop()
            exit(
                f"{failure}  {Fore.RED}{url} si not a valid onion url.\n"
                "   Check the url."
            )
        if valid_url(url):
            try:
                ...
                if res.status_code == 200:
                    ...
                    spinner.stop()
                    exit(
                        f"{failure}  {Fore.RED}{url} request is not successful.\n"
                        "   Check the url."
                    )
            except httpx.HTTPError as e:
                spinner.stop()
                exit(
                    f"{failure}  {Fore.RED}{url} request is not successful : {e}.\n"
                    "   Check the url."
                )
        else:
            spinner.stop()
            exit(
                f"{failure}  {Fore.RED}{url} doesn't seem to be well formatted as a valid url.\n"
                "   Check the url."
            )
