    ...
    def ini_connection(self):
        ...
        try:
            ...
            try:
                ...
                self.ini_connection()
                print(
                    f"Your tor configuration (tor path and password,...) is not set or is not right.\n"
                    f"{e}"
                )
...
def create_tor_hash(tor_pw, tor_path):
    try:
        ...
        exit(f"{failure}{Fore.RED}  Tor can't be found. Check its path.\n"
...
def setup() -> None:
    import pwinput
    print(f"{warning}{Fore.YELLOW} You have to install tor first. If you haven't, see README.md.\n"
    ...
    if len(tor_pass) == 0:
        exit(f"{failure}{Fore.RED}  You should enter a password.")
    tor_path = input(
        r"➡️ Tor binary path (e.g. C:\Tor\tor.exe for windows,"
        """\n   /usr/bin/tor or tor for linux) : """
    )
    if len(tor_path) == 0:
        exit(f"{failure}{Fore.RED}  You should enter a path.")
