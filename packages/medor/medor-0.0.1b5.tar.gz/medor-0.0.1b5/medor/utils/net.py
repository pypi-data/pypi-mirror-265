...
class Net:
    def __init__(
        self, onion: bool = False, proxy: str or None or dict[str, str] = None, timeout: float = 5.0
    ):
    ...
    def valid_site_url(self, url: str) -> None:
        if self.onion and not self.valid_onion(url):
            spinner.stop()
            exit(
                f"{failure}  {Fore.RED}{url} is not a valid onion url.\n"
                "   Check the url."
            )
        if valid_url(url):
            try:
    ...
    def update_ua(self) -> None:
        ...
        if ua is None:
            spinner.stop_and_persist(
                symbol=warning, text="User-Agents can't be updated"
            )
        else:
            with open(Path(self.medor_path, "uas.py"), "w", encoding="utf-8)") as f:
                f.write(ua)
            importlib.reload(uas)
            spinner.stop_and_persist(symbol=success, text="User-Agents checked")
        ...
        try:
            ...
            if res.status_code == 200:
                if proxy and (res.text == real_ip):
                    spinner.stop()
                    exit(
                        f"❌  {Fore.RED}Proxy check failed:\n"
                        f"   Your proxy IP ({proxy}) is the same as your real ip ({real_ip})."
                    )
        except httpx.HTTPError as e:
            spinner.stop()
            exit(
                f"❌  {Fore.RED}Proxy {proxy} check failed : {e}.\n"
