...
from medor.utils.util import success, warning, spinner


class Net:
    def __init__(
        self, onion: bool = False, proxy: str or None or dict[str, str] = None
    ):
    ...
    def connect(self, url, rtype="get", headers=None, content=None) -> httpx.Response:
        ...
        with httpx.Client(headers=headers, proxy=self.proxy) as c:
            if rtype == "get":
                res = c.get(url)
            if rtype == "post":
