...
def json_to_row_cells(data, row_num) -> List[Cell]:
    """Convert JSON data to a list of cells for a single row."""
    cells_list = []
    cell_data = zip(data.get("numberFormat", [])[0], data.get("values", [])[0])
    for i, cell in enumerate(cell_data):
        cell_type, cell_value = cell
        cells_list.append(Cell(column_name="", data_type=cell_type, value=cell_value, row_num=row_num, col_num=i + 1))
    return cells_list
