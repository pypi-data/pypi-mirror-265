...
class QuarkProxy(object):
    ...
    def connect(self):
        ...
        with open(router, 'r') as f:
            ...
            for chip, value in table.items():
                ...
                # trying to self.router
                self.syscon[chip] = connect('QuarkProxy',
                                            host=value['host'],
                                            port=value.get('port', 3088))
    ...
    def submit(self, task: dict, block: bool = False):
        ...
        try:
            ...
            eid = task['metainfo']['other']['quafu']['tid']  # external id
    ...
    def update(self, backend: str, info: dict):
        quafuos = str(Path.home()/'Desktop/quafuos')
        if quafuos not in sys.path:
            print('adding quafuos')
            sys.path.append(quafuos)
        with open(f'{backend}.json', 'w') as f:
            f.write(json.dumps(info, indent=4))
        from call_QuafuOS import call_local_backend_api
    ...
    def sysinfo(self):
        while True:
            if self.router:
                ...
                for chip, conn in self.syscon.items():
                    # if chip != 'Baiwang':
                    #     continue
                    info[chip] = conn.sysinfo()
                    try:
                        print(self.perf.format(**({'chip': chip} | info[chip])))
                    except Exception as e:
