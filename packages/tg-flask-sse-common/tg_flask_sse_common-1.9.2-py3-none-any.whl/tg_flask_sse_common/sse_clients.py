...
class SseClients(object):
    ...
    def add_heartbeat(self):
        ...
        for channel, sse_connect_list in self.sse_global_clients.items():
            for sse_connect in sse_connect_list:
                ...
                need_release = sse_connect.get(self.Field.LATER_RELEASE, False)
                if not need_release:
                    continue

                # 问题:
                # 因为flask是无法监听到连接是否关闭的，所以需要定时发送心跳包
                # 一旦客户端关闭连接，就会收不到心跳包，协程将断开连接，while将直接退出
                # 且服务端无法感知，所以LATER_RELEASE标识位将无法置为true，所以此连接对象将一直存在
                #
                # 解决措施:
    ...
    def listen_message(self, channel):
        ...
        if not latest_sse_connect:
            print({
                'title': 'sse-log-当前连接全部等待关闭中，请重新连接',
                'channel': channel,
        ...
        while (datetime.now() - connect_time).seconds < self.max_connect_time:
            print({
                'title': 'sse-log-内存队列-开始轮训',
                'channel': channel,
                'connect_time': datetime.strftime(connect_time, '%Y-%m-%d %H:%M:%S'),
                'latest_sse_connect': latest_sse_connect,
                'clients': self.sse_global_clients,
                'channel_connect_count': len(sse_connect_list),
                'client_id': client_id,
                'channel_count': self.count(),
                'time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        if run_rsp == SseListenRsp.TIMEOUT_CONNECT:
            ...
            print({
                'title': 'sse-log-收到同一节点重复channel连接-关闭轮训',
                'channel': channel,
                'connect_time': datetime.strftime(connect_time, '%Y-%m-%d %H:%M:%S'),
                'channel_connect_count': len(sse_connect_list),
                'ln_id': self.local_node_id,
            ...
            print({
                'title': 'sse-log-收到其他节点重复channel连接-关闭轮训',
                'channel': channel,
                'connect_time': datetime.strftime(connect_time, '%Y-%m-%d %H:%M:%S'),
                'channel_connect_count': len(sse_connect_list),
                'ln_id': self.local_node_id,
