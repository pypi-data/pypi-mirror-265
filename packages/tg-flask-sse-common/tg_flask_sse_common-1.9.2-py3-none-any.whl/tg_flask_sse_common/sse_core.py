...
class Sse(object):
    ...
    def default_generator(self, channel):
        ...
        订阅消息，监听内存中的消息队列
        """
        for message in self.sse_clients.listen_message(channel):
            # 消息非法
            if not message:
            ...
            # 系统断开消息
            if message.event in [
                SseSystemEventType.D_REPEAT_CONNECT,
                SseSystemEventType.REPEAT_CONNECT,
                SseSystemEventType.ALL_DISCONNECT,
                SseSystemEventType.TIMEOUT_CONNECT,
                SseSystemEventType.END
            ]:
                self.sse_redis_pub_sub.disconnect(channel)
