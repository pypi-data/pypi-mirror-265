        ...
        if len(atexts) > 0 and atexts[0] != "-":
            for atext in atexts:
                for m in re.finditer(atext, document.text):
                    annotations.append(Annotation(label=alabel, labelName=alabelName, start=m.start(), end=m.end(), text=atext))
...
def add_categories(document: Document, params: OpenAICompletionBaseParameters, result: str):
    ...
    for matchNum, match in enumerate(matches, start=1):
        alabel = match.group('label')
        alabelName = candidate_names.get(alabel, None)
        categories.append(Category(label=alabel, labelName=alabelName))
