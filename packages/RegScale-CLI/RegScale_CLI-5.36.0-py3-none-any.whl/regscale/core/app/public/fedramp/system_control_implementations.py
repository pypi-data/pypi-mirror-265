...
def process_parts(
    ...
    :param List[dict] existing_controls: A list of existing controls
    ...
    for cmp_uuid in {cmp["component_uuid"] for cmp in parts}:
        if single_system is False:
            pass
    for imp in imps:
        cnt = [SecurityControl.from_dict(cntrl) for cntrl in existing_controls if cntrl["id"] == imp["controlID"]]
        if cnt:
            control = cnt.pop()
        else:
        ...
        opts = [
            ...
            if option["securityControlId"] == imp["controlID"] and option["name"] == status
        ]
        if opts:
            option = opts.pop()
        else:
        ...
        params = [
            part
            for part in parts
            if "prm" in part["part_id"] and control["controlId"] == part["part_id"].split("_prm")[0]
        ]
...
def update_parameters(
    trv: FedrampTraversal, existing_parameters: List[dict], parameters: List[dict], control: dict, imp: dict
) -> list[dict]:
    """
    Update parameters
    ...
    :param dict control: A control object
    ...
    for param in parameters:
        dat = [
            ...
            if par["parameterId"] == param["part_id"] and par["securityControlId"] == control["id"]
        ]
        if dat:
...
def post_options(trv: FedrampTraversal, options: list[ImplementationOption]) -> list[dict]:
    """
