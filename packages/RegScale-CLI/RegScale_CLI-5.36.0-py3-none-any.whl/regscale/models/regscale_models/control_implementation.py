...
class ControlImplementation(RegScaleModel):
    ...
    def _get_additional_endpoints(cls) -> ConfigDict:
        ...
        return ConfigDict(  # type: ignore
            ...
            update=cls._module_slug_id_url,
            delete=cls._module_slug_id_url,
            get=cls._module_slug_id_url,
    ...
    @classmethod
    def get_object(cls, object_id: Union[str, int]) -> Optional["ControlImplementation"]:  # type ignore
        """
        Get an object by ID.
        ...
        if response and response.ok:
            ...
            response_json["status"] = (
                response_json.get("status", None) or ControlImplementationStatus.NotImplemented.value
            )
    ...
    def get_list_by_plan(cls, plan_id: int) -> List["ControlImplementation"]:
        ...
        :rtype: List[ControlImplementation]
        """
        response = cls.api_handler.get(endpoint=cls.get_endpoint("get_list_by_plan").format(intSecurityPlan=plan_id))
        security_controls = []
        if response and response.ok:
            for ci in response.json():
                if ci := cls.get_object(object_id=ci["id"]):
    ...
    def get_control_map_by_plan(cls, plan_id: int) -> Dict[str, int]:
        ...
        if response and response.ok:
            return {ci["control"]["controlId"].lower(): ci["id"] for ci in response.json()}
        return {}
    ...
    @staticmethod
    def _extract_text_and_log(element: Element, debug_logger: logging.Logger) -> Optional[str]:
        """
    ...
    def _update_implementation_status(element: Element, imp: "ControlImplementation") -> None:
        ...
        :rtype: None
        """
        if "name" in element.attrib and element.attrib["name"] == "implementation-status":
            imp.status = "Fully Implemented" if element.attrib.get("value") == "implemented" else "Not Implemented"
