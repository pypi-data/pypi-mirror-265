...
pub fn create_module(py: Python) -> Result<&PyModule, PyErr> {
    ...
    module.add_class::<TimeExtrapolation>()?;
...
fn dataclass_registry() -> DataclassRegistry {
    ...
    registry.insert_with_sample(&core_model::model::lorenz_96::Lorenz96Parameters {
        k: core_model::model::lorenz_96::K::MIN,
        forcing: Lorenz96Forcing::Const(core_model::model::lorenz_96::Const::new(0.0)),
    });
    registry.insert_with_sample(&core_model::model::lorenz_96::Lorenz96Parameters {
        k: core_model::model::lorenz_96::K::MIN,
        forcing: Lorenz96Forcing::Distr(
            core_model::model::lorenz_96::Distr::new(standard_normal()),
        ),
    });
...
///
/// - [`Model::lorenz_63`] creates a Lorenz63 model
/// - [`Model::lorenz_96_with_const_forcing`] creates a basic Lorenz96 model
///   with constant forcing
/// - [`Model::lorenz_96_with_stochastic_forcing`] creates a basic Lorenz96
...
///     extrapolation=TimeExtrapolation.EulerSmoothing,
...
impl Model {
    ...
    pub fn state(this: &PyCell<Self>) -> Result<&PyAny, PyErr> {
        ...
        namedtuple.call1(PyTuple::new(
            ...
            core_model::model::StateViewMut::iter_mut(&mut state)
                .map(|state| {
                    ...
                    #[allow(unsafe_code)]
                    unsafe {
                        numpy::PyArrayDyn::borrow_from_array(&state, this)
                    }
                })
                .collect::<Vec<_>>(),
        ))
    ...
    pub fn __next__(this: &PyCell<Self>) -> Result<Option<&PyAny>, PyErr> {
        {
            let mut slf: PyRefMut<Self> = this.try_borrow_mut()?;
            let slf: &mut Self = &mut slf;
    ...
    /// Creates a Lorenz63 model with `params`, a time `extrapolation` scheme,
    /// and the timestep `$\Delta t$` `dt`.
    ///
    /// The Lorenz63 model [^1] is described by the following sytem of
    /// equations:
    ///
    /// ```math
    /// \begin{align}
    ...
    ///
    /// [SIGNATURE]: # "(params: types.Lorenz63Parameters, extrapolation: TimeExtrapolation, dt: float) -> Model"
    #[must_use]
    pub fn lorenz_63(
    ...
    #[staticmethod]
    #[allow(clippy::needless_pass_by_value)]
    /// Creates a Lorenz96 model with `params`, a time `extrapolation` scheme,
    /// and the timestep `$\Delta t$` `dt`.
    ///
    /// The model's forcing `$F$` is constant.
    ///
    /// The basic Lorenz96 model [^2] is described by the following equation:
    ///
    /// ```math
    /// \begin{align}
    ///     \frac{\partial X_k}{\partial t} &= -X_{k-2} \cdot X_{k-1} + X_{k-1}
    /// \cdot X_{k+1} - X_{k} + F \end{align}
    /// ```
    ///
    ...
    ///
    /// [SIGNATURE]: # "(params: types.Lorenz96Parameters, extrapolation: TimeExtrapolation, dt: float) -> Model"
    #[must_use]
    ...
    ///
    /// The basic Lorenz96 model [^3] is described by the following equation:
    ///
    /// ```math
    /// \begin{align}
    ///     \frac{\partial X_k}{\partial t} &= -X_{k-2} \cdot X_{k-1} + X_{k-1}
    ...
    ///   [Accessed: 18th March 2024].
    ///
    /// [SIGNATURE]: # "(params: types.Lorenz96Parameters, extrapolation: TimeExtrapolation, dt: float, seed: int) -> Model"
    #[must_use]
    ...
    /// a time `extrapolation` scheme, a `boundary` condition, and the timestep
    /// `$\Delta t$` `dt`.
    ///
    ...
    /// `$(x_{dim},)$`.
    ///
    /// [SIGNATURE]: # "(params: types.LinadvParameters, extrapolation: TimeExtrapolation, boundary: Boundary, dt: float) -> Model"
    #[must_use]
    pub fn linadv(
    ...
    /// shape `$(x_{dim},)$`.
    ///
    /// [SIGNATURE]: # "(params: types.OneDSWParameters, extrapolation: TimeExtrapolation, boundary: Boundary, dt: float) -> Model"
    #[must_use]
    pub fn onedsw(
    ...
    /// shape `$(y_{dim}, x_{dim})$`.
    ///
    /// [SIGNATURE]: # "(params: types.TwoDSWParameters, extrapolation: TimeExtrapolation, boundary: Boundary, dt: float) -> Model"
    #[must_use]
    pub fn twodsw(
...
///
/// - [`TimeExtrapolation::EulerSmoothing`]: The model is advanced using the
///   Euler Smoothing method, which averages between the current state of a
///   field `$F$` and a double-[`TimeExtrapolation::Direct`] estimate for state
///   after two timesteps:
///
///     ```math
///     \begin{align}
...
///             \frac{\partial F}{\partial t}
///         \right)}_{t} \newline
///         F_{(t + 2 \Delta t)} &= F_{(t + \Delta t)} + \Delta t \cdot {\left(
///             \frac{\partial F}{\partial t}
///         \right)}_{(t + \Delta t)} \newline
///         F_{(t + \Delta t)} &= \left(
///             F_{t} + F_{(t + 2 \Delta t)}
...
///     ```
///
/// - [`TimeExtrapolation::LeapFrog`]: The model is advanced using the Leap Frog
...
///
/// - [`TimeExtrapolation::RungeKutta4`]: The model is advanced using the 4th
///   order Runge-Kutta method, which averages the estimates of the first
///   derivative tendencies of a field `$F$` at `$t$`, `$t + 0.5 \cdot \Delta
///   t$`, and `$t + \Delta t$`:
///
///     ```math
///     \begin{align}
...
pub enum TimeExtrapolation {
    /// The model is advanced using the direct Forward Euler method, which
    /// uses the first derivative.
    Direct,
    /// The model is advanced using the Euler Smoothing method, which averages
    /// between the current state of a field `$F$` and a
    /// double-[`TimeExtrapolation::Direct`] estimate for state after two
    /// timesteps.
    EulerSmoothing,
