...
class City(GenericCRUD):
    ...
    def get_city_id_by_city_name_state_id(self, city_name: str, country_id: int = None, state_id: int = None,
        ...
        where_clause = f"title=%s AND lang_code=%s"
        params = (city_name, lang_code.value)
        if country_id is not None:
            where_clause += f" AND country_id=%s"
            params += (country_id,)
        if state_id is not None:
            where_clause += f" AND state_id=%s"
            params += (state_id,)
