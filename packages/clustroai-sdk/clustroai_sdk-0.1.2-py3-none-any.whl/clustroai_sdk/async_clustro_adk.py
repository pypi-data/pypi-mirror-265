...
class AsyncClustroAI:
    ...
    async def _send_request(self, method, path, data=None):
        ...
        async with httpx.AsyncClient(verify=self._verify) as client:
            response = await client.request(method, f'{self._base_url}/{path}', headers=headers, json=data,
    ...
    async def create_model(self, name: str, model_type: str, model_code_version: str, model_code_repo_url: str,
        ...
        return await self._send_request('POST', 'models', payload)
    ...
    async def list_models(self) -> List[Dict[str, str]]:
        ...
        """
        return await self._send_request('GET', 'models')

    async def get_model(self, model_id: str) -> Dict[str, str]:
        """
        get a model
        waring: When users who are not in the 'admin' role call this API, we limit its usage to 60 calls per minute
        """
        assert model_id is not None
        return await self._send_request('GET', f'models/{model_id}')
    ...
    async def update_model(self, model_id: str, **kwargs):
        ...
        """
        assert model_id is not None
        return await self._send_request('PUT', f'models/{model_id}', kwargs)
    ...
    async def delete_model(self, model_id: str):
        ...
        """
        assert model_id is not None
        return await self._send_request('DELETE', f'models/{model_id}')

    async def list_public_models(self, username: str = "") -> List[Dict[str, str]]:
        """
    ...
    async def get_public_models(self, username: str, model_name: str) -> Dict[str, str]:
        """
        ...
        return await self._send_request('GET', f"public_models/{username}/{model_name}")

    async def list_inference_jobs(self) -> List[Dict[str, str]]:
        """
        list all inference jobs
        waring: When users who are not in the 'admin' role call this API, we limit its usage to 60 calls per minute
        """
        return await self._send_request('GET', 'inference_jobs')

    async def get_inference_job(self, inference_job_id: str) -> Dict[str, str]:
        """
        get inference job
        waring: When users who are not in the 'admin' role call this API, we limit its usage to 60 calls per minute
        """
        assert inference_job_id is not None
        return await self._send_request('GET', f'inference_jobs/{inference_job_id}')

    async def update_inference_job(self, inference_job_id: str, **kwargs):
        """
        update inference job
        waring : We only allow users with the 'admin' role to update model
        """
        assert inference_job_id is not None
        return await self._send_request('PUT', f'inference_jobs/{inference_job_id}', kwargs)

    async def delete_inference_job(self, inference_job_id: str):
        """
        delete inference job
        waring : We only allow users with the 'admin' role to delete model
        """
        assert inference_job_id is not None
        return await self._send_request('DELETE', f'inference_jobs/{inference_job_id}')

    async def create_inference_job(self, model_id: str, name: str, **kwargs):
        """
        ...
        return await self._send_request('POST', 'inference_jobs', payload)
    ...
    async def list_tasks(self) -> List[Dict[str, str]]:
        ...
        waring: When users who are not in the 'admin' role call this API, we limit its usage to 60 calls per minute
        """
        return await self._send_request('GET', 'tasks')

    async def get_task(self, task_id: str) -> Dict[str, str]:
        """
        get a task
        waring: When users who are not in the 'admin' role call this API, we limit its usage to 60 calls per minute
        """
        assert task_id is not None
        return await self._send_request('GET', f'tasks/{task_id}')

    async def list_worker(self) -> List[Dict[str, str]]:
        """
        list all workers
        waring: When users who are not in the 'admin' role call this API, we limit its usage to 60 calls per minute
        """
        return await self._send_request('GET', 'workers')
    ...
    async def get_worker(self, worker_id: str) -> Dict[str, str]:
        ...
        return await self._send_request('GET', f'workers/{worker_id}')

    async def update_worker(self, worker_id: str, **kwargs):
        """
        update a worker
        waring : We only allow users with the 'admin' role to update worker
        """
        assert worker_id is not None
        return await self._send_request('PUT', f'workers/{worker_id}', kwargs)
    ...
    async def delete_worker(self, worker_id: str):
        ...
        """
        assert worker_id is not None
        return await self._send_request('DELETE', f'workers/{worker_id}')

    async def create_worker(self, name: str = None, worker_type: str = None, job_assignment_type: str = None,
        ...
        if name:
            payload['name'] = name
        return await self._send_request('POST', 'workers', payload)

    async def run(self, input: str, model_id_or_name: str = None, job_id: str = None, sync: bool = False):
        """
        run job
        ...
        return await self._send_request('POST', path, {"input": input})

    async def username_run(self, input: str, username, model_name, sync: bool = False):
        """
