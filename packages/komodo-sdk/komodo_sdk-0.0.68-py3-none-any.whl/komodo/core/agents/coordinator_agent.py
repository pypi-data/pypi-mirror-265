...
class CoordinatorAgent(KomodoAgent):
    ...
    tasks and order in which to call the other agents. For each task breakdown step, describe your 
    thinking to the tool "chain_of_thought" and then proceed to invoke the tools in the right order.
    ...
    def __init__(self, appliance: KomodoApp, run_agent_as_tool, run_workflow_as_tool):
        ...
        path = locations.appliance_data(appliance.shortcode)
        cache = locations.cache()

        self.add_tool(DirectoryReader(path))
        self.add_tool(FileReader(path, cache))
