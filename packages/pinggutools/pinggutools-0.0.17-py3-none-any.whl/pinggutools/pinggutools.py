...
if csdir=="":
    ...
    csdir =csdir +csdir.split ("pinggutools")[0 ][-1 ]#
...
def get_directory_path(directory_path):
    ...
    global csdir
    # 检查目录是否存在 A.txt 文件
    if not (os.path.isfile(os.path.join(directory_path, '0（范例）质量评估.xls'))):
        # 解压 A.zip 到指定目录
...
def Topentable(methon):
    ...
    try:
        ...
        if "报告类型-新的" in ori.columns:
            biaozhun= pd.read_excel(
                    peizhidir+"0（范例）质量评估.xls", sheet_name="药品", header=0, index_col=0
                ).reset_index()
            ori["报告编码"]=ori["报告表编码"]
        ...
        if "皮损部位" in ori.columns:
            biaozhun= pd.read_excel(
                    peizhidir+"0（范例）质量评估.xls", sheet_name="化妆品", header=0, index_col=0
                ).reset_index()
            ori["报告编码"]=ori["报告表编号"]
    ...
    if yongtu == 1:
        if len(biaozhun) == 0:
            ...
            try:
                ...
                text.insert(END, "\n您使用配置表文件夹中的“0（范例）质量评估.xls“作为评分标准。")
                text.see(END)
                # r=pdb.to_dict()
                # text.insert(END,r)
            except:
...
def Tpinggu():
    ...
    biaozhun = pd.read_excel(
        peizhidir+"0（范例）质量评估.xls", sheet_name=0, header=0, index_col=0
    ).reset_index()
    #print(ms)
    if "专家打分-不良反应名称" in ms:    
        biaozhun = pd.read_excel(peizhidir+"0（范例）质量评估.xls", sheet_name="药品", header=0, index_col=0).reset_index()    
        #print(biaozhun)
    if "专家打分-化妆品名称" in ms:    
        biaozhun = pd.read_excel(peizhidir+"0（范例）质量评估.xls", sheet_name="化妆品", header=0, index_col=0).reset_index()    
    if "专家打分-是否需要开展产品风险评价" in ms:    
        biaozhun = pd.read_excel(peizhidir+"0（范例）质量评估.xls", sheet_name="器械持有人", header=0, index_col=0).reset_index()          
    ...
    writer_dafen = pd.ExcelWriter(str(ddename) + "最终打分" + ".xlsx")
