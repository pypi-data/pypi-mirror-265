...
class BaseBlue:
    ...
    def __load_api(cls, item_list):
        for item in item_list:
            ...
            len_arr = len(item)
            if len_arr < 2:
                if hasattr(item, 'as_view') or hasattr(item, 'wsrouter'):
                    item = [f'/{cls.name}', item]
                else:
                    continue
            version = cls.conf.VER_CODE if (len_arr < 3) else item[2]
            rpath, handler = item[0], item[1]
            name = handler.__name__ if (len_arr < 4) else item[3]
            if rpath and rpath[0] != '/':
                rpath = f'/{rpath}'
            if hasattr(handler, 'as_view'):
                cls.bpo.add_route(handler.as_view(), rpath, version=version, name=name)
                continue
            if hasattr(handler, 'wsrouter'):
                cls.bpo.add_websocket_route(handler().wsrouter, rpath, version=version, name=name)
            hasattr(handler, 'init_loop_task') and cls.__loop_handlers.append(handler)
