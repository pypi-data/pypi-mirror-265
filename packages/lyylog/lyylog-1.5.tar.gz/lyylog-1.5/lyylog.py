...
exe_file_name_without_extension = os.path.splitext(exe_file_name)[0]
log_filename_prefix = f"log_{exe_file_name_without_extension}"
...
def log(*args, subdir_name="lyylog", prefix=log_filename_prefix, if_print=True):
    ...
    # 如果需要，直接向标准输出写入消息
    if if_print and sys.stdout is not None:
