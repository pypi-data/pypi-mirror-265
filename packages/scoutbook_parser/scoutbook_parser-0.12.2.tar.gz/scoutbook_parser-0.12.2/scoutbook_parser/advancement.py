...
def record_advancement_line(target, line):
    ...
    target["Date"] = find_min_date(line)  # find earliest date, which will usually be the date signed off, rather than approved or recorded
...
def record_rank_requirement(scout, award_type, line):
    ...
    if rank not in scout["Ranks"]:
        scout["Ranks"][rank] = {}
    if "Requirements" not in scout["Ranks"][rank]:
        scout["Ranks"][rank]["Requirements"] = {}
    scout["Ranks"][rank]["Requirements"][requirement] = {}
    record_advancement_line(scout["Ranks"][rank]["Requirements"][requirement], line)
    ...
    with open(file, "r") as f:
        ...
        for line in reader:
            ...
            match line["Advancement Type"].strip().split():
                case ("Rank",) | ("Adventure",) | ("Award",) | ( "Merit", "Badge",
                ) as award_type:
