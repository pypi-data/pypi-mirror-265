...
data["options test"] = [str(i) + " helo" for i in data["channel"]]
options = {
    "channel": "exponent-mode=fixed, fixed-exponent=2, uncertainty-mode=separate",
}
...
separate_uncertainties(data, inplace=True)
# x = transform_dataframe_to_latex_ready(data, siunitx_column_option=options)
join_uncertainties(data, inplace=True)
print(data)
