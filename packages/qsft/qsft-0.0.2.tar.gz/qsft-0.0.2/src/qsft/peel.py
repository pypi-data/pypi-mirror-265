...
from .utils import bin_to_dec, qary_vec_to_dec, sort_qary_vecs, calc_hamming_weight, dec_to_qary_vec
...
class QSFT:
    ...
    def transform(self, signal, verbosity=0, report=False, timing_verbose=False, **kwargs):
        ...
        while cont_peeling and num_peeling < peeling_max and iter_step < max_iter:
            ...
            # if there were no single-tons, terminate
            if len(singletons) == 0:
                cont_peeling = False
        ...
        for k, value in result: # iterating over (i, j)s
            loc.add(tuple(k))
            if tuple(k) in qsft_counts:
                qsft[tuple(k)] = (qsft[tuple(k)] * qsft_counts[tuple(k)] + value) / (qsft_counts[tuple(k)] + 1)
                qsft_counts[tuple(k)] = qsft_counts[tuple(k)] + 1
            else:
                qsft[tuple(k)] = value
                qsft_counts[tuple(k)] = 1
        if timing_verbose:
