...
from qleegss.dataload.load_eeg_mr4_one_night import load_eeg_mr4_one_night
from qleegss.plot.plot_data_preview import plot_preview_mr4
from qleegss.pdf.data_preview import generate_data_preview
from qleegss.dataload.load_eeg_mr2_one_night import load_eeg_mr2_one_night
...
class DataHandler:
    ...
    def load_eeg_x8_one_night(self):
        self.eeg, self.start_time, self.end_time,  self.disconnect_rate, self.package_loss_rate = load_eeg_x8_one_night(self.eeg_path)

    def load_eeg_mr4_one_night(self):
        self.ecg, self.emg, self.eeg0, self.eeg1, self.start_time, self.end_time, self.disconnect_rate, self.package_loss_rate, \
            self.header_mac, self.box_mac = load_eeg_mr4_one_night(self.eeg_path)

    def load_eeg_mr2_one_night(self):
        self.eeg0, self.eeg1, self.start_time, self.end_time, self.disconnect_rate, self.package_loss_rate, \
            self.header_mac, self.box_mac = load_eeg_mr2_one_night(self.eeg_path)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 4通道小鼠
            ...
            # 加载数据
            data.load_eeg_mr4_one_night() if data.eeg_path is not None else None
            data.load_acc() if data.acc_path is not None else None
            logging.info('load data success.')
            # data preview
        ...
        # 2通道小鼠
            ...
            # 加载数据
            data.load_eeg_mr2_one_night() if data.eeg_path is not None else None
            data.load_acc() if data.acc_path is not None else None
            logging.info('load data success.')
            # data preview
