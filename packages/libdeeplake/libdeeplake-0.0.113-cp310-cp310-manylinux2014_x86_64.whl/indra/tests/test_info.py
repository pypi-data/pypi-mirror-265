...
def test_bbox(get_pytest_local_path):
    ...
    with ds:
        ...
        assert (
            ds.bboxes.info.coords == ds.bboxes.info["coords"] == {"type": 0, "mode": 2}
        )
...
def test_info_new_methods(get_pytest_local_path):
    ...
    info = pickle.dumps(ds.info)
    new_info = pickle.loads(info)
    assert len(new_info) == 1
    assert 1 in new_info
    assert new_info[1] == "world"
