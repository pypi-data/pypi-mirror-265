    ...
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request) -> Response:
            ...
            if request.scope['path'] == '/time':
                plogger.info('Request Cost: {}s'.format(round(time.time() - before, 2)))
                plogger.info('Response Content: {}'.format(response.body.decode('utf-8')))
            else:
                klogger.info('Request Cost: {}s'.format(round(time.time() - before, 2)))
...
class Interceptor(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: HttpRequest, call_next: RequestResponseEndpoint) -> HttpResponse:
        ...
        if request.scope['path'] == '/time':
            plogger.info('Request URL: {} {}'.format(request.method, request.url))
            response = await call_next(request)
            plogger.info('Response HTTP Status Code: {}'.format(response.status_code))
        else:
