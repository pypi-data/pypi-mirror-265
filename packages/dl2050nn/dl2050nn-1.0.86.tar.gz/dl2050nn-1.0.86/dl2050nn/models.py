...
class SignalNet(nn.Module):
    def __init__(self, ch, n, c, p=0., show=False):
        ...
        szs = [(n1,sz)]
        for _ in range(3):
            cnn += [Conv1D(n1, n2, stride=2)]
            n1, sz = n2, int(sz/2)
            szs += [(n1,sz)]
        bottle = int((int(math.log2(sz)))/2)
        for _ in range(bottle):
            cnn += [ResBlock1D(n1, n1*2, ks=3, stride=2)]
            n1, sz = n1*2, int(sz/2)
            szs += [(n1,sz)]
            cnn += [nn.MaxPool1d(2)]  # AvgPool1d, MaxPool1d
            sz = int(sz/2)
            szs += [(n1,sz)]
        cnn += [nn.AdaptiveMaxPool1d(1)]
        cnn += [Flatten()]
        self.cnn = nn.Sequential(*cnn)
        head = [Linear(n1, 1024, p=p)]
        head += [LinearFinal(1024, c, p=None)]
        self.head = nn.Sequential(*head)
        if show: model_stats(self, bottle=bottle, szs=szs, show_szs=True)

    def forward(self, x): return self.head(self.cnn(x))
    def get_emb(self, x): return self.cnn(x).detach().cpu().numpy()
