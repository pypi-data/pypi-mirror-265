...
def plot_confusion(cm, cm_p, figsize=(8, 2), cbar=False, p=None):
    ...
    plt.pause(.001)
    if p is not None:
        plt.savefig(p, facecolor='g', bbox_inches="tight", pad_inches=0.3, transparent=True)
...
def plot_roc(y, y2):
    fpr,tpr,_ = metrics.roc_curve(y, y2)
    auc = np.trapz(tpr, fpr)
    precision,recall,_ = metrics.precision_recall_curve(y, y2)
    auc_pr = np.trapz(precision, recall)
...
class Results_Clsf():
    ...
    def stats(self, thresholds=10):
        if self.c!=2: raise ValueError('More than two classes')
        return df_stats_binary(self.preds, self.actuals)
    def stats_cum(self, thresholds=10):
        if self.c!=2: raise ValueError('More than two classes')
