...
def get_flat_layers(model):
    children = list(model.children())
    return [model] if len(children) == 0 else [ci for c in children for ci in get_flat_layers(c)]

def model_freeze_layers(model, freeze):
    layers = get_flat_layers(model)
    for p in [p for i,l in enumerate(layers) for p in l.parameters() if i<freeze]:
