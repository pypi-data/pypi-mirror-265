...
class BaseConfig:
    def __init__(
        ...
        from abfy.src.utils.config_utils import set_metrics
    ...
    def _validate_ab_config(self):
        ...
        if self.interaction:
            if len(self.experiment_groups) > 2:
                raise InputConfigError(
                    "abfy only support two way interaction of two experiment groups! There are more than two experiment_groups in the config."
                )
