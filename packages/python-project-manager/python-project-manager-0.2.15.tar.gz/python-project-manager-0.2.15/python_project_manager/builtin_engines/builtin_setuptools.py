...
def verify(_method, **_kwargs) -> bool:
    ...
    if missing_packages:
        ...
        print(f'Run `pip install {" ".join(missing_packages)}` to install the missing packages.')
        return False
...
def _build():
    os.system(Config.parse(f'{VENV} && ppm version inc -t && python -m build', Config._value_config))
def _install():
    os.system(Config.parse(f'{VENV} && pip install %dist_dir%/%twine.wheel%-%version%-py3-none-any.whl --force-reinstall', Config._value_config))
def _uninstall():
    os.system(Config.parse(f'{VENV} && pip uninstall %dist_dir%/%twine.wheel%-%version%-py3-none-any.whl', Config._value_config))
def _publish():
    os.system(Config.parse(f'{VENV} && IF EXIST %dist_dir% (del /S /Q %dist_dir%\*) && python -m build && twine upload -u %twine.username% -p %twine.password% -r pypi %dist_dir%/*', Config._value_config))
def _publish_patch():
    os.system(Config.parse(f'{VENV} && ppm version inc -p 1 && {_publish_command}', Config._value_config))
def _publish_minor():
    os.system(Config.parse(f'{VENV} && ppm version inc -m 1 && {_publish_command}', Config._value_config))
def _publish_major():
    os.system(Config.parse(f'{VENV} && ppm version inc -M 1 && {_publish_command}', Config._value_config))