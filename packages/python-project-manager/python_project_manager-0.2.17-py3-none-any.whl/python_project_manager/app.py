...
@click.option('--engine', '-e', type=str, default='',
    help='Choose the engine \'module\' to use. Built in engines are \'ppm-builtin-setuptools\'')
...
def init(project_name: str, engine: str, force: bool) -> None:
    ...
    if pass_command_to_engine('verify', _verify, _engine=engine,
        force=force, project_name=project_name, engine=engine):
...
@click.option('--raw', '-r', is_flag=True, help='Run the script without any modifications')
def run(script_name, raw) -> None:
    '''
