...
from typing import List, Dict, Callable, Any, Type, Optional, MutableMapping, Union, Tuple, Set
...
class RestFast(object):
    ...
    def __init__(self, host, http2: bool = False, retry_times: int = 3, retry_backoff_factor: int = 5):
        ...
        if http2:
            self.__session.mount(self.__host, HTTP20Adapter(max_retries=retry))
...
class Rest(object):
    ...
    def __initialize(self, file: str = None, server_name: str = None, host: str = None, headers: Dict[str, str] = None,
        ...
        self.__session: Session = Session()
        if http2:
            self.__session.mount(self.host, HTTP20Adapter())
        if file:
            path = Path(file)
