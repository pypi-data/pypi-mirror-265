...
from relationalai.rel_utils import assert_no_problems
from relationalai.loaders.loader import emit_delete_import, import_file, list_available_resources
...
class Resources(ResourceProvider):
    ...
    def list_imports(self, model:str):
        return [*list_available_resources(self, model, self.get_default_engine()).values()]
    ...
    def delete_import(self, import_name: str, model:str):
        res = self.exec_raw(model, self.get_default_engine(), emit_delete_import(import_name), False)
    ...
    def exec_raw(self, database:str, engine:str, raw_code:str, readonly=True, raw_results=True, inputs: dict = {}):
        try:
            ...
            setattr(err, "msg",  f" {res.get('message', '')} {res.get('details', '')}")
            raise
