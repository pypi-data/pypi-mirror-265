...
def show_transactions(transactions, limit, all_users):
    ...
    if len(transactions):
        ...
        for txn in transactions:
            if not all_users and txn.get("created_by") != config.get("user"):
                continue
            if added >= limit:
                break
            ...
            if duration is None:
                created_on = txn["created_on"].replace(tzinfo=datetime.timezone.utc)
                duration = (datetime.datetime.now(datetime.timezone.utc) - created_on).total_seconds() * 1000
