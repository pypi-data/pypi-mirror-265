...
class Resources(ResourceProvider):
    ...
    def list_imports(self, model:str) -> list[Import]:
        results = self._exec(f"""
            select FQ_OBJECT_NAME
            from {APP_NAME}.api.data_streams
            where RAI_DATABASE='{model}';
        """)
        if not results:
            return []
        return [cast(Import, {"name": name, "type": "snowflake_table"}) for (name,) in results.fetchall()]
    ...
    def cancel_pending_transactions(self):
        for txn_id in self._pending_transactions:
            self._exec(f"CALL {APP_NAME}.api.CANCEL_TRANSACTION('{txn_id}');")
