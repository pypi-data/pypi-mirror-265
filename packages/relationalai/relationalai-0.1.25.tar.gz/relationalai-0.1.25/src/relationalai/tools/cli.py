...
def import_source_csv_options_flow(provider: ResourceProvider, source: ImportSourceFile, default_options:dict) -> dict:
    default_schema = {k.strip(): rel_schema_to_type(v.lower()) for k, v in default_options.get("schema", {}).items()}
    if source.is_url():
        # @FIXME: Should maybe prompt user to provide a schema manually for urls?
        return {**default_options, "schema": default_schema}

    schema, csv_chunk = CSVLoader.guess_schema(source.raw_path)
    ...
    for field, type in schema.items():
        table.add_row(field, type.name, f"[dim]{csv_chunk[field][0]}")

    rich.print(table)
    if controls.confirm(f"Use this schema for {source.name}:", True):
        return {**default_options, "schema": schema}
    else:
        ...
        print()
        rich.print(f"You can manually specify column types using schema arguments. For example, to load the column [cyan]{field}[/cyan] as a [green]string[/green], run:")
        print()
        rich.get_console().print(f"[dim]    rai {prev_cmd_args}{saved_args}[/dim] schema:[cyan]{field}[/cyan]=[green]string[/green]", highlight=False)
        divider()
        sys.exit(0)
