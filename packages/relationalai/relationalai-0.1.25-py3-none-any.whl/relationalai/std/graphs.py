...
class Compute:
    ...
    def cosine_similarity(self, node1, node2):
        ...
        return self._lib.similarity.cosine_similarity(self._graph, node1, node2)

    def jaccard_similarity(self, node1, node2):
        return self._lib.similarity.jaccard_similarity(self._graph, node1, node2)
    ...
    def louvain(self, node):
        config = self._config(graph=self._graph)
        return self._lib.community_detection.louvain(config, node)
...
class Graph:
    def __init__(self, model:dsl.Graph, undirected=False, weighted=False, default_weight=1.0):
        ...
        if weighted:
            create_graph = f"""create_graph[{{(:weight, {self.edges._prop("weight")._name})}}, {{(:is_weighted) ; {"" if undirected else "(:is_directed)"} }}]"""
        else:
            create_graph = f"{graph_type}[{self.edges._type._name}]"

        self.model.install_raw(f"""
        bound {self.nodes._type._name}
        bound {self.edges._type._name}
        {f"bound {self.edges._prop('weight')._name}" if weighted else ""}
    ...
    def visualize(self, three=False, style: dict | None = None, use_cache = False, **kwargs):
        ...
        fig = vis(graph_dict, edge_label_data_source='label', node_label_data_source='label', edge_curvature=0.4, **kwargs)
        return fig
