...
@LastEditTime: 2024-03-29 15:53:26
...
class OrderBaseModel(FrameBaseModel):
    ...
    def sync_shake_shop_pay_order(self, app_id, act_id, module_id, user_id, login_token, handler_name, request_code, is_log=False, asset_type=3, while_count=1, ascription_type=0, reward_order_status_list = [105, 2, 101, 3, 5]):
        ...
        :last_editors: HuangJianYi
        """
        frane_base_model = FrameBaseModel(context=self)
        try:
            ...
            if invoke_result_data.success == True:
                ...
                shake_shop_base_model = ShakeShopBaseModel(context=self, app_key=shake_shop_config["app_key"], app_secret=shake_shop_config["app_secret"], shop_id=shake_shop_config["shop_id"])
                ...
                order_data = []
                if act_info_dict['start_date'] != "":
                    invoke_result_data = shake_shop_base_model.get_user_order_list(user_info_dict["open_id"], 100, 0, TimeHelper.format_time_to_timestamp(act_info_dict['start_date']), while_count=while_count, is_log=is_log)
                    if invoke_result_data.success == True:
                        order_data = invoke_result_data.data
                else:
                    invoke_result_data = shake_shop_base_model.get_user_order_list(user_info_dict["open_id"], 100, 0, while_count=while_count, is_log=is_log)
                    if invoke_result_data.success == True:
                        order_data = invoke_result_data.data
                    ...
                    for order in reward_order_list:
                        try:
                            ...
                            if order["sub_order_id"] not in pay_order_no_list and int(order["order_status"]) in reward_order_status_list and order["after_sale_info"]["refund_status"] != '3': # 订单状态;1 待确认/待支付-订单创建完毕;105-已支付; 2-备货中； 101-部分发货; 3-已发货（全部发货）;4- 已取消;5 已完成（已收货）;
                                ...
                                third_pay_order.sku_name = self.json_dumps(order['spec'])
                            ...
                            continue
                    if len(pay_order_list) > 0:
                        result = ThirdPayOrderModel(context=self).set_sub_table(app_id).add_list(pay_order_list)
                        if result == True and buy_num > 0:
    ...
    def _get_shake_shop_pay_order_no_list(self, app_id, user_id):
        ...
        pay_order_cache_key = f"tt_pay_order_no_list:appid_{app_id}_userid_{user_id}"
        pay_order_no_list = redis_init.lrange(pay_order_cache_key,0,-1)
        pay_order_list = []
        is_add = False
        if not pay_order_no_list or len(pay_order_no_list)<=0:
