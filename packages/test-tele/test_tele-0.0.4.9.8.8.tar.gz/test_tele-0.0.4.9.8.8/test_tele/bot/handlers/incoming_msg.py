...
from test_tele.bot.bot_header import loop_message, youtube_url_validation, instagram_url_validation #, hentaifox_to_telegraph
...
async def check_prefix(query: str):
    ...
    if query.startswith('.px'):
        ...
        mode = {}
        keywords = await instagram_url_validation(query.replace(".ig", "").strip())
...
async def get_images_dict(query, keywords, offset):
    if query.startswith('.px'):
        ...
        from test_tele.features.extractors.instagram import get_images_list
        lists = await get_images_list(keywords)
    return lists
...
async def download_media_url_handler(event, is_premium: bool=False):
    ...
    if yt_link:
        ...
        with yt_dlp.YoutubeDL({'quiet': True, 'format': "mp4", 'outtmpl': f'outputs/{video_title}.%(ext)s', 'postprocessors': []}) as ydl:
...
def get_incoming_msg_handlers_telethon(val) -> dict:
    ...
    if val == 0: # bot
        incoming_msg_handlers_for_bot = {
            ...
            "get_images": (get_images_from_inline, events.NewMessage(pattern=r".(?:px|rp|ig)")),
        }
