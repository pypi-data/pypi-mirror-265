...
class ManifiestoInfo (EcuInfo):
	...
	def getVehiculoRemolqueInfo (self, vehicleType):
		keysVehiculo = {"marca": "04_Camion_Marca", "anho": "05_Camion_AnoFabricacion", 
		                "placaPais": "06_Camion_PlacaPais", 
		                "chasis": "07_Camion_Chasis", "certificado": "08_Certificado_Habilitacion"}
		keyRemolque = {"marca": "09_Remolque_Marca", "anho": "10_Remolque_AnoFabricacion", 
		               "placaPais": "11_Remolque_PlacaPais", 
		...
		def getValidValue (value):
			if value.upper().startswith ("XX") or value.upper () == "N/A" or value is None:
				return None
			else: 
				return value

		try:
			certificado = self.getCheckCertificadoString (keys ["certificado"], vehicleType)

			if certificado:
				vehiculo ["marca"]       = getValidValue (Utils.getValue (self.fields, keys ["marca"]))
				vehiculo ["anho"]        = getValidValue (Utils.getValue (self.fields, keys ["anho"]))
				placaPaisText            = getValidValue (Utils.getValue (self.fields, keys ["placaPais"]))
				if placaPaisText:
					placaPais            = Extractor.getPlacaPais (placaPaisText) if placaPaisText else None
					vehiculo ["pais"]    = getValidValue (placaPais ["pais"])
					vehiculo ["placa"]   = getValidValue (placaPais ["placa"])
				vehiculo ["chasis"]      = getValidValue (Utils.getValue (self.fields, keys ["chasis"]))
				vehiculo ["certificado"] = certificado
				vehiculo ["tipo"]        = getValidValue (Utils.addLow ("CAMION" if vehicleType=="vehiculo" else "SEMIRREMOLQUE"))
