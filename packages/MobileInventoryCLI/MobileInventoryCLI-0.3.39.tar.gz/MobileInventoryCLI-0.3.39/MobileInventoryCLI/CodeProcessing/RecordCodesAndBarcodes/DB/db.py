...
class RTemplate:
    ...
    def init(self,*args,**kwargs):
        for i in kwargs:
            fields=[z.name for z in self.__table__.columns]
            if i in fields:
                setattr(self,i,kwargs.get(i))
            else:
                raise Exception(f"Invalid field try:{fields}, instead of '{i}'")


    def __str__(self,name_color=Fore.light_magenta,fieldColor=Fore.light_yellow,value_color=Fore.light_green):
        m=[]
        m1=[]
        m.append(f"{name_color}{self.__class__.__name__}{Style.reset}(")
        default=fieldColor
        default_v=value_color
        for num,i in enumerate(self.__table__.columns):
            if num%2==0:
                fieldColor=Fore.dark_goldenrod
                value_color=Fore.cyan
            else:
                fieldColor=default
                value_color=default_v
            m1.append(f'\t{fieldColor}{i}{Style.reset}={value_color}{getattr(self,i.name)}{Style.reset}')
        m.append(',\n'.join(m1))
        m.append(")")
        return '\n'.join(m)

    def __repr__(self):
        return self.__str__(name_color='',fieldColor='',value_color='')

class RecieptBilling(BASE,RTemplate):
    __tablename__="RecieptBilling"
    RecieptBillingId=Column(Integer,primary_key=True)
    SellerOrBusinessName=Column(String)
    SellerOrBusinessAddress=Column(String)
    RetailersPermitSerial=Column(String)
    RetailersCertificateOfRegistration=Column(String)
    PurchaserName=Column(String)
    PurchaserAddress=Column(String)
    DateSold=Column(Date)
    PaymentForm=Column(String)
    def __init__(self,**kwargs):
        self.init(**kwargs)

class RecieptEntry(BASE,RTemplate):
    __tablename__="RecieptEntry"
    RecieptEntryId=Column(Integer,primary_key=True)
    RecieptBillingId=Column(Integer)

    EntryName=Column(String)
    EntryNote=Column(String)
    EntrySize=Column(String)
    ...
    QtySold=Column(Integer)
    def __init__(self,**kwargs):
        self.init(**kwargs)

RecieptBilling.metadata.create_all(ENGINE)
RecieptEntry.metadata.create_all(ENGINE)

class AdditionalExpenses(BASE,RTemplate):
    __tablename__="AdditionalExpenses"
    ExpenseName=Column(String)
    AdditionalExpensesId=Column(Integer,primary_key=True)
    ExpenseValue=Column(Float)
    ExpenseNote=Column(String)
    ExpenseDate=Column(Date)
    def __init__(self,**kwargs):
        self.init(**kwargs)

    def helpTxt(self,sprint=True):
        h='''
when needing to take into account tax info like say getting net from gross
z=AdditionalExpenses(
    ExpenseName="Income Tax",
    ExpenseValue=0.178,
    ExpenseNote="TAx on my paycheck"
    ExpenseDate=None,
    ExpenseId=1
)
gross=100 //could be $100 dollar check
net=gross-(gross*z.ExpenseValue) // after tax
'''
        if sprint:
            print(h)
        return h

AdditionalExpenses.metadata.create_all(ENGINE)