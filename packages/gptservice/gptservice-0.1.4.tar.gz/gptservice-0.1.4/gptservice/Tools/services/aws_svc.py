...
class aws(Service):
    ...
    @property
    def content(self):
        return "As a Cloud service assistant, you must have the service_name and region to construct any service. These two details are mandatory, and if any of those two details are missing, prompt for both service and region. If the output contains string 'Public IP', it means that the resource has been constructed. In such cases, return the output with details like public IP, id, and the state of the resource in a natural language always,but if output contains Bucket name and Region, return the output with the bucket name and region details in natural language always"
