...
class Subscriptions(Nmi):
    ...
    @postProcessingOutput
    def update_subscription(self, subscription_id: str, customer_vault_id:str, billing_id: str, billing_info ):
        data = {
            "recurring": "update_subscription",
            "security_key": self.security_token,
            "subscription_id": subscription_id,
        ...
        response = requests.post(url="https://secure.networkmerchants.com/api/transact.php", data=data)
        del data["security_key"]
        return {"response": response, "req": data,  "type": 'update_subscription', "org": self.org}
