...
def rename(paths, rename_dirs, regex, regex_eval, hidden):
    ...
    if (not renamed or renamed == original):
        utils.log.fatal('nothing''s changed')

    renamed = renamed.split('\n')

    if (len(renamed) != len(to_rename)):
        utils.log.fatal('wrong number of rows')
    ...
    for d, pairs in utils.group_by(lambda x: dirname(x[0]), zip(to_rename, renamed)).items():
        ...
        if dirOps is None:
            utils.log.fatal('rename conflict')
    ...
    try:
        ...
        if filter:
            tags = {k.strip().lower(): v.strip() if isinstance(v, str) else
                    ...
                    if k.strip() in {'FileType', 'Identifier', 'Title', 'BookName', 'Author', 'Publisher', 'Creator', 'Date', 'Artist', 'TrackID', 'Album', 'ContentCreateDate', 'Year'}}
            if 'date' not in tags and ('contentcreatedate' in tags or 'year' in tags):
                tags['date'] = tags.get('contentcreatedate') or tags.get('year')
                if 'contentcreatedate' in tags:
                    del tags['contentcreatedate']
                if 'year' in tags:
                    del tags['year']
        return tags
