...
class WA:
    ...
    def sql_connection(self, uid, pwd, server, database = 'Vista_Replication_PRD', driver = 'SQL Server'):
        uid = parse.quote_plus(uid)
        pwd = parse.quote_plus(pwd)
        self.engine = create_engine(f"mssql+pyodbc://{uid}:{pwd}@{server}/{database}?driver=ODBC+Driver+17+for+SQL+Server&TrustServerCertificate=yes")
        try:
