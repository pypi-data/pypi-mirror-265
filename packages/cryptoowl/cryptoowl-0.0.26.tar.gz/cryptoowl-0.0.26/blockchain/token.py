...
class Token:
    @classmethod
    def get_token_by_symbol(cls, symbol, limit=1):
        query = GET_TOKEN_DETAILS_FROM_SYMBOL_QUERY
        if data := read_db.fetchall(query=query, values={"symbol": symbol, "limit": limit}):
            chain, token_id, liquidity, marketcap, vol_24_hr, token_symbol = data[0]
            return True, {"token_id": token_id, "chain": chain, "token_symbol": token_symbol.upper()}
        else:
            return False, f"No token found for {symbol}"

    @classmethod
    def get_token_for_multiple_symbol(cls, symbols, limit=1):
