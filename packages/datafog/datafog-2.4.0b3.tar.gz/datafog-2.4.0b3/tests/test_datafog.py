...
def test_upload_file_pptx(datafog):
    uploaded_pptx = "tests/files/input_files/abc.pptx"
    result = datafog.upload_file(uploaded_file_path=uploaded_pptx)
    file_text = result[
        uploaded_pptx.split("/")[-1]
    ]  # Extract the text using the file name as key
    assert "RAG" in file_text  
    assert "Zetta" in file_text
    assert "DataFog" in file_text
    assert "RAG" in file_text
    assert "PII" not in file_text
    assert "SkyFlow" in file_text
    assert "Gretel" not in file_text
