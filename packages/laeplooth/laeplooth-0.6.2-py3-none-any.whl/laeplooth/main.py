...
class Translator:
    ...
    def spoonerism2syl(self, syl, origInSyl):
        ...
        #  ใหลครุย ใลครุย
        if syl[posSwap1].endswith('หล') and syl[posSwap2][0] in thai_consonants and syl[posSwap2][0] != 'ห':
    ...
    def check_syllable(self):
        ...
        # ตลาด -> ตะ หลาด
        #  prevent อย, คล
        if len(self.syl[0]) > 3 and all(char in thai_consonants for char in self.syl[0][:2]) and (self.syl[0][1] != 'ว') and (self.syl[0][2] != 'ะ') and 'รร' not in self.syl[0] and self.syl[0][:2] != 'อย' and self.syl[0][:2] != 'คล':
