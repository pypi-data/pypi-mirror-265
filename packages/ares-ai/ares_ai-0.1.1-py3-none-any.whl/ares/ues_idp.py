...
def ues_idp_config(in_domain_prompts_dataset: str, unlabeled_evaluation_set: str, context_relevance_system_prompt: str, answer_relevance_system_prompt: str, answer_faithfulness_system_prompt: str, model_choice: str) -> dict:
    ...
    if model_choice in gpt_models:
        with tqdm(total=5000, desc="Evaluating large subset with {model_choice}") as pbar:
            for index, row in unlabeled_evaluation_set[:5000].iterrows():
        ...
        with tqdm(total=5000, desc=f"Evaluating large subset with {model_choice}") as pbar:
            for index, row in unlabeled_evaluation_set[:5000].iterrows():
                # Extract query, document, and answer from the row
                query = row['Query']
                document = row['Document']
