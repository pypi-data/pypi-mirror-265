...
def plot_loglog_convergence(step, erros, **kwargs):
    """
    ...
    """
    fig = plt.figure()
    plt.title(kwargs.get('title', 'Error Analysis'))
    plt.xlabel(kwargs.get('x_label', 'Step size h (log scale)'))
    plt.ylabel(kwargs.get('y_label', 'Error (log scale)'))
    plt.loglog(step, step, linestyle=":", label="$y=x$")
    plt.loglog(step, step**2, linestyle=":", label="$y=x^2$")
    plt.loglog(step, erros, label=kwargs.get('label1', 'equidistant'))
    if 'errot' in kwargs:
        plt.loglog(kwargs['stept'], kwargs['errot'], label=kwargs.get('label2', 'nonequidistant'))
    plt.legend()
    
    save_title = kwargs.get('save_title', None) 
