...
if NETBOX_CURRENT_VERSION >= version.parse("3.2"):
    from nb_service_ntt.views_3_x import ServiceChangeLogView as ObjectChangeLogView
else:
...
urlpatterns = [
    ...
    path(
        "service/<int:pk>/edit/", views.ServiceEditView.as_view(), name="service_edit"
    ),
    path(
        "service/<int:pk>/diagram/",
        views.ServiceDiagramView.as_view(),
        name="service_diagram",
    ),
    path("service/<int:pk>/IC/", views.ServiceICView.as_view(), name="service_IC"),
    path(
        "service/<int:pk>/relation/",
        views.ServiceRelationView.as_view(),
        name="service_relation",
    ),
    path("relation/add", views.RelationEditView.as_view(), name="relation_add"),
    path(
        "relation/<int:pk>/edit/",
    ...
    path(
        "pentest/<int:pk>/edit/", views.PenTestEditView.as_view(), name="pentest_edit"
    ),
    path(
        "pentest/<int:pk>/delete/",
        views.PenTestDeleteView.as_view(),
        name="pentest_delete",
    ),
    ...
    path(
        "application/<int:pk>/devices",
        views.ApplicationDevicesView.as_view(),
        name="application_devices",
    ),
    path(
        "application/<int:pk>/vms",
        views.ApplicationVMsView.as_view(),
        name="application_vms",
    ),
    path(
        "application/add", views.ApplicationEditView.as_view(), name="application_add"
    ),
    path(
...
NETBOX_CURRENT_VERSION = version.parse(settings.VERSION)
if NETBOX_CURRENT_VERSION >= version.parse("3.2"):
    from nb_service_ntt.views_3_x import ServiceChangeLogView, ApplicationChangeLogView

    urlpatterns_3_2 = [
        path(
            "service/<int:pk>/changelog",
            ServiceChangeLogView.as_view(),
            name="service_changelog",
            kwargs={"model": models.Service},
        ),
        path(
            "application/<int:pk>/changelog",
            ApplicationChangeLogView.as_view(),
