...
class PacksNameCache(caching.IntegralRuntimeCache):
    ...
    def get(self, dimensions, default=None):
        result = super().get(dimensions, default)

        if result:
            short_name = result[2].get_short_name()

            if short_name in ControllerCache.overrides:
                pack_instance = ControllerCache.overrides[short_name]
                result = (pack_instance.__class__, pack_instance.get_absolute_url(), pack_instance)
