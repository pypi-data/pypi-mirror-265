...
def get_object_or_application_logic_exc(model, *args, **kwargs):
    """Обёртка для встроенного шортката get_object_or_404.

    Args:
        model: класс модели
        *args: аргументы для get().
        **kwargs: аргументы для get().

    Returns: полученный объект указанной модели

    Raises: вызывает исключение ApplicationLogicException в случаях, если записи не существует или
        существует больше чем 1 запись
    """

    try:
        result = get_object_or_404(model, *args, **kwargs)
    except (Http404, model.MultipleObjectsReturned):
        raise ApplicationLogicException('Получено ноль или несколько записей!')
