...
def generate_hash_unique(name, hash=hashlib.md5):
    ...
    hash_content = "puremlHash".join(
        [org_id, name, time_current, string_random]
    )
...
def generate_hash_for_file(
    file_path, name: str, hash=hashlib.md5, is_empty=False
):

    if is_empty:
        hash_value = generate_hash_unique(name=name)
    else:
        hash_obj = hash()
