...
def add(
    ...
    print()
    console = Console()
    table = Table("Sr.No.", "Integration Name")
    sr_no = 1
    for integration in backend_schema.INTEGRATIONS:
        table.add_row(str(sr_no), backend_schema.INTEGRATIONS[integration]["name"])
        sr_no += 1
    console.print(table)
    print()
    ...
    while not secret_name:
        secret_name = typer.prompt("Enter a unique secret name for this integration")
