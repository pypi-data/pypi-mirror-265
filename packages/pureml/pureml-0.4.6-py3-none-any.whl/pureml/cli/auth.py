...
def signup(
    ...
    try:
        ...
        logger.debug("Signup data", data=data)
        ...
        logger.info("Response from backend", response_status=response.status_code)

        if not response.ok:
            logger.error("Could not create account", response=response)
            print(f"[red]Could not create account! Please try again later[/red]")
            return
        elif response.status_code == 202:
            print(
                f"[yellow]Successfully created your account! You need to verify your email to login![/yellow]"
            )
            print(f"[green]You can then login using `pureml auth login`[/green]")
        else:
            print(f"[green]Successfully created your account![/green]")
            print(f"[green]You can now login using `pureml auth login`[/green]")
    except Exception as e:
        print(f"[red]Could not create account! Please try again later[/red]")
        logger.error("Exception occurred while siging up", error=e)
    ...
    try:
        ...
        if interactive:
            ...
            if response.ok:
                ...
                logger.error("Unable to login to your account", response=response)
                print(f"[red]Unable to login to your account![/red]")
        ...
        # Browser based login
            ...
            if not response.ok:
                ...
                print(f"[red]Unable to create session! Please try again later[/red]")
                return

            session_id = response.json()["data"][0]["session_id"]

            # Generater link & Open browser
            link = urljoin(frontend_base_url, f"auth/verify-session/{session_id}")
            print(link)
                ...
                while True:
                    ...
                    if response.ok:
                        ...
                        if response.status_code == 404:
                            print(
                                f"[red]Session not found! Please try again later[/red]"
                            )
                            return
                        elif response.status_code == 403:
                            print(
                                f"[red]Session expired or invalid device! Please try again later[/red]"
                            )
                            return
        ...
        print(f"[red]Unable to contact the server! Please try again later[/red]")
...
def status():
    ...
    logger.info("User status", exists=exists)
    if exists:
