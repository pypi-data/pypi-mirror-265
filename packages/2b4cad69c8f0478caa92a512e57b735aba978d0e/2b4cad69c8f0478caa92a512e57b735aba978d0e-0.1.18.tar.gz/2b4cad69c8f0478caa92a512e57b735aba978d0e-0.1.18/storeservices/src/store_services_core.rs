...
pub struct StoreServicesCoreADIProxy {
    ...
    fairplay_import_keybag: sysv64_type!(fn(context: u64, keybag: *const u8, keybag_length: u32, out_result: *mut u64, out_flag: *mut u64) -> i32),

    fairplay_get_kbsync: sysv64_type!(fn(context: u64, ds_id: i64, zero: u32, flag: u64, out_kbsync: *mut *const u8, out_kbsync_length: *mut u32) -> i32),
    ...
    fn get_kbsync(&mut self, context: u64, ds_id: i64, flag: u64) -> PyResult<KBSyncResult> {
        unsafe {
            ...
            if result == 0 {
                ...
                kbsync_data.copy_from_slice(std::slice::from_raw_parts(kbsync, kbsync_length as usize));

                Ok(KBSyncResult { kbsync: kbsync_data })
            } else {
                Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
