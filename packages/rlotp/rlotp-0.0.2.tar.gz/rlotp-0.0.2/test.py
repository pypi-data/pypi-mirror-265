...
class HOTPExampleValuesFromTheRFC(unittest.TestCase):
    def test_match_rfc(self):
        ...
        hotp = rlotp.HOTP("GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ")
    ...
    def test_provisioning_uri(self):
        hotp = rlotp.HOTP("wrn3pqx5uqxqvnqr", name="mark@percival")
        ...
        self.assertEqual(hotp.provisioning_uri(), rlotp.parse_uri(hotp.provisioning_uri()).provisioning_uri())

        hotp = rlotp.HOTP("wrn3pqx5uqxqvnqr", name="mark@percival", initial_count=12)
        url = urlparse(hotp.provisioning_uri())
        self.assertEqual(url.scheme, "otpauth")
        self.assertEqual(url.netloc, "hotp")
        self.assertEqual(url.path, "/mark%40percival")
        ...
        hotp = rlotp.HOTP("wrn3pqx5uqxqvnqr", name="mark@percival", issuer="FooCorp!")
        url = urlparse(hotp.provisioning_uri())
        self.assertEqual(url.scheme, "otpauth")
        self.assertEqual(url.netloc, "hotp")
        ...
        hotp = rlotp.HOTP(key, digits=8, digest=hashlib.sha256, name="baco@peperina", issuer="FooCorp")
        url = urlparse(hotp.provisioning_uri())
        self.assertEqual(url.scheme, "otpauth")
        self.assertEqual(url.netloc, "hotp")
        ...
        hotp = rlotp.HOTP(key, digits=8, name="baco@peperina", issuer="Foo Corp", initial_count=10)
        url = urlparse(hotp.provisioning_uri())
        self.assertEqual(url.scheme, "otpauth")
        self.assertEqual(url.netloc, "hotp")
...
class TOTPExampleValuesFromTheRFC(unittest.TestCase):
    ...
    def test_match_rfc(self):
        for digest, secret in self.RFC_VALUES:
            totp = rlotp.TOTP(base64.b32encode(secret), 8, digest)
    ...
    def test_match_rfc_digit_length(self):
        totp = rlotp.TOTP("GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ")
    ...
    def test_match_google_authenticator_output(self):
        totp = rlotp.TOTP("wrn3pqx5uqxqvnqr")
    ...
    def test_provisioning_uri(self):
        totp = rlotp.TOTP("wrn3pqx5uqxqvnqr", name="mark@percival")
        ...
        totp = rlotp.TOTP("wrn3pqx5uqxqvnqr", name="mark@percival", issuer="FooCorp!")
        url = urlparse(totp.provisioning_uri())
        self.assertEqual(url.scheme, "otpauth")
        self.assertEqual(url.netloc, "totp")
        self.assertEqual(url.path, "/FooCorp%21:mark%40percival")
        ...
        totp = rlotp.TOTP(key, digits=8, name="baco@peperina", issuer="FooCorp")
        url = urlparse(totp.provisioning_uri())
        self.assertEqual(url.scheme, "otpauth")
        self.assertEqual(url.netloc, "totp")
        self.assertEqual(url.path, "/FooCorp:baco%40peperina")
    ...
    def test_random_key_generation(self):
        self.assertEqual(len(rlotp.random_base32()), 32)
        self.assertEqual(len(rlotp.random_base32(length=34)), 34)
        self.assertEqual(len(rlotp.random_hex()), 40)
        self.assertEqual(len(rlotp.random_hex(length=42)), 42)
        with self.assertRaises(ValueError):
            rlotp.random_base32(length=31)
        with self.assertRaises(ValueError):
            rlotp.random_hex(length=39)


class SteamTOTP(unittest.TestCase):
    def test_match_examples(self):
        steam = rlotp.contrib.Steam("BASE32SECRET3232")
        ...
        steam = rlotp.contrib.Steam("FMXNK4QEGKVPULRTADY6JIDK5VHUBGZW")
...
class CompareDigestTest(unittest.TestCase):
    method = staticmethod(rlotp.utils.compare_digest)
...
class StringComparisonTest(CompareDigestTest):
    method = staticmethod(rlotp.utils.strings_equal)
...
class ParseUriTest(unittest.TestCase):
    ...
    def test_algorithms(self):
        ...
        otp = rlotp.parse_uri("otpauth://totp?algorithm=SHA1&secret=GEZDGNBV&algorithm=SHA1&period=60")
        self.assertEqual(hashlib.sha1, otp.digest)
        ...
        otp = rlotp.parse_uri("otpauth://hotp?algorithm=SHA1&secret=GEZDGNBV&algorithm=SHA1")
        self.assertEqual(hashlib.sha1, otp.digest)
        self.assertEqual(otp.at(0), "734055")
        ...
        otp = rlotp.parse_uri("otpauth://hotp?algorithm=SHA1&secret=GEZDGNBV&algorithm=SHA1&counter=1")
        self.assertEqual(hashlib.sha1, otp.digest)
        ...
        otp = rlotp.parse_uri("otpauth://totp?algorithm=SHA1&secret=GEZDGNBV&algorithm=SHA256")
        self.assertEqual(hashlib.sha256, otp.digest)
        self.assertEqual(otp.at(0), "918961")
        self.assertEqual(otp.at(9000), "934470")

        otp = rlotp.parse_uri("otpauth://totp?algorithm=SHA1&secret=GEZDGNBV&algorithm=SHA512")
        ...
        otp = rlotp.parse_uri(otp.provisioning_uri(name="n", issuer_name="i", image="https://test.net/test.png"))
        self.assertEqual(hashlib.sha512, otp.digest)

        otp = rlotp.parse_uri("otpauth://totp/Steam:?secret=FMXNK4QEGKVPULRTADY6JIDK5VHUBGZW&encoder=steam")
        self.assertEqual(type(otp), rlotp.contrib.Steam)
        ...
        otp = rlotp.parse_uri("otpauth://totp/Steam:?secret=FMXNK4QEGKVPULRTADY6JIDK5VHUBGZW&period=15&digits=7&encoder=steam")
        self.assertEqual(type(otp), rlotp.contrib.Steam)
        self.assertEqual(otp.at(0), "C5V56")
        self.assertEqual(otp.at(30), "QJY8Y")
        self.assertEqual(otp.at(60), "R3WQY")
