...
kegg_background_gene_set = KEGG_DATA_DIR / Path('kegg_background_V' + str(VERSION_NUM) + '.txt')
...
def _build_dre_mapped_pathway(human_pathways_path, human_pathways_dir, dre_mapped_dir):
    ...
    for pathway_id in human_pathways_df['Pathway ID']:
        ...
        out_file_name = dre_mapped_dir / Path('dreM'+stripped_pathway_id+'.txt')
...
def _create_kegg_background():
    # Directory containing gene ID files
    directory = zebrafish_pathways_dir
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.txt'):  # Assuming all files end with .txt
            ...
            # Read gene IDs from the file and add them to the set
            unique_gene_ids.update(_read_gene_ids_from_file(file_path))

    # Write unique gene IDs to an output file
    output_file = os.path.join(KEGG_DATA_DIR, 'kegg_background_V'+str(VERSION_NUM)+'.txt')
    with open(output_file, 'w') as file:
        # Write the header
        file.write("NCBI Gene ID\n")
        for gene_id in sorted(unique_gene_ids):  # Sort for consistency
            file.write(gene_id + '\n')
