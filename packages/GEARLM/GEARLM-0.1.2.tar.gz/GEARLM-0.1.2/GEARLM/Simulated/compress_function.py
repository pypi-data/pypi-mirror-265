...
def compress_insert_function(
    ...
    if compress_config.compress_method[layer_idx] == "groupquantization_kc_vt_preserving":
        ...
        if previous_value is not None:
            previous_value[:,:,compress_config.start_saving[layer_idx]:-compress_config.locality_saving[layer_idx],:] = (
                fake_groupwise_token_asymmetric_quantization(
                    previous_value[:,:,compress_config.start_saving[layer_idx]:-compress_config.locality_saving[layer_idx],:],
                    compress_config.quantize_bit[layer_idx],
                    128,
    ...
    if compress_config.compress_method[layer_idx] == "outliterquantize_with_lrap_preserving":
        smaller_dim = seq_len if seq_len <= num_head * sep_dim else num_head * sep_dim
        smaller_dim = int(smaller_dim)
        rank_k = int(smaller_dim * compress_config.rank[layer_idx])
        rank_v = int(smaller_dim * compress_config.rankv[layer_idx])
        # [bsz, num_head, seq_len, sep_dim]
        
        previous_key[:,:,compress_config.start_saving[layer_idx]:-compress_config.locality_saving[layer_idx],:] = fake_outquant_with_lrap(
            previous_key[:,:,compress_config.start_saving[layer_idx]:-compress_config.locality_saving[layer_idx],:],
            compress_config.quantize_bit[layer_idx],
            rank_k,
            compress_config.loop[layer_idx],
        ...
        if previous_value is not None:
            previous_value[:,:,compress_config.start_saving[layer_idx]:-compress_config.locality_saving[layer_idx],:] = fake_outquant_with_lrap(
                previous_value[:,:,compress_config.start_saving[layer_idx]:-compress_config.locality_saving[layer_idx],:],
                compress_config.quantize_bit[layer_idx],
                rank_v,
                compress_config.loop[layer_idx],
