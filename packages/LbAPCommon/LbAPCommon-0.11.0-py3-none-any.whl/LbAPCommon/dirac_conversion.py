...
def group_in_to_requests(jobs_data):
    ...
    for node in G.nodes():
        if G.in_degree(node) > 1:
            raise InvalidAPJob(f"Job {node} has more than one parent.")

    # Find roots (nodes with in-degree == 0)
    roots = [node for node, degree in G.in_degree() if degree == 0]

    # Function to get paths from root to leaf
    def get_paths_from_root(G, root):
        paths = []
        for node in nx.descendants(G, root):
            # Since it's a linear path, just find the path from root to each node
    ...
    all_paths = []
    for root in roots:
        paths_from_root = get_paths_from_root(G, root)
        if paths_from_root:  # If there are paths from this root
            all_paths.extend(paths_from_root)
        else:  # If the root is a standalone node
