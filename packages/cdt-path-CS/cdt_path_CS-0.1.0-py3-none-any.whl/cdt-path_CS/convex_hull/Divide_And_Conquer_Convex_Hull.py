...
from Display import *
# Incremental_Convex_Hull(P)
def Divide_And_Conquer_Convex_Hull(P):
	return Divide_And_Conquer_Convex_Hull_Sorted(Simplify_With_X_i(P[P[:,0].argsort()]))[0]

def Divide_And_Conquer_Convex_Hull_Sorted(P):
	i=0
	l=len(P)
	if l<6:
	...
	A, i_axmax=Divide_And_Conquer_Convex_Hull_Sorted(P[:l//2])
	B, i_bxmax=Divide_And_Conquer_Convex_Hull_Sorted(P[l//2:])
	return Combin_Convex_Hull_M_2(A,B,i_axmax,i_bxmax)
	

def Tangent_U(A,B,a1):
	b=0
	la=len(A)
	lb=len(B)
