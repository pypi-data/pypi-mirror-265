...
class Scanner:
    ...
    def open_browser(self):
        ...
        edge_options.add_argument("--profile-directory=Temporal")
        ...
        edge_options.add_argument("--disable-web-security")  # Deshabilita la política de mismo origen
        edge_options.add_argument("--allow-file-access-from-files")  # Permite el acceso a archivos locales
        ...
        while end is False:
            time.sleep(1)
            try:
                if self.driver.current_url != url and self.driver.current_url != "data:,":
                    try:
                        if self.driver.execute_script(f"localStorage.getItem('myPort')") != str(self.port_api):
                            self.driver.execute_script(f"localStorage.setItem('myPort', '{self.port_api}')")
                    except WebDriverException:
                        print("Reintentando cargar el puerto en el localstorage.")
                    url = self.driver.current_url
                    url_domain = self.get_url_domain(url)
                    if domain != url_domain:
                        domain = url_domain
                        self.event_recorder_handler({"event": "url log",
                                                     "target": url,
                                                     "value": url,
                                                     "frame": None})
    ...
    def event_recorder_handler(self, event_recorded):
        """
            Description:
        ...
        if event_recorded['event'] == 'log user input':
            ...
            if not self.eventsRecordedList or not self.compare_events_and_ignore_key(self.eventsRecordedList[-1], event_recorded, 'order'):
                event_recorded['order'] = self.recorder_elements_order
    ...
    def event_logger_message(self, event):
        ...
        if event['event'] == 'delete card step':
            self._api.callback_refresh_message(f"Se elimina el paso número {event['value'] + 1}")
    ...
    def normalizer_list_to_json(self, list_element: list):
        ...
        for object_element in list_element:
            #new_object_name = self.generate_random_name(object_element["TAGNAME"])
    ...
    async def inspector_mode(self, activated, recorder_mode = False):
        ...
        if activated:
            if recorder_mode:
                self.driver.execute_script(f"window.inspectorCalledFrom = 'recorder';")
                self.driver.execute_script("activateLocationModeOn()")
            else:
                self.driver.execute_script(f"window.inspectorCalledFrom = 'inspector';")
                self.driver.execute_script("activateLocationModeOn()")
        else:
