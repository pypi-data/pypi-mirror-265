...
function addRecordedStepsToHtmlLanding() {
            ...
            <div id="ActionBox${index}">
                <div class="row valign-wrapper margin-tb-1">
                    <div class="col s12 blue-grey lighten-5 rounded-tiny padding-tb-1 z-depth-1">
                        ...
                        <div class="valign-wrapper margin-tb-1">
            ...
            <div id="ActionBox${index}">
                <div class="row valign-wrapper margin-tb-1">
                    <div class="col s12 blue-grey lighten-5 rounded-tiny padding-tb-1 z-depth-1">
                        ...
                        <div class="valign-wrapper margin-tb-1">
                            <span class="left margin-l-1 material-icons">send</span>
                            <input type="text" disabled value='${element.value}'>
                        </div>
                    </div>
                </div>
    ...
    update_dropdowns();
    // scroll hacia el ultimo elemento de la lista, si existe
...
function addObjectRow(listObjects) {
    ...
    bloqueHTML = '<span class="new badge btn-floating teal darken-2" data-badge-caption="" id="cantElements">' + numberElement + '</span>'
...
function addInfractionRow(listInfractions) {
    ...
    for (const category in groupedInfractions) {
        ...
        var generarBloqueTags = function (categoryTags) {
            ...
            for (let tag of categoryTags) {
                if (tag == "wcag2a" || tag == "wcag2aa") {
                    bloqueTags += '<div class="chip red darken-2 white-text">' + tag + '</div>';
                } else {
        ...
        var bloqueHTML =
            `<div class="card blue-grey lighten-5 z-depth-1 rounded-small">
                ...
                <div class="card-action blue-grey lighten-3 rounded-small">${generarBloqueTags(categoryTags)}
                </div>
            </div>`;
    ...
    bloqueHTML = '<span class="new badge btn-floating red darken-2" data-badge-caption="" id="cantInfractions">' + numberInfraction + '</span>'
    containerLoader.innerHTML = bloqueHTML;
...
function activateEditElement(buttonEdit) {
    ...
    var bloqueHTML =
        `<button class="btn teal darken-1" id="saveEditElement" onclick="saveElementXpath(this)">
...
function highlightElement(buttonAction) {
    ...
    pywebview.api.call_highlight_element(valueXpath.value, nameObject.textContent);
}

function copyElement(buttonAction) {
    // LLama a la funcion call_highlight cuando se presiona un boton desde la vista.
    const nameObject = buttonAction.parentNode.parentNode.querySelector("span").textContent;
...
function deleteActionsCards() {
    const modalInstance = M.Modal.getInstance(document.getElementById('deleteStepsModal'));
    deleteAllSteps()
    modalInstance.close()
}
