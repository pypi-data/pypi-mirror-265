...
class Api:
    ...
    def call_save_to_file(self, list_element):
        ...
        """
        Api._scanner.save_to_file(Api.normalizer_dict_list(list_element))
        self.callback_refresh_message(f"Guardando archivo: nombre_archivo.json")
    ...
    def generate_script(self):
        """ diccionario_entrante = [
                {
                    "ORDER": 1,
                    "EVENT": {
                        "TYPE": "click",
                        "TARGET": {
                            "TAGNAME": "<button>_Texto_Bienvenidos",
                            "XPATH": "//button[@id=\"APjFqb\"]",
                            "PARENT": {
                                "FRAME": "IFRAME",
                                "XPATH": "//iframe[@class='iframe']"
                            }
                        },
                        "VALUE": None
                    }
                },
                {
                    ...
                    "EVENT": {
                        "TYPE": "send_keys",
                        "TARGET": {
                            "TAGNAME": "<input>_ingreso_mail",
                            "XPATH": "//input[@id=\"APjFqb\"]",
                            "PARENT": {
                                "FRAME": "IFRAME",
                                "XPATH": "//iframe[@class='iframe']"
                            }
        ...
        PythonScriptToFile(diccionario_entrante).main() """
        lista_original = Api._scanner.get_elemets_clicked_list()
        if lista_original is not []:
            print(lista_original)
            ScriptToFile(lista_original).main()
        else:
    ...
    def reset_recorder_steps(self):
        Api._scanner.event_recorder_handler({'event': 'reset recorder steps'})

    def delete_card_step(self, step_index):
        Api._scanner.event_recorder_handler({
