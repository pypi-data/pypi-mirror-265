...
class NewteraException(Exception):
    """Base Newtera exception."""


class InvalidResponseError(NewteraException):
...
class ServerError(NewteraException):
...
A = TypeVar("A", bound="NewteraError")


class NewteraError(NewteraException):
    """
    ...
    def copy(self, code: str, message: str) -> NewteraError:
