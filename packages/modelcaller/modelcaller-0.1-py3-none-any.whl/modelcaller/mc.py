...
class ModelCaller(): # main MC class
    ...
    def wrap_host(self, kind='function'): # wrap this MC around a host (model or function) with optional context  (replacing the current host)
        def decorator(host):
            def wrapper(*xs, cvals=None):
                cvals = cvals or list() # set default
                self._ncargs = len(cvals)
                if  kind == 'model':
                    nfargs = len(xs) - self._ncargs # number of fn args
