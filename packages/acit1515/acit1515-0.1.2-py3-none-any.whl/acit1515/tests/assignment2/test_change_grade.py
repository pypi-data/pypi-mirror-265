...
class Change_Grade(Test):
    ...
    def test1_returns(self):
        ...
        valid_dir_string = "/".join(__file__.split('\\')[:-1]) + '/transcripts'
    ...
    def test2_hardcoded(self):
        ...
        dir = Path("/".join(__file__.split('\\')[:-1]))
        ...
        try:
            ...
            if data == more_data or data["grades"] == more_data["grades"]:
                returns = False
    ...
    def test3_valid(self):
        ...
        if not returns:
            print(Fore.RED + f"FAIL: {self.name} does not successfully update a transcript file (-1 mark)" + Fore.RESET)
