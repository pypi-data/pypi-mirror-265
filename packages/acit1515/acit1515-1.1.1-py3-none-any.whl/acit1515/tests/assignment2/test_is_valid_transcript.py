...
class Is_Valid_Transcript(Test):
    ...
    def test4_valid(self):
        ...
        if not self.submission.is_valid_transcript(correct):
            returns = False
        
        if self.submission.is_valid_transcript(incorrect):
            returns = False
        
        if not returns:
            print(Fore.RED + f"FAIL: {self.name} does not return a valid value for correct/incorrect transcripts (-1 mark)" + Fore.RESET)
            return { 'deduction': 1, 'continue': False }

        print(Fore.GREEN + f"PASS: {self.name} returned a valid value for correct/incorrect transcripts (4 marks)" + Fore.RESET)
        return { 'deduction': 0, 'continue': True }