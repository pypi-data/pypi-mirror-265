...
def run_test(test, input_file):
    try:
        path_parts = input_file.split(sep)
        mod = path_parts[-1]
        ...
        results = []
        directory = "/".join(os.path.realpath(__file__).split(sep)[:-1])
...
def run_tests(test_suite, target_directory):
    ...
    if not res['ERROR_STATE']:
        for submission in res['output_path'].glob('*.py'):
            print(Fore.CYAN + f'\nRUNNING TESTS FOR .{sep}{submission}' + Fore.RESET)
            try:
                run_test(test_suite, f'.{sep}{submission}')
                print()
            except Exception as e:
