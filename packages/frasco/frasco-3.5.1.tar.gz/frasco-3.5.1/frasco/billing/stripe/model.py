...
class StripeSubscriptionModelMixin(StripeModelMixin):
    ...
    def get_stripe_product_price(self, plan=None):
        prices = stripe.Price.list(lookup_keys=[plan or self.plan_name])
        if not len(prices.data):
            return stripe.Price.retrieve(plan)
    ...
    def subscribe_stripe_plan(self, plan=None, quantity=1, item_options=None, trial_end=None, force=False, **kwargs):
        ...
        price = self.get_stripe_product_price(plan)
        item = dict(price=price, quantity=quantity)
        if item_options:
