...
def compute_aggregate_materialization_schema_from_view_schema(
    ...
    if not is_aggregate:
        if "DEV_EMBEDDINGS_INPUT_COLUMNS" not in feature_view_args.options:
            return view_schema
        col_names = feature_view_args.options["DEV_EMBEDDINGS_INPUT_COLUMNS"].split(",")
        embedding_schema = core_schema.Schema.from_dict(
            {f"{col}_embedding": tecton_types.ArrayType(tecton_types.Float32Type()) for col in col_names}
        )
