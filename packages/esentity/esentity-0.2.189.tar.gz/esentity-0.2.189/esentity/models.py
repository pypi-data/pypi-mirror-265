...
class ElasticEntity(BaseEntity):
    ...
    def aggs_request(cls, aggs, _range=None, _from=0, _count=0, _sort={}, _must=[], **kwargs):
        if kwargs:
            def pre_field(k, v):
                return k if isinstance(v, (bool, int)) else '{0}.keyword'.format(k)
            _must = _must + [{"terms" if isinstance(v, list) else "term": {pre_field(k, v): v}} for k, v in kwargs.items()]
        ...
        if _range:
            if isinstance(_range, dict):
                ...
                _f, _s, _e, _tz = _range
                _must.append({"range": {_f: {"gte": _s, "lte": _e, "time_zone": _tz}}})
...
class Page(ElasticEntity):
    ...
    @classmethod
    def provider_by_context(cls, country, _exclude=[], _exclude_params={}, _count=12, _page=1, _offset=None, _source=["title", "alias", "logo"], _aggs={}, _sorting=None, _locale=None, **params):
        filter_list =  [ 
            {"term": {"is_active": True}},
            {"term": {"is_draft": False}},
            {"term": {"is_sandbox": False}},
