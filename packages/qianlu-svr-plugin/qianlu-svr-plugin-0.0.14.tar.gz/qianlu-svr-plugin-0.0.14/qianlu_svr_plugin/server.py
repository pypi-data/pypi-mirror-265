...
class QianluService(object):
    ...
    def on_message(self, client, message):
        ...
        msg['from'] = to
        if len(dataObj) > 0:
            if "serviceID" in dataObj and 'functionName' in dataObj and dataObj['serviceID'] == "onMsg" and dataObj['functionName'] == "onMsg":
                try:
                    if self.onMessageFunction is not None:
                        result = self.onMessageFunction(dataObj['args'])
