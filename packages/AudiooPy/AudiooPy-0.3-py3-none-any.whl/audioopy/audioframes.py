...
class AudioFrames(object):
    ...
    def resample(self, in_rate, out_rate=16000):
        ...
        try:
            ...
            logging.warning("Deprecated. Since 'audioop' will be removed of the Python standard library, "
                            "re-sampling will not be fully available from Python 3.13. "
                            "Any help is welcome to implement a self-implemented solution.")
            try:
                import audioop
    ...
    def _re_sample(self, in_rate, out_rate):
        ...
        """
        AudioFrames._re_sample_valid_rates(in_rate, out_rate)
    ...
    @staticmethod
    def _re_sample_valid_rates(in_rate, out_rate):
        """Check if given frame rates are supported for re-sampling.

        :raise: NotImplementedError

        """
        if in_rate < out_rate:
            raise NotImplementedError("re-sample error: Only down-sampling is available. "
                                      "Can't increase framerate from {:d} to {:d}Hz"
                                      "".format(in_rate, out_rate))
        if out_rate != 16000:
            raise NotImplementedError("re-sample error: Only down-sampling to 16kHz is available. "
                                      "Can't decrease framerate from {:d} to {:d}Hz"
                                      "".format(in_rate, out_rate))
        if in_rate not in (192000, 96000, 48000, 32000, 16000):
            raise NotImplementedError("re-sample error: Only down-sampling from 48 or 32kHz is available. "
                                      "Can't decrease framerate from the exotic rate of {:d} to {:d}Hz"
                                      "".format(in_rate, out_rate))
