...
class Router(RouterBase):
    ...
    def get_match_ignore_method(self, request: Request) -> Optional[RouteMatch]:
        ...
        """
        test = self.get_match_by_path_any_method(request._path)
        print(f"Match {test}")
    ...
    def get_match_by_path_any_method(self, path: AnyStr) -> Optional[RouteMatch]:
        ...
        for method, routes in self.routes.items():
            print(f"Checking method {method} (routes: {routes})")
            for route in routes:
                print(f"Checking route {route} (method {method})")
                match = route.match_by_path(bytes_value)
                print(f"Check Match: {match}")
                if match:
                    print(f"Found match for {path}: {match}")
                    return match
        return None
