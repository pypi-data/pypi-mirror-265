...
class Build:
    ...
    customized menu-based TUI that will guide the user through all the vailable
    options.
    ...
    @param data:
        ...
        - "trigger":
            ...
            - When "type" is "positional argument", "trigger" has a special
              meaning (a "positional argument" does not have an acompanying
        ...
        - "value":
            ...
            For "option" and "positional argument" can be any any of this:
            ...
            4. (2) followed by a pipe ("|") followed by (3). The default value
               will be that of the referenced environment variable if it is set
               or (2) otherwise.
    ...
    @param env_help
        Text to show when a option parameter is configure to take its default
    ...
    @param style
        ...
        - "enpoint" : style used in menu entries that run the final command when
                      selected
        - "entry"   : style used in menu entries that have a flag, option or
                      positional argument value associated
    ...
    def __init__(
        ...
        self._qualifier = {
            ...
            "entry"      : "x['type'] in ['flag', 'option', 'positional argument']",
        }
    ...
    def _resulting_command(self):
        ...
        for entry, path in self._traverse():
            ...
            elif entry["type"] == "option" and entry["value"]:
                cmd.append(entry["trigger"])
                if " " in entry["value"]:
            ...
            elif entry["type"] == "positional argument" and entry["value"]:
                if " " in entry["value"]:
                    value = "\"" + entry["value"] + "\""
                else:
                    value = entry["value"]
        ...
        # "positional arguments" always go at the end of the cmd line. First of
    ...
    def _process_input(self, key, stdscr):
        ...
        if key == "j" or key == "KEY_DOWN":
            ...
            if x[self._current_selection[-1]]["type"] == "menu":
            ...
            elif x[self._current_selection[-1]]["type"] in ["option",
                                                            "positional argument"]:
                ...
                def convert(key):
                    if key == ord("\b") or key == ord("\x7f"):
                        return curses.KEY_BACKSPACE
                    return key

                for c in x[self._current_selection[-1]]["value"]:
                    textbox.do_command(c)

                curses.curs_set(1)
                textbox.edit(convert)
                curses.curs_set(0)

                x[self._current_selection[-1]]["value"] = \
                        textbox.gather().strip()
