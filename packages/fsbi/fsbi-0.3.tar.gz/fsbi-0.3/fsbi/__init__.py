...
expand.i = {None:'', 'i':'Italic', 'o':'Oblique'}

def fsbi(files, list=False, restore=False, stretch=False, write=False):
	r = []
	for f in sum([glob.glob(f'{f}/*' if os.path.isdir(f) else f) for f in files], []):
		try:
			r.append((f, (n := expand(f) if restore else compact(f,stretch)) ))
			if list: print(f'{f} -> {n}')
...
def main():
	a = argparse.ArgumentParser(usage='fsbi [-lrsw] [file] ...', add_help=False)
	...
	a.add_argument('-s', action='store_true', help='stretch 5 if normal')
	a.add_argument('-w', action='store_true', help='write')
	a = a.parse_args()
	fsbi(a.file, a.l or not a.w, a.r, a.s, a.w)
