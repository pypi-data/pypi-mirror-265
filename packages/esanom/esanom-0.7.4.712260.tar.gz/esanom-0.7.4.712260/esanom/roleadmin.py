...
def sample_data( ) :
    ...
    options = {
        "api" : { "name" : _util.get_test_uid( postfix = f"NOTIFICATIONEMAIL_MODEL" ) , "enable" : 1 } ,
        "group" : [ config[ f"group0_name" ] ] 
    } 
    create_model( options )

    # model : math_increment
    options = {
        "api" : { "name" : _util.get_test_uid( postfix = f"MATHINCREMENT_MODEL" ) , "enable" : 1 } ,
        "group" : [ config[ f"group0_name" ] ] 
    } 
    create_model( options )
    ...
    # model : test_nop
    options = {
        "api" : { "name" : _util.get_test_uid( postfix = f"TESTNOP_MODEL" ) , "enable" : 1 } ,
        "group" : [ config[ f"group0_name" ] ] 
    } 
    create_model( options )
