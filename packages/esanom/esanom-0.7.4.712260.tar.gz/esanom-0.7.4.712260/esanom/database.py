...
def init( ) :
    ...
    # FIXME TODO find a better way to detect tahtw e don't want database init
    ...
    # FIXME TODO keep api as first table to create otherwise it fails (see below)
    ...
    for sql_file in sql_files :
        ...
        try :
            ...
            # FIXME TODO keep api as first table to create otherwise it fails
...
def init_tables( ) :
    ...
    api_id = insert_fromdict( "api" , { "name" : "admin" , "email" : _config.get( "admin_api_email" ) , "ip" : _config.get( "admin_api_ip" ) , "enable" : 1 , "email_confirmed" : 1 } )
    role_id = insert_fromdict( "role" , { "name" : "admin" , "enable" : 1 } )
    insert_fromdict( "api_role" , { "api_id" : api_id , "role_id" : role_id } )

    # user/model/dashboard role
    role_id = insert_fromdict( "role" , { "name" : "user" , "enable" : 1 } )
    role_id = insert_fromdict( "role" , { "name" : "model" , "enable" : 1 } )
    role_id = insert_fromdict( "role" , { "name" : "dashboard" , "enable" : 1 } )
...
def wait_until_ready( retries = 10 ) :
...
def io_info( model_name ) :
    ...
    for mport_row in mport_rows :
        ...
        for port_pin_row in port_pin_rows :
            ...
            pin_data = { "name" : pin_row[ "name" ] , "cardinal" : port_pin_row[ "cardinal" ] }
...
def io_insert( task_uid , rdata , pred_io_uid = None) :
...
def model_load_ports( rolemodel_id , ports ) :
    ...
    for port_data in ports :
        ...
        port_name = f"{rolemodel_row['name']}:{port_data['name']}"
        ...
        for pin_data in port_data[ "pins" ] :
            ...
            if pin_row_global == None :
                pin_name = f"{rolemodel_row['name']}:{pin_data['name']}"
            ...
            for unit_data in pin_data[ "units" ] :
                ...
                unit_name = f"{rolemodel_row['name']}:{unit_data['name']}"
...
def handle_overrides( task_uid ) :
    ...
    for mport_row in mport_rows :
        ...
        for mpin_row in mpin_rows :
            handle_override( task_row[ "id" ] , mpin_row[ "id" ] , mpin_row[ "override" ] )
...
def pin_override_map( override_key , task_id ) :
    ...
    if override_key == "pipeline_name" :

        pipeline_task_row = db_query_select_row( "SELECT * from pipeline_task WHERE task_id=%s OR next_task_id=%s LIMIT 1" , [ task_id , task_id ] )
        pipeline_row = db_query_select_row( "SELECT * from pipeline WHERE id=%s LIMIT 1" , [ pipeline_task_row[ "pipeline_id" ] ] )

        return( f"Pipeline Name : {pipeline_row['name']}" )
