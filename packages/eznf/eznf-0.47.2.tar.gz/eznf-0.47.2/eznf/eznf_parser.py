...
class Not:
    ...
    def to_cls(self):
        ...
        raise NotImplementedError(f"Tseint transformation not yet implemented for {self.child}")
...
class Implies:
    ...
    def to_cls(self):
        ...
        if isinstance(self.left, Literal):
            ...
            return Or(Or(Not(self.left.left), Not(self.left.right)), self.right).to_cls()
            ...
            return And(Or(And(self.left.left, Not(self.left.right)), self.right), 
                        Or(And(self.left.right, Not(self.left.left)), self.right)).to_cls()
            # (a <-> b) -> c
            # a ^ {-b} -> c
            # 
        else:
            raise NotImplementedError(f"Implies not implemented for this case: {self.left}, {self.right}")
