...
class Modeler:
    ...
    def add_clause(self, clause) -> None:
        ...
        for lit in numerical_clause:
            if abs(lit) not in self._rvarmap:
                self.add_var(f"_anonymous_var_by_number_{abs(lit)}", var_number=abs(lit))
    ...
    def at_most_k(self, variables, k) -> None:
        hsh = hash(tuple(variables))
        self.add_svar(f"__counting_vars_{hsh}", "COUNTING_VARS", variables=variables)
        if k >= len(variables):
            return # nothing to enforce in this case :) it's vacuously true
        self.add_clauses([-self.v(f"__counting_vars_{hsh}_{k+1}")])
...
class CAtMostOne:
    ...
    def to_clauses_o(self) -> [[int]]:
        if len(self.variables) <= 4:
            ...
            return CAtMostOne(head, self.modeler).to_clauses_o() + CAtMostOne(tail, self.modeler).to_clauses_o()
    ...
    def to_clauses_2(self) -> [[int]]:
        if len(self.variables) <= 4:
            ...
            return CAtMostOne(head, self.modeler).to_clauses_2() + CAtMostOne(tail, self.modeler).to_clauses_2()
