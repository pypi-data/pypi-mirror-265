...
from dharitri_sdk_core.codec import decode_unsigned_number
from dharitri_sdk_core.transaction_parsers.token_operations_outcome_parser_types import (
    AddQuantityOutcome, BurnOutcome, BurnQuantityOutcome, DCTIssueOutcome,
...
class TokenOperationsOutcomeParser:
    ...
    def parse_issue_fungible(self, transaction: ITransactionOnNetwork) -> DCTIssueOutcome:
        ...
        return DCTIssueOutcome(token_identifier)

    def parse_issue_non_fungible(self, transaction: ITransactionOnNetwork) -> DCTIssueOutcome:
        wrapper = TransactionOnNetworkWrapper.from_transaction(transaction)
        wrapper.ensure_no_error()
    ...
    def parse_issue_semi_fungible(self, transaction: ITransactionOnNetwork) -> DCTIssueOutcome:
        wrapper = TransactionOnNetworkWrapper.from_transaction(transaction)
        wrapper.ensure_no_error()
    ...
    def parse_register_meta_dct(self, transaction: ITransactionOnNetwork) -> DCTIssueOutcome:
        wrapper = TransactionOnNetworkWrapper.from_transaction(transaction)
        wrapper.ensure_no_error()

        event = wrapper.find_single_event_by_identifier("registerMetaDCT")
        token_identifier = self._extract_token_identifier(event)
        return DCTIssueOutcome(token_identifier)
    ...
    def parse_register_and_set_all_roles(self, transaction: ITransactionOnNetwork) -> RegisterAndSetAllRolesOutcome:
        ...
        event_set_role = wrapper.find_single_event_by_identifier("DCTSetRole")
    ...
    def parse_set_special_role(self, transaction: ITransactionOnNetwork) -> SetSpecialRoleOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTSetRole")
    ...
    def parse_nft_create(self, transaction: ITransactionOnNetwork) -> NFTCreateOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTNFTCreate")
        token_identifier = self._extract_token_identifier(event)
    ...
    def parse_local_mint(self, transaction: ITransactionOnNetwork) -> MintOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTLocalMint")
        user_address = event.address.bech32()
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
    ...
    def parse_local_burn(self, transaction: ITransactionOnNetwork) -> BurnOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTLocalBurn")
        user_address = event.address.bech32()
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
    ...
    def parse_pause(self, transaction: ITransactionOnNetwork) -> PausingOutcome:
        ...
        _ = wrapper.find_single_event_by_identifier("DCTPause")
    ...
    def parse_unpause(self, transaction: ITransactionOnNetwork) -> PausingOutcome:
        ...
        _ = wrapper.find_single_event_by_identifier("DCTUnPause")
        return PausingOutcome()
    ...
    def parse_freeze(self, transaction: ITransactionOnNetwork) -> FreezingOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTFreeze")
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
    ...
    def parse_unfreeze(self, transaction: ITransactionOnNetwork) -> FreezingOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTUnFreeze")
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
        balance = self._extract_amount(event)
        user_address = self._extract_address(event)
    ...
    def parse_wipe(self, transaction: ITransactionOnNetwork) -> WipingOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTWipe")
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
        balance = self._extract_amount(event)
        user_address = self._extract_address(event)
    ...
    def parse_update_attributes(self, transaction: ITransactionOnNetwork) -> UpdateAttributesOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTNFTUpdateAttributes")
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
    ...
    def parse_add_quantity(self, transaction: ITransactionOnNetwork) -> AddQuantityOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTNFTAddQuantity")
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
    ...
    def parse_burn_quantity(self, transaction: ITransactionOnNetwork) -> BurnQuantityOutcome:
        ...
        event = wrapper.find_single_event_by_identifier("DCTNFTBurn")
        token_identifier = self._extract_token_identifier(event)
        nonce = self._extract_nonce(event)
