...
from dharitri_sdk_core.interfaces import IAddress, IToken, ITokenTransfer
from dharitri_sdk_core.transaction import Transaction
from dharitri_sdk_core.transaction_factories.token_transfers_data_builder import \
...
class IConfig(Protocol):
    ...
    gas_limit_dct_transfer: int
    gas_limit_dct_nft_transfer: int
    gas_limit_multi_dct_nft_transfer: int
...
class TransferTransactionsFactory:
    ...
    def create_transaction_for_dct_token_transfer(self,
                                                   sender: IAddress,
                                                   receiver: IAddress,
        ...
        if len(token_transfers) == 0:
            ...
            if self.token_computer.is_fungible(transfer.token):
                data_parts = self._data_args_builder.build_args_for_dct_transfer(transfer)
                extra_gas_for_transfer = self.config.gas_limit_dct_transfer + ADDITIONAL_GAS_FOR_DCT_TRANSFER
            else:
                data_parts = self._data_args_builder.build_args_for_single_dct_nft_transfer(transfer, receiver)
                extra_gas_for_transfer = self.config.gas_limit_dct_nft_transfer + ADDITIONAL_GAS_FOR_DCT_NFT_TRANSFER
                receiver = sender
        else:
            data_parts = self._data_args_builder.build_args_for_multi_dct_nft_transfer(receiver, token_transfers)
            extra_gas_for_transfer = self.config.gas_limit_multi_dct_nft_transfer * len(token_transfers) + ADDITIONAL_GAS_FOR_DCT_NFT_TRANSFER
            receiver = sender
