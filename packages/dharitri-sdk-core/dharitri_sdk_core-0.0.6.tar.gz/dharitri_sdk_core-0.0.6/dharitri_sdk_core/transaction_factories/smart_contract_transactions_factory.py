...
class SmartContractTransactionsFactory:
    ...
    def create_transaction_for_execute(self,
        ...
        if native_transfer_amount and number_of_tokens:
            raise BadUsageError("Can't send both native token and custom tokens(DCT/NFT)")
        ...
        if len(token_transfers) == 1:
            ...
            if self.token_computer.is_fungible(transfer.token):
                data_parts = self._data_args_builder.build_args_for_dct_transfer(transfer=transfer)
            else:
                data_parts = self._data_args_builder.build_args_for_single_dct_nft_transfer(
                    transfer=transfer, receiver=receiver)
                receiver = sender
        elif len(token_transfers) > 1:
            data_parts = self._data_args_builder.build_args_for_multi_dct_nft_transfer(
