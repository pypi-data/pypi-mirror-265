...
class IDCTIssueConfiguration(ITransactionBuilderConfiguration, Protocol):
    gas_limit_dct_issue: IGasLimit
    issue_cost: ITransactionValue
    dct_contract_address: IAddress


class DCTIssueBuilder(TransactionBuilder):
    def __init__(self,
                 config: IDCTIssueConfiguration,
                 issuer: IAddress,
                 token_name: str,
                 token_ticker: str,
                 initial_supply: int,
        ...
        self.gas_limit_dct_issue = config.gas_limit_dct_issue

        self.sender = issuer
        self.receiver = config.dct_contract_address
    ...
    def _estimate_execution_gas(self) -> IGasLimit:
        return self.gas_limit_dct_issue
