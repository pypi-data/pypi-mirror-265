...
import numpy as np
from typing import Sequence, overload, Any, TypeVar, SupportsFloat
...
class CartesianStateDerivative:
    ...
    def asarray(self):
        """Return state as numpy array"""
        return np.array([self.dx, self.dy, self.dz, self.ddx, self.ddy, self.ddz])
...
class KeplerianState:
    ...
    def _constrain_angle(self, angle: Vector, limits: tuple[float, float]):
        ...
        if limits[0] != 0.0:
            raise NotImplementedError("Non-zero lower limits are not supported yet")
