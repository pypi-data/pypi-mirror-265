...
def sanityze_path_to_file(path: typing.Union[str, None]) -> typing.Union[str, None]:
    if path is None:
        return None
...
def sanityze_port(port: typing.Union[int, None]) -> typing.Union[int, None]:
...
def create_and_sanitize_user_creds(args) -> UserCredentials:
    ...
    if is_default_pkcs and (args.cert != DEFAULT_USER_CERT_PATH or args.key != DEFAULT_USER_KEY_PATH):
        args.pkcs = None
    cert = sanityze_path_to_file(args.cert)
    key = sanityze_path_to_file(args.key)
    pkcs = sanityze_path_to_file(args.pkcs)
...
def main():
    ...
    try:  # noqa: WPS225
        if args.which is None:
            ...
            register_port = sanityze_port(args.register_port)
        ...
        if args.which == _COMMAND_NEW_VM:
            user_creds = create_and_sanitize_user_creds(args)
            disk = sanityze_path_to_file(args.disk)
