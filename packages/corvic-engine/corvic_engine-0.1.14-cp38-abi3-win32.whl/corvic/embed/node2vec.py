...
class KeyedVectors:
    ...
    def __init__(self, *, dim: int, index_to_key: pl.Series, node_ids: list[str]):
...
class Space:
    ...
    def __init__(
        ...
        *,
        ...
        columns = (
            nodes.select(pl.struct(self._node_ids).alias("id"))
            if len(self._node_ids) != 1
            else nodes.select(pl.col(self._node_ids).alias("id"))
        )
        nodes = nodes.with_columns(columns)
    ...
    def make_keyed_vectors(self, dim: int) -> KeyedVectors:
        ...
        return KeyedVectors(
            dim=dim, index_to_key=self._index_to_key, node_ids=self._node_ids
        )
