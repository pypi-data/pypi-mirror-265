...
class Karpet:
    ...
    def fetch_crypto_historical_data(self, symbol=None, slug=None):
        """
        ...
        * total_volume
        ...
        search_data = self.get_quick_search_data()

        if not len(search_data):
            raise Exception("No search data available.")

        found_item = list(filter(lambda item: item["slug"] == slug, search_data))
        ...
        # Compile URLs.
        url = (
            f"https://www.coingecko.com/price_charts/{found_item[0]['id']}/usd/max.json"
        )
        url_market_cap = (
            f"https://www.coingecko.com/market_cap/{found_item[0]['id']}/usd/max.json"
        )
    ...
    def _get_coin_slug_from_params(self, symbol=None, slug=None):
        ...
        if (not symbol and not slug) or (symbol and slug):
