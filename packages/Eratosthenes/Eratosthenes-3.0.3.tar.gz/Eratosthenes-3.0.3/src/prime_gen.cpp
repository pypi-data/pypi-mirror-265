...
std::vector<BigInteger> SieveOfEratosthenes(const BigInteger& n)
{
    ...
    DispatchQueue dispatch(threadCount);
    dispatch.finish();
...
size_t CountPrimesTo(const BigInteger& n)
{
...
std::vector<BigInteger> SegmentedSieveOfEratosthenes(BigInteger n)
{
    ...
    std::vector<BigInteger> _knownPrimes = SieveOfEratosthenes(limit_simple);
    std::set<BigInteger> knownPrimes(_knownPrimes.begin(), _knownPrimes.end());
    _knownPrimes.clear();
    ...
    // we're free to continue to up to x * x,
    // then we synchronize.
    BigInteger threadBoundary = *knownPrimes.rbegin() + 1U;
    threadBoundary *= threadBoundary;
    std::mutex knownPrimesMutex;
    DispatchQueue dispatch(threadCount);
    dispatch.finish();
    dispatch.resetResult();
    ...
    while (low < nCardinality)
    {
        ...
        if (threadBoundary <= fHi) {
            dispatch.finish();
            threadBoundary = *knownPrimes.rbegin() + 1U;
            threadBoundary *= threadBoundary;
        }

        std::vector<BigInteger> localPrimes(knownPrimes.begin(), knownPrimes.end());
        dispatch.dispatch([fHi, low, high, localPrimes, &knownPrimes, &knownPrimesMutex]() {
            ...
            for (size_t k = 1U; k < localPrimes.size(); ++k) {
                BigInteger p = localPrimes[k];
                if ((p * p) > fHi) {
                    break;
                }
            ...
            for (size_t o = 1U; o <= cardinality; ++o) {
                ...
                std::lock_guard<std::mutex> lock(knownPrimesMutex);
    ...
    return std::vector<BigInteger>(knownPrimes.begin(), knownPrimes.end());
}

size_t SegmentedCountPrimesTo(BigInteger n)
{
    // TODO: This should scale to the system.
    // Assume the L1 cache limit is 128 KB.
    // We save half our necessary bytes by
    ...
    const BigInteger sqrtnp1 = (sqrt(n) + 1U) | 1U;
    const std::vector<BigInteger> knownPrimes = SieveOfEratosthenes(sqrtnp1);
    ...
    // We have chosen segment size as sqrt(n).
    const size_t nCardinality = backward2(n);
    size_t low = backward2(sqrtnp1);
    size_t high = backward2(sqrtnp1) + limit;
    ...
    while (low < nCardinality)
    {
        ...
        dispatch.dispatch([low, high, &knownPrimes, &count, &countMutex]() {
            const BigInteger fHi = forward2(high);
            const BigInteger fLo = forward2(low);
            ...
            // Use the primes found by the simple sieve to find
            // primes in current range.
            for (size_t k = 1U; k < knownPrimes.size(); ++k) {
                BigInteger p = knownPrimes[k];
                if ((p * p) > fHi) {
                    break;
                }
