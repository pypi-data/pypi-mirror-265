...
def determine_cyp2d6_cnv(sample, vcf_dir=None):
    file = os.path.join(vcf_dir, f"No_SULT1A1_{sample}.CEL.vcf")
    file_gz = file + ".gz"
...
def _parse_pharmcat_output(
    ...
    compiled_data = []
    with open(file) as f:
        person_id = os.path.basename(file).split(".")[1]
        ...
        for gene_data in json_data["results"]:
            ...
            if gene == "DPYD":
                ...
                for call_data in gene_data["diplotypes"]:
                    call = call_data["name"]
                    if gene == "G6PD":
                        if xy_ids is not None and person_id in xy_ids:
                ...
                if len(genotypes) == 1:
                    ...
                    if gene == "G6PD" and xy_ids is not None and person_id in xy_ids:
                ...
                if gene == "CYP2D6":
                    if vcf_dir is None:
                        ...
                        cyp2d6_cnv = determine_cyp2d6_cnv(
                            sample=person_id, vcf_dir=vcf_dir
                        )
                    if (
