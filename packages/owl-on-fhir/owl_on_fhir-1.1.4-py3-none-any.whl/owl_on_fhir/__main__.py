...
def _run_shell_command(command: str, cwd_outdir: str = None, verbose=False) -> subprocess.CompletedProcess:
    ...
    if (stderr and 'Unable to create a system terminal, creating a dumb terminal' not in stderr
        and 'warning' not in stderr.lower()):
...
def write_concept_maps(
    obograph_path: str, owl_path: str, outdir: str = None, code_system_id: str = None, verbose=True, retain_intermediaries=False
):
    ...
    try:
        ...
        print('Converting: Obographs -> SSSOM')
        ...
        df = pd.read_csv(outpath_sssom, sep='\t', comment='#')
        if len(df) > 0:
            df = parse_sssom_table(outpath_sssom)
            with open(outpath_concept_map, "w") as file:
                write_fhir_json(df, file)
        else:
            print('Warning: Tried creating ConceptMaps, but was not first able to render any SSSOM mappings. Skipping.',
                  file=sys.stderr)
    finally:
        if not retain_intermediaries:
            os.remove(outpath_metadata)
...
def owl_to_fhir(
    ...
    try:
        ...
        if include_conceptmap:
            write_concept_maps(intermediary_path, input_path, out_dir, code_system_id, retain_intermediaries)
    # Cleanup
    finally:
        if not retain_intermediaries:
