...
from roc.dingo.tools import query_db, valid_time, \
    round_up, gen_sql_filters
from roc.dingo.constants import PIPELINE_DATABASE, TIME_DAILY_STRFORMAT, \
    TRYOUTS, TIME_WAIT_SEC, SQL_LIMIT, SBM_SCI_PKT_LIST, SBM_SCI_PKT_MAX_NB
...
class UpdateSbmRetrieved(Task):
    ...
    def run(self):
        ...
        sbm_trange_func = {
            1: self.compute_sbm1_trange,
            2: self.compute_sbm2_trange
        }
        ...
        for i, current_sbm in enumerate(results):
            ...
            current_sbm_start, current_sbm_end = sbm_trange_func[current_sbm_type](current_sbm)
    ...
    def compute_sbm1_trange(event_data: dict) -> Tuple[int, int]:
        ...
        if end_time_dt3 > end_time:
            logger.debug(f'DT1_SBM1[{dt1}] + DT3_SBM1[{dt3}] > 0.5 * DT2_SBM1[{dt2}] for SBM1 event at {utc_time} [{cuc_time}]')
    ...
    def query_sbm_tm_data(self, sbm_type: int,
                       sbm_obt_start: int,
                       sbm_obt_end: int,
                       model=TmLog) -> List[dict]:
        """
        ...
        """
        # Build query filters
        filters = [model.palisade_id == current_tm
                   for current_tm in SBM_SCI_PKT_LIST[sbm_type]]
        filters = or_(*filters)
        ...
        # Query database
        return query_db(self.session, model,
                            filters=filters,
                            tryouts=self.tryouts,
                            wait=self.wait,
                            limit=self.limit)
    ...
    def update_sbm_log(self, sbm_type: int,
                       sbm_tm_data: pd.core.frame.DataFrame,
                       sbm_log_data: list,
                       model=SbmLog):
        """
        ...
        :param sbm_log_data: sbm_log data for the current event
        ...
        for current_packet in SBM_SCI_PKT_LIST[sbm_type]:
            ...
            if current_length.count() == 0:
                continue
            sbm_info[current_packet]['size'] = int(sum(current_length) + (7 * sbm_info[current_packet]['count']))
            sbm_info[current_packet]['percent'] = 100.0 * (float(sbm_info[current_packet]['count']) /
                                                   float(SBM_SCI_PKT_MAX_NB[sbm_type][current_packet]))

            # Compute total size, count and percent
            total_size += sbm_info[current_packet]['size']
            total_count += sbm_info[current_packet]['count']
    ...
    def get_sbm_sci_pkt_names(sbm_type: int, no_comp=False):
        ...
        """
        if no_comp:
            pkt_list = list(set([current_packet.replace('_C', '')
                        for current_packet in SBM_SCI_PKT_LIST[sbm_type]]))
        else:
