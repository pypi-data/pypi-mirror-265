...
from roc.dingo.constants import PIPELINE_DATABASE, \
    ...
    BIA_SWEEP_LOG_PACKETS, CIWT0130TM, CIWT0131TM, \
    NAIF_SOLO_ID, PACKET_DATA_GROUP, EVENT_TM_APID, TC_ACK_APID, HK_TM_APID, \
    ACQ_TIME_PNAMES
...
class L0ToDb(Task):
    ...
    def run(self):
        ...
        try:
            ...
            logger.exception(
                f'Initializing inputs has failed for job {self.job_id}!')
            self.pipeline.exit()
            return
    ...
    def packet_to_dataframe(self, l0, packet_name):
        ...
        if 'compressed' in l0[packet_name].keys():
            ...
            packet_df.loc[where_compressed,
                          'palisade_id'] = packet_name.split('/')[-1] + '_C'
        ...
        if packet_type == 'TM':
            ...
            packet_info['cuc_coarse_time'] = packet_df['time'].apply(
                lambda x: x[0])
            packet_info['cuc_fine_time'] = packet_df['time'].apply(
                lambda x: x[1])
            ...
            packet_df = pd.concat(
                    [packet_df, pd.DataFrame.from_dict(acq_time_info)],
                    axis=1)
    ...
    def _insert_packet_param(self, packet_name, packet_data):
        ...
        if packet_name in SBM_LOG_PACKETS:
            ...
            columns = get_columns(model,
                                  remove=['id', 'retrieved_time'])
            sbm_type = int(packet_name[-1])
            packet_data['sbm_type'] = [sbm_type] * n_packet
            packet_data['selected'] = [False] * n_packet
            packet_data['retrieved'] = [{'size': 0, 'percent': 0, 'stats': {}}] * n_packet
            packet_data['status'] = [null()] * n_packet
        ...
        try:
            ...
            logger.error(f'Bulk insertion in table {model} may be uncompleted '
