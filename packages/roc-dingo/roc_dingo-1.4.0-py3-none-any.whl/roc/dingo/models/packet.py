...
class TmLog(Base):
    ...
    binary = NonNullColumn(TEXT(), nullable=True,
                           descr='Packet raw binary data (in hexadecimal)')
    data = NonNullColumn(JSONB(), nullable=True,
                           descr='Packet source data (JSON format)')
    sha = NonNullColumn(TEXT(),
                        descr='Packet sha (hexdigest)')
    sequence_cnt = NonNullColumn(BIGINT(), nullable=True,
                                 descr='Packet sequence counter')
    cuc_coarse_time = NonNullColumn(BIGINT(),
                                 descr='Packet creation time (CUC coarse part)')
    cuc_fine_time = NonNullColumn(INTEGER(), nullable=True,
                                 descr='Packet creation time (CUC fine part)')
    acq_coarse_time = NonNullColumn(BIGINT(), nullable=True,
                                 descr='Acquisition time (CUC coarse part)')
    acq_fine_time = NonNullColumn(INTEGER(), nullable=True,
                                 descr='Acquisition time (CUC fine part)')
