...
def change_edge_between_task_nodes(node1,node2):
    ...
    property2 = f'{{last_block: "{node2["last_block"]}", level: "{node2["level"]}", caas_job: "{node2["caas_job"]}", name: "{node2["name"]}", parent_task: "{node2["parent_task"]}", task_id: "{node2["task_id"]}", status: "{node2["status"]}", block_identifier: "{node2["block_identifier"]}"}}'
    
    query = session.run(f" match (n1:node{property1})-[old:`new`]->(n2:node{property2}) merge (n3:node{property1})-[fresh:`success`]->(n4:node{property2}) delete old").data()
