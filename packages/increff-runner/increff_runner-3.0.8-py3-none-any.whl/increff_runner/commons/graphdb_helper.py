...
def get_next_task_nodes(node,task_id,level):
    ...
    query = session.run(f"match (n:node{property})-[r:{task_id}]->(m) return m").data()
    next_nodes = []
    for node in query:
        next_nodes.append(node['m'])
...
def check_last_block_status(task_id,node):
    ...
    query = session.run(f"match (n:node{property}) return count(*)").data()
    total = query[0]['count']
    completed = session.run(f"match (n:node{property}) where n.status = 'SUCCESS' return count(*)").data()[0]['count']
    if total == completed:
        return True
    return False
