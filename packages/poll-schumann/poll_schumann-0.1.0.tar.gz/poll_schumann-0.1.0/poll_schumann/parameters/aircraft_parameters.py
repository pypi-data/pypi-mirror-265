...
from poll_schumann.parameters import constants
...
def impact_pressure_max_operating_limits(max_mach_num: float) -> float:
    ...
    return constants.p_surface * (
        (1.0 + 0.5 * (constants.gamma - 1.0) * v_cas_mo_over_c_msl**2) ** (constants.gamma / (constants.gamma - 1.0)) - 1.0
    )
...
def crossover_pressure_altitude(max_mach_num: float, p_i_max: float) -> float:
    ...
    """
    return p_i_max / (
        0.5 * constants.gamma * max_mach_num**2 * (1.0 + (max_mach_num**2 / 4.0) + (max_mach_num**4 / 40.0))
    )