...
from poll_schumann.parameters import units, operational_limits as op_lim, jet, atmospheric_parameters as atm_params
...
class FuelFlow():
    ...
    def __call__(
        ...
        true_airspeed, mach_num = jet.clip_mach_number(speed_current, air_temperature, max_mach)

        # Reynolds number
        reynolds = atm_params.reynolds_number(self.aircraft_parameters['wing_span'], self.aircraft_parameters['wing_aspect_ratio'], mach_num, air_temperature, air_pressure)
        ...
        path_angle = math.degrees((altitude_next - altitude_current)*ft / (true_airspeed * delta_time)) # approximation for small angles
        dv_dt = jet.acceleration(true_airspeed, delta_time)

        # aircraft performance parameters
        c_lift = atm_params.lift_coefficient(self.aircraft_parameters['wing_surface_area'], mass_current, air_pressure, air_temperature, mach_num, path_angle)
        c_f = atm_params.skin_friction_coefficient(reynolds)
        c_drag_0 = atm_params.zero_lift_drag_coefficient(c_f, self.aircraft_parameters['psi_0'])
        e_ls = atm_params.oswald_efficiency_factor(c_drag_0, self.aircraft_parameters)
        c_drag_w = atm_params.wave_drag_coefficient(mach_num, c_lift, self.aircraft_parameters)
        c_drag = atm_params.airframe_drag_coefficient(c_drag_0, c_drag_w, c_lift, e_ls, self.aircraft_parameters['wing_aspect_ratio'])

        # engine parameters and fuel consumption
        thrust = atm_params.thrust_force(mass_current, c_lift, c_drag, dv_dt, path_angle)

        c_t = atm_params.engine_thrust_coefficient(thrust, mach_num, air_pressure, self.aircraft_parameters['wing_surface_area'])
        c_t_eta_b = atm_params.thrust_coefficient_at_max_efficiency(mach_num, self.aircraft_parameters['m_des'], self.aircraft_parameters['c_t_des'])

        # correct fuel flow
        c_t_available = op_lim.max_available_thrust_coefficient(air_temperature, mach_num, c_t_eta_b, self.aircraft_parameters)
        c_t = np.clip(c_t, 0, c_t_available)

        # engine efficiency
        eta_over_eta_b_min = 0.5
        engine_efficiency = atm_params.overall_propulsion_efficiency(mach_num, c_t, c_t_eta_b, self.aircraft_parameters, eta_over_eta_b_min)

        # fuel flow
        q_fuel = 43.13e6
        fuel_flow = atm_params.fuel_mass_flow_rate(air_pressure, air_temperature, mach_num, c_t, engine_efficiency, self.aircraft_parameters["wing_surface_area"], q_fuel)
        ...
        fuel_flow = atm_params.fuel_flow_correction(
            fuel_flow,
            altitude_current,
            air_temperature,
            air_pressure,
