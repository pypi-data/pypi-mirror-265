...
title_all="药械妆不良反应报表统计分析工作站 V"+version_now
title_all2="药械妆不良反应报表统计分析工作站 V"+version_now
# 第二部分：函数模块 ##################################################################
...
def get_directory_path(directory_path):
    ...
    global csdir
    # 检查目录是否存在 A.txt 文件
    if not (os.path.isfile(os.path.join(directory_path, '0（范例）比例失衡关键字库.xls'))):
        # 解压 A.zip 到指定目录
...
def TOOL_guizheng(data,methon,rt):
	...
	if methon==0:#选择文件自定义规整
		...
		text.insert(END,"\n开展报告单位和监测机构名称规整...")
		listxx1 = pd.read_excel(peizhidir+"0（范例）上报单位.xls",sheet_name="报告单位",header=0,index_col=0,).fillna("没有定义好X").reset_index()
		listxx2 = pd.read_excel(peizhidir+"0（范例）上报单位.xls",sheet_name="监测机构",header=0,index_col=0,).fillna("没有定义好X").reset_index()
		listxx3 = pd.read_excel(peizhidir+"0（范例）上报单位.xls",sheet_name="地市清单",header=0,index_col=0,).fillna("没有定义好X").reset_index()				
...
def TOOL_person(data):
	"""化妆品评表人员专项统计（帮助林科做的）"""
	listxx2 = pd.read_excel(peizhidir+"0（范例）注册单位.xlsx",sheet_name="专家列表",header=0,index_col=0,).reset_index()
	for ids, cols in listxx2.iterrows():
...
def TOOLS_autocount(mydata, methon):
    ...
    data_org = pd.read_excel(
        peizhidir+"0（范例）上报单位.xls", sheet_name="监测机构", header=0, index_col=0
    ).reset_index()
    data_user = pd.read_excel(
        peizhidir+"0（范例）上报单位.xls", sheet_name="报告单位", header=0, index_col=0
    ).reset_index()
    ...
    if methon == "化妆品":
        ...
        orgcount0 = Countall(mydata).df_org("监测机构")
        orgcount0=pd.merge(orgcount0, data_org, on="监测机构", how="left")
        orgcount0=orgcount0[["监测机构序号","监测机构","化妆品数量指标","报告数量","审核通过数"]].sort_values(by=["监测机构序号"], ascending=True, na_position="last" ).fillna(0)
        ...
        usercount0=pd.merge(usercount0, data_user, on=["监测机构","单位名称"], how="left")
        usercount0=pd.merge(usercount0, data_org[["监测机构序号","监测机构"]], on="监测机构", how="left")
        usercount0=usercount0[["监测机构序号","监测机构","单位名称","化妆品数量指标","报告数量","审核通过数"]].sort_values(by=["监测机构序号","报告数量"], ascending=[True,False], na_position="last" ).fillna(0)
        a=["化妆品数量指标","审核通过数","报告数量"]
        usercount0[a] = usercount0[a].apply(lambda x: x.astype(int) )	

        erji=pd.merge(data_user_erji,usercount0, on=["监测机构","单位名称"], how="left").sort_values(by=["监测机构"], ascending=True, na_position="last" ).fillna(0)
        erji=erji[(erji["审核通过数"]<1)]
        erji=erji[["监测机构","单位名称","报告数量","审核通过数"]]
...
def PROGRAM_Menubar(win,ori_owercount_easyread, methon, ori):
	...
	if ini["模式"]=="药品":
		...
		keti_Bar.add_command(label="新的不良反应检测(证号)", command=lambda: PROGRAM_thread_it(TOOLS_get_new, ori,"证号")) 
		keti_Bar.add_command(label="新的不良反应检测(品种)", command=lambda: PROGRAM_thread_it(TOOLS_get_new, ori,"品种"))     
	...
	TOOL_Bar.add_command(		
		label="数据规整（报告单位）", command=lambda: TOOL_guizheng(ori_owercount_easyread,2,False))	
	TOOL_Bar.add_command(		
		label="数据规整（产品名称）", command=lambda: TOOL_guizheng(ori_owercount_easyread,3,False))				
	TOOL_Bar.add_command(		
		label="脱敏保存", command=lambda: TOOLS_data_masking(ori_owercount_easyread))	
	TOOL_Bar.add_separator() 
	TOOL_Bar.add_command(		
		label="评价人员（广东化妆品）", command=lambda: TOOL_person(ori_owercount_easyread))		
	TOOL_Bar.add_command(	
		label="意见反馈", command=lambda: PROGRAM_helper(["","  药械妆不良反应报表统计分析工作站","  开发者：蔡权周","  邮箱：411703730@qq.com","  微信号：sysucai","  手机号：18575757461"]))	
	TOOL_Bar.add_command(	
