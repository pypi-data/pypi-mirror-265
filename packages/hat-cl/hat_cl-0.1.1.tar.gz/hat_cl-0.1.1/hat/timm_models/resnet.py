...
from timm.models.resnet import BasicBlock, ResNet, _cfg, default_cfgs

# noinspection PyProtectedMember
from hat.modules._base import HATPayloadCarrierMixin
from hat.payload import HATPayload
from hat.types_ import HATConfig
from hat.utils import load_from_base_module_state_dict
...
class HATResNet(ResNet_, HATPayloadCarrierMixin):
    ...
    def load_pretrained(self, checkpoint_path):
        ...
        The name of the function must be `load_pretrained` to be compatible
        with `timm`. See `timm.models.helpers.load_custom_pretrained` for
        more details. To use this function, the model best be created with
        `pretrained_custom_load=True` in the `build_model_with_cfg` function.
        """
        state_dict = torch.load(checkpoint_path, map_location="cpu")
        load_from_base_module_state_dict(self, state_dict)
