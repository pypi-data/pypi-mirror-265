...
class TaskDependentModuleABC(
    ...
    def load_from_base_module(self, base_module: nn.Module):
        """Load from a base module.

        Args:
            base_module: The base module to load from.
    ...
    def load_from_base_module_state_dict(
        ...
        """Load the state dict of a base module.

        Args:
            base_module_state_dict: The state dict of the base module
                to load from.
...
class TaskIndexedModuleListABC(
    ...
    def load_from_base_module_state_dict(
        ...
        """Load the state dict of the base module to the task dependent
        module. Similar to `load_from_base_module` but with state dict.

        Args:
            base_module_state_dict: The state dict of the base module to load
                the state dict from.
        ...
        """
        for __task_id in range(self.num_tasks):
            self[__task_id].load_state_dict(base_module_state_dict)
...
class HATMaskedModuleABC(
    ...
    def load_from_base_module_state_dict(
        ...
        """Load the state dict of the base module."""
        self.weight.data.copy_(base_module_state_dict["weight"])
        if self.bias is not None:
            self.bias.data.copy_(base_module_state_dict["bias"])
