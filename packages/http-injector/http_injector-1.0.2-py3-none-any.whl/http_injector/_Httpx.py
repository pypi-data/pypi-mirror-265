...
class Adapter:
    ...
    def __new__(cls, timeout: int, headers: Dict[str, str], proxy_url: Optional[str] = None) -> Client:
        ...
        return httpx.Client(http2=True, cert=context, verify=context, proxies=proxy_url, headers=header, default_encoding=cls.__autodetect__, timeout=timeouts, transport=transport, event_hooks=dict(response = [raise_on_4xx_5xx]))