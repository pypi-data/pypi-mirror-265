...
from aiask.consts import config_path, api_key_key, app_directory

def check_for_config():
    # TODO: specifically check for existence of file rather than keys?
    config = get_config()
    if not config.get("API_KEY", None):
        raise Exception("Config not set up properly.  Have you run `aiask init`?")
...
def get_api_key():
    ...
    if api_key == None:
        print(Fore.RED + "Unable to find API key.  Have you created run `aiask init`?" + Style.RESET_ALL)
        return
    return api_key
