...
async def load(
    ...
    if not subcommands:
        active_subcommands = subcommands = (
            hub._dynamic.config.get("subcommands") or {}
        ).get(cli, {})
    else:
        active_subcommands = subcommands
...
async def parse_cli(
    ...
    if not cli:
        return {}
    parser = hub.lib.argparse.ArgumentParser(description=f"{cli} CLI parser")
    subparsers = {}

    # Add subcommands to the parser
    for subcommand, opts in subcommands.items():
        subparsers[subcommand] = parser.add_subparsers(dest=subcommand, **opts)
    ...
    for name, opts in active_cli.items():
        ...
        arg_subparsers = opts.pop("subparsers", [])

        # If choices is a string then assume it is a ref on the hub and get a list of loaded mods
        choices = opts.get("choices")
        if isinstance(choices, str):
            choices = (c for c in hub[choices]._loaded.keys() if c != "init")
        ...
        if not arg_subparsers or "__global__" in arg_subparsers:
            target_parser.add_argument(cli_name, *options, **opts)
        else:
            for p in arg_subparsers:
                subparsers[p].add_argument(cli_name, *options, **opts)
