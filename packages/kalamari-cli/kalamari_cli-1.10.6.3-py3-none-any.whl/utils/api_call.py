...
if API_URL is None:
    raise ValueError("API_URL environment variable not set. Please set the API_URL environment variable.")
...
def api_call(api_key, endpoint, req_type, data=None, files=None):
    ...
    if req_type == "POST":
        if files:
            ...
            response = requests.post(url, headers=headers, files=files, timeout=TIMEOUT_TIME)
        else:
            # For regular POST requests with JSON data
            response = requests.post(url, headers=headers, json=data, timeout=TIMEOUT_TIME)
    elif req_type == "GET":
