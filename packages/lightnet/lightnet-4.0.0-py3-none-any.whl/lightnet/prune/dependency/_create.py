...
import logging
import functools
from lightnet._imports import onnx, onnx_numpy_helper
from ._tree import Node, traverse_depth_first, traverse_parents_breadth_first
from ._type import NodeType, get_node_type
...
def create_module_weight(nodetype, element, graph, model, rootnode, parent):
    ...
    try:
        ...
        log.debug('Could not get PyTorch module for [%s]', name)
        module = None
...
def create_concat(element, graph, model, rootnode, parent):
    ...
    for gn in graph.node:
        if gn.output[0] in inputs and gn.op_type == 'Constant':
            ...
            if value != 1:
                raise NotImplementedError('ATen: cat (Not concatenating on channel dimension [1])')
    ...
    if name == '"flip"':
        ...
        for gn in graph.node:
            if gn.output[0] in inputs and gn.op_type == 'Constant':
                value = onnx_numpy_helper.to_array(onnx.helper.get_attribute_value(gn.attribute[0]))
                if value == 1:
                    raise NotImplementedError('ATen: flip (Flipping on channel dimension [1])')
