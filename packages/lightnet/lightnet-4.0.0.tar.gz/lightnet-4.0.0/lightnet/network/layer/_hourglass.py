...
class HourGlass(nn.Module):
    ...
    .. figure:: /.static/api/hourglass.*
       ...
       The HourGlass module consists of 4 different blocks: upper, down1, inner, down2.
       You can modify each of these blocks by providing the correct make function and (optionally) keyword arguments. |br|
       It will then recursively call `order` number of hourglasses, whilst removing the first number from the `channels` each time.
    ...
    def __init__(
        ...
        if order > 1:
            ...
            inner = make_inner(c2, c2, **self._get_current(inner_kwargs, {}))

        self.layers = ParallelSum(OrderedDict([
            ('upper', make_upper(c1, c1, **self._get_current(upper_kwargs, {}))),
            ('down', nn.Sequential(OrderedDict([
                ('down1', make_down1(c1, c2, **self._get_current(down1_kwargs, {}))),
                ('inner', inner),
                ('down2', make_down2(c2, c1, **self._get_current(down2_kwargs, {}))),
            ]))),
        ]))
